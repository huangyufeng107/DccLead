...
function viewCrowdSmartCodes(id) {
  ...
  body.innerHTML = `
    <div style="margin-bottom: 14px; background: #f8fafc; padding: 12px; border-radius: 6px; border-left: 4px solid #2563eb;">
      ...
      <div class="summary-primary-value">${escapeHtml(item.crowdTag)}</div>
      <div style="font-size: 12px; color: #64748b; margin-top: 4px;">主键ID: ${escapeHtml(item.id)}</div>
    </div>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 6px;">
      <div>
        <div class="summary-code-value">${escapeHtml(item.smartCode?.code || '-')}</div>
...
registerUiAction('resource-unfilled-export-batches', exportResourceUnfilledBatchDetails);
...
function renderResourceUnfilledBatchDetailPage(batch) {
  ...
  return `<div class="detail-tabs"><button class="detail-tab active" type="button" onclick="switchLeadDetailTab(this, 'resourceUnfilledBatchOverviewPanel')">执行概览</button><button class="detail-tab" type="button" onclick="switchLeadDetailTab(this, 'resourceUnfilledBatchRowsPanel')">批量明细</button><button class="detail-tab" type="button" onclick="switchLeadDetailTab(this, 'resourceUnfilledBatchLogPanel')">执行日志</button></div>
    ...
    <div class="detail-tab-panel worry-free-batch-detail-panel" id="resourceUnfilledBatchRowsPanel"><div class="worry-free-batch-layout">${summaryCards}<div class="worry-free-batch-section"><div class="worry-free-batch-section-head"><div class="worry-free-batch-section-title">批量明细</div></div><div class="lead-table-wrap"><table class="lead-table resource-unfilled-batch-lead-table"><thead><tr><th>批次号</th><th>留资未满编码</th><th>培育线索编码</th><th>培育任务编码</th><th>系统状态</th><th>同步时间</th><th>客户信息</th><th>意向车系</th><th>数据来源</th><th>线索来源</th><th>SmartCode</th><th>人群包标识</th><th>当前时段设置容量</th><th>当前时段剩余容量</th></tr></thead><tbody>${pageRows.map((row, index) => { const suffix = String(row.nurtureLeadCode || row.unfilledId || '').replace(/^PYXS|^LZWM/, ''); return `<tr><td>LZWM-${batch.date.replace(/-/g, '')}-001</td><td>${escapeHtml(row.unfilledId)}</td><td>${escapeHtml(row.nurtureLeadCode)}</td><td>${escapeHtml(`PYRW${suffix}`)}</td><td>${escapeHtml(getSystemStatus(row))}</td><td>${escapeHtml(formatSyncTime(row, start + index))}</td><td><div class="list-customer-cell"><div class="list-customer-name">${escapeHtml(row.customerName)}</div><div class="list-customer-phone">${escapeHtml(row.phone)}</div></div></td><td>${escapeHtml(row.intentSeries)}</td><td>${escapeHtml(row.leadType)}</td><td>${escapeHtml(row.leadSource)}</td><td>${escapeHtml(row.smartCode)}</td><td>${escapeHtml(row.crowdPackageTag)}</td><td>${escapeHtml(formatSlotQuantity(row.timeSlotCapacity, start + index))}</td><td>${escapeHtml(formatSlotQuantity(row.timeSlotRemaining, start + index))}</td></tr>`; }).join('')}</tbody></table></div>${paginationMarkup}</div></div></div>
