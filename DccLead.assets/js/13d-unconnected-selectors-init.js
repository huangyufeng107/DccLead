...
function syncUnconnectedBelongTypeFields(row) {
  ...
  const onclickChannel = readonly ? '' : 'onclick="toggleUnconnectedChannelPicker()"';
  const onclickSmartCode = readonly ? '' : 'onclick="toggleUnconnectedSmartCodePicker()"';
  ...
  if (belongType === '渠道编码') {
    ...
    subContainer.innerHTML = `
      ...
      <div class="tag-picker project-name-picker ${readonly ? 'readonly' : ''}">
        <button class="tag-picker-trigger placeholder" id="unconnectedChannelTrigger" type="button" ${onclickChannel}>请选择渠道编码</button>
        <div class="tag-picker-panel" id="unconnectedChannelPanel">
          <div class="tag-picker-toolbar">
            ...
            <div class="tag-picker-actions">
              <button class="tag-picker-action" type="button" onclick="selectAllUnconnectedChannelOptions()">全选</button>
              <button class="tag-picker-action" type="button" onclick="clearUnconnectedChannelOptions()">清空</button>
            </div>
            `}
          </div>
          ${readonly ? '' : `
          <div class="tag-picker-search">
            <input type="search" id="unconnectedChannelSearchInput" placeholder="搜索渠道编码" oninput="filterUnconnectedChannelOptions(this.value)" />
          </div>
          `}
          <div class="tag-picker-list" id="unconnectedChannelList">
            ${assignmentChannelCodeOptions.map(item => `
              <label class="tag-option ${selectedValues.includes(item) ? 'selected' : ''}" data-channel-option="${escapeAttr(item)}">
                <input type="checkbox" name="unconnectedChannelValue" value="${item}" ${selectedValues.includes(item) ? 'checked' : ''} ${disabledAttr} onchange="toggleUnconnectedChannelOption(this)" />
                ${item}
              </label>
            `).join('')}
          </div>
    ...
    subContainer.innerHTML = `
      <div class="smart-code-input-mode">
        ...
        <div class="series-mode-toggle">
          <button class="duration-tab ${inputMode === 'picker' ? 'active' : ''} ${readonly ? 'disabled' : ''}" type="button" id="unconnectedSmartCodePickerModeBtn" ${readonly ? '' : 'onclick="switchUnconnectedSmartCodeInputMode(\'picker\')"'}>下拉勾选</button>
          <button class="duration-tab ${inputMode === 'manual' ? 'active' : ''} ${readonly ? 'disabled' : ''}" type="button" id="unconnectedSmartCodeManualModeBtn" ${readonly ? '' : 'onclick="switchUnconnectedSmartCodeInputMode(\'manual\')"'}>手动输入</button>
        </div>
      </div>
      <div id="unconnectedSmartCodePickerInputPanel" style="${inputMode === 'manual' ? 'display:none' : ''}">
        <div class="tag-picker project-name-picker ${readonly ? 'readonly' : ''}">
          <button class="tag-picker-trigger placeholder" id="unconnectedSmartCodeTrigger" type="button" ${onclickSmartCode}>请选择SmartCode</button>
          <div class="tag-picker-panel" id="unconnectedSmartCodePanel">
            <div class="tag-picker-toolbar">
              ...
              ${readonly ? '' : `
              <div class="tag-picker-actions">
                <button class="tag-picker-action" type="button" onclick="selectAllUnconnectedSmartCodeOptions()">全选</button>
                <button class="tag-picker-action" type="button" onclick="clearUnconnectedSmartCodeOptions()">清空</button>
              </div>
              `}
            </div>
            ${readonly ? '' : `
            <div class="tag-picker-search">
              <input type="search" id="unconnectedSmartCodeSearchInput" placeholder="搜索SmartCode" oninput="filterUnconnectedSmartCodeOptions(this.value)" />
            </div>
            `}
            <div class="tag-picker-list" id="unconnectedSmartCodeList">
              ${assignmentSmartCodeOptions.map(item => `
                <label class="tag-option ${selectedValues.includes(item) ? 'selected' : ''}" data-sc-option="${escapeAttr(item)}">
                  <input type="checkbox" name="unconnectedSmartCodeValue" value="${item}" ${selectedValues.includes(item) ? 'checked' : ''} ${disabledAttr} onchange="toggleUnconnectedSmartCodeOption(this)" />
                  ${item}
                </label>
              `).join('')}
            </div>
      ...
      <div id="unconnectedSmartCodeManualInputPanel" style="${inputMode === 'manual' ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" id="unconnectedSmartCodeManualInput" rows="4" placeholder="可一次粘贴多个 SmartCode，支持换行、逗号、分号或空格分隔" ${disabledAttr}>${escapeHtml(manualValue)}</textarea>
