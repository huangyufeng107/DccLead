...
for (let i = 0; i < 5; i++) {
  nurtureTaskItems.push({
    ...
    criticalReminder: i === 2 ? {
      reasons: ['同店2次留资（广州天河店）', '私域高频互动（近3天）'],
      message: '客户近期在本店重复留资，并在私域场景持续互动；当前任务已逾期，建议优先跟进。'
    } : null,
    leadInfo: makeLeadInfo({
...
function renderCriticalReminder(task) {
  if (task.isDriveTask || task.tag !== '关键提醒') return '';
  ...
  const message = reminder.message || '该线索命中关键行为提醒，建议优先跟进。';
  return `<section class="nt-critical-reminder" aria-label="关键行为强提醒">
    <div class="nt-critical-reminder-icon" aria-hidden="true">!</div>
    <div class="nt-critical-reminder-body">
      <div class="nt-critical-reminder-title"><strong>关键行为强提醒</strong><span class="nt-critical-reminder-tags">${reasons.map((reason, index) => `<span class="nt-critical-reminder-tag ${index === 0 ? 'primary' : 'secondary'}">${esc(reason)}</span>`).join('')}</span></div>
      <p>${esc(message)}</p>
    </div>
  </section>`;
}
