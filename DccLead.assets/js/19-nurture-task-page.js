...
const nurtureTaskItems = [
  {
    id: 'NT-T1', queue: 'today', name: '吴女士', phone: '11700000007', level: 'C',
  ...
  {
    id: 'NT-T2', queue: 'today', name: '林先生', phone: '11400000018', level: 'C',
  ...
  {
    id: 'NT-DR1', queue: 'drive_new', name: '张先生', phone: '138****8888', level: 'H',
    dueLabel: '今日 14:00', tag: '系统取消', isDriveTask: true,
    driveInfo: {
      bookingCode: 'YYTJ2026090800125', scheduleCode: 'NEVSP202609080045',
      driveType: '到店试驾', driveVehicle: '粤AC6666（N7·曜石黑）',
      driveSeries: 'N7（AE01）— 高级灰', driveTimeSlot: '14:00–14:30',
      appointmentDate: '2026-09-10', driveStatus: '逾期未到店',
      cancelTag: '系统取消', dccInitialStatus: '待挽回'
    },
    leadInfo: makeLeadInfo({
      phone: '138****8888', ipLocation: '广东广州', leadType: '试驾预约线索',
      leadSource: 'AI外呼排程', firstLeadStatus: '已预约试驾', firstIntentLevel: 'H',
      intentCar: 'N7', latestIntentSeries: 'N7', followCount: '2次',
      createTime: '2026-09-08 09:00:00', leadStatus: '逾期未到店',
      leadDesc: '客户原预约今日 14:00 试驾 N7，专营店核销为逾期未到店。需坐席开展流失访谈并争取二次激活重新排程。',
      intentStore: '广州东风日产天河店', buyTime: '1个月内', carModel: 'N7 2026款 四驱版',
      leadRemark: '专营店反馈：客户超时1小时未到店，电话未接通，转入总部坐席开展流失排查与二次邀约。',
      leadCode: '1846296533270700', taskCode: '202609091400000001D',
      channelName: 'AI外呼排程', projectName: 'N7试驾招募'
    }),
    formData: {
      contactStatus: 'CONNECTED',
      unfulfilledReason: '暂存点单',
      visitResult: 'dispatch_store',
      intentLevel: 'H',
      intentStore: 'DLR-GD-GZ-001',
      leadType: '',
      nextVisitTime: '',
      visitDesc: '客户表示今日因公司临时召开紧急会议未能如约到店，对N7四驱版仍然保持浓厚兴趣。已协助重新预约本周六下午14:00到广州天河店试驾，客户已确认同意赴约。'
    },
    timelineEvents: [
      { type: 'lead', title: 'AI外呼建单成功', time: '2026-09-08 09:00', desc: 'AI外呼确认试驾四要素，生成预约单号 YYTJ2026090800125' },
      { type: 'call', title: '中台回执确立', time: '2026-09-08 09:02', desc: 'NEV中台落库成功，生成排程编码 NEVSP202609080045 并直派广州天河店' },
      { type: 'lead', title: '专营店核销未到店', time: '2026-09-10 15:00', desc: '超过预约时段，专营店在系统核销为【逾期未到店】，触发总部试驾挽回工单' }
    ],
    visitRecords: [
      {
        round: 2,
        ...
        intentLevel: 'A',
        leadType: '高意向定单线索',
        intentStore: '成都东风日产高新店',
        nextVisitTime: '2026-09-28 10:00',
        followStatus: '跟进中',
        agent: 'seat_sz_05',
        time: '2026-09-24 14:30',
        desc: '客户已于昨日完成深度试驾，对NX8底盘舒适性及隔音表现高度认可，正在对比竞品，已向客户提供置换补贴政策，推进国庆前下订。'
      },
      {
        round: 1,
        isLatest: false,
        ...
        followStatus: '跟进中',
        agent: 'seat_sz_05',
        time: '2026-09-16 15:50',
        desc: '客户工作繁忙，已确认工作日下午到店试驾。'
      }
    ],
...
// 试驾排程补充示例：覆盖阶段五真实断点（逾期未到店、临时取消）
function makeDriveScheduleTask(data) {
  return {
    isDriveTask: true,
    tag: data.tag || '逾期未到店',
    level: data.level || 'B',
    driveInfo: {
      ...
      driveType: '到店试驾',
      driveVehicle: data.driveVehicle,
      driveSeries: data.driveSeries,
      driveTimeSlot: data.driveTimeSlot,
      appointmentDate: data.appointmentDate,
      driveStatus: data.driveStatus || '逾期未到店',
      cancelTag: data.cancelTag || '-',
      dccInitialStatus: '待挽回'
    },
    leadInfo: makeLeadInfo({
      phone: data.phone, ipLocation: data.ipLocation, leadType: '试驾预约线索',
      leadSource: data.leadSource || 'AI外呼排程', firstLeadStatus: '已预约试驾', firstIntentLevel: data.level,
      intentCar: data.series, latestIntentSeries: data.series, followCount: data.followCount,
      createTime: data.createTime, leadStatus: data.driveStatus || '逾期未到店', leadDesc: data.leadDesc,
      intentStore: data.store, buyTime: data.buyTime, carModel: data.carModel,
      leadRemark: data.leadRemark, leadCode: data.leadCode, taskCode: data.taskCode,
      channelName: data.leadSource || 'AI外呼排程', projectName: '试驾流失挽回'
    }),
    formData: {
      contactStatus: data.contactStatus || 'CONNECTED',
      unfulfilledReason: data.unfulfilledReason || '暂存点单',
      visitResult: data.visitResult || 'dispatch_store',
      intentLevel: data.level || 'B',
      intentStore: data.storeCode || 'DLR-GD-GZ-001',
      leadType: '',
      nextVisitTime: '',
      visitDesc: data.visitDesc || '客户反馈因行程安排冲突未能到店，经坐席电话沟通，已协调顺延排期。'
    },
    timelineEvents: [{ type: 'lead', title: '试驾断点触发', time: data.createTime.slice(0, 16), desc: data.leadDesc }],
    visitRecords: [],
    customerProfile: { ownerTags: {}, buyIntent: { awareness: '基本了解', planTime: data.buyTime, ownBrand: '-', competitors: '-', focus: ['试驾体验', '金融方案'] } },
    wishlist: [],
    ...data
  };
}
...
nurtureTaskItems.push(
  makeDriveScheduleTask({
    id: 'NT-DR2', queue: 'drive_today', name: '李女士', phone: '136****5268', level: 'A', dueLabel: '今日 17:30', tag: '临时取消',
    bookingCode: 'YYTJ2026090900181', scheduleCode: 'NEVSP202609090050', driveVehicle: '粤AD9888（N6·珠光白）', driveSeries: 'N6（E03）— 智享版', driveTimeSlot: '17:30–18:00', appointmentDate: '2026-09-10', driveStatus: '已取消', cancelTag: '用户取消',
    ipLocation: '广东广州', leadSource: '人工坐席回访建单', series: 'N6', followCount: '1次', createTime: '2026-09-09 11:15:00', leadDesc: '客户临时因出差电话告知取消试驾，需回访排查顾虑并协助顺延改期。', store: '广州东风日产天河店', storeCode: 'DLR-GD-GZ-001', buyTime: '1个月内', carModel: 'N6 2026款 智享版', leadRemark: '客户表示对N6外观与空间很喜欢，希望能周末重新约试驾。', leadCode: '1846296533270721', taskCode: '2026090917300000031', gender: '女',
    visitRecords: []
  }),
  makeDriveScheduleTask({
    id: 'NT-DR3', queue: 'drive_overdue', name: '陈先生', phone: '139****3176', level: 'B', dueLabel: '逾期 2 h', tag: '临时取消',
    bookingCode: 'YYTJ2026090800156', scheduleCode: 'NEVSP202609080048', driveVehicle: '粤AE3108（天籁·曜石黑）', driveSeries: '天籁（L34）— 智驾版', driveTimeSlot: '10:00–10:30', appointmentDate: '2026-09-09', driveStatus: '已取消', cancelTag: '门店取消',
    ipLocation: '广东佛山', leadSource: 'AI外呼排程', series: '天籁', followCount: '2次', createTime: '2026-09-08 15:40:00', leadDesc: '专营店因试驾车辆调配冲突取消，需总部坐席致电安抚并协助更换门店或改期。', store: '广州东风日产天河店', storeCode: 'DLR-GD-GZ-001', buyTime: '3个月内', carModel: '天籁 2.0T 智驾版', leadRemark: '已取消，坐席需致电致歉并协助改约就近门店。', leadCode: '1846296533270722', taskCode: '2026090910000000032', gender: '男',
    visitRecords: []
  }),
  makeDriveScheduleTask({
    id: 'NT-DR4', queue: 'drive_future', name: '王女士', phone: '135****8602', level: 'B', dueLabel: '3d后 09:30', tag: '门店取消',
    bookingCode: 'YYTJ2026091000203', scheduleCode: 'NEVSP202609100054', driveVehicle: '粤AB5201（探陆·砂岩棕）', driveSeries: '探陆（T33）— 四驱版', driveTimeSlot: '09:30–10:00', appointmentDate: '2026-09-12', driveStatus: '已取消', cancelTag: '门店取消',
    ipLocation: '广东深圳', leadSource: '人工坐席回访建单', series: '探陆', followCount: '1次', createTime: '2026-09-10 09:05:00', leadDesc: '探陆四驱版试驾逾期未到店，需坐席跟进了解未到店原因并重新激活。', store: '深圳东风日产南山店', storeCode: 'DLR-GD-SZ-002', buyTime: '1个月内', carModel: '探陆 380VC-TURBO 四驱版', leadRemark: '计划再次电话核实客户意向。', leadCode: '1846296533270723', taskCode: '2026091209300000033', gender: '女',
    visitRecords: []
  })
);
...
nurtureTaskItems.push(
  ...
  },
  {
    ...
    criticalReminder: {
      reasons: ['同店2次留资（上海浦东店）', '私域高频互动（近3天）'],
      message: '客户近期在本店重复留资，并持续关注车型内容，建议本次回访优先确认购车计划与到店意向。'
    },
    leadInfo: makeLeadInfo({
...
const overdueReminderReasons = {
  0: ['同店2次留资（北京朝阳店）'],
  1: ['私域高频互动（近3天）'],
  2: ['同店2次留资（广州天河店）', '私域高频互动（近3天）'],
  3: ['同店3次留资（深圳南山店）']
};
for (let i = 0; i < 5; i++) {
  const reminderReasons = overdueReminderReasons[i] || null;
  nurtureTaskItems.push({
    id: `NT-OV${i+1}`, queue: 'overdue', name: overdueNames[i], phone: overduePhones[i],
    level: overdueLevels[i], dueLabel: i === 0 ? '逾期 1 d' : `逾期 ${i + 1} h`, tag: reminderReasons ? '关键提醒' : '', isDriveTask: false,
    criticalReminder: reminderReasons ? { reasons: reminderReasons } : null,
    leadInfo: makeLeadInfo({
...
let criticalReminderCountdownTimer = null;
let criticalReminderHideTimer = null;
let activeCriticalReminderTaskId = null;
let criticalReminderVisit = 0;
const hiddenCriticalReminderVisits = new Set();
let aiDriveSourceReminderCountdownTimer = null;
let aiDriveSourceReminderHideTimer = null;
let activeAiDriveSourceReminderTaskId = null;
let aiDriveSourceReminderVisit = 0;
const hiddenAiDriveSourceReminderVisits = new Set();
...
window.showNurtureTaskPage = function() {
  ...
  activeCriticalReminderTaskId = null;
  if (typeof window.__renderSupervisorNurturePage === 'function') {
    window.__renderSupervisorNurturePage();
  } else {
...
function renderNurtureTaskTopbar() {
  return `<header class="nt-page-topbar">
    ...
    <div class="nt-page-actions">
      <button type="button" class="nt-page-action nt-page-action-timeline" data-nt-top-action="open-timeline" title="查看客户时光轴">
...
// ==================== 左栏渲染 ====================
// 试驾排程场景：提取“取消发起方”标准标签与优先级视觉配色（异常取消线索 100% 具备）
function getDriveCancelPartyBadge(item) {
  const cancelTag = item.driveInfo?.cancelTag || item.cancelTag;
  if (cancelTag === '门店取消' || cancelTag === '专营店取消') {
    return {
      text: '门店取消',
  ...
  const fallback = (cancelTag && cancelTag !== '-') ? cancelTag : '系统取消';
  return {
    text: fallback,
...
function getNurtureLeadStatusBadge(status) {
  const s = (status && status !== '-') ? status : '培育中';
  if (s.includes('待首次') || s.includes('新任务')) {
    return {
      text: s,
      style: 'background:#ecfdf5; color:#059669; border:1px solid #a7f3d0;'
    };
  }
  if (s.includes('逾期') || s.includes('关键提醒') || s.includes('紧急')) {
    return {
      text: s,
      style: 'background:#fef2f2; color:#dc2626; border:1px solid #fecaca;'
    };
  }
  if (s.includes('回访') || s.includes('提交') || s.includes('已完成')) {
    return {
      text: s,
...
function renderTaskCard(item) {
  ...
  const isCritical = item.tag === '关键提醒' || (item.isDriveTask && item.queue === 'drive_overdue');
  const intentSeries = item.leadInfo?.latestIntentSeries || item.leadInfo?.intentCar || '—';
  const driveBadge = item.isDriveTask ? getDriveCancelPartyBadge(item) : null;
  return `<button type="button" class="nt-task-card ${isActive?'active':''} ${isCritical?'critical':''}" data-task-id="${item.id}" data-queue="${item.queue}">
    <div class="nt-task-card-top">
      <span class="nt-task-identity">
        ...
        ${isCritical && !item.isDriveTask ? '<span class="nt-task-tag">关键提醒</span>' : ''}
        ${driveBadge ? `<span class="nt-task-tag" style="${driveBadge.style} font-size:10px; padding:1px 5px; border-radius:3px; font-weight:600; flex-shrink:0; white-space:nowrap;">${esc(driveBadge.text)}</span>` : ''}
      </span>
...
function renderLeadInfoSection(task) {
  ...
  if (isDrive) {
    ...
    const seriesName = drive.driveSeries ? drive.driveSeries.split('—')[0].trim() : (info.latestIntentSeries || info.intentCar || '—');
    const vehicleConfig = info.carModel || drive.driveVehicle || '—';
    const storeName = info.intentStore || task.store || '广州东风日产天河店';
    const cancelReasonText = drive.cancelReason || task.cancelReason || (drive.cancelTag && drive.cancelTag !== '-' ? (drive.cancelTag === '系统取消' ? '系统取消（逾期未到店核销）' : drive.cancelTag) : '逾期未到店核销');
    const cancelParty = (drive.cancelTag && drive.cancelTag !== '-') ? drive.cancelTag : '系统取消';

    groups = [
      { title: '试驾信息', summary: [drive.driveStatus, drive.appointmentDate, drive.driveTimeSlot].filter(value=>value&&value!=='-').join(' · '), fields: [
        ['试驾来源渠道', info.leadSource || task.leadSource || 'AI外呼排程', false],
        ['预约单号', drive.bookingCode || '-', true],
        ['试驾排程编码', drive.scheduleCode || '-', true],
        ['试驾类型', drive.driveType || '到店试驾', false],
        ['预约车系', seriesName, false],
        ['预约车型配置', vehicleConfig, false],
        ['预约专营店', storeName, false],
        ['预约试驾日期', drive.appointmentDate || '-', false],
        ['预约试驾时段', drive.driveTimeSlot || '-', false],
        ['试驾履约状态', drive.driveStatus || task.tag || '-', false],
        ['取消发起方', cancelParty, false],
        ['取消原因说明', cancelReasonText, false]
      ]},
      { title: '线索与意向信息', summary: [seriesName, info.firstIntentLevel&&info.firstIntentLevel!=='-'?`${info.firstIntentLevel}级`:'', storeName].filter(value=>value&&value!=='-').join(' · '), fields: [
        ['联系电话', info.phone, true],
        ['线索类型', info.leadType || '试驾预约线索', false],
        ['首次线索状态', info.firstLeadStatus || '已预约试驾', false],
        ['累计跟进次数', info.followCount || '1次', false],
        ['当前线索状态', info.leadStatus || drive.driveStatus || '已取消', false],
        ['预约专营店', storeName, false],
        ['最新意向车型', info.carModel || vehicleConfig, false],
        ['VIN码', info.vin || '-', false],
        ['IP归属地', info.ipLocation || '-', false],
        ['线索来源', info.leadSource || 'AI外呼排程', false],
        ['首次意向级别', info.firstIntentLevel ? `${info.firstIntentLevel}级` : '-', false],
        ['原始意向车辆', info.intentCar || seriesName, false],
        ['最新意向车系', info.latestIntentSeries || seriesName, false],
        ['首次留资时间', info.createTime || '-', true],
        ['预计购车时间', info.buyTime || '-', false],
        ['购车门店', info.purchaseStore || '-', false],
        ['线索备注', info.leadRemark || '-', true],
        ['线索描述', info.leadDesc || '-', true]
      ]},
      { title: '业务标识信息', summary: drive.bookingCode&&drive.scheduleCode?'预约单号、排程编码已关联':'查看业务编码与渠道信息', fields: [
        ['培育线索编码', info.leadCode || '-', true],
        ['回访工单编码', info.taskCode || '-', true],
        ['渠道名称', info.channelName || info.leadSource || 'AI外呼排程', false],
        ['媒体名称', info.mediaName || '-', false],
        ['大项目名', info.projectName || '试驾流失挽回', false],
        ['是否经过AI外呼', info.aiCalled || (info.leadSource && info.leadSource.includes('AI') ? '是' : '否'), false]
      ]}
    ];
  } else {
    groups = [
...
function renderTimelineEntry(task) {
  ...
  return `<section class="nt-section nt-timeline-entry ${collapsed?'collapsed':''}" id="ntTimelineEntry">
    <button type="button" class="nt-section-header nt-section-header-link" data-section-toggle="timeline" data-open-tab="timeline">
      ...
      <span class="nt-section-arrow-r" aria-hidden="true"><svg viewBox="0 0 16 16" focusable="false"><path d="M5 3.5 10 8l-5 4.5"/></svg></span>
    </button>
  </section>`;
}

// ==================== 试驾排程【回访提交】专属枚举与联动配置 ====================
// 1:1 引用系统数据字典 CONTACT_STATUS（未接通激活配置“接触状态”选项共用）
const driveContactStatusOptions = [
  { value: 'CONNECTED', label: '正常接通' },
  { value: 'NO_ANSWER', label: '无人接听' },
  { value: 'WECHAT_FOLLOW_UP', label: '企微跟进' },
  { value: 'CUSTOMER_REJECTED', label: '客户拒绝' },
  { value: 'BUSY_SECRETARY', label: '忙音/秘书台' },
  { value: 'EMPTY_NUMBER', label: '空号' },
  { value: 'SUSPENDED', label: '停机' },
  { value: 'POWERED_OFF', label: '关机' }
];

const unfulfilledReasonOptions = [
  { value: '薅羊毛客户（如抽奖领礼品）', label: '薅羊毛客户（如抽奖领礼品）' },
  { value: '通话秒挂/不说话', label: '通话秒挂/不说话' },
  { value: '非客户主动留资（扫共享授权/误点留资）', label: '非客户主动留资（扫共享授权/误点留资）' },
  { value: '异常客群无意向（老人小孩/语言不通/投诉/骂人）', label: '异常客群无意向（老人小孩/语言不通/投诉/骂人）' },
  { value: '无意向（表示不关注/不考虑/无需求/无购车意向）', label: '无意向（表示不关注/不考虑/无需求/无购车意向）' },
  { value: '异常线索（高度配合/声音一致/主动告知假线索）', label: '异常线索（高度配合/声音一致/主动告知假线索）' },
  { value: '暂存点单', label: '暂存点单' },
  { value: '特殊报备', label: '特殊报备' },
  { value: '当天内回流', label: '当天内回流' }
];

const driveLeadTypeOptions = [
  { value: '试驾留资线索', label: '试驾留资线索' },
  { value: '一般留资线索', label: '一般留资线索' }
];

// 页面白名单保持原有顺序和保存值，显示名称动态引用公共字典。
const driveVisitResultOptions = [
  { value: '', label: '请选择' },
  { value: 'next_visit', label: '下次回访', dictionaryCode: 'NEXT_FOLLOW_UP' },
  { value: 'visit_completed', label: '回访完成', dictionaryCode: 'VISIT_COMPLETED' },
  { value: 'dispatch_store', label: '下发门店', dictionaryCode: 'DISPATCH_STORE' }
];

function getDriveVisitResultOptions() {
  const items = typeof DICTIONARY_ITEMS !== 'undefined' ? DICTIONARY_ITEMS.RETURN_VISIT_RESULT : [];
  return driveVisitResultOptions.map(option => ({
    ...option,
    label: (items || []).find(item => item.code === option.dictionaryCode)?.name || option.label
  }));
}

// ==================== 阶段五：试驾排程专属【回访提交】表单 ====================
function renderDriveRecoveryForm(task) {
  const f = task.formData || {};
  const cs = f.contactStatus ?? 'CONNECTED';
  const unfulfilledDisabled = !['CONNECTED', 'connected', 'CUSTOMER_REJECTED', 'rejected'].includes(cs);
  const vr = f.visitResult !== undefined ? f.visitResult : (['CONNECTED', 'connected'].includes(cs) ? 'dispatch_store' : '');
  const isNextVisit = vr === 'next_visit' || vr === '下次回访';
  const lc = intentLevelOptions.find(l => l.value === (f.intentLevel || task.level)) || {};

  const selectHtml = (field, label, options, currentVal, placeholder, disabled, isNav, isReq) => {
    const hasVal = !!currentVal;
    return `<div class="nt-form-item ${disabled ? 'nt-form-item-disabled' : ''}" data-field="${field}">
      <label>${isReq ? '<span class="req" style="color:#ef4444;margin-right:2px;">*</span>' : ''}${esc(label)}</label>
      ${isNav ?
        `<button type="button" class="nt-form-ctl nt-select-nav" ${disabled ? 'disabled' : ''} data-action="open-store-picker">
          ...
          <span class="nt-select-arrow-r" aria-hidden="true"><svg viewBox="0 0 16 16" focusable="false"><path d="M5 3.5 10 8l-5 4.5"/></svg></span>
        </button>`
        :
        `<div class="nt-custom-select" data-select="${field}" ${disabled ? 'data-disabled="1"' : ''}>
          <button type="button" class="nt-select-trigger" ${disabled ? 'disabled' : ''}>
            <span class="nt-select-value ${!hasVal ? 'placeholder' : ''}" ${hasVal && field === 'intentLevel' ? `style="color:${lc.color}"` : ''}>${hasVal ? esc(options.find(o => o.value === currentVal || o.label === currentVal || (o.value && currentVal && o.value.toLowerCase() === String(currentVal).toLowerCase()))?.label || currentVal) : esc(placeholder)}</span>
            ${hasVal && !disabled ? `<span class="nt-select-clear" data-clear="${field}" role="button" aria-label="清除" tabindex="0"><svg viewBox="0 0 16 16" focusable="false" aria-hidden="true"><path d="m5 5 6 6M11 5l-6 6"/></svg></span>` : ''}
            <span class="nt-select-arrow" aria-hidden="true"><svg viewBox="0 0 16 16" focusable="false"><path d="m3.5 6 4.5 4 4.5-4"/></svg></span>
          </button>
          <div class="nt-select-dropdown">
            ${options.map(o => `<div class="nt-select-option ${currentVal === o.value || (o.value && currentVal === o.label) || (o.value && currentVal && o.value.toLowerCase() === String(currentVal).toLowerCase()) ? 'selected' : ''}" data-option="${esc(o.value)}">${field === 'intentLevel' ? `<span style="color:${o.color}">${esc(o.label)}</span>` : esc(o.label)}</div>`).join('')}
          </div>
        </div>`
      }
    </div>`;
  ...
  return `<section class="nt-section nt-visit-form-section" id="ntVisitForm">
    <div class="nt-section-header static">
      <span class="nt-section-title">回访提交</span>
    </div>
    <div class="nt-section-body">
      <div class="nt-form-grid">
        <!-- 1、接触状态 -->
        ${selectHtml('contactStatus', '接触状态', driveContactStatusOptions, cs, '请选择接触状态', false, false, true)}

        <!-- 2、未履约原因（与接触状态联动：当接触状态=正常接听、客户拒绝时才解锁填写） -->
        ${selectHtml('unfulfilledReason', '未履约原因', unfulfilledReasonOptions, unfulfilledDisabled ? '' : f.unfulfilledReason, unfulfilledDisabled ? '需接听或拒绝后解锁' : '请选择未履约原因', unfulfilledDisabled, false, false)}

        <!-- 3、回访结果（4项枚举：请选择、下次回访、回访完成、下发门店） -->
        ${selectHtml('visitResult', '回访结果', getDriveVisitResultOptions(), vr, '请选择', false, false, true)}

        <!-- 4、下次回访时间（回访结果=下次回访时激活并标为必填） -->
        <div class="nt-form-item ${!isNextVisit ? 'nt-form-item-disabled' : ''}" data-field="nextVisitTime">
          <label>${isNextVisit ? '<span class="req" style="color:#ef4444;margin-right:2px;">*</span>' : ''}下次回访时间</label>
          <input type="datetime-local" class="nt-form-ctl nt-input" data-drive-date="nextVisitTime" aria-label="下次回访时间" ${!isNextVisit ? 'disabled' : ''} value="${esc(isNextVisit ? (f.nextVisitTime || '').replace(' ', 'T') : '')}"/>
        </div>

        <!-- 5、意向级别 -->
        ${selectHtml('intentLevel', '意向级别', intentLevelOptions, f.intentLevel ?? task.level, '请选择意向级别', false, false, true)}

        <!-- 6、线索类型 -->
        ${selectHtml('leadType', '线索类型', driveLeadTypeOptions, f.leadType ?? '', '请选择线索类型', false, false, true)}

        <!-- 7、意向门店 -->
        ${selectHtml('intentStore', '意向门店', intentStoreOptions, f.intentStore || 'DLR-GD-GZ-001', '请选择意向专营店', false, true, true)}

        <!-- 8、回访描述（整宽多行文本域） -->
        <div class="nt-form-item nt-form-item-wide" data-field="visitDesc">
          <label><span class="req" style="color:#ef4444;margin-right:2px;">*</span>回访描述</label>
          <textarea class="nt-form-ctl nt-textarea" data-input="visitDesc" rows="3" placeholder="请输入本次试驾后回访详细纪要（沟通内容、未履约原因分析、后续跟进结论等）">${esc(f.visitDesc || '')}</textarea>
        </div>
      </div>

      <div class="nt-form-actions">
        <button type="button" class="nt-btn nt-btn-default" data-action="drive-pause">回访暂存</button>
        <button type="button" class="nt-btn nt-btn-primary" data-action="drive-submit">回访提交</button>
      </div>
    </div>
  </section>`;
}
...
function renderVisitForm(task) {
  if (task.isDriveTask) return renderDriveRecoveryForm(task);
  const f = task.formData || {};
      ...
      ${isNav?
        ...
        `<div class="nt-custom-select" data-select="${field}" ${disabled?'data-disabled="1"':''}>
          <button type="button" class="nt-select-trigger" ${disabled?'disabled':''}>
            ...
            ${hasVal?`<span class="nt-select-clear" data-clear="${field}" role="button" aria-label="清除" tabindex="0"><svg viewBox="0 0 16 16" focusable="false" aria-hidden="true"><path d="m5 5 6 6M11 5l-6 6"/></svg></span>`:''}
            <span class="nt-select-arrow" aria-hidden="true"><svg viewBox="0 0 16 16" focusable="false"><path d="m3.5 6 4.5 4 4.5-4"/></svg></span>
          </button>
          <div class="nt-select-dropdown">
...
function renderSmartRating(task) {
  ...
  return `<div class="nurture-rating-panel nt-smart-rating-panel">
    ...
    <section class="rating-summary-block"><div class="rating-summary-copy">${summary}</div></section>
    <section class="rating-features-block">
      <h5>客户特征</h5>
      <div class="tag-summary-block"><h6>画像小结</h6><p>${esc(basicTags[0][1])}、${esc(cityText)}；计划${esc(planTime)}购车，预算${esc(budget)}；倾向${esc(payment)}，近三月访问${esc(platformVisits)}。</p></div>
  ...
  if (task.isDriveTask) {
    if (!records.length) {
      return `<div class="nt-records-view">
        <div class="nt-records-header">
          <h3 class="nt-records-title">回访记录</h3>
          <span class="nt-records-count-badge">累计回访 0 次</span>
        </div>
        <div class="nt-empty" style="padding:32px 0;"><div>暂无回访记录</div></div>
      </div>`;
    }

    const countBadge = `<span class="nt-records-count-badge">累计回访 ${records.length} 次</span>`;
    ...
    return `<div class="nt-records-view">
      ...
      <div class="nurture-timeline nt-drive-timeline">${records.map((r, idx) => {
        // 坐席人工回访记录（四层紧凑卡片流）
        const isLatest = idx === 0;
        const roundNum = r.round || (records.length - idx);

        // 接触状态标签颜色映射
        const cs = r.contactStatus || r.status || '正常接通';
        let csClass = 'gray';
        if (cs === '正常接通' || cs === '正常接听') csClass = 'green';
        else if (cs === '企微跟进') csClass = 'blue';
        else if (cs === '客户拒绝') csClass = 'red';

        // 回访结果标签颜色映射
        const vr = r.visitResult || r.result || '下次回访';
        let vrClass = 'blue';
        if (vr === '回访完成' || vr.includes('完成') || vr.includes('提升') || vr.includes('订金')) vrClass = 'green';
        else if (vr === '下发门店' || vr.includes('下发')) vrClass = 'cyan';
        else if (vr === '下次回访') vrClass = 'blue';

        const level = r.intentLevel || '';
        const store = r.intentStore || '—';
        const leadType = r.leadType || '试驾留资线索';
        const reason = r.unfulfilledReason || '';
        const nextTime = r.nextVisitTime || '';
        const desc = r.desc || r.visitDesc || '—';

        return `
          <article class="nurture-timeline-item nt-drive-rec-item ${isLatest ? 'is-latest' : ''}">
            <span class="nurture-timeline-dot ${isLatest ? 'dot-latest' : 'dot-history'}" aria-hidden="true"></span>
            
            <!-- 第1层：轮次标题与坐席/时间 -->
            <div class="nt-rec-header">
              <div class="nt-rec-round-title">
                <strong>第 ${roundNum} 次回访</strong>
                ${isLatest ? '<span class="nt-tag-chip blue">最新记录</span>' : '<span class="nt-tag-chip gray">历史记录</span>'}
              </div>
              <div class="nt-rec-meta-info">
                <span class="agent">@${esc(r.agent || '坐席')}</span>
                <time class="time">${esc(r.time || '')}</time>
              </div>
            </div>

            <!-- 第2~4层：结构化卡片主体 -->
            <div class="nt-rec-body-card ${isLatest ? 'latest-card' : ''}">
              <!-- 第2层：核心状态徽章行 -->
              <div class="nt-rec-badge-row">
                <span class="nt-badge ${csClass}">${esc(cs)}</span>
                <span class="nt-badge ${vrClass}">${esc(vr)}</span>
                ${level ? `<span class="nt-badge level">${esc(level)}级意向</span>` : ''}
              </div>

              <!-- 第3层：核心属性网格 -->
              <div class="nt-rec-grid">
                <div class="nt-rec-grid-item">
                  <span class="grid-label">意向门店</span>
                  <span class="grid-val font-semibold" title="${esc(store)}">${esc(store)}</span>
                </div>
                <div class="nt-rec-grid-item">
                  <span class="grid-label">线索类型</span>
                  <span class="grid-val" title="${esc(leadType)}">${esc(leadType)}</span>
                </div>
              </div>

              <!-- 未履约原因预警行（若存在） -->
              ${reason ? `
                <div class="nt-rec-alert-row">
                  <span class="alert-label">未到店原因:</span>
                  <span class="alert-text">${esc(reason)}</span>
                </div>
              ` : ''}

              <!-- 下次回访时间（若存在） -->
              ${nextTime && nextTime !== '-' ? `
                <div class="nt-rec-next-row">
                  <span class="next-label">下次回访:</span>
                  <span class="next-time font-mono">${esc(nextTime)}</span>
                </div>
              ` : ''}

              <!-- 第4层：回访详细纪要长文本气泡 -->
              <div class="nt-rec-desc-box">
                <div class="desc-box-label">回访详细纪要</div>
                <p class="desc-box-content">${esc(desc)}</p>
              </div>
            </div>
          </article>
        `;
...
function renderCustomerProfile(task) {
  ...
  return `<div class="nt-profile">
    ...
    ${activeProfileSubTab==='buyIntent'?`<div class="nt-p-body">
      ...
      <div class="nt-p-row"><label>已有车辆品牌</label><span>${esc(bi.ownBrand||'-')}</span><button class="nt-p-more">更多<svg viewBox="0 0 16 16" focusable="false" aria-hidden="true"><path d="M5 3.5 10 8l-5 4.5"/></svg></button></div>
      <div class="nt-p-row"><label>关注竞品</label><span>${esc(bi.competitors||'-')}</span><button class="nt-p-more">更多<svg viewBox="0 0 16 16" focusable="false" aria-hidden="true"><path d="M5 3.5 10 8l-5 4.5"/></svg></button></div>
...
function renderCriticalReminder(task) {
  const reminderVisitKey = task.id + '@' + criticalReminderVisit;
  if (task.isDriveTask || task.tag !== '关键提醒' || hiddenCriticalReminderVisits.has(reminderVisitKey)) return '';
  const reminder = task.criticalReminder || {};
  const reasons = Array.isArray(reminder.reasons) && reminder.reasons.length ? reminder.reasons : ['关键行为命中'];
  return `<section class="nt-critical-reminder" data-critical-reminder data-reminder-visit="${esc(reminderVisitKey)}" aria-label="关键行为强提醒">
    <div class="nt-critical-reminder-icon" aria-hidden="true">!</div>
    <div class="nt-critical-reminder-body">
      <div class="nt-critical-reminder-title"><strong>关键行为强提醒</strong><span class="nt-critical-reminder-tags">${reasons.map((reason) => `<span class="nt-critical-reminder-tag ${String(reason).includes('私域高频互动') ? 'secondary' : 'primary'}">${esc(reason)}</span>`).join('')}</span></div>
    </div>
    <button type="button" class="nt-critical-reminder-action" data-critical-reminder-toggle>10 秒后自动隐藏</button>
  </section>`;
}

function bindCriticalReminderCountdown(page) {
  if (criticalReminderCountdownTimer) {
    window.clearInterval(criticalReminderCountdownTimer);
    criticalReminderCountdownTimer = null;
  }
  if (criticalReminderHideTimer) {
    window.clearTimeout(criticalReminderHideTimer);
    criticalReminderHideTimer = null;
  }
  const reminder = page.querySelector('[data-critical-reminder]');
  if (!reminder) return;

  const action = reminder.querySelector('[data-critical-reminder-toggle]');
  let seconds = 10;
  const hideReminder = function() {
    if (criticalReminderCountdownTimer) window.clearInterval(criticalReminderCountdownTimer);
    criticalReminderCountdownTimer = null;
    if (criticalReminderHideTimer) window.clearTimeout(criticalReminderHideTimer);
    criticalReminderHideTimer = null;
    hiddenCriticalReminderVisits.add(reminder.dataset.reminderVisit);
    reminder.remove();
  };
  criticalReminderCountdownTimer = window.setInterval(function() {
    seconds -= 1;
    if (seconds <= 0) { hideReminder(); return; }
    action.textContent = seconds + ' 秒后自动隐藏';
  }, 1000);
  criticalReminderHideTimer = window.setTimeout(hideReminder, 10000);
  action.addEventListener('click', function() {
    hideReminder();
  });
}

function isAiDriveRecoveryTask(task) {
  const source = task?.leadInfo?.leadSource || task?.leadSource || '';
  return Boolean(task?.isDriveTask && String(source).includes('AI'));
}

function getAiDriveAssignmentLabel(task) {
  return task?.assignmentMode === 'supervisor' || task?.assignmentMode === '主管分配'
    ? '主管分配'
    : '系统自动分配';
}

function renderAiDriveSourceReminder(task) {
  const reminderVisitKey = task.id + '@' + aiDriveSourceReminderVisit;
  if (!isAiDriveRecoveryTask(task) || hiddenAiDriveSourceReminderVisits.has(reminderVisitKey)) return '';
  return `<section class="nt-ai-drive-source-reminder" data-ai-drive-source-reminder data-reminder-visit="${esc(reminderVisitKey)}" aria-label="AI外呼试驾回访任务提示">
    <span class="nt-ai-drive-source-reminder-icon" aria-hidden="true"><svg viewBox="0 0 16 16" focusable="false"><circle cx="8" cy="8" r="6.25"></circle><path d="M8 7v3.25M8 4.9h.01"></path></svg></span>
    <span class="nt-ai-drive-source-reminder-content"><strong>AI外呼试驾排程回访</strong><span>· ${esc(getAiDriveAssignmentLabel(task))}</span></span>
    <button type="button" class="nt-ai-drive-source-reminder-action" data-ai-drive-source-reminder-dismiss>10 秒后自动收起</button>
  </section>`;
}

function bindAiDriveSourceReminder(page) {
  if (aiDriveSourceReminderCountdownTimer) {
    window.clearInterval(aiDriveSourceReminderCountdownTimer);
    aiDriveSourceReminderCountdownTimer = null;
  }
  if (aiDriveSourceReminderHideTimer) {
    window.clearTimeout(aiDriveSourceReminderHideTimer);
    aiDriveSourceReminderHideTimer = null;
  }
  const reminder = page.querySelector('[data-ai-drive-source-reminder]');
  if (!reminder) return;

  const action = reminder.querySelector('[data-ai-drive-source-reminder-dismiss]');
  let seconds = 10;
  const hideReminder = function() {
    if (aiDriveSourceReminderCountdownTimer) window.clearInterval(aiDriveSourceReminderCountdownTimer);
    aiDriveSourceReminderCountdownTimer = null;
    if (aiDriveSourceReminderHideTimer) window.clearTimeout(aiDriveSourceReminderHideTimer);
    aiDriveSourceReminderHideTimer = null;
    hiddenAiDriveSourceReminderVisits.add(reminder.dataset.reminderVisit);
    reminder.remove();
  };
  aiDriveSourceReminderCountdownTimer = window.setInterval(function() {
    seconds -= 1;
    if (seconds <= 0) { hideReminder(); return; }
    action.textContent = seconds + ' 秒后自动收起';
  }, 1000);
  aiDriveSourceReminderHideTimer = window.setTimeout(hideReminder, 10000);
  action.addEventListener('click', hideReminder);
}

function renderTaskOverview(task) {
  const isDrive = Boolean(task.isDriveTask);
  const info = task.leadInfo || {};
  const configured = task.aiInsight || {};
  const hasCommunicationInsight = !isDrive && (configured.score != null || !!configured.summary);
  const ratingByLevel = {
    H: { text:'高', score:80, color:'#be123c' },
    A: { text:'中', score:68, color:'#d97706' },
    B: { text:'中', score:65, color:'#d97706' },
  ...
  const currentLeadStatus = info.leadStatus || task.tag || '培育中';
  ...
  const isOverdueTask = task.queue === 'drive_overdue' || task.queue === 'normal_overdue' || String(task.dueLabel || '').includes('逾期');
  const dueHtml = isOverdueTask ? `<strong style="color:#dc2626;">${esc(dueText)}</strong>` : esc(dueText);
  ...
  const driveInfo = task.driveInfo || {};
  const driveSummary = isDrive ? `客户原预约试驾<strong>${esc(driveInfo.appointmentDate || '')} ${esc(driveInfo.driveTimeSlot || '')}</strong>核销为<strong>${esc(task.tag || '逾期未到店')}</strong>。建议重点排查未到店真实顾虑，打消客户抗拒，核心争取<strong>【二次激活，重新预约排程】</strong>。` : '';
  const summary = configured.summary || (isDrive ? driveSummary : `当前评为<strong>${esc(rating.text)}等级</strong>：计划<strong>${esc(planTime)}</strong>购车，关注<strong>${esc(intentSeries)}</strong>；已产生<strong>${esc(followCount)}次</strong>跟进。建议结合客户关注点持续推进，优先确认购车计划与到店意向。`);
  return `<section class="nt-task-overview ${hasCommunicationInsight ? 'has-communication-insight' : 'no-communication-insight'}">
    <header class="nt-overview-head">
      <div class="nt-overview-customer">
        ...
        <div class="nt-overview-meta">${isDrive ? (() => {
          const badge = getDriveCancelPartyBadge(task);
          return `任务编号 ${esc(task.id)} · ${esc(badge.text)} · ${dueHtml}`;
        })() : (() => {
          const badge = getNurtureLeadStatusBadge(currentLeadStatus);
          return `任务编号 ${esc(task.id)} · ${esc(badge.text)} · ${dueHtml}`;
        })()}</div>
      </div>
    ...
    ${renderCriticalReminder(task)}
    ${renderAiDriveSourceReminder(task)}
    ${hasCommunicationInsight ? `<div class="nt-overview-insight">
      <h3>沟通提示</h3>
...
function renderRightPanel(task) {
  ...
  // 试驾排程任务专属：辅助信息栏移除红框入口（搜索框、智能预评、客户档案、更多），纯粹聚焦【线索详情】与【回访记录】
  if (isDrive) {
    if (activeRightTab !== 'details' && activeRightTab !== 'records' && activeRightTab !== 'timeline') {
    ...
    if (activeRightTab === 'timeline') {
      driveTabs.push({ k:'timeline', n:'时光轴' });
    }
    return `<aside class="nt-right-panel nt-right-panel-drive">
      ...
      <div class="nt-right-tabs nt-right-tabs-drive">
        ${driveTabs.map(t=>`<button type="button" class="nt-rt-tab ${activeRightTab===t.k?'active':''}" data-rtab="${t.k}">${esc(t.n)}${t.c!=null?`<span class="nt-rt-count">${t.c}</span>`:''}</button>`).join('')}
      </div>
...
window.renderNurtureTaskPage = function() {
  const page = document.getElementById('nurtureMyTaskWorkspace') || document.getElementById('nurtureTaskPage');
  if (!page) return;
  const task = getActiveTask();
  if (task.id !== activeCriticalReminderTaskId) {
    activeCriticalReminderTaskId = task.id;
    criticalReminderVisit += 1;
  }
  if (task.id !== activeAiDriveSourceReminderTaskId) {
    activeAiDriveSourceReminderTaskId = task.id;
    aiDriveSourceReminderVisit += 1;
  }
  ...
  page.innerHTML = `<div class="nt-page-shell">
    ...
    <div class="nt-workspace ${isNurtureAssistantCollapsed ? 'assistant-collapsed' : ''}">
      ...
      <main class="nt-center-panel">
        ...
        <div class="nt-center-scroll-body">${renderVisitForm(task)}</div>
      </main>
...
function bindEvents() {
  const page = document.getElementById('nurtureMyTaskWorkspace') || document.getElementById('nurtureTaskPage');
  if (!page) return;
  bindCriticalReminderCountdown(page);
  bindAiDriveSourceReminder(page);
  page.onclick = function(e) {
    ...
    if (topAction) {
      ...
      if (action === 'open-timeline') {
        isNurtureAssistantCollapsed = false;
    ...
    if (tc) {
      ...
      if (t) {
        activeNurtureTaskScope = t.isDriveTask ? 'drive' : 'nurture';
        if (t.isDriveTask) {
          if (activeRightTab !== 'details' && activeRightTab !== 'records' && activeRightTab !== 'timeline') {
            activeRightTab = 'details';
          }
    ...
    if (clr) {
      e.stopPropagation();
      setFormField(clr.dataset.clear, '');
      renderNurtureTaskPage();
      return;
    }
    ...
    // select展开/收起 (仅当点击触发按钮 .nt-select-trigger 时响应)
    const trigger = e.target.closest('.nt-select-trigger');
    if (trigger) {
      const sel = trigger.closest('.nt-custom-select');
    ...
    if (rt) {
      ...
      rightTabMoreOpen = false;
      if (rt.classList.contains('nurture-rating-result-link') || rt.classList.contains('nt-timeline-edge-tab')) isNurtureAssistantCollapsed = false;
      renderNurtureTaskPage(); return;
    }
    ...
    if (act) {
      ...
      if (a==='drive-pause') { showToast?.('试驾回访记录已暂存', true); return; }
      if (a==='drive-submit') {
        const t = getActiveTask();
        if (!t) return;
        const f = t.formData || {};
        if (!f.contactStatus) {
          showToast?.('请选择接触状态', false);
          return;
        }
        if (!f.visitResult) {
          showToast?.('请选择回访结果', false);
          return;
        }
        if (!(f.intentLevel ?? t.level)) {
          showToast?.('请选择意向级别', false);
          return;
        }
        if (!f.leadType) {
          showToast?.('请选择线索类型', false);
          return;
        }
        if (f.visitResult === 'next_visit' && !f.nextVisitTime) {
          showToast?.('回访结果为下次回访时，必须选择下次回访时间', false);
          return;
        }
        if (!f.visitDesc || !f.visitDesc.trim()) {
          showToast?.('请填写回访描述', false);
          return;
        }

        const csItem = driveContactStatusOptions.find(o => o.value === f.contactStatus || (o.value && f.contactStatus && o.value.toLowerCase() === String(f.contactStatus).toLowerCase()));
        const vrItem = getDriveVisitResultOptions().find(o => o.value === f.visitResult || o.label === f.visitResult)
          || driveVisitResultOptions.find(o => o.label === f.visitResult);
        const csLabel = csItem ? csItem.label : f.contactStatus;
        const vrLabel = vrItem && vrItem.value ? vrItem.label : f.visitResult;
        ...
        const stObj = intentStoreOptions.find(s => s.code === f.intentStore || s.name === f.intentStore);
        const storeName = stObj ? stObj.name : (f.intentStore || '成都东风日产高新店');
        const ltObj = driveLeadTypeOptions.find(o => o.value === f.leadType || o.label === f.leadType);
        const leadTypeName = ltObj ? ltObj.label : f.leadType;

        // 计算新轮次序号
        const nextRound = (t.visitRecords || []).length + 1;
        ...
        const newRecord = {
          ...
          agent: '张敏',
          time: timeStr,
          desc: f.visitDesc.trim(),
          remark: f.unfulfilledReason ? `未履约原因：${f.unfulfilledReason}` : '',
          contactStatus: csLabel,
          unfulfilledReason: f.unfulfilledReason || '',
          visitResult: vrLabel,
          intentLevel: f.intentLevel || t.level || 'B',
          intentStore: storeName,
          leadType: leadTypeName,
          nextVisitTime: f.nextVisitTime || '',
          followStatus: (f.visitResult === 'next_visit' || vrLabel === '下次回访') ? '跟进中' : ((f.visitResult === 'dispatch_store' || vrLabel === '下发门店') ? '已下发' : '已办结')
        };
        if (!t.visitRecords) t.visitRecords = [];
        t.visitRecords.forEach(r => { r.isLatest = false; });
        t.visitRecords.unshift(newRecord);
        activeRightTab = 'records';

        if (f.visitResult === 'next_visit' || f.visitResult === '下次回访') {
          showToast?.('回访记录提交成功！已生成下次回访任务并同步更新', true);
        } else if (f.visitResult === 'dispatch_store' || f.visitResult === '下发门店') {
          showToast?.('回访记录提交成功！试驾任务已下发至对应意向专营店', true);
        } else {
          showToast?.('回访记录提交成功！该试驾流失回访任务已归档闭环', true);
        }
        renderNurtureTaskPage();
        return;
      }
  ...
  // 试驾排程回访使用真实日期时间选择，保存格式保持与历史记录一致。
  page.querySelectorAll('[data-drive-date]').forEach(inp=>{
    inp.addEventListener('click', function(){
      if (this.disabled) return;
      try { this.showPicker?.(); } catch (_) { /* 保留浏览器原生输入交互 */ }
    });
    inp.addEventListener('change', function(){
      if (this.disabled) return;
      setFormField(this.dataset.driveDate, this.value.replace('T', ' '));
    });
  });
...
function setFormField(field, value) {
  ...
  if (field === 'contactStatus') {
    if (!task.isDriveTask) {
      ...
      if (!['CONNECTED', 'connected', 'CUSTOMER_REJECTED', 'rejected'].includes(value)) {
        task.formData.unfulfilledReason = '';
      }
    }
  }
  // 联动：切换回访结果清空原因与试驾下次回访时间联动
  if (field === 'visitResult') {
    task.formData.resultReason = '';
    if (task.isDriveTask) {
      if (value !== 'next_visit' && value !== '下次回访') {
        task.formData.nextVisitTime = '';
      }
    }
  }
  moreActionsOpen = false;
