...
// 试驾排程补充示例：覆盖当日跟进、逾期与未来任务队列，便于演示完整处理流程。
function makeDriveScheduleTask(data) {
  return {
    ...
    driveInfo: {
      ...
      cancelTag: '-',
      dccInitialStatus: '待回访'
    },
    leadInfo: makeLeadInfo({
      phone: data.phone, ipLocation: data.ipLocation, leadType: '试驾预约线索',
      leadSource: data.leadSource, firstLeadStatus: '已预约试驾', firstIntentLevel: data.level,
      intentCar: data.series, latestIntentSeries: data.series, followCount: data.followCount,
      createTime: data.createTime, leadStatus: data.driveStatus, leadDesc: data.leadDesc,
      ...
      channelName: data.channelName, projectName: '试驾预约跟进'
    }),
    formData: { contactStatus: '', gender: data.gender, visitResult: '', intentLevel: data.level, altPhone: '', lastVisitTime: '-', nextVisitTime: '', intentCar: data.series, intentStore: data.storeCode, purchaseType: '', planBuyTime: data.buyTime, wecomStatus: '', expectStoreTime: data.appointmentDate, resultReason: '', visitDesc: '', remark: '' },
    timelineEvents: [{ type: 'lead', title: '试驾预约', time: data.createTime.slice(0, 16), desc: data.leadDesc }],
    visitRecords: [],
...
nurtureTaskItems.push(
  makeDriveScheduleTask({
    id: 'NT-DR2', queue: 'drive_today', name: '李女士', phone: '136****5268', level: 'A', dueLabel: '今日 17:30',
    bookingCode: 'YYSD2026090900181', scheduleCode: 'NEVSP202609090050', driveVehicle: '粤AD9888（N6·珠光白）', driveSeries: 'N6（E03）— 智享版', driveTimeSlot: '17:30–18:00', appointmentDate: '2026-09-10', driveStatus: '待确认',
    ipLocation: '广东广州', leadSource: '小程序预约试驾', series: 'N6', followCount: '1次', createTime: '2026-09-09 11:15:00', leadDesc: '预约 N6 智享版到店试驾，待确认同行人数。', store: '广州东风日产天河店', storeCode: 'DLR-GD-GZ-001', buyTime: '1个月内', carModel: 'N6 2026款 智享版', leadRemark: '下班前确认到店时间与试驾车辆。', leadCode: '1846296533270721', taskCode: '2026090917300000031', channelName: '小程序', gender: '女'
  }),
  makeDriveScheduleTask({
    id: 'NT-DR3', queue: 'drive_overdue', name: '陈先生', phone: '139****3176', level: 'B', dueLabel: '逾期 2 h',
    bookingCode: 'YYSD2026090800156', scheduleCode: 'NEVSP202609080048', driveVehicle: '粤AE3108（天籁·曜石黑）', driveSeries: '天籁（L34）— 智驾版', driveTimeSlot: '10:00–10:30', appointmentDate: '2026-09-09', driveStatus: '待确认',
    ipLocation: '广东佛山', leadSource: '门店活动', series: '天籁', followCount: '2次', createTime: '2026-09-08 15:40:00', leadDesc: '车展现场预约天籁试驾，尚未确认到店。', store: '广州东风日产天河店', storeCode: 'DLR-GD-GZ-001', buyTime: '3个月内', carModel: '天籁 2.0T 智驾版', leadRemark: '已逾期，优先确认客户是否保留试驾安排。', leadCode: '1846296533270722', taskCode: '2026090910000000032', channelName: '车展活动', gender: '男'
  }),
  makeDriveScheduleTask({
    id: 'NT-DR4', queue: 'drive_future', name: '王女士', phone: '135****8602', level: 'B', dueLabel: '2d后 09:30',
    bookingCode: 'YYSD2026091000203', scheduleCode: 'NEVSP202609100054', driveVehicle: '粤AB5201（探陆·砂岩棕）', driveSeries: '探陆（T33）— 四驱版', driveTimeSlot: '09:30–10:00', appointmentDate: '2026-09-12', driveStatus: '待确认',
    ipLocation: '广东深圳', leadSource: '官网预约试驾', series: '探陆', followCount: '0次', createTime: '2026-09-10 09:05:00', leadDesc: '官网预约探陆四驱版试驾，计划周末到店。', store: '深圳东风日产南山店', storeCode: 'DLR-GD-SZ-002', buyTime: '1个月内', carModel: '探陆 380VC-TURBO 四驱版', leadRemark: '提前一天电话确认试驾时间。', leadCode: '1846296533270723', taskCode: '2026091209300000033', channelName: '官网', gender: '女'
  })
);
...
function renderTaskOverview(task) {
  const info = task.leadInfo || {};
  const configured = task.aiInsight || {};
  const hasCommunicationInsight = configured.score != null || !!configured.summary;
  const ratingByLevel = {
    H: { text:'高', score:80, color:'#be123c' },
    A: { text:'中', score:68, color:'#d97706' },
    B: { text:'中', score:65, color:'#d97706' },
...
window.renderNurtureTaskPage = function() {
  ...
  page.innerHTML = `<div class="nt-page-shell">
    ...
    <div class="nt-workspace ${isNurtureAssistantCollapsed ? 'assistant-collapsed' : ''}">
      ...
      <button type="button" class="nt-timeline-edge-tab" data-rtab="timeline" title="查看客户时光轴" aria-label="查看客户时光轴"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg><span>时光轴</span></button>
