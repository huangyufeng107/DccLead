...
// 1:1 引用系统试驾排程【回访结果】下拉枚举（依据现网实际选项：下次回访、回访完成、下发门店）
const driveVisitResultOptions = [
  { value: '', label: '请选择' },
  { value: 'next_visit', label: '下次回访' },
  { value: 'visit_completed', label: '回访完成' },
  { value: 'dispatch_store', label: '下发门店' }
];
      ...
      ${isNav ?
        ...
        `<div class="nt-custom-select" data-select="${field}" ${disabled ? 'data-disabled="1"' : ''}>
          <button type="button" class="nt-select-trigger" ${disabled ? 'disabled' : ''}>
            ...
            ${hasVal && !disabled ? `<span class="nt-select-clear" data-clear="${field}">×</span>` : ''}
  ...
  return `<section class="nt-section nt-visit-form-section" id="ntVisitForm">
    ...
    <div class="nt-section-body">
      <div class="nt-form-grid">
        ...
        ${selectHtml('visitResult', '回访结果', driveVisitResultOptions, vr, '请选择', false, false, true)}
        ...
        <div class="nt-form-item ${!isNextVisit ? 'nt-form-item-disabled' : ''}" data-field="nextVisitTime">
          ...
          <input type="text" class="nt-form-ctl nt-input nt-date-input" data-date="nextVisitTime" placeholder="${isNextVisit ? '请选择回访日期' : '回访结果为下次回访时填写'}" readonly ${!isNextVisit ? 'disabled' : ''} value="${esc(isNextVisit ? (f.nextVisitTime || '') : '')}"/>
        </div>
        ...
        <!-- 6、线索类型 -->
        ${selectHtml('leadType', '线索类型', driveLeadTypeOptions, f.leadType || '试驾留资线索', '请选择线索类型', false, false, true)}
      ...
      <div class="nt-form-actions">
        ...
        <button type="button" class="nt-btn nt-btn-default" data-action="drive-pause">暂存</button>
        <button type="button" class="nt-btn nt-btn-primary" data-action="drive-submit">提交回访记录</button>
        <div class="nt-more-wrap">
          ...
          <div class="nt-more-menu ${moreActionsOpen ? 'show' : ''}">
            <button type="button" data-action="sms">下发关怀短信</button>
            <button type="button" data-action="store-query">专营店车辆库存</button>
            <button type="button" data-action="drive-record">历史试驾记录</button>
          </div>
        </div>
      </div>
    </div>
  ...
  page.onclick = function(e) {
    ...
    if (act) {
      ...
      if (a==='drive-submit') {
        ...
        const vrItem = driveVisitResultOptions.find(o => o.value === f.visitResult || o.label === f.visitResult);
        ...
        const leadTypeName = ltObj ? ltObj.label : (f.leadType || '试驾留资线索');
