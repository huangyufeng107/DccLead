 ...
 */
(function () {
  const REPORT_PATH = 'DccLead.assets/employee-dimension-report.html?v=2026091413';
  const FRAME_ID = 'sdEmployeeOperationalReport';
  const EMPLOYEE_HEADER_TEXT = '按日期、时段及 10 项筛选维度查看员工执行数据，任务、话单、有效接通与后端转化分别按对应口径统计。';
  let headerObserver;
  ...
  function applyEmployeeHeader(root) {
    ...
    if (desc && desc.textContent !== EMPLOYEE_HEADER_TEXT) desc.textContent = EMPLOYEE_HEADER_TEXT;
  }
