...
(function () {
  ...
  window.toggleAiSummaryHelp = function (event) {
    event?.stopPropagation();
    const trigger = event?.currentTarget;
    const popover = trigger?.parentElement?.querySelector('.ai-summary-help-popover');
    if (!popover) return;
    document.querySelectorAll('.ai-summary-help-popover.is-open').forEach((item) => {
  ...
  window.showAiTestDriveScheduleDetail = function (id) {
    ...
    page.innerHTML = `<div class="detail-page-header"><div><div class="detail-page-title">中台推送预约线索详情</div><div class="detail-page-subtitle">NEV培育 / 试驾排程 / 中台推送监控 / 预约详情</div></div><button class="btn-secondary" type="button" onclick="returnAiTestDriveScheduleList()">返回列表</button></div>
      <div class="ai-schedule-detail-grid">
        ...
        <section class="card ai-schedule-detail-card"><div class="section-title">确认的试驾信息</div><div class="ai-detail-kv"><div><span>客户信息</span><b>${escapeHtml(row.customerName || (row.customer ? row.customer.split('/')[0].trim() : '—'))} <span style="font-size:12px; font-weight:400; color:#64748b; font-family:monospace; margin-left:4px;">(${escapeHtml(row.phone || (row.customer && row.customer.split('/')[1] ? row.customer.split('/')[1].trim() : '—'))})</span></b></div><div><span>意向车系</span><b>${escapeHtml(row.series)}</b></div><div><span>试驾专营店</span><b>${escapeHtml(row.dealer)}</b></div><div><span>预约日期/时段</span><b>${escapeHtml(row.date)} ${escapeHtml(row.time)}</b></div></div><div class="ai-transcript">${row.channel === 'AI外呼排程' ? `<div class="ai-transcript-heading"><span>AI外呼录音摘要</span><button type="button" class="ai-summary-help-trigger" aria-label="查看AI外呼录音摘要生成规则" onclick="toggleAiSummaryHelp(event)">!</button><div class="ai-summary-help-popover" role="dialog"><div class="ai-summary-help-title">AI外呼录音摘要生成规则</div><div class="ai-summary-help-body"><p><b>数据来源：</b>AI外呼通话录音及通话过程中的结构化识别结果。</p><p><b>生成步骤：</b>语音转写后，提取客户确认的车系、门店、预约日期与时段、试驾意向及外呼结果标签，由摘要模型压缩为可读纪要。</p><p><b>展示原则：</b>仅保留与试驾排程和后续跟进有关的事实，不替代 NEV 中台回执，也不等同于人工坐席沟通纪要。</p><p><b>无摘要时：</b>显示“暂无AI外呼录音摘要”，不使用人工纪要补写。</p></div></div></div>` : `<span>坐席建单沟通纪要</span>`}<p>${escapeHtml(row.transcript)}</p></div></section>
        <section class="card ai-schedule-detail-card"><div class="section-title">DCC 与 NEV 处理状态</div><div class="ai-detail-kv"><div><span>DCC处理结果</span>${statusTag(row.dcc)}</div><div><span>NEV推送状态</span>${statusTag(row.nev)}</div><div><span>最近处理时间</span><b>${escapeHtml(row.updated)}</b></div><div><span>后续处理方</span><b>NEV线索中台</b></div></div><div class="ai-state-note">数据传输遵循通话后结果标签统一裁决；若标签判定为不下发或转人工，预建工单自动作废并同步中台记录。</div>${row.nev === '待重试' || row.nev === '推送超时' || row.nev === '推送失败' ? `<button class="btn-add ai-detail-retry" type="button" onclick="retryAiTestDrivePush('${row.id}')">重试推送 NEV</button>` : ''}</section>
      </div>`;
  };
        ...
        <div style="background:#fff; width:94%; max-width:1300px; height:88vh; border-radius:12px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25); display:flex; flex-direction:column; overflow:hidden;">
          <header style="padding:16px 24px; background:#1e293b; color:#fff; display:flex; justify-content:space-between; align-items:center; border-bottom:3px solid #2563eb;">
            ...
            <div style="display:flex; gap:10px; align-items:center;">
              <button type="button" style="color:#93c5fd; border:1px solid #3b82f6; background:rgba(37,99,235,0.25); padding:6px 14px; font-size:12px; font-weight:600; border-radius:4px; cursor:pointer;" onclick="window.open('DccLead.assets/docs/DccLead_试驾排程场景旅程开发说明.html?v=2026092306', '_blank')">在新窗口打开完整手册 ↗</button>
          ...
          <div style="flex:1; overflow:hidden; background:#f8fafc;">
            <iframe src="DccLead.assets/docs/DccLead_试驾排程场景旅程开发说明.html?v=2026092306" style="width:100%; height:100%; border:none;"></iframe>
          </div>
