...
function renderSmsTable(data) {
  ...
  tbody.innerHTML = data.map(function(tpl, i) {
    ...
    return '<tr><td class="col-num">' + (i+1) + '</td><td><strong>' + escapeSmsHtml(tpl.name) + '</strong></td><td><span class="tag tag-processing">' + escapeSmsHtml(tpl.type) + '</span></td><td style="max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#595959" title="' + escapeSmsHtml(tpl.content) + '">' + escapeSmsHtml(tpl.content) + '</td><td><span class="tag ' + statusTag + '">' + escapeSmsHtml(tpl.status) + '</span></td><td style="color:#8c8c8c">' + escapeSmsHtml(tpl.createTime) + '</td><td><div class="sms-template-actions"><a href="javascript:void(0)" class="action-link" onclick="previewSms(' + tpl.id + ')">预览</a><a href="javascript:void(0)" class="action-link" onclick="editSms(' + tpl.id + ')">编辑</a><a href="javascript:void(0)" class="action-link" style="color:' + (tpl.status === '启用' ? '#ff4d4f' : '#52c41a') + '" onclick="toggleSmsStatus(' + tpl.id + ')">' + (tpl.status === '启用' ? '停用' : '启用') + '</a><a href="javascript:void(0)" class="action-link" style="color:#ff4d4f" onclick="deleteSms(' + tpl.id + ')">删除</a></div></td></tr>';
  }).join('');
}
...
function renderAllocationRulesPage() {
  ...
  page.innerHTML = 
    ...
    '<div class="card alloc-config-card">' +
      ...
      '<!-- R渠道分配 -->' +
      '<div class="alloc-form-row" style="margin-bottom:28px;">' +
        '<div class="alloc-form-label">R渠道分配：</div>' +
        '<div class="alloc-form-content">' +
          '<div class="alloc-channel-row">' +
            '<div class="alloc-channel-dropdown-container">' +
              ...
              '</div>' +
            '</div>' +
            '<div class="alloc-channel-hint">支持设置多渠道优先级队列；系统分派时自上而下依次匹配，最上方 R 渠道的任务优先分配。</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
