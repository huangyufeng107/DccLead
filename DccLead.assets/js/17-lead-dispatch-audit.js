// ===== AI Outbound Frequency Audit (线索分发审核) =====
...
function initLeadDispatchAuditPage() {
  ...
  page.innerHTML = `
    <div class="page-hero">
      <div>
        <div class="page-title">线索分发审核 · AI冷线索审核</div>
    ...
    <div class="card">
      <div class="section-header">
        ...
        <div class="action-btns" style="gap:8px;">
          <button class="btn-secondary" type="button" ${renderUiActionCallback('audit-rule-open')}>审核规则配置</button>
        ...
        <table class="data-table audit-lead-table" style="min-width: 2480px;">
          <thead>
            <tr>
              ...
              <th>意向车系</th>
              <th>意向专营店</th>
              <th style="width:90px;">门店冷线索</th>
              <th style="width:90px;">总部冷线索</th>
              <th style="width:90px;">三无忧线索</th>
              <th style="width:130px;">近${aiColdAuditRuleConfig.windowDays}天跟进频次</th>
              ...
              <th style="width:180px; text-align:left; padding-left:22px;">审核操作</th>
            </tr>
          </thead>
...
function renderLeadAuditSummary() {
  ...
  const cards = [
    ...
    { label: '已下发', value: approved, color: '#0f766e' },
...
function renderLeadAuditTable() {
  ...
  body.innerHTML = pageRows.length ? pageRows.map(item => {
    ...
    const isStore = item.coldTag === '门店冷线索' ? '<span style="color:#0f766e; font-weight:600;">是</span>' : '<span style="color:#64748b;">否</span>';
    const isHq = item.coldTag === '总部冷线索' ? '<span style="color:#0f766e; font-weight:600;">是</span>' : '<span style="color:#64748b;">否</span>';
    const isWorryFree = item.coldTag === '三无忧线索' ? '<span style="color:#0f766e; font-weight:600;">是</span>' : '<span style="color:#64748b;">否</span>';
    ...
    return `
      <tr class="audit-record-row ${isTranscriptOpenThis ? 'active' : ''}" ${renderUiActionCallback('audit-dispatch-open-transcript', [item.callId])}>
        ...
        <td>${isHq}</td>
        <td>${isWorryFree}</td>
        <td><span class="rule-chip blue" style="font-weight:400;">第 ${item.callCount} 次</span></td>
        ...
        <td style="text-align:left; padding-left:22px;">
          <div class="action-btns audit-action-cell" style="justify-content:flex-start;">
            ${isPending 
...
function openAiColdAuditRulePage() {
  ...
  setPageName('线索管理 / 线索分发审核 / AI冷线索审核 / 审核规则配置');
  page.innerHTML = `
    <div class="detail-page-header audit-rule-page-header">
      <div>
        <div class="detail-page-title">审核规则配置</div>
        <div class="detail-page-subtitle">线索管理 / 线索分发审核 / AI冷线索审核 / 审核规则配置</div>
      </div>
...
function closeAiColdAuditRulePage() {
  ...
  setPageName('线索管理 / 线索分发审核 / AI冷线索审核');
...
function renderDispatchSettingsStep() {
  ...
  if (dispatchSettingsCurrentStep === 1) {
    ...
    const selectedBatchRows = !isSingle
      ? dispatchSettingsSelectedIds.map(id => leadAuditList.find(item => item.id === id)).filter(Boolean)
      : [];
    const dealerFieldMarkup = keepOriginalIntent
      ? `<div class="form-group batch-original-intent-summary" style="width:100%;">
          <div class="form-label">原意向信息 <span class="form-field-hint">本次审核将按每条线索原车系、原专营店下发</span></div>
          <div class="batch-original-intent-list">
            ${selectedBatchRows.map(item => `<div class="batch-original-intent-row"><span>${escapeHtml(item.name)}（${escapeHtml(item.displayPhone)}）</span><b>${escapeHtml(item.carSeries)}</b><em>${escapeHtml(item.dealer)}</em></div>`).join('')}
          </div>
        </div>`
      : `<div class="form-group" style="width:100%;">
          ...
          <div class="resource-unfilled-smartcode-combobox" style="position:relative;">
            ...
            <div class="resource-unfilled-smartcode-dropdown wizard-dealer-dropdown" id="wizardFormDealerDropdown" style="width:100%; box-sizing:border-box; z-index:9999;">
              <div class="resource-unfilled-smartcode-dropdown-head"><span>可售专营店</span><button type="button" onclick="clearWizardDealer()">清空选择</button></div>
              <div class="wizard-dealer-filter-bar"><input class="form-input" id="wizardDealerSearch" placeholder="搜索专营店名称/编码" oninput="filterWizardDealerOptions(this.value)" /><select class="form-input" id="wizardDealerProvince" onchange="updateWizardDealerCityOptions()"></select><select class="form-input" id="wizardDealerCity" onchange="filterWizardDealerOptions()"></select></div>
    ...
    const batchHelperText = keepOriginalIntent
      ? '本次将按每条线索原有的意向车系和专营店分别下发，不会改写原意向信息。'
      : `已选择统一意向车系“${escapeHtml(selectedSeries)}”。请选择该车系的可售专营店；确认后将统一更新所选线索的车系和专营店。`;
    ...
    body.innerHTML = `
      ...
      <div class="dispatch-rule-form" style="padding:0;">
        <div class="dispatch-form-grid" style="display:flex; flex-direction:column; gap:16px;">
          ...
          ${!isSingle ? `<div class="form-field-helper-label">${batchHelperText}</div>` : ''}
...
registerUiActionCallback('audit-dispatch-settings-series-change', (selectedSeries, event) => {
  // 统一事件委托同时监听 click/change/input；仅在真正改选后重新渲染，避免点击下拉时阻断原生展开。
  if (event?.type !== 'change') return;
  const body = document.getElementById('dispatchSettingsModalBody');
  const series = selectedSeries || document.getElementById('wizardFormCarSeries')?.value || '';
  if (!body) return;
  // 车系选择决定批量审核模式：保留原意向显示逐条下发明细；选择具体车系后显示统一门店选择。
...
function switchLeadDispatchAuditTab(tab = 'lead-dispatch-audit-ai-cold') {
  ...
  if (tab === 'lead-dispatch-audit-ai-cold') {
    ...
    setPageName('线索管理 / 线索分发审核 / AI线索分发');
    ...
    setPageName('线索管理 / 线索分发审核 / 人工线索分发');
...
function renderWizardDealerOptions(selectedDealer = '') {
  ...
  const seriesSelect = document.getElementById('wizardFormCarSeries');
  const isKeepOriginal = seriesSelect?.value === keepOriginalIntentValue;
  ...
  list.innerHTML = eligibleDealers.map(item => {
    ...
    return `
      <label class="dealer-option ${isSelected ? 'selected' : ''}" data-dealer-option="${escapeHtml(item.name)}" data-dealer-province="${escapeHtml(item.province)}" data-dealer-city="${escapeHtml(item.city)}">
        ...
        <span>
          <div class="dealer-name">${escapeHtml(item.name)}</div>
          <div class="dealer-meta">${escapeHtml(item.code)}｜${escapeHtml(region)}</div>
        </span>
      </label>
    `;
  }).join('');
  ...
  if (empty) {
    if (!eligibleDealers.length && isKeepOriginal) {
      empty.textContent = '所选线索意向车系无共同可售门店，请选择具体意向车系后筛选可售专营店';
    } else {
      empty.textContent = '暂无符合筛选条件的可售专营店';
    }
  }
  filterWizardDealerOptions();
}
...
function selectWizardDealerFromPicker(input) {
  ...
  document.querySelectorAll('#wizardFormDealerList .dealer-option').forEach(option => {
...
function clearWizardDealer() {
  ...
  document.querySelectorAll('#wizardFormDealerList .dealer-option').forEach(option => option.classList.remove('selected'));
