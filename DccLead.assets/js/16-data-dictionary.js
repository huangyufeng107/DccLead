  ...
  container.innerHTML = `
    ...
    <div class="dictionary-layout">
      <aside class="dictionary-type-panel">
        ...
        ${DICTIONARY_TYPES.map(item => `<button type="button" class="dictionary-type-item ${item.code === type.code ? 'active' : ''}" onclick="selectDictionaryType('${item.code}')"><span><b>${dictionaryEscape(item.name)}</b><small>${dictionaryEscape(item.scope)} · ${item.count} 项</small></span><span class="dictionary-chevron">›</span></button>`).join('')}
      </aside>
      <section class="dictionary-content-panel">
        <div class="dictionary-title-row"><div><div class="dictionary-title">${dictionaryEscape(type.name)}</div><div class="dictionary-subtitle">${dictionaryEscape(type.description)}</div></div><div class="dictionary-title-actions">${type.importable === false ? '' : '<button class="btn-secondary" type="button" onclick="openDictionaryImportWizard()">导入数据</button><button class="btn-secondary" type="button" onclick="exportDictionaryItems()">导出数据</button>'}<button class="btn-add" type="button" onclick="openDictionaryItemModal()">新增数据</button></div></div>
        ${renderPolicyRuleNote('dictionaryRuleNote', '停用后，历史数据仍保留原值；新增或编辑业务数据时不可再选择该字典项。', { style: 'margin-top:16px' })}
        <div class="dictionary-filter-row unified-filter-query" id="dictionaryFilterQuery"><input class="form-input" value="${dictionaryEscape(dictionaryKeyword)}" placeholder="搜索编码、名称或说明" oninput="filterDictionaryItems(this.value)" /><select class="filter-select" onchange="filterDictionaryStatus(this.value)"><option value="">全部状态</option><option value="启用" ${dictionaryStatusFilter === '启用' ? 'selected' : ''}>启用</option><option value="停用" ${dictionaryStatusFilter === '停用' ? 'selected' : ''}>停用</option></select><button class="btn-secondary" type="button" onclick="resetDictionaryFilters()">重置</button></div>
