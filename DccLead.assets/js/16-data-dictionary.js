...
// ========== NEV可搜索单选下拉最小样式补充（复用tag-picker主体样式） ==========
(function injectNevSinglePickerStyles() {
  if (document.getElementById('nev-single-picker-styles')) return;
  const style = document.createElement('style');
  style.id = 'nev-single-picker-styles';
  style.textContent = `
    .nev-single-picker { width: 100%; position: relative; z-index: 100; }
    .nev-single-picker .tag-picker-trigger { width: 100%; min-height: 40px; box-sizing: border-box; text-align: left; justify-content: space-between; font-size: 14px; }
    .nev-single-picker .tag-picker-list { display: block; max-height: 240px; overflow-y: auto; padding: 6px 0; grid-template-columns: none; gap: 0; }
    .nev-single-picker .nev-option-hidden { display: none !important; }
    .nev-single-picker .tag-option {
      display: block;
      width: 100%;
    ...
    .nev-single-picker .tag-option::before, .nev-single-picker .tag-option input[type="checkbox"] { display: none; }
    .nev-single-picker .tag-option:hover { background-color: #f5f7fa; }
    .nev-single-picker .tag-option.selected { background-color: #eff6ff; color: #2563eb; font-weight: 600; }
    .nev-single-picker .tag-picker-search { padding: 8px; border-bottom: 1px solid #f0f0f0; }
    .nev-single-picker .tag-picker-search input {
      width: 100%;
      padding: 7px 10px;
    ...
    .nev-single-picker .tag-picker-search input:focus { border-color: #409eff; }
    .nev-single-picker .tag-picker-empty { padding: 20px; text-align: center; color: #909399; font-size: 13px; }
  `;
...
const DICTIONARY_TYPES = [
  ...
  { code: 'OUTBOUND_TYPE', name: '外呼类型', scope: '业务字典', description: '按所属外呼系统关联维护及车系外呼场景ID配置', count: 8, importable: false },
  ...
  { code: 'RETURN_VISIT_RESULT', name: '回访结果', scope: '业务字典', description: '回访结果公共枚举；下发门店、回访完成仅供试驾排程任务使用，各页面保留独立选项范围', count: 11, importable: false },
  { code: 'SKYNET_SMARTCODE', name: '天网smartcode', scope: '业务字典', description: '天网策略 SmartCode 编码映射与算法分类配置共用', count: 8, importable: false },
  ...
  { code: 'INTENT_LEVEL', name: '意向级别', scope: '业务字典', description: 'AI与人工外呼配置共用；各页面按现有业务规则筛选意向级别', count: 8, importable: false },
  { code: 'INITIAL_LEAD_STATUS', name: '线索状态', scope: '业务字典', description: '初始线索状态、首次线索状态等字段共用', count: 8, importable: false },
  { code: 'NEV_DEALER_WHITELIST', name: '经销商白名单', scope: '业务字典', description: '留资未满-经销商白名单配置', count: 2, importable: true },
  { code: 'NEV_TEST_DRIVE_STORE', name: '试驾排程门店', scope: '业务字典', description: '留资未满-试驾排程门店配置', count: 2, importable: true },
  { code: 'NEV_TEST_DRIVE_CITY', name: '试驾排程城市', scope: '业务字典', description: '留资未满-试驾排程城市配置', count: 2, importable: true },
  { code: 'NEV_TEST_DRIVE_DEALER', name: '赛马门店', scope: '业务字典', description: '留资未满-赛马门店配置（支持关联区域、小组、排程分数）', count: 7, importable: true },
  { code: 'NEV_HORSE_RACE_GROUP', name: '赛马小组', scope: '业务字典', description: '留资未满-赛马小组配置', count: 12, importable: true }
];
...
const DICTIONARY_ITEMS = {
  ...
  OUTBOUND_TYPE: [
    { code: 'KDXF_COLD', name: '科大讯飞-冷线索', outboundSystem: '科大', isSeriesRelated: '是', scenarioId: '', seriesScenarioMappings: [{ series: 'N6', scenarioId: 'KDXF_N6_1001' }, { series: '轩逸', scenarioId: 'KDXF_SYLPHY_1002' }, { series: '天籁', scenarioId: 'KDXF_TEANA_1003' }], defaultScenarioId: 'KDXF_DEF_9999', order: 10, status: '启用', refs: 6, description: '按车系差异化配置外呼话术场景' },
    { code: 'KDXF_UNFILLED', name: '科大讯飞-留资未满', outboundSystem: '科大', isSeriesRelated: '否', scenarioId: 'KDXF_UNFILLED_8001', seriesScenarioMappings: [], defaultScenarioId: '', order: 20, status: '启用', refs: 4, description: '统一使用全局外呼场景 ID' },
    { code: 'YIZHI_COLD', name: '一知-冷线索', outboundSystem: '一知', isSeriesRelated: '是', scenarioId: '', seriesScenarioMappings: [{ series: '逍客', scenarioId: 'YZ_QASHQAI_201' }, { series: '奇骏', scenarioId: 'YZ_XTRAIL_202' }], defaultScenarioId: 'YZ_DEF_2000', order: 30, status: '启用', refs: 3, description: '按车系差异化外呼场景' },
    { code: 'YIZHI_COLD_LLM', name: '一知-冷线索（大模型）', outboundSystem: '一知', isSeriesRelated: '否', scenarioId: 'YZ_LLM_GLOBAL_3001', seriesScenarioMappings: [], defaultScenarioId: '', order: 40, status: '启用', refs: 2, description: '大模型统一全局场景' },
    { code: 'BINGLAN_NEW', name: '冰兰-新线索', outboundSystem: '冰兰', isSeriesRelated: '否', scenarioId: 'BL_NEW_5001', seriesScenarioMappings: [], defaultScenarioId: '', order: 50, status: '启用', refs: 5, description: '通用新线索统一场景' },
    { code: 'BINGLAN_EXISTING', name: '冰兰-保客', outboundSystem: '冰兰', isSeriesRelated: '是', scenarioId: '', seriesScenarioMappings: [{ series: 'ARIYA', scenarioId: 'BL_ARIYA_601' }, { series: '探陆', scenarioId: 'BL_PATHFINDER_602' }], defaultScenarioId: '', order: 60, status: '启用', refs: 4, description: '按车系保客回访场景' },
    { code: 'YIZHI_NEV_LLM', name: '一知-大模型NEV新线索', outboundSystem: '一知', isSeriesRelated: '否', scenarioId: 'YZ_NEV_7001', seriesScenarioMappings: [], defaultScenarioId: '', order: 70, status: '启用', refs: 3, description: '外部外呼系统映射值' },
    { code: 'YIZHI_EXISTING', name: '一知-保客', outboundSystem: '一知', isSeriesRelated: '否', scenarioId: 'YZ_EXISTING_8001', seriesScenarioMappings: [], defaultScenarioId: '', order: 80, status: '启用', refs: 2, description: '外部外呼系统映射值' }
  ],
  OUTBOUND_TAG: [
  ...
  RETURN_VISIT_RESULT: [
    { code: 'DISPATCH_STORE', name: '下发门店', order: 100, status: '启用', refs: 1, description: '仅用于培育任务－我的任务－试驾排程任务' },
    { code: 'VISIT_COMPLETED', name: '回访完成', order: 110, status: '启用', refs: 1, description: '仅用于培育任务－我的任务－试驾排程任务' },
  ...
  SKYNET_SMARTCODE: [
    { code: 'SC_HIGH_INTENT_ACTIVATE', name: 'AI挖掘-高意向激活', order: 10, status: '启用', refs: 0, description: '天网SmartCode高意向实时唤醒策略映射' },
    { code: 'SC_UNFILLED_WAKEUP', name: 'AI挖掘-留资未满唤醒', order: 20, status: '启用', refs: 0, description: '天网SmartCode留资未满自动补全唤醒' },
    { code: 'SC_COLD_LEAD_REACTIVATE', name: 'AI挖掘-冷线索重激活', order: 30, status: '启用', refs: 0, description: '天网SmartCode冷线索AI多维度提权策略' },
    { code: 'SC_PRIVATE_DOMAIN_FREQ', name: 'AI挖掘-私域高频互动', order: 40, status: '启用', refs: 0, description: '天网SmartCode私域用户高频互动行为触发' },
    { code: 'SC_STORE_VISIT_REINVITE', name: 'AI挖掘-到店试驾二次邀约', order: 50, status: '启用', refs: 0, description: '天网SmartCode试驾断点二次到店邀约模型' },
    { code: 'SC_DEFEAT_RECOVERY', name: 'AI挖掘-战败线索挽回', order: 60, status: '启用', refs: 0, description: '天网SmartCode战败客群AI防流失模型' },
    { code: 'SC_DORMANT_REENGAGE', name: 'AI挖掘-休眠失联重触', order: 70, status: '启用', refs: 0, description: '天网SmartCode长期未联系休眠客群重触' },
    { code: 'SC_DEFAULT_FALLBACK', name: 'AI挖掘-通用兜底策略', order: 80, status: '启用', refs: 0, description: '天网SmartCode默认通用兜底编码' }
  ],
  INTENT_SERIES: [
  ...
  INTENT_LEVEL: [
    { code: 'E', name: 'E - E(智能体级别)', order: 45, status: '启用', refs: 2, description: '人工回访工单、高德地图配置中下次回访可用' },
    { code: 'H', name: 'H - H(高意向)', order: 10, status: '启用', refs: 12, description: '高意向线索' },
    { code: 'A', name: 'A - A(一周内买车)', order: 20, status: '启用', refs: 10, description: '计划一周内购车' },
    { code: 'B', name: 'B - B(计划三个月内买车)', order: 30, status: '启用', refs: 8, description: '计划三个月内购车' },
    { code: 'C', name: 'C - C(计划三个月后买车)', order: 40, status: '启用', refs: 6, description: '计划三个月后购车' },
    { code: 'F', name: 'F - F(战败)', order: 50, status: '启用', refs: 5, description: '战败线索' },
    { code: 'L', name: 'L - L(休眠)', order: 60, status: '启用', refs: 4, description: '休眠线索' },
    { code: 'N', name: 'N - N(阶段性战败)', order: 70, status: '启用', refs: 3, description: '阶段性战败线索' }
  ],
  INITIAL_LEAD_STATUS: [
  ...
  NEV_DEALER_WHITELIST: [
    { code: 'DL20240001', name: '广州东风南方中大店', order: 10, status: '启用', refs: 0, description: '经销商白名单-广州区域' },
    { code: 'DL20240002', name: '深圳东风南方华旭店', order: 20, status: '启用', refs: 0, description: '经销商白名单-深圳区域' }
  ],
  NEV_TEST_DRIVE_STORE: [
    { code: 'ST001', name: '广州天河体验中心', order: 10, status: '启用', refs: 0, description: '试驾排程门店-广州天河' },
    { code: 'ST002', name: '深圳南山体验中心', order: 20, status: '启用', refs: 0, description: '试驾排程门店-深圳南山' }
  ],
  NEV_TEST_DRIVE_CITY: [
    { code: 'CITY_GZ', name: '广州市', order: 10, status: '启用', refs: 0, description: '试驾排程城市-广东省广州市' },
    { code: 'CITY_ND', name: '宁德市', order: 20, status: '启用', refs: 0, description: '测试' }
  ],
  NEV_TEST_DRIVE_DEALER: [
    { code: 'A0906', name: '沈阳汇丰沈大', province: '辽宁', city: '沈阳', region: '和平区', relatedGroup: '王者队', scheduleScore: '85', order: 10, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'A0910', name: '沈阳中晨诚隆', province: '辽宁', city: '沈阳', region: '铁西区', relatedGroup: '荣耀队', scheduleScore: '82', order: 20, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'A0902', name: '沈阳新世纪', province: '辽宁', city: '沈阳', region: '沈河区', relatedGroup: '巅峰队', scheduleScore: '78', order: 30, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'A0909', name: '沈阳新世纪浑南', province: '辽宁', city: '沈阳', region: '浑南区', relatedGroup: '卓越队', scheduleScore: '80', order: 40, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'A0937', name: '沈阳东风南方辽沈', province: '辽宁', city: '沈阳', region: '大东区', relatedGroup: '领航队', scheduleScore: '88', order: 50, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'B0104', name: '北京东风南方亮马', province: '北京', city: '北京', region: '朝阳区', relatedGroup: '飞跃队', scheduleScore: '90', order: 60, status: '启用', refs: 0, description: '初始化赛马门店' },
    { code: 'S2935', name: '测试经销商', province: '', city: '', region: '', relatedGroup: '', scheduleScore: '12', order: 70, status: '启用', refs: 0, description: '12' }
  ],
  NEV_HORSE_RACE_GROUP: [
    { code: 'HRG_A1', name: '王者队', order: 10, status: '启用', refs: 0, description: '赛马小组-华南一区' },
    { code: 'HRG_A2', name: '荣耀队', order: 20, status: '启用', refs: 0, description: '赛马小组-华南二区' },
    { code: 'HRG_A3', name: '巅峰队', order: 30, status: '启用', refs: 0, description: '赛马小组-华东一区' },
    { code: 'HRG_A4', name: '卓越队', order: 40, status: '启用', refs: 0, description: '赛马小组-华东二区' },
    { code: 'HRG_A5', name: '领航队', order: 50, status: '启用', refs: 0, description: '赛马小组-华北一区' },
    { code: 'HRG_A6', name: '飞跃队', order: 60, status: '启用', refs: 0, description: '赛马小组-华北二区' },
    { code: 'HRG_A7', name: '冲锋队', order: 70, status: '启用', refs: 0, description: '赛马小组-西南一区' },
    { code: 'HRG_A8', name: '雄鹰队', order: 80, status: '启用', refs: 0, description: '赛马小组-西南二区' },
    { code: 'HRG_A9', name: '猛虎队', order: 90, status: '启用', refs: 0, description: '赛马小组-东北一区' },
    { code: 'HRG_A10', name: '战狼队', order: 100, status: '启用', refs: 0, description: '赛马小组-东北二区' },
    { code: 'HRG_A11', name: '尖刀队', order: 110, status: '启用', refs: 0, description: '赛马小组-华中一区' },
    { code: 'HRG_A12', name: '精英队', order: 120, status: '启用', refs: 0, description: '赛马小组-华中二区' }
  ]
};

const NEV_SELECT_OPTIONS = {
  NEV_TEST_DRIVE_STORE: ['广州天河体验中心', '深圳南山体验中心', '北京朝阳体验中心', '上海浦东体验中心', '成都武侯体验中心', '杭州西湖体验中心', '武汉江汉体验中心'],
  NEV_TEST_DRIVE_CITY: ['广州市', '深圳市', '北京市', '上海市', '成都市', '杭州市', '武汉市', '南京市', '重庆市', '宁德市', '沈阳市', '大连市'],
  NEV_TEST_DRIVE_DEALER: ['广州东风南方汽车销售服务有限公司', '深圳东风南方汽车销售服务有限公司', '北京东风南方汽车销售服务有限公司', '上海东风南方汽车销售服务有限公司', '沈阳汇丰沈大', '沈阳中晨诚隆', '沈阳新世纪', '沈阳新世纪浑南', '沈阳东风南方辽沈', '北京东风南方亮马']
};

const NEV_DEALER_ADDRESS_MAP = {
  '广州东风南方汽车销售服务有限公司': { province: '广东', city: '广州', region: '天河区' },
  '深圳东风南方汽车销售服务有限公司': { province: '广东', city: '深圳', region: '南山区' },
  '北京东风南方汽车销售服务有限公司': { province: '北京', city: '北京', region: '朝阳区' },
  '上海东风南方汽车销售服务有限公司': { province: '上海', city: '上海', region: '浦东新区' },
  '沈阳汇丰沈大': { province: '辽宁', city: '沈阳', region: '和平区' },
  '沈阳中晨诚隆': { province: '辽宁', city: '沈阳', region: '铁西区' },
  '沈阳新世纪': { province: '辽宁', city: '沈阳', region: '沈河区' },
  '沈阳新世纪浑南': { province: '辽宁', city: '沈阳', region: '浑南区' },
  '沈阳东风南方辽沈': { province: '辽宁', city: '沈阳', region: '大东区' },
  '北京东风南方亮马': { province: '北京', city: '北京', region: '朝阳区' }
};
...
let selectedDictionaryCode = 'SKYNET_SMARTCODE';
...
function renderDictionaryManager() {
  ...
  const isNevDealerDictionary = type.code === 'NEV_TEST_DRIVE_DEALER';
  const relationHeader = isAbnormalReasonDictionary 
    ? '<th>适用线索状态</th>' 
    : (isOutboundTypeDictionary 
      ? '<th>所属外呼系统</th><th>关联车系</th><th>外呼场景 ID 配置</th>' 
      : (isNevDealerDictionary
        ? '<th>关联省份</th><th>关联城市</th><th>关联区域</th><th>赛马小组</th><th>排程分数</th>'
        : ''));
  const relationCell = item => {
    ...
    if (isOutboundTypeDictionary) {
      const isRelated = item.isSeriesRelated === '是';
      const mappings = item.seriesScenarioMappings || [];
      const mappingTitle = mappings.map(m => m.series + ' ➔ ' + m.scenarioId).join('\n') + (item.defaultScenarioId ? '\n默认兜底: ' + item.defaultScenarioId : '');
      const scenarioText = isRelated
        ? (mappings.length > 0
          ? `<span class="scenario-tag-badge" title="${dictionaryEscape(mappingTitle)}">已映射 ${mappings.length} 个车系</span>`
          : '<span class="dictionary-empty">未配置车系</span>')
        : (item.scenarioId ? `<code>${dictionaryEscape(item.scenarioId)}</code>` : '<span class="dictionary-empty">未配置</span>');
      return `<td><span class="dictionary-system-tag">${dictionaryEscape(getOutboundTypeOutboundSystem(item) || '未配置')}</span></td>
              <td><span class="status-pill-tag ${isRelated ? 'status-pill-yes' : 'status-pill-no'}">${isRelated ? '是' : '否'}</span></td>
              <td>${scenarioText}</td>`;
    }
    if (isNevDealerDictionary) {
      return `<td>${dictionaryEscape(item.province || '-')}</td>
              <td>${dictionaryEscape(item.city || '-')}</td>
              <td>${dictionaryEscape(item.region || '-')}</td>
              <td>${dictionaryEscape(item.relatedGroup || '-')}</td>
              <td><strong>${dictionaryEscape(item.scheduleScore || '-')}</strong></td>`;
    }
    return '';
  };
  let colSpanCount = 7;
  if (isAbnormalReasonDictionary) colSpanCount = 8;
  if (isOutboundTypeDictionary) colSpanCount = 10;
  if (isNevDealerDictionary) colSpanCount = 12;
  ...
  container.innerHTML = `
    ...
    <div class="dictionary-layout">
      ...
      <section class="dictionary-content-panel">
        <div class="dictionary-title-row"><div><div class="dictionary-title">${dictionaryEscape(type.name)}</div><div class="dictionary-subtitle">${dictionaryEscape(type.description)}</div></div><div class="dictionary-title-actions">${type.importable === false ? '' : '<button class="btn-secondary" type="button" data-ui-action="dictionary-import-open">导入数据</button><button class="btn-secondary" type="button" data-ui-action="dictionary-export-items">导出数据</button>'}<button class="btn-add" type="button" data-ui-action="dictionary-item-open-create">新增数据</button></div></div>
...
registerUiAction('dictionary-select-type', (target, event) => { if (event.type === 'click') selectDictionaryType(target.dataset.dictionaryCode); });
registerUiAction('dictionary-filter-keyword', (target, event) => { if (event.type === 'input') filterDictionaryItems(target.value); });
registerUiAction('dictionary-filter-status', (target, event) => { if (event.type === 'change') filterDictionaryStatus(target.value); });
registerUiAction('dictionary-filter-reset', (target, event) => { if (event.type === 'click') resetDictionaryFilters(); });

function getDictionaryItem(code) { return (DICTIONARY_ITEMS[selectedDictionaryCode] || []).find(item => item.code === code); }

function renderOutboundTypeScenarioConfig(isRelated, itemData = null) {
  const container = document.getElementById('outboundTypeScenarioConfigArea');
  if (!container) return;

  if (isRelated === '否') {
    const scenarioVal = itemData ? (itemData.scenarioId || '') : (document.getElementById('dictionaryItemScenarioId')?.value || '');
    container.innerHTML = `
      <div class="form-group">
        <div class="form-label">外呼场景 ID <span class="required">*</span></div>
        <input class="form-input" id="dictionaryItemScenarioId" value="${dictionaryEscape(scenarioVal)}" placeholder="请输入统一外呼场景 ID，例：SCENE_1001">
        <div class="form-hint">未关联车系时，所有归属于该外呼类型的任务统一使用此场景 ID。</div>
      </div>
    `;
  } else {
    const mappings = itemData?.seriesScenarioMappings || [];
    const defaultScenarioId = itemData ? (itemData.defaultScenarioId || '') : (document.getElementById('dictionaryItemDefaultScenarioId')?.value || '');

    container.innerHTML = `
      <div class="form-group">
        <div class="form-label">车系 - 外呼场景映射配置 <span class="required">*</span></div>
        <div class="series-scenario-table-wrap">
          <table class="series-scenario-table">
            <thead>
              <tr>
                <th style="width: 45%;">目标车系 <span class="required">*</span></th>
                <th style="width: 45%;">外呼场景 ID <span class="required">*</span></th>
                <th style="width: 10%; text-align: center;">操作</th>
              </tr>
            </thead>
            <tbody id="seriesScenarioRowsContainer">
            </tbody>
          </table>
          <div class="series-scenario-table-actions">
            <button type="button" class="btn-secondary btn-sm" onclick="addSeriesScenarioRow()">
              + 添加车系映射
            </button>
          </div>
        </div>
        <div class="form-hint">为不同意向车系单独配置对应的外呼场景 ID。</div>
      </div>
      <div class="form-group" style="margin-top: 10px;">
        <div class="form-label">默认兜底场景 ID <small style="color:#8c8c8c;font-weight:normal">(选填)</small></div>
        <input class="form-input" id="dictionaryItemDefaultScenarioId" value="${dictionaryEscape(defaultScenarioId)}" placeholder="请输入默认兜底场景 ID，例：SCENE_DEF_000">
        <div class="form-hint">未在上方表格中单独配置的目标车系，外呼时将自动回退使用此场景 ID。</div>
      </div>
    `;

    const tbody = document.getElementById('seriesScenarioRowsContainer');
    if (mappings && mappings.length > 0) {
      mappings.forEach(m => addSeriesScenarioRow(m.series, m.scenarioId));
    } else {
      addSeriesScenarioRow();
    }
  }
}

function addSeriesScenarioRow(selectedSeries = '', scenarioIdVal = '') {
  const tbody = document.getElementById('seriesScenarioRowsContainer');
  if (!tbody) return;

  const allSeries = (DICTIONARY_ITEMS.INTENT_SERIES || [])
    .filter(s => s.status === '启用')
    .map(s => s.name);
  if (allSeries.length === 0) {
    allSeries.push('N6', '轩逸', '天籁', '逍客', '奇骏', 'ARIYA', '探陆', '其他车系');
  }

  const tr = document.createElement('tr');
  tr.className = 'series-scenario-row';

  const optionsHtml = allSeries.map(s => {
    return `<option value="${dictionaryEscape(s)}" ${s === selectedSeries ? 'selected' : ''}>${dictionaryEscape(s)}</option>`;
  }).join('');

  tr.innerHTML = `
    <td>
      <select class="form-input series-select">
        <option value="">请选择车系</option>
        ${optionsHtml}
      </select>
    </td>
    <td>
      <input class="form-input scenario-input" value="${dictionaryEscape(scenarioIdVal)}" placeholder="请输入对应的外呼场景 ID" />
    </td>
    <td style="text-align: center;">
      <button type="button" class="btn-text-danger" onclick="removeSeriesScenarioRow(this)" title="删除此映射">删除</button>
    </td>
  `;
  tbody.appendChild(tr);
}

function removeSeriesScenarioRow(btn) {
  const tr = btn.closest('tr');
  const tbody = tr?.parentElement;
  if (tbody && tbody.querySelectorAll('tr').length <= 1) {
    showToast('请至少保留一行车系场景映射', false);
    return;
  }
  tr?.remove();
}

function toggleOutboundTypeSeriesRelated(value) {
  renderOutboundTypeScenarioConfig(value);
}

function autoGenerateNevCode(name) {
  const codeInput = document.getElementById('dictionaryItemCode');
  if (!codeInput || !name) return;
  const typeCode = selectedDictionaryCode;
  const prefixMap = {
    NEV_TEST_DRIVE_STORE: 'ST',
    NEV_TEST_DRIVE_CITY: 'CITY',
    NEV_TEST_DRIVE_DEALER: 'DLR'
  };
  const prefix = prefixMap[typeCode] || 'NEV';
  const existingCodes = new Set((DICTIONARY_ITEMS[typeCode] || []).map(i => i.code));
  let seq = 1;
  let code;
  do {
    code = `${prefix}_${String(seq).padStart(3, '0')}`;
    seq++;
...
function openDictionaryItemModal(code = '') {
  ...
  const isOutboundTypeDictionary = type.code === 'OUTBOUND_TYPE';
  const isNevDealerDictionary = type.code === 'NEV_TEST_DRIVE_DEALER';
  const isNevSelectDictionary = ['NEV_TEST_DRIVE_STORE', 'NEV_TEST_DRIVE_CITY', 'NEV_TEST_DRIVE_DEALER'].includes(type.code);
  const nevSelectOptions = NEV_SELECT_OPTIONS[type.code] || [];
  const nevFieldLabel = {
    NEV_TEST_DRIVE_STORE: '试驾排程门店',
    NEV_TEST_DRIVE_CITY: '试驾排程城市',
    NEV_TEST_DRIVE_DEALER: '赛马门店'
  }[type.code] || '显示名称';
  ...
  const horseRaceGroups = (DICTIONARY_ITEMS.NEV_HORSE_RACE_GROUP || []).filter(g => g.status === '启用').sort((a, b) => a.order - b.order);
  ...
  const nameField = isNevSelectDictionary
    ? `<div class="form-group">
        <div class="form-label">${nevFieldLabel} <span class="required">*</span></div>
        ${renderSearchableSelect('nevItemNamePicker', 'dictionaryItemName', nevFieldLabel, nevSelectOptions, item?.name || '', '请选择', '点击展开可搜索选择，选择后字典项编码将自动生成。')}
      </div>`
    : `<div class="form-group"><div class="form-label">显示名称 <span class="required">*</span></div><input class="form-input" id="dictionaryItemName" value="${dictionaryEscape(item?.name || '')}" placeholder="请输入显示名称"></div>`;
  const groupSelectHtml = isNevDealerDictionary
    ? renderSearchableSelect('nevGroupPicker', 'nevDealerGroup', '赛马小组', horseRaceGroups.map(g => g.name), item?.relatedGroup || '', '请选择', '')
    : '';
  const nevDealerFields = isNevDealerDictionary
    ? `<div class="form-group"><div class="form-label">关联省份</div><input class="form-input is-autofill-readonly" id="nevDealerProvince" value="${dictionaryEscape(item?.province || '')}" placeholder="选择门店后自动回填" readonly></div>
       <div class="form-group"><div class="form-label">关联城市</div><input class="form-input is-autofill-readonly" id="nevDealerCity" value="${dictionaryEscape(item?.city || '')}" placeholder="选择门店后自动回填" readonly></div>
       <div class="form-group"><div class="form-label">关联区域</div><input class="form-input is-autofill-readonly" id="nevDealerRegion" value="${dictionaryEscape(item?.region || '')}" placeholder="选择门店后自动回填" readonly></div>
       <div class="form-hint" style="margin-top:-8px;margin-bottom:16px;">关联省份、城市、区域根据所选门店自动解析回填，无需手动填写。</div>
       <div class="form-group"><div class="form-label">赛马小组 <span class="required">*</span></div>${groupSelectHtml}</div>
       <div class="form-group"><div class="form-label">排程分数 <span class="required">*</span></div><input type="text" inputmode="numeric" class="form-input" id="nevDealerScore" value="${dictionaryEscape(item?.scheduleScore || '')}" placeholder="请输入非负整数" oninput="this.value=this.value.replace(/[^0-9]/g,'')"><div class="form-hint">非负整数，用于试驾排程优先级排序，数字越大优先级越高。</div></div>`
    : '';
  
  const isSeriesRelatedVal = item?.isSeriesRelated || '否';
  const isSeriesRelatedField = isOutboundTypeDictionary
    ? `<div class="form-group">
        <div class="form-label">是否关联车系 <span class="required">*</span></div>
        <div class="radio-group-row">
          <label><input type="radio" name="dictionaryItemIsSeriesRelated" value="否" ${isSeriesRelatedVal === '否' ? 'checked' : ''} onchange="toggleOutboundTypeSeriesRelated(this.value)"> 否 (全局统一场景)</label>
          <label><input type="radio" name="dictionaryItemIsSeriesRelated" value="是" ${isSeriesRelatedVal === '是' ? 'checked' : ''} onchange="toggleOutboundTypeSeriesRelated(this.value)"> 是 (按车系差异化配置)</label>
        </div>
        <div class="form-hint">开启后可针对不同意向车系配置独立的外呼场景 ID。</div>
      </div>
      <div id="outboundTypeScenarioConfigArea"></div>`
    : '';
  ...
  modal.innerHTML = `<div class="modal dictionary-item-modal"><div class="modal-header"><div class="modal-title">${item ? '编辑字典项' : '新增字典项'}</div><button class="modal-close" type="button" data-ui-action="dictionary-item-close">×</button></div><div class="modal-body"><div class="dictionary-modal-type">所属字典：<b>${dictionaryEscape(type.name)}</b><span>${dictionaryEscape(type.scope)}</span></div><div class="form-group"><div class="form-label">字典项编码 <span class="required">*</span></div><input class="form-input" id="dictionaryItemCode" value="${dictionaryEscape(item?.code || '')}" placeholder="选择名称后自动生成" ${item || isNevSelectDictionary ? 'disabled' : ''}><div class="form-hint">${isNevSelectDictionary ? '选择显示名称后自动生成编码' : '编码保存后不可修改，用于系统和历史数据识别。'}</div></div>${nameField}${relationField}${outboundSystemField}${isSeriesRelatedField}${nevDealerFields}<div class="form-group"><div class="form-label">排序</div><input class="form-input" id="dictionaryItemOrder" type="number" min="1" value="${item?.order || 99}"><div class="form-hint">数字越小，页面下拉列表中越靠前。</div></div><div class="form-group"><div class="form-label">说明</div><textarea class="form-textarea" id="dictionaryItemDesc" placeholder="请输入添加原因或使用说明">${dictionaryEscape(item?.description || '')}</textarea><div class="form-hint">对应现网"添加原因"字段。</div></div>${item?.refs ? `<div class="dictionary-reference-warning">该字典项已被 ${item.refs} 处业务数据或规则引用。可修改名称、排序或停用；如需移除，请先替换引用关系。</div>` : ''}</div><div class="modal-footer"><button class="btn-secondary" type="button" data-ui-action="dictionary-item-close">取消</button><button class="btn-primary" type="button" onclick="saveDictionaryItem('${code}')">保存</button></div></div>`;
  modal.classList.add('show');

  if (isOutboundTypeDictionary) {
    renderOutboundTypeScenarioConfig(isSeriesRelatedVal, item);
  }
}

registerUiAction('dictionary-item-open-create', openDictionaryItemModal);

function closeDictionaryItemModal() { document.getElementById('dictionaryItemModal')?.classList.remove('show'); }
registerUiAction('dictionary-item-close', closeDictionaryItemModal);

function saveDictionaryItem(existingCode = '') {
  const code = document.getElementById('dictionaryItemCode')?.value.trim();
  ...
  let outboundSystem = '';
  let isSeriesRelated = '否';
  let scenarioId = '';
  let seriesScenarioMappings = [];
  let defaultScenarioId = '';

  if (selectedDictionaryCode === 'OUTBOUND_TYPE') {
    outboundSystem = document.getElementById('dictionaryItemOutboundSystem')?.value || '';
    if (!outboundSystem) { showToast('请选择所属外呼系统', false); return; }

    const radioChecked = document.querySelector('input[name="dictionaryItemIsSeriesRelated"]:checked');
    isSeriesRelated = radioChecked ? radioChecked.value : '否';

    if (isSeriesRelated === '否') {
      scenarioId = document.getElementById('dictionaryItemScenarioId')?.value.trim() || '';
      if (!scenarioId) { showToast('请输入外呼场景 ID', false); return; }
    } else {
      defaultScenarioId = document.getElementById('dictionaryItemDefaultScenarioId')?.value.trim() || '';
      const rows = document.querySelectorAll('#seriesScenarioRowsContainer tr');
      const selectedSeriesSet = new Set();
      
      for (let row of rows) {
        const seriesVal = row.querySelector('.series-select')?.value.trim();
        const scenarioVal = row.querySelector('.scenario-input')?.value.trim();
        if (!seriesVal) { showToast('请在所有车系映射行中选择目标车系', false); return; }
        if (!scenarioVal) { showToast('请在所有车系映射行中输入外呼场景 ID', false); return; }
        if (selectedSeriesSet.has(seriesVal)) { showToast(`车系「${seriesVal}」在映射表中重复配置，请合并或删除`, false); return; }
        selectedSeriesSet.add(seriesVal);
        seriesScenarioMappings.push({ series: seriesVal, scenarioId: scenarioVal });
      }

      if (seriesScenarioMappings.length === 0) {
        showToast('关联车系为“是”时，请至少配置一个车系映射关系', false); return;
      }
    }
  }

  if (!code || !/^[A-Za-z0-9][A-Za-z0-9_-]*$/.test(code)) { showToast('请输入由字母、数字、下划线或中划线组成的字典项编码', false); return; }
  ...
  let province = '', city = '', region = '', relatedGroup = '', scheduleScore = '';
  if (selectedDictionaryCode === 'NEV_TEST_DRIVE_DEALER') {
    province = document.getElementById('nevDealerProvince')?.value.trim() || '';
    city = document.getElementById('nevDealerCity')?.value.trim() || '';
    region = document.getElementById('nevDealerRegion')?.value.trim() || '';
    relatedGroup = document.getElementById('nevDealerGroup')?.value || '';
    scheduleScore = document.getElementById('nevDealerScore')?.value.trim() || '';
    if (!relatedGroup) { showToast('请选择赛马小组', false); return; }
    if (!scheduleScore) { showToast('请输入排程分数', false); return; }
    if (isNaN(Number(scheduleScore)) || Number(scheduleScore) < 0) { showToast('排程分数必须是非负数字', false); return; }
  }

  const items = DICTIONARY_ITEMS[selectedDictionaryCode] || (DICTIONARY_ITEMS[selectedDictionaryCode] = []);
  const extraFields = selectedDictionaryCode === 'ABNORMAL_REASON' 
    ? { applicableLeadStatuses } 
    : (selectedDictionaryCode === 'OUTBOUND_TYPE' ? { outboundSystem, isSeriesRelated, scenarioId, seriesScenarioMappings, defaultScenarioId } 
      : (selectedDictionaryCode === 'NEV_TEST_DRIVE_DEALER' ? { province, city, region, relatedGroup, scheduleScore } : {}));

  if (existingCode) {
    const item = getDictionaryItem(existingCode);
    if (!item) return;
    Object.assign(item, { name, order, description, ...extraFields });
    showToast('字典项已保存', true);
  } else {
    if (items.some(item => item.code === code)) { showToast('字典项编码已存在', false); return; }
    items.push({ code, name, order, description, status: '启用', refs: 0, ...extraFields });
...
function openDictionaryImportWizard() {
  ...
  modal.innerHTML = `<div class="modal assignment-import-modal"><div class="modal-header"><div class="modal-title" id="dictionaryImportTitle">导入${dictionaryEscape(type.name)}</div><button class="modal-close" type="button" data-ui-action="dictionary-import-close">✕</button></div><div class="modal-body" id="dictionaryImportBody"></div><div class="modal-footer" id="dictionaryImportFooter"></div></div>`;
  modal.classList.add('show');
...
function renderDictionaryImportComplete() {
  ...
  return `<div class="assignment-import-complete"><div class="import-complete-title">导入完成</div><div class="assignment-import-complete-desc">新增 <strong>${result.created}</strong> 项，更新 <strong>${result.updated}</strong> 项，失败 <strong>${result.failed}</strong> 项。</div></div>`;
}

function renderDictionaryImportFooter() {
  const pass = dictionaryImportState.validatedRows.filter(row => row.valid).length;
  if (dictionaryImportState.step === 1) return `<button class="btn-cancel" type="button" data-ui-action="dictionary-import-close">取消</button><button class="btn-save" type="button" ${dictionaryImportState.validatedRows.length ? '' : 'disabled'} onclick="goDictionaryImportPreview()">下一步</button>`;
  ...
  return `<button class="btn-save" type="button" data-ui-action="dictionary-import-close">完成</button>`;
}

registerUiAction('dictionary-import-close', closeDictionaryImportWizard);
...
// ========== 通用可搜索单选下拉组件 ==========
function renderSearchableSelect(pickerId, hiddenInputId, label, options, selectedValue, placeholder, hint) {
  const displayText = selectedValue || '请选择';
  const placeholderClass = selectedValue ? '' : 'placeholder';
  return `
    <div class="tag-picker nev-single-picker" id="${pickerId}">
      <button type="button" class="tag-picker-trigger ${placeholderClass}" onclick="toggleNevSearchableSelect(event, '${pickerId}', '${hiddenInputId}')">
        <span data-picker-summary>${dictionaryEscape(displayText)}</span>
      </button>
      <div class="tag-picker-panel" id="${pickerId}Panel" onclick="event.stopPropagation()">
        <div class="tag-picker-search">
          <input type="text" id="${pickerId}Search" placeholder="搜索${label}" oninput="filterNevSearchableOptions(this.value, '${pickerId}')" onclick="event.stopPropagation()" />
        </div>
        <div class="tag-picker-list" id="${pickerId}List">
          ${options.map(opt => `
            <label class="tag-option ${selectedValue === opt ? 'selected' : ''}" data-value="${dictionaryEscape(opt)}" onclick="selectNevSearchableOption('${dictionaryEscape(opt)}', '${pickerId}', '${hiddenInputId}'); event.stopPropagation()">
              <span>${dictionaryEscape(opt)}</span>
            </label>
          `).join('')}
        </div>
        <div class="tag-picker-empty" id="${pickerId}Empty" style="display:none;">暂无匹配选项</div>
      </div>
    </div>
    <input type="hidden" id="${hiddenInputId}" value="${dictionaryEscape(selectedValue || '')}" />
    ${hint ? `<div class="form-hint">${hint}</div>` : ''}
  `;
}

// ========== NEV可搜索单选下拉交互 ==========
let _nevSearchableClickOutside = null;
let _currentActivePicker = null;

function toggleNevSearchableSelect(event, pickerId, hiddenInputId) {
  ...
  // 先关闭所有其他打开的下拉
  document.querySelectorAll('.nev-single-picker .tag-picker-panel.show').forEach(p => {
    if (p !== panel) {
      p.classList.remove('show');
      p.closest('.nev-single-picker').querySelector('.tag-picker-trigger').classList.remove('open');
    }
  });
  ...
  if (isOpen) {
    ...
    _currentActivePicker = null;
    if (_nevSearchableClickOutside) {
      document.removeEventListener('click', _nevSearchableClickOutside);
    }
  } else {
    panel.classList.add('show');
    trigger.classList.add('open');
    _currentActivePicker = pickerId;
    setTimeout(() => {
      const searchInput = document.getElementById(pickerId + 'Search');
      if (searchInput) {
        searchInput.value = '';
        searchInput.focus();
        filterNevSearchableOptions('', pickerId);
      }
    }, 50);
    _nevSearchableClickOutside = function(e) {
    ...
    setTimeout(() => document.addEventListener('click', _nevSearchableClickOutside), 0);
  }
}

function filterNevSearchableOptions(keyword, pickerId) {
  const pid = pickerId || _currentActivePicker;
  if (!pid) return;
  const list = document.getElementById(pid + 'List');
  const empty = document.getElementById(pid + 'Empty');
  if (!list) return;
  ...
  options.forEach(opt => {
    ...
    const match = !kw || value.toLowerCase().includes(kw);
    opt.classList.toggle('nev-option-hidden', !match);
    if (match) visible++;
  });
  if (empty) empty.style.display = visible === 0 ? '' : 'none';
}

function selectNevSearchableOption(value, pickerId, hiddenInputId) {
  const pid = pickerId || _currentActivePicker;
  const hid = hiddenInputId || 'dictionaryItemName';
  const hiddenInput = document.getElementById(hid);
  const summary = document.querySelector(`#${pid} [data-picker-summary]`);
  const trigger = document.querySelector(`#${pid} .tag-picker-trigger`);
  const panel = document.getElementById(pid + 'Panel');
  const options = document.querySelectorAll(`#${pid}List .tag-option`);
  ...
  // 如果是名称选择，自动生成编码
  if (hid === 'dictionaryItemName') {
    autoGenerateNevCode(value);
  }
  // 如果是赛马门店选择，自动回填地址
  autoFillNevDealerAddress(value);
...
function autoFillNevDealerAddress(value) {
  const provinceInput = document.getElementById('nevDealerProvince');
  const cityInput = document.getElementById('nevDealerCity');
  const regionInput = document.getElementById('nevDealerRegion');
  if (provinceInput && cityInput && regionInput) {
    const address = NEV_DEALER_ADDRESS_MAP[value];
    if (address) {
