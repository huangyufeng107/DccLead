...
function renderColdLeadDefinitionPage() {
  ...
  page.innerHTML = `
    <div class="page-hero">
      <div>
        <div class="page-title">门店冷线索规则</div>
    ...
    <div class="card">
      <div class="section-header">
        <div class="section-title">门店冷线索规则列表</div>
        <button class="btn-add strategy-add-btn" type="button" onclick="openColdLeadDefinitionModal('add')">新增规则</button>
      </div>
...
function renderColdLeadDefinitionSummary() {
  ...
  const items = [
    { label: '规则总数', value: coldLeadDefinitionRules.length },
...
function renderColdLeadDefinitionTable() {
  ...
  `).join('') : '<tr><td colspan="5"><div class="empty-state">暂无匹配的门店冷线索规则</div></td></tr>';
}
...
function openColdLeadDefinitionModal(mode, id = null) {
  ...
  const titleMap = { add: '新增门店冷线索规则', edit: '编辑门店冷线索规则', view: '查看门店冷线索规则' };
  document.getElementById('leadDispatchRuleModalTitle').textContent = titleMap[mode] || '门店冷线索规则';
  document.getElementById('leadDispatchRuleModalBody').innerHTML = mode === 'view' ? `
    <div class="rule-detail-layout"><section class="rule-detail-section"><div class="rule-detail-title">规则详情</div><div class="rule-detail-body">
  ...
  ` : `
    <div class="dispatch-rule-form">
      <section class="dispatch-form-section"><div class="dispatch-section-title">基础信息</div><div class="dispatch-form-grid">
        ...
        <div class="form-group wide"><div class="form-label">备注</div><textarea class="form-textarea" id="coldLeadFormRemark" placeholder="填写规则说明">${escapeHtml(target.remark || '')}</textarea></div>
      </div></section>
    </div>
  `;
...
function saveColdLeadDefinition() {
  ...
  if (duplicate) { showToast('该线索状态已存在规则', false); return; }
...
function deleteColdLeadDefinition(id) {
  ...
  if (!rule) return;
  if (!confirm(`确认删除门店冷线索规则「${rule.leadStatus}」？`)) return;
  coldLeadDefinitionRules = coldLeadDefinitionRules.filter(item => item.id !== id);
  showToast('门店冷线索规则已删除', true);
  renderColdLeadDefinitionPage();
}
