...
function renderColdLeadDefinitionPage() {
  ...
  page.innerHTML = `
    ...
    <div class="filter-row">
      ...
      <input class="lead-input" style="width:130px; margin-right: 12px;" id="coldLeadKeywordFilter" placeholder="请输入" ${renderUiActionCallback('cold-lead-filter')} />
      <span class="filter-label">线索状态：</span>
      <select class="filter-select" id="coldLeadStatusFilter" ${renderUiActionCallback('cold-lead-filter')}>
        <option value="">全部</option>
        ${leadDispatchLeadStatusOptions.filter(Boolean).map(status => `<option value="${status}">${status}</option>`).join('')}
      </select>
      <span class="filter-label">状态：</span>
      <select class="filter-select" id="coldLeadEnabledFilter" ${renderUiActionCallback('cold-lead-filter')}>
        <option value="">全部</option>
        <option value="启用">启用</option>
        <option value="停用">停用</option>
      </select>
      <button class="btn-secondary" type="button" ${renderUiActionCallback('cold-lead-reset')}>重置</button>
    </div>
    <div class="card">
      <div class="section-header">
        <div class="section-title">门店冷线索配置列表</div>
        <button class="btn-add strategy-add-btn" type="button" ${renderUiActionCallback('cold-lead-open-modal', ['add'])}>新增配置</button>
      </div>
      ...
      <div class="pagination">
        ...
        <div class="pagination-btns">
          <select class="hit-page-size" id="coldLeadDefinitionPageSize" ${renderUiActionCallback('cold-lead-page-size', ['$value'])}>
            <option value="5">每页 5 条</option>
            <option value="10">每页 10 条</option>
            <option value="20">每页 20 条</option>
            <option value="50">每页 50 条</option>
          </select>
          <button class="page-btn" type="button" ${renderUiActionCallback('cold-lead-page', [-1])}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></button>
          <select class="hit-page-size" id="coldLeadDefinitionPageSelect" ${renderUiActionCallback('cold-lead-page-select', ['$value'])}></select>
          <button class="page-btn" type="button" ${renderUiActionCallback('cold-lead-page', [1])}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
      </div>
    </div>
  `;
...
function getFilteredColdLeadDefinitionRules() {
  const keyword = document.getElementById('coldLeadKeywordFilter')?.value.trim().toLowerCase() || '';
  ...
  return coldLeadDefinitionRules
    .filter(rule => {
      ...
      const text = [
        rule.leadStatus,
        rule.remark,
        `CLD-${String(rule.id).padStart(3, '0')}`
    ...
    <tr>
      ...
      <td>
        <div class="rule-text-stack">
          <div class="rule-text-main">${rule.leadStatus}</div>
          <div class="rule-muted">规则ID: CLD-${String(rule.id).padStart(3, '0')}</div>
        </div>
      </td>
      ...
      <td><div class="action-btns"><button class="action-btn view" type="button" ${renderUiActionCallback('cold-lead-open-modal', ['view', rule.id])}>查看</button><button class="action-btn edit" type="button" ${renderUiActionCallback('cold-lead-open-modal', ['edit', rule.id])}>编辑</button><button class="action-btn delete" type="button" ${renderUiActionCallback('cold-lead-delete', [rule.id])}>删除</button></div></td>
    </tr>
  `).join('') : '<tr><td colspan="5"><div class="empty-state">暂无匹配的门店冷线索配置</div></td></tr>';
}

function resetColdLeadDefinitionFilters() {
  ['coldLeadKeywordFilter', 'coldLeadStatusFilter', 'coldLeadEnabledFilter'].forEach(id => {
...
function openColdLeadDefinitionModal(mode, id = null) {
  ...
  document.getElementById('leadDispatchRuleModalFooter').innerHTML = mode === 'view'
    ? `<button class="btn-cancel" type="button" ${renderUiActionCallback('cold-lead-close', ['leadDispatchRuleModal'])}>关闭</button>`
    : `<button class="btn-cancel" type="button" ${renderUiActionCallback('cold-lead-close', ['leadDispatchRuleModal'])}>取消</button><button class="btn-save" type="button" ${renderUiActionCallback('cold-lead-save')}>保存</button>`;
...
// 门店冷线索配置的动态入口统一通过受控回调桥接。
registerUiActionCallback('cold-lead-filter', filterColdLeadDefinitionTable);
registerUiActionCallback('cold-lead-reset', resetColdLeadDefinitionFilters);
registerUiActionCallback('cold-lead-open-modal', openColdLeadDefinitionModal);
registerUiActionCallback('cold-lead-page-size', changeColdLeadDefinitionPageSize);
registerUiActionCallback('cold-lead-page', changeColdLeadDefinitionPage);
registerUiActionCallback('cold-lead-page-select', selectColdLeadDefinitionPage);
registerUiActionCallback('cold-lead-delete', deleteColdLeadDefinition);
registerUiActionCallback('cold-lead-close', (...args) => closeModal(...args));
registerUiActionCallback('cold-lead-save', saveColdLeadDefinition);
