...
function switchSettingsSubPage(sub) {
  ...
  const subLabels = { profile: '个人资料', security: '帐号安全', oplog: '操作日志' };
...
function renderProfilePage() {
  // Nothing dynamic needed for the profile page — it's static HTML in the panel.
}
...
function renderOpLogPagination(total) {
  ...
  el.innerHTML = `
    ...
    <div class="oplog-page-btns">
      <button class="oplog-page-btn" onclick="opLogGoPage(1)" ${opLogCurrentPage === 1 ? 'disabled' : ''}>«</button>
      <button class="oplog-page-btn" onclick="opLogGoPage(${opLogCurrentPage - 1})" ${opLogCurrentPage === 1 ? 'disabled' : ''}>‹</button>
      <button class="oplog-page-btn" onclick="opLogGoPage(${opLogCurrentPage + 1})" ${opLogCurrentPage >= totalPages ? 'disabled' : ''}>›</button>
      <button class="oplog-page-btn" onclick="opLogGoPage(${totalPages})" ${opLogCurrentPage >= totalPages ? 'disabled' : ''}>»</button>
    </div>
  `;
}
