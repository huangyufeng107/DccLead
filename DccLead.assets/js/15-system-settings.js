...
function renderOpLogTable() {
  ...
  if (slice.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:#64748b;padding:32px 0;">暂无操作日志</td></tr>`;
  } else {
...
function renderOpLogPagination(total) {
  ...
  const isPrevDisabled = opLogCurrentPage <= 1;
  const isNextDisabled = opLogCurrentPage >= totalPages;
  el.innerHTML = `
    <span class="oplog-page-info">共 <strong>${total}</strong> 条记录，第 ${opLogCurrentPage} / ${totalPages} 页</span>
    <div class="oplog-page-btns">
      <button class="oplog-page-btn ${isPrevDisabled ? 'disabled' : ''}" type="button" data-ui-action="settings-oplog-go-page" data-page="1" ${isPrevDisabled ? 'disabled' : ''}>«</button>
      <button class="oplog-page-btn ${isPrevDisabled ? 'disabled' : ''}" type="button" data-ui-action="settings-oplog-go-page" data-page="${opLogCurrentPage - 1}" ${isPrevDisabled ? 'disabled' : ''}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <button class="oplog-page-btn ${isNextDisabled ? 'disabled' : ''}" type="button" data-ui-action="settings-oplog-go-page" data-page="${opLogCurrentPage + 1}" ${isNextDisabled ? 'disabled' : ''}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
      <button class="oplog-page-btn ${isNextDisabled ? 'disabled' : ''}" type="button" data-ui-action="settings-oplog-go-page" data-page="${totalPages}" ${isNextDisabled ? 'disabled' : ''}>»</button>
    </div>
  `;
}
