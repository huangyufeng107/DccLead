...
function renderLeadDispatchUpdateSummary(rule) {
  const items = [
    ...
    rule.updateSmartCode ? `新SmartCode：${rule.updateSmartCode}` : ''
  ].filter(Boolean);
...
function resetLeadDispatchFilters() {
  ['leadDispatchCallTypeFilter', 'leadDispatchTargetFilter', 'leadDispatchEnabledFilter'].forEach(id => {
...
function getLeadDispatchAbnormalReasonOptions(leadStatus = '', currentReason = '') {
  ...
  const baseOptions = leadDispatchAbnormalReasonMap[leadStatus || ''] || [''];
...
function getLeadDispatchIntentLevelOptions(statusValue = '', currentValue = '') {
  const baseOptions = (statusValue || '') === '总部_高意向' ? highIntentLevelOptions : leadDispatchIntentLevelOptions;
...
function syncIntentLevelByLeadStatus(statusValue, intentLevelSelectId) {
  ...
  intentLevelSelect.disabled = locked;
  const currentValue = intentLevelSelect.value || '';
  const options = getLeadDispatchIntentLevelOptions(statusValue || '', currentValue);
  const selected = getLeadStatusLockedIntentLevel(statusValue || '', currentValue);
  intentLevelSelect.innerHTML = renderLeadDispatchOptionList(options, selected, locked ? '不更新' : '请选择意向级别');
}
...
function updateLeadDispatchAbnormalReasonOptions() {
  ...
  const options = getLeadDispatchAbnormalReasonOptions(statusSelect.value || '', currentValue);
  reasonSelect.innerHTML = renderLeadDispatchOptionList(options, getSelectedLeadDispatchAbnormalReason(statusSelect.value || '', options.includes(currentValue) ? currentValue : ''), isLeadStatusAbnormalReasonLocked(statusSelect.value || '') ? '不更新' : '请选择异常原因');
  if (!shouldShowReason) reasonSelect.value = '';
  syncIntentLevelByLeadStatus(statusSelect.value || '', 'dispatchFormIntentLevel');
}
  ...
  return `
    <div class="tag-picker dispatch-tag-picker" data-kind="${kind}">
      ...
      <div class="tag-picker-panel" id="dispatch${kind}TagPanel">
        <div class="tag-picker-toolbar">
          <span id="dispatch${kind}TagCount">已选 0 / ${leadDispatchTagOptions.length}</span>
          <div class="tag-picker-actions">
        ...
        <div class="dispatch-form-grid">
          ...
          <div class="form-group wide">
            ...
            <div class="date-range">
              ...
              <input class="lead-input" id="dispatchFormDurationSeconds" type="number" min="0" value="${escapeAttr(rule.durationSeconds || '')}" placeholder="秒数" />
              <select class="lead-select" id="dispatchFormDurationLogic">
                <option value="且" ${rule.durationLogic === '且' ? 'selected' : ''}>且</option>
                <option value="或" ${rule.durationLogic === '或' ? 'selected' : ''}>或</option>
              </select>
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group wide">
            <div class="form-label">是否有意向专营店 <span class="required">*</span></div>
            <select class="form-input" id="dispatchFormHasDealer">
              <option value="不限" ${rule.hasIntentDealer === '不限' ? 'selected' : ''}>不限</option>
      ...
      <section class="dispatch-form-section">
        ...
        <div class="dispatch-action-card">
          <div class="dispatch-form-grid">
            ...
            <div class="form-group">
              <div class="form-label">新SmartCode</div>
            ...
            <div class="form-group" id="dispatchFormAbnormalReasonGroup" style="${shouldShowLeadDispatchAbnormalReason(rule.updateLeadStatus || '') ? '' : 'display:none'}">
              ...
              <select class="form-input" id="dispatchFormAbnormalReason" ${isLeadStatusAbnormalReasonLocked(rule.updateLeadStatus || '') ? 'disabled' : ''}>
                ${renderLeadDispatchOptionList(getLeadDispatchAbnormalReasonOptions(rule.updateLeadStatus || '', rule.abnormalReason || ''), getSelectedLeadDispatchAbnormalReason(rule.updateLeadStatus || '', rule.abnormalReason || ''), isLeadStatusAbnormalReasonLocked(rule.updateLeadStatus || '') ? '不更新' : '请选择异常原因')}
              </select>
            </div>
            <div class="form-group wide">
              <div class="form-label">意向级别更新为 <span class="required">*</span></div>
              <select class="form-input" id="dispatchFormIntentLevel" ${isLeadStatusIntentLevelLocked(rule.updateLeadStatus || '') ? 'disabled' : ''}>
                ${renderLeadDispatchOptionList(getLeadDispatchIntentLevelOptions(rule.updateLeadStatus || '', rule.updateIntentLevel || ''), getLeadStatusLockedIntentLevel(rule.updateLeadStatus || '', rule.updateIntentLevel || ''), isLeadStatusIntentLevelLocked(rule.updateLeadStatus || '') ? '不更新' : '请选择意向级别')}
              </select>
            </div>
          </div>
        </div>
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        ['是否有意向专营店', rule.hasIntentDealer || '不限']
      ]
    },
    {
      title: '执行动作',
      fields: [
        ...
        ['新SmartCode', rule.updateSmartCode || '不更新'],
...
function saveLeadDispatchRule() {
  ...
  if (!payload.hasIntentDealer) { showToast('请选择是否有意向专营店', false); return; }
