...
function renderLeadDispatchConfigPage() {
  ...
  page.innerHTML = `
    ...
    <div class="filter-row">
      ...
      <input class="lead-input" style="width:130px; margin-right: 12px;" id="leadDispatchKeywordFilter" placeholder="请输入" ${renderUiActionCallback('dispatch-filter')} />
      <span class="filter-label">外呼类型：</span>
      <select class="filter-select" id="leadDispatchCallTypeFilter" ${renderUiActionCallback('dispatch-filter')}>
        <option value="">全部</option>
        ${unifiedPolicyCallTypeOptions.map(type => `<option value="${type}">${type}</option>`).join('')}
      </select>
      <span class="filter-label">下发动作：</span>
      <select class="filter-select" id="leadDispatchTargetFilter" ${renderUiActionCallback('dispatch-filter')}>
        <option value="">全部</option>
      ...
      <select class="filter-select" id="leadDispatchEnabledFilter" ${renderUiActionCallback('dispatch-filter')}>
        <option value="">全部</option>
        <option value="启用">启用</option>
        <option value="停用">停用</option>
      </select>
      <button class="btn-secondary" type="button" ${renderUiActionCallback('dispatch-reset')}>重置</button>
    </div>
    ...
    <div class="card">
      <div class="section-header">
        ...
        <div class="action-btns">
          <button class="btn-add strategy-add-btn" type="button" ${renderUiActionCallback('dispatch-open-modal', ['add'])}>新增配置</button>
        </div>
      </div>
      ...
      <div class="pagination">
        ...
        <div class="pagination-btns">
          <select class="hit-page-size" id="leadDispatchPageSize" ${renderUiActionCallback('dispatch-page-size', ['$value'])}>
          ...
          <select class="hit-page-size" id="leadDispatchPageSelect" ${renderUiActionCallback('dispatch-page-select', ['$value'])}></select>
...
function renderLeadDispatchRuleTable() {
  ...
  body.innerHTML = pageRows.length ? pageRows.map(rule => `
    <tr>
      ...
      <td>
        <div class="action-btns">
          <button class="action-btn view" type="button" ${renderUiActionCallback('dispatch-open-modal', ['view', rule.id])}>查看</button>
          <button class="action-btn edit" type="button" ${renderUiActionCallback('dispatch-open-modal', ['edit', rule.id])}>编辑</button>
          <button class="action-btn delete" type="button" ${renderUiActionCallback('dispatch-delete', [rule.id])}>删除</button>
        </div>
      </td>
    </tr>
...
function renderCallTypeTagPicker({ idPrefix, inputName, options = [], selected = [], placeholder = '请选择外呼类型', readonly = false }) {
  ...
  const onclickAttr = readonly ? '' : renderUiActionCallback('dispatch-calltype-toggle', [idPrefix]);
  return `
      ...
      <div class="tag-picker-panel" id="${idPrefix}Panel">
        <div class="tag-picker-toolbar">
          ...
          <div class="tag-picker-actions">
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-calltype-all', [idPrefix])}>全选</button>
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-calltype-clear', [idPrefix])}>清空</button>
          </div>
          `}
        </div>
        <div class="tag-picker-list">
          ${options.map(type => `
            <label class="tag-option">
              <input type="checkbox" name="${inputName}" value="${escapeAttr(type)}" ${safeSelected.includes(type) ? 'checked' : ''} ${disabledAttr} ${renderUiActionCallback('dispatch-calltype-option', [idPrefix, '$self'])} />
              ${escapeHtml(type)}
            </label>
          `).join('')}
        </div>
...
function openLeadDispatchRuleModal(mode, id = null) {
  ...
  document.getElementById('leadDispatchRuleModalFooter').innerHTML = mode === 'view'
    ? `<button class="btn-cancel" type="button" ${renderUiActionCallback('dispatch-close-modal')}>关闭</button>`
    : `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('dispatch-close-modal')}>取消</button>
      <button class="btn-save" type="button" ${renderUiActionCallback('dispatch-save')}>保存</button>
    `;
...
function renderLeadDispatchOptionList(options, selectedValue = '', emptyText = '不更新') {
  ...
  return list.map(value => `
    <option value="${escapeAttr(value)}" ${value === selected ? 'selected' : ''}>${escapeHtml(value && leadDispatchIntentLevelOptions.includes(value) ? getSharedIntentLevelLabel(value) : (value || emptyText))}</option>
  `).join('');
}
...
function getLeadDispatchFormLeadStatusOptions() {
  return getManagedDictionaryOptionNames('INITIAL_LEAD_STATUS', leadDispatchFormLeadStatusOptions)
    .filter(status => !['总部_暂败', '总部_异地'].includes(status));
}
...
// Dictionary names are presentation only. Preserve the legacy values used by AI rules.
function getSharedIntentLevelLabel(value = '') {
  const code = String(value).split(' - ')[0];
  const items = typeof DICTIONARY_ITEMS !== 'undefined' ? DICTIONARY_ITEMS.INTENT_LEVEL : null;
  return items?.find(item => item.code === code)?.name || value;
}

function getAiIntentLevelDictionaryValues() {
  const allowed = leadDispatchIntentLevelOptions.filter(Boolean);
  const items = typeof DICTIONARY_ITEMS !== 'undefined' ? DICTIONARY_ITEMS.INTENT_LEVEL : null;
  if (!Array.isArray(items)) return allowed;
  return items.filter(item => item.status === '启用')
    .slice().sort((a, b) => a.order - b.order)
    .map(item => allowed.find(value => value.split(' - ')[0] === item.code))
    .filter(Boolean);
}

function getLeadDispatchIntentLevelOptions(statusValue = '', currentValue = '') {
  const managedOptions = getAiIntentLevelDictionaryValues();
...
function syncIntentLevelByLeadStatus(statusValue, intentLevelSelectId) {
  ...
  const options = getLeadDispatchIntentLevelOptions(statusValue || '');
  const selected = getLeadStatusLockedIntentLevel(
    statusValue || '',
...
function renderLeadDispatchTagPicker(kind, inputName, selectedTags = [], placeholder = '请选择外呼标签') {
  ...
  return `
    <div class="tag-picker dispatch-tag-picker" data-kind="${kind}">
      <button class="tag-picker-trigger placeholder" id="dispatch${kind}TagTrigger" type="button" ${renderUiActionCallback('dispatch-tag-toggle', [kind])}>${placeholder}</button>
      <div class="tag-picker-panel" id="dispatch${kind}TagPanel">
        <div class="tag-picker-toolbar">
          <span id="dispatch${kind}TagCount">已选 0 / ${tagOptions.length}</span>
          <div class="tag-picker-actions">
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-tag-all', [kind])}>全选</button>
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-tag-clear', [kind])}>清空</button>
          </div>
        </div>
        <div class="tag-picker-search">
          <input type="search" placeholder="搜索${kind === 'Include' ? '包含' : '不包含'}标签" ${renderUiActionCallback('dispatch-tag-filter', [`dispatch${kind}TagPanel`, '$value'])} />
        </div>
        <div class="tag-picker-list">
          ${tagOptions.map(tag => `
            <label class="tag-option">
              <input type="checkbox" name="${inputName}" value="${escapeAttr(tag)}" ${safeSelected.includes(tag) ? 'checked' : ''} ${renderUiActionCallback('dispatch-tag-option', ['$self', kind])} />
              ${tag}
            </label>
          `).join('')}
        </div>
...
function renderLeadDispatchCallStatusPicker(selectedStatuses = []) {
  ...
  return `
    <div class="tag-picker dispatch-tag-picker">
      <button class="tag-picker-trigger placeholder" id="dispatchCallStatusTrigger" type="button" ${renderUiActionCallback('dispatch-status-toggle')}>请选择通话状态</button>
      <div class="tag-picker-panel" id="dispatchCallStatusPanel">
        <div class="tag-picker-toolbar">
          ...
          <div class="tag-picker-actions">
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-status-all')}>全选</button>
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('dispatch-status-clear')}>清空</button>
          </div>
        </div>
        <div class="tag-picker-list">
          ${leadDispatchCallStatusOptions.map(status => `
            <label class="tag-option">
              <input type="checkbox" name="dispatchCallStatus" value="${escapeAttr(status)}" ${safeSelected.includes(status) ? 'checked' : ''} ${renderUiActionCallback('dispatch-status-option', ['$self'])} />
              ${status}
            </label>
          `).join('')}
        </div>
  ...
  return `
    ...
    <div class="condition-row">
      ...
      <div class="condition-toggle">
        <button type="button" class="condition-btn ${isAnd ? 'active' : ''}" id="dispatch${kind}And" ${renderUiActionCallback('dispatch-tag-logic', [kind, '且'])}>且 (AND)</button>
        <button type="button" class="condition-btn ${!isAnd ? 'active' : ''}" id="dispatch${kind}Or" ${renderUiActionCallback('dispatch-tag-logic', [kind, '或'])}>或 (OR)</button>
      </div>
...
function renderLeadDispatchCallStatusLogic(value = '或') {
  ...
  return `
    ...
    <div class="condition-row">
      ...
      <div class="condition-toggle">
        <button type="button" class="condition-btn ${isAnd ? 'active' : ''}" id="dispatchCallStatusAnd" ${renderUiActionCallback('dispatch-status-logic', ['且'])}>且 (AND)</button>
        <button type="button" class="condition-btn ${!isAnd ? 'active' : ''}" id="dispatchCallStatusOr" ${renderUiActionCallback('dispatch-status-logic', ['或'])}>或 (OR)</button>
      </div>
...
function renderLeadDispatchDurationLogic(value = '且') {
  ...
  return `
    ...
    <div class="condition-row">
      ...
      <div class="condition-toggle">
        <button type="button" class="condition-btn ${isAnd ? 'active' : ''}" id="dispatchDurationAnd" ${renderUiActionCallback('dispatch-duration-logic', ['且'])}>且 (AND)</button>
        <button type="button" class="condition-btn ${!isAnd ? 'active' : ''}" id="dispatchDurationOr" ${renderUiActionCallback('dispatch-duration-logic', ['或'])}>或 (OR)</button>
      </div>
        ...
        <div class="dispatch-action-card">
          <div class="dispatch-form-grid">
            ...
            <div class="form-group">
              ...
              <select class="form-input" id="dispatchFormLeadStatus" ${renderUiActionCallback('dispatch-sync-abnormal')}>
                ${renderLeadDispatchOptionList(getLeadDispatchFormLeadStatusOptions(), rule.updateLeadStatus || '', '不更新')}
              </select>
            </div>
...
registerUiActionCallback('dispatch-filter', filterLeadDispatchRuleTable);
registerUiActionCallback('dispatch-reset', resetLeadDispatchFilters);
registerUiActionCallback('dispatch-open-modal', openLeadDispatchRuleModal);
registerUiActionCallback('dispatch-delete', deleteLeadDispatchRule);
registerUiActionCallback('dispatch-page-size', changeLeadDispatchPageSize);
registerUiActionCallback('dispatch-page', changeLeadDispatchPage);
registerUiActionCallback('dispatch-page-select', selectLeadDispatchPage);
registerUiActionCallback('dispatch-close-modal', () => closeModal('leadDispatchRuleModal'));
registerUiActionCallback('dispatch-save', saveLeadDispatchRule);
registerUiActionCallback('dispatch-calltype-toggle', toggleCallTypeTagPicker);
registerUiActionCallback('dispatch-calltype-all', selectAllCallTypeTagPicker);
registerUiActionCallback('dispatch-calltype-clear', clearCallTypeTagPicker);
registerUiActionCallback('dispatch-calltype-option', toggleCallTypeTagOption);
registerUiActionCallback('dispatch-tag-toggle', toggleLeadDispatchTagPicker);
registerUiActionCallback('dispatch-tag-all', selectAllLeadDispatchTags);
registerUiActionCallback('dispatch-tag-clear', clearLeadDispatchTags);
registerUiActionCallback('dispatch-tag-filter', filterTagPickerOptions);
registerUiActionCallback('dispatch-tag-option', toggleLeadDispatchTagOption);
registerUiActionCallback('dispatch-tag-logic', setLeadDispatchTagLogic);
registerUiActionCallback('dispatch-status-toggle', toggleLeadDispatchCallStatusPicker);
registerUiActionCallback('dispatch-status-all', selectAllLeadDispatchCallStatuses);
registerUiActionCallback('dispatch-status-clear', clearLeadDispatchCallStatuses);
registerUiActionCallback('dispatch-status-option', toggleLeadDispatchCallStatusOption);
registerUiActionCallback('dispatch-status-logic', setLeadDispatchCallStatusLogic);
registerUiActionCallback('dispatch-duration-logic', setLeadDispatchDurationLogic);
registerUiActionCallback('dispatch-sync-abnormal', updateLeadDispatchAbnormalReasonOptions);
