...
function openTranscriptByCallId(callId, shouldScroll = true) {
  ...
  if (activeAudioCallId !== callId) {
    activeAudioCallId = callId;
    audioPlaying = true;
    audioProgressSeconds = 0;
    audioDurationSeconds = typeof parseDurationToSeconds === 'function' ? parseDurationToSeconds(record.transcript?.duration || '02:00') : 120;
    if (typeof updateRecordAudioPlayer === 'function') {
      updateRecordAudioPlayer(record);
    }
    if (typeof startRecordAudioTimer === 'function') {
...
function closeTranscript() {
  ...
  if (typeof stopRecordAudio === 'function') {
    stopRecordAudio();
  }
}
