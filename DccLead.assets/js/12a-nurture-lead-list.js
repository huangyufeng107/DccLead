  ...
  list.innerHTML = dealers.length
    ? dealers.map(item => `
      <label class="dealer-option">
        <input type="checkbox" value="${item.code}" ${selectedDealerCodes.includes(item.code) ? 'checked' : ''} ${renderUiActionCallback('nurture-lead-dealer-toggle', ['$self'])} />
        <span>
...
function renderNurtureLeadTable() {
  ...
  const pageInfoEl = document.getElementById('nurtureLeadPageInfo');
  if (pageInfoEl) {
    pageInfoEl.textContent = `共 ${rows.length} 条记录，当前第 ${nurtureLeadCurrentPage} / ${totalPages} 页`;
    syncPaginationButtons(pageInfoEl.closest('.pagination'), nurtureLeadCurrentPage, totalPages);
  }
  ...
  document.getElementById('nurtureLeadTableHead').innerHTML = `
    <tr>
      ${columns.map(col => `
        <th ${renderUiActionCallback('nurture-lead-sort', [col.key])}>
  ...
  document.getElementById('nurtureLeadTableBody').innerHTML = pageRows.length
    ? pageRows.map(row => `
      <tr>
        ...
        <td><button class="action-btn view" type="button" ${renderUiActionCallback('nurture-lead-open-detail', [row.leadCode])}>查看</button></td>
      </tr>
    `).join('')
...
function formatNurtureLeadCell(row, key) {
  ...
  if (key === 'status') {
    ...
    return `<span class="tag-chip ${cls}" style="font-weight:400;">${value}</span>`;
  }
  if (key === 'intentLevel') return `<span class="tag-chip orange" style="font-weight:400;">${value}</span>`;
  if (key === 'customerName') return `<div class="list-customer-cell"><div class="list-customer-name">${row.customerName || '—'}</div><div class="list-customer-phone">${row.phone || '—'}</div></div>`;
  return value;
}
    ...
    <div class="field-picker-grid">
      ${nurtureLeadColumns.map(col => `
        <label class="field-check">
          <input type="checkbox" value="${col.key}" ${visibleNurtureLeadFields.includes(col.key) ? 'checked' : ''} ${renderUiActionCallback('nurture-lead-field-toggle', ['$self'])} />
          ${col.label}
        </label>
      `).join('')}
    </div>
...
function openLeadDetail(leadCode) {
  ...
  bindNurtureLeadCriticalReminderCountdown(document.getElementById('leadDetailPageBody'));
}

let nurtureLeadCriticalReminderCountdownTimer = null;
let nurtureLeadCriticalReminderHideTimer = null;

function bindNurtureLeadCriticalReminderCountdown(page) {
  if (nurtureLeadCriticalReminderCountdownTimer) window.clearInterval(nurtureLeadCriticalReminderCountdownTimer);
  if (nurtureLeadCriticalReminderHideTimer) window.clearTimeout(nurtureLeadCriticalReminderHideTimer);
  nurtureLeadCriticalReminderCountdownTimer = null;
  nurtureLeadCriticalReminderHideTimer = null;
  const reminder = page?.querySelector('[data-nurture-lead-critical-reminder]');
  if (!reminder) return;
  const action = reminder.querySelector('[data-nurture-lead-critical-reminder-toggle]');
  let seconds = 10;
  const hideReminder = () => {
    if (nurtureLeadCriticalReminderCountdownTimer) window.clearInterval(nurtureLeadCriticalReminderCountdownTimer);
    if (nurtureLeadCriticalReminderHideTimer) window.clearTimeout(nurtureLeadCriticalReminderHideTimer);
  ...
  nurtureLeadCriticalReminderCountdownTimer = window.setInterval(() => {
    seconds -= 1;
    if (seconds <= 0) return hideReminder();
    action.textContent = seconds + ' 秒后自动隐藏';
  }, 1000);
  nurtureLeadCriticalReminderHideTimer = window.setTimeout(hideReminder, 10000);
...
function renderLeadDetailPageContent(lead) {
  ...
  // 原生集成：频繁留资强提醒横幅 & 时间轴旅程卡片
  const isFrequent = (lead.phone === '138****8810' || lead.phone === '138****8888' || lead.id === 'LEAD-001' || lead.leadCode === 'LEAD-001' || (lead.leadCode && lead.leadCode.includes('001')));
  
  const frequentBanner = isFrequent ? `
    <section class="nt-critical-reminder" data-nurture-lead-critical-reminder aria-label="关键行为强提醒">
      <div class="nt-critical-reminder-icon" aria-hidden="true">!</div>
      <div class="nt-critical-reminder-body"><div class="nt-critical-reminder-title"><strong>关键行为强提醒</strong><span class="nt-critical-reminder-tags"><span class="nt-critical-reminder-tag primary">同店2次留资（广州天河店）</span><span class="nt-critical-reminder-tag secondary">私域高频互动（近3天）</span></span></div></div>
      <button type="button" class="nt-critical-reminder-action" data-nurture-lead-critical-reminder-toggle>10 秒后自动隐藏</button>
    </section>
  ` : '';
  ...
  return `
    ...
    <div class="detail-tabs">
      <button class="detail-tab active" type="button" ${renderUiActionCallback('nurture-lead-detail-tab', ['$self', 'leadDetailInfoPanel'])}>线索详情</button>
      <button class="detail-tab" type="button" ${renderUiActionCallback('nurture-lead-detail-tab', ['$self', 'leadTaskPanel'])}>培育任务</button>
      <button class="detail-tab" type="button" ${renderUiActionCallback('nurture-lead-detail-tab', ['$self', 'leadSyncLogPanel'])}>接收日志</button>
      <button class="detail-tab" type="button" ${renderUiActionCallback('nurture-lead-detail-tab', ['$self', 'leadSystemLogPanel'])}>系统日志</button>
    </div>
...
// 培育线索列表的动态表格与详情页入口统一通过受控回调桥接。
registerUiActionCallback('nurture-lead-dealer-toggle', toggleDealerSelection);
registerUiActionCallback('nurture-lead-sort', sortNurtureLeadBy);
registerUiActionCallback('nurture-lead-open-detail', openLeadDetail);
registerUiActionCallback('nurture-lead-field-toggle', toggleNurtureLeadField);
registerUiActionCallback('nurture-lead-detail-tab', switchLeadDetailTab);
