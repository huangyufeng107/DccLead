...
function openMetricRuleModal(metric) {
  ...
  const rules = {
    ...
    '同步失败量': {
      ...
      formula: '同步接口返回非 200 或超时（≥ 5000ms）的去重任务数量。',
      sla: 'SLA 统计窗口 15 分钟，若失败率 ≥ 5% 则触发 P2 级业务预警。',
      selfHealing: '系统内置指数退避重试算法（按 5m、15m、30m 间隔），最多自动重试 3 次。若 3 次后仍失败，则生成运维待办事件并触发责任人通知。'
    },
    '滚存待推送': {
