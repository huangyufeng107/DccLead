...
function renderPreDialerMetricCard(name, value, subtitle, category) {
  ...
  return `<button class="predial-overview-card predial-metric-card${catClass}${alertClass}" type="button" onclick="showPreDialerMetricInfo('${name}', this)"><div class="metric-label">${name}</div><div class="metric-value">${value}</div><div class="metric-sub">${subtitle}</div></button>`;
}
...
function renderPreDialerReportPage(page) {
  ...
  page.innerHTML = `
    <div class="predial-report-shell worry-free-dashboard">
      ...
      <section class="card">
        <div class="lead-toolbar"><div class="lead-toolbar-left"><div class="section-title">明细列表</div></div><div class="lead-toolbar-right"><button class="btn-secondary" type="button" onclick="exportPreDialerTasks()">导出数据</button></div></div>
...
function showPreDialerMetricInfo(metric, element) {
  ...
  const action = `<button class="metric-info-popover-action" type="button" onclick="openMetricRuleModal('${metric}')">点击查看指标规则 →</button>`;
