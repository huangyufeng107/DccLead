...
function renderBlacklistDealerListPage() {
  ...
  if (info) info.textContent = `共 ${rows.length} 条记录，当前第 ${blacklistDealerCurrentPage} / ${totalPages} 页`;
  ...
  body.innerHTML = pageRows.map((d, index) => {
    ...
    return `
      <tr>
        ...
        <td>${carSeriesText}</td>
        <td>${attrTypeText}</td>
        <td><span class="tag-chip green">${escapeDealerHtml(d.addType || '人工手动添加')}</span></td>
