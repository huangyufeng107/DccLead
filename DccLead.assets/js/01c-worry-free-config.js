...
function getWorryFreeBatchLeadRows(detail) {
  ...
  return [
    [`HQ${dateKey}00188`, `PYXS${dateKey}0001`, `PYRW${dateKey}0128`, '培育中', '—', 'A', '陆先生', '138****6721', 'N6', '广州东风日产天河店', 'N6', 'SC-N6-0891', `${detail.date} 17:10:00`, '4', '历史试驾过 / 历史到店过', 'P1', '成功', detail.status === '执行中' ? '同步中' : '已同步', '—'],
    [`HQ${dateKey}00231`, `PYXS${dateKey}0002`, `PYRW${dateKey}0136`, '休眠未购', '资金不足', 'B', '陈女士', '186****4309', 'NX8', '深圳东风日产南山店', 'NX8', 'SC-NX8-2048', `${detail.date} 16:42:00`, '3', '门店跟进次数 ≥ 3次', 'P2', '成功', '已同步', '—'],
    [`HQ${dateKey}00309`, `PYXS${dateKey}0003`, `PYRW${dateKey}0142`, '无效', '明确空号', 'H', '王先生', '159****2840', 'N7', '杭州东风日产滨江店', 'N7', 'SC-N7-7712', `${detail.date} 15:28:00`, '1', '休眠未购', 'P3', '失败', '同步失败', '短信发送失败'],
    [`HQ${dateKey}00476`, `PYXS${dateKey}0004`, `PYRW${dateKey}0155`, '休眠失联', '三天四次无法接通', 'C', '李女士', '137****9088', 'N6', '上海东风日产浦东店', 'N6', 'SC-N6-5520', `${detail.date} 14:55:00`, '5', '休眠失联', 'P4', '成功', '已同步', '—'],
    [`HQ${dateKey}00518`, `PYXS${dateKey}0005`, `PYRW${dateKey}0162`, '待培育', '—', 'A', '赵先生', '135****2190', 'NX8', '成都东风日产高新店', 'NX8', `SC-${prefix}-4219`, `${detail.date} 13:36:00`, '0', '—', 'P100', '成功', '待同步', '等待DCC任务生成']
  ];
}
...
function addWorryFreeScriptRule() {
  const tbody = document.getElementById('worryFreeScriptRuleBody');
  if (!tbody) return;
  const selectedSeries = getSelectedWorryFreeSeries();
  let seriesOptionsHtml = '';
  if (selectedSeries.length === 0) {
    seriesOptionsHtml = `<option value="">请选择车系</option>`;
  } else {
    seriesOptionsHtml = selectedSeries.map(series => `<option value="${series}">${series}</option>`).join('');
  }
  tbody.insertAdjacentHTML('beforeend', `
    <tr>
      <td>
      ...
      <td>
        <select class="worry-free-table-select">
          <option selected>一知-冷线索</option>
          <option>一知-冷线索（大模型）</option>
          <option>一知-大模型NEV新线索</option>
          <option>科大讯飞-冷线索</option>
          <option>科大讯飞-留资未满</option>
          <option>冰兰-新线索</option>
          <option>冰兰-保客</option>
          <option>一知-保客</option>
        </select>
      </td>
