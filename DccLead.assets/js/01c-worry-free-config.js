...
function renderWorryFreeRows(rows) {
  return rows.map(row => {
    const cells = row.map((cell, index) => {
      if (index === 7) {
        return `<td><div class="list-customer-cell"><div class="list-customer-name">${cell}</div><div class="list-customer-phone">${row[8] || '—'}</div></div></td>`;
      }
      if (index === 8) return '';
      return `<td>${cell}</td>`;
    }).join('');
...
let worryFreeBatchDetailFilters = { headquarterLeadId: '', systemStatus: '', customerName: '', phone: '', intentSeries: '', dealerCodes: [], layer: '', smartCode: '', nurtureLeadCode: '', startDate: '', endDate: '' };
let worryFreeBatchDetailDealerCodes = [];
...
function getFilteredWorryFreeBatchLeadDetailRows(detail) {
  ...
  return getWorryFreeBatchLeadDetailRows(detail).filter(row =>
    ...
    (!(filter.dealerCodes || []).length || filter.dealerCodes.some(code => dealerOptions.find(item => item.code === code)?.name === row[10])) &&
...
function renderWorryFreeBatchLeadDetailTable(detail) {
  const columns = worryFreeBatchDetailColumns;
...
function openWorryFreeBatchDetail(date) {
  ...
  worryFreeBatchDetailCurrentPage = 1;
  worryFreeBatchDetailFilters = { headquarterLeadId: '', systemStatus: '', customerName: '', phone: '', intentSeries: '', dealerCodes: [], layer: '', smartCode: '', nurtureLeadCode: '', startDate: '', endDate: '' };
  worryFreeBatchDetailDealerCodes = [];
      ...
      <div class="worry-free-batch-layout">
        ...
        <div class="worry-free-batch-stack">
          ...
          <div class="worry-free-batch-section">
            ...
            <div class="lead-table-wrap">
              <table class="lead-table worry-free-overview-detail-table">
                <thead><tr><th>意向车系</th><th>外呼类型</th><th>累计总量</th><th>去重过滤</th><th>短信成功</th><th>DCC培育任务</th><th>异常原因</th><th>滚存待推送</th></tr></thead>
...
function updateWorryFreeBatchDetailDealerCities() {
  ...
  citySelect.innerHTML = `<option value="">选择城市</option>${cities.map(city => `<option value="${city}">${city}</option>`).join('')}`;
  if (cities.includes(current)) citySelect.value = current;
  renderWorryFreeBatchDetailDealerPicker();
}

function initWorryFreeBatchDetailDealerPicker() {
  const provinceSelect = document.getElementById('worryFreeDealerProvince');
  if (!provinceSelect) return;
  const provinces = [...new Set(dealerOptions.map(item => item.province))];
  provinceSelect.innerHTML = `<option value="">选择省份</option>${provinces.map(province => `<option value="${province}">${province}</option>`).join('')}`;
  updateWorryFreeBatchDetailDealerCities();
}

function toggleWorryFreeBatchDetailDealerPicker() {
  document.getElementById('worryFreeDealerPickerPanel')?.classList.toggle('show');
  renderWorryFreeBatchDetailDealerPicker();
}

function renderWorryFreeBatchDetailDealerPicker() {
  const dealers = getVisibleWorryFreeBatchDetailDealers();
  const list = document.getElementById('worryFreeDealerOptionList');
  if (!list) return;
  list.innerHTML = dealers.length ? dealers.map(item => `<label class="dealer-option"><input type="checkbox" value="${item.code}" ${worryFreeBatchDetailDealerCodes.includes(item.code) ? 'checked' : ''} onchange="toggleWorryFreeBatchDetailDealer(this)" /><span><div class="dealer-name">${item.name}</div><div class="dealer-meta">${item.code}｜${item.province} ${item.city} ${item.district}</div></span></label>`).join('') : '<div class="empty-state" style="padding:24px 12px">暂无匹配门店</div>';
  const visibleCodes = dealers.map(item => item.code);
  const selectedCount = visibleCodes.filter(code => worryFreeBatchDetailDealerCodes.includes(code)).length;
  const all = document.getElementById('worryFreeDealerSelectAll');
  const count = document.getElementById('worryFreeDealerSelectedCount');
  if (all) all.checked = dealers.length > 0 && selectedCount === dealers.length;
  if (count) count.textContent = `已选 ${worryFreeBatchDetailDealerCodes.length} / ${dealers.length}`;
  const trigger = document.getElementById('worryFreeDealerSelectTrigger');
  if (trigger) {
    const names = dealerOptions.filter(item => worryFreeBatchDetailDealerCodes.includes(item.code)).map(item => item.name);
    trigger.textContent = names.length ? `${names.slice(0, 2).join('、')}${names.length > 2 ? ` 等${names.length}家` : ''}` : '暂未选择门店';
    trigger.classList.toggle('has-value', names.length > 0);
  }
}

function toggleWorryFreeBatchDetailDealer(input) {
  worryFreeBatchDetailDealerCodes = input.checked ? [...new Set([...worryFreeBatchDetailDealerCodes, input.value])] : worryFreeBatchDetailDealerCodes.filter(code => code !== input.value);
  renderWorryFreeBatchDetailDealerPicker();
}

function toggleAllVisibleWorryFreeBatchDetailDealers(checked) {
  const visibleCodes = getVisibleWorryFreeBatchDetailDealers().map(item => item.code);
  worryFreeBatchDetailDealerCodes = checked ? [...new Set([...worryFreeBatchDetailDealerCodes, ...visibleCodes])] : worryFreeBatchDetailDealerCodes.filter(code => !visibleCodes.includes(code));
  renderWorryFreeBatchDetailDealerPicker();
}

function clearWorryFreeBatchDetailDealers() {
  worryFreeBatchDetailDealerCodes = [];
  renderWorryFreeBatchDetailDealerPicker();
}
...
function mountWorryFreeBatchDetailFilters() {
  ...
  initWorryFreeBatchDetailDealerPicker();
}

function queryWorryFreeBatchDetail() {
  const value = id => document.getElementById(id)?.value.trim() || '';
  worryFreeBatchDetailFilters = { headquarterLeadId: value('worryFreeDetailHeadquarterLeadId'), systemStatus: value('worryFreeDetailSystemStatus'), customerName: value('worryFreeDetailCustomerName'), phone: value('worryFreeDetailPhone'), intentSeries: value('worryFreeDetailIntentSeries'), dealerCodes: [...worryFreeBatchDetailDealerCodes], layer: value('worryFreeDetailLayer'), smartCode: value('worryFreeDetailSmartCode'), nurtureLeadCode: value('worryFreeDetailNurtureLeadCode'), startDate: value('worryFreeDetailStartDate'), endDate: value('worryFreeDetailEndDate') };
  worryFreeBatchDetailCurrentPage = 1;
  refreshWorryFreeBatchLeadDetailTable();
}
...
const worryFreeBatchDetailColumns = [
  '批次号',
  '总部线索ID',
  '培育线索编码',
  '培育任务编码',
...
registerUiAction('worry-free-export-details', exportWorryFreeDetails);
