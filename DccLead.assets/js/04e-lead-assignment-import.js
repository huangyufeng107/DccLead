...
registerUiActionCallback('assignment-import', openLeadAssignmentImportWizard);
  ...
  return `
    <div class="assignment-import-panel">
      ...
      <div class="action-btns">
        <button class="btn-secondary" type="button" data-ui-action="assignment-import-download-template">下载模板</button>
      </div>
      <label class="assignment-import-upload">
        <input id="leadAssignmentImportFile" type="file" accept=".xls,.xlsx,.xml,.csv,.tsv,.txt" ${renderUiActionCallback('assignment-import-file', ['$self'])} />
        <div>
...
function renderLeadAssignmentImportFooter() {
  ...
  if (step === 1) {
    return `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('assignment-import-close', ['leadAssignmentImportModal'])}>取消</button>
      <button class="btn-save" type="button" ${leadAssignmentImportState.validatedRows.length ? '' : 'disabled'} ${renderUiActionCallback('assignment-import-preview')}>下一步</button>
    `;
  }
  if (step === 2) {
    return `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('assignment-import-back')}>上一步</button>
      <button class="btn-save" type="button" ${validCount ? '' : 'disabled'} ${renderUiActionCallback('assignment-import-confirm')}>确认导入</button>
    `;
  }
  return `<button class="btn-save" type="button" ${renderUiActionCallback('assignment-import-close', ['leadAssignmentImportModal'])}>完成</button>`;
}
...
registerUiAction('assignment-import-download-template', downloadLeadAssignmentImportTemplate);
registerUiActionCallback('assignment-import-file', input => handleLeadAssignmentImportFile(input.files && input.files[0]));
registerUiActionCallback('assignment-import-close', (...args) => closeModal(...args));
registerUiActionCallback('assignment-import-preview', goLeadAssignmentImportPreview);
registerUiActionCallback('assignment-import-back', backLeadAssignmentImportUpload);
registerUiActionCallback('assignment-import-confirm', confirmLeadAssignmentImport);
