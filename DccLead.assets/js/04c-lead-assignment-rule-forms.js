...
function renderAssignmentSeriesMultiPicker(inputName, selectedValues = [], placeholder = '请选择意向车系') {
  ...
  return `<div class="tag-picker assignment-series-multi-picker"><button class="tag-picker-trigger ${selected.length ? '' : 'placeholder'}" type="button" data-placeholder="${escapeAttr(placeholder)}" onclick="toggleAssignmentSeriesMultiPicker(this)">${escapeHtml(triggerText)}</button><div class="tag-picker-panel"><div class="tag-picker-toolbar"><span class="assignment-series-selected-count">已选 ${selected.length} / ${seriesIntentOptions.length}</span><div class="tag-picker-actions"><button class="tag-picker-action" type="button" onclick="selectAllAssignmentSeriesOptions(this)">全选</button><button class="tag-picker-action" type="button" onclick="clearAssignmentSeriesOptions(this)">清空</button></div></div><div class="tag-picker-list">${seriesIntentOptions.map(item => `<label class="tag-option ${selected.includes(item) ? 'selected' : ''}"><input type="checkbox" name="${escapeAttr(inputName)}" value="${escapeAttr(item)}" ${selected.includes(item) ? 'checked' : ''} onchange="syncAssignmentSeriesMultiPicker(this)" />${escapeHtml(item)}</label>`).join('')}</div></div></div>`;
}
