function openLeadAssignmentRuleModal(mode, id = null) {
  ...
  document.getElementById('leadDispatchRuleModalFooter').innerHTML = mode === 'view'
    ? `<button class="btn-cancel" type="button" ${renderUiActionCallback('assignment-close-modal')}>关闭</button>`
    : `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('assignment-close-modal')}>取消</button>
      <button class="btn-save" type="button" ${renderUiActionCallback('assignment-save-rule')}>保存</button>
  `;
...
registerUiActionCallback('assignment-open-rule', openLeadAssignmentRuleModal);
...
const seriesIntentOptions = ['全部车系', 'N6', '轩逸', '天籁', '逍客', '奇骏', 'ARIYA', '探陆'];

function formatAssignmentSeriesSelection(selectedValues = [], placeholder = '请选择意向车系') {
  const selected = Array.isArray(selectedValues) ? selectedValues : [];
  return selected.length ? `${selected.slice(0, 3).join('、')}${selected.length > 3 ? ` 等${selected.length}项` : ''}` : placeholder;
}

// 多选控件始终只回填前三项，避免编辑已有规则时的长文本撑高输入框。
function formatAssignmentMultiSelectSummary(selectedValues = [], placeholder = '请选择') {
  const selected = Array.isArray(selectedValues) ? selectedValues : [];
  return selected.length ? `${selected.slice(0, 3).join('、')}${selected.length > 3 ? ` 等${selected.length}项` : ''}` : placeholder;
}

function renderAssignmentSeriesMultiPicker(inputName, selectedValues = [], placeholder = '请选择意向车系') {
  const selected = Array.isArray(selectedValues) ? selectedValues : [];
  const triggerText = formatAssignmentSeriesSelection(selected, placeholder);
  const isAllSelected = selected.includes('全部车系');
  return `<div class="tag-picker assignment-series-multi-picker"><button class="tag-picker-trigger ${selected.length ? '' : 'placeholder'}" type="button" data-placeholder="${escapeAttr(placeholder)}" ${renderUiActionCallback('assignment-series-toggle', ['$self'])}>${escapeHtml(triggerText)}</button><div class="tag-picker-panel"><div class="tag-picker-toolbar"><span class="assignment-series-selected-count">已选 ${selected.length} / ${seriesIntentOptions.length}</span><div class="tag-picker-actions"><button class="tag-picker-action" type="button" ${renderUiActionCallback('assignment-series-all', ['$self'])}>全选</button><button class="tag-picker-action" type="button" ${renderUiActionCallback('assignment-series-clear', ['$self'])}>清空</button></div></div><div class="tag-picker-list">${seriesIntentOptions.map(item => {
    const isDisableItem = isAllSelected && item !== '全部车系';
    const opacityStyle = isDisableItem ? 'style="opacity:0.45; cursor:not-allowed;"' : '';
    return `<label class="tag-option ${selected.includes(item) ? 'selected' : ''}" ${opacityStyle}><input type="checkbox" name="${escapeAttr(inputName)}" value="${escapeAttr(item)}" ${selected.includes(item) ? 'checked' : ''} ${isDisableItem ? 'disabled' : ''} ${renderUiActionCallback('assignment-series-sync', ['$self'])} />${escapeHtml(item)}</label>`;
  }).join('')}</div></div></div>`;
}

function toggleAssignmentSeriesMultiPicker(trigger) {
  const picker = trigger.closest('.assignment-series-multi-picker');
  if (!picker) return;
  document.querySelectorAll('.assignment-series-multi-picker .tag-picker-panel').forEach(panel => {
    if (panel !== picker.querySelector('.tag-picker-panel')) panel.classList.remove('show');
  });
  picker.querySelector('.tag-picker-panel')?.classList.toggle('show');
}

function refreshAssignmentSeriesMultiPicker(picker) {
  if (!picker) return;
  const selected = Array.from(picker.querySelectorAll('input[type="checkbox"]:checked')).map(input => input.value);
  const trigger = picker.querySelector('.tag-picker-trigger');
  const count = picker.querySelector('.assignment-series-selected-count');
  if (trigger) {
    trigger.textContent = formatAssignmentSeriesSelection(selected, trigger.dataset.placeholder || '请选择意向车系');
    trigger.classList.toggle('placeholder', !selected.length);
  }
  if (count) count.textContent = `已选 ${selected.length} / ${seriesIntentOptions.length}`;
}

function syncAssignmentSeriesMultiPicker(input) {
  const picker = input.closest('.assignment-series-multi-picker');
  if (!picker) return;
  ...
  if (val === '全部车系') {
    inputs.forEach(i => {
      if (i.value !== '全部车系') {
        ...
        const opt = i.closest('.tag-option');
        if (opt) {
          opt.classList.remove('selected');
          opt.style.opacity = isChecked ? '0.45' : '1';
          opt.style.cursor = isChecked ? 'not-allowed' : 'pointer';
        }
      }
    });
  } else if (isChecked) {
    const allInput = inputs.find(i => i.value === '全部车系');
    if (allInput) {
      allInput.checked = false;
      allInput.closest('.tag-option')?.classList.remove('selected');
    }
  }
  input.closest('.tag-option')?.classList.toggle('selected', input.checked);
  refreshAssignmentSeriesMultiPicker(picker);
}

function selectAllAssignmentSeriesOptions(button) {
  const picker = button.closest('.assignment-series-multi-picker');
  picker?.querySelectorAll('input[type="checkbox"]').forEach(input => { input.checked = true; input.closest('.tag-option')?.classList.add('selected'); });
  refreshAssignmentSeriesMultiPicker(picker);
}

function clearAssignmentSeriesOptions(button) {
  const picker = button.closest('.assignment-series-multi-picker');
  picker?.querySelectorAll('input[type="checkbox"]').forEach(input => { input.checked = false; input.closest('.tag-option')?.classList.remove('selected'); });
  refreshAssignmentSeriesMultiPicker(picker);
}

registerUiActionCallback('assignment-series-toggle', toggleAssignmentSeriesMultiPicker);
registerUiActionCallback('assignment-series-sync', syncAssignmentSeriesMultiPicker);
registerUiActionCallback('assignment-series-all', selectAllAssignmentSeriesOptions);
registerUiActionCallback('assignment-series-clear', clearAssignmentSeriesOptions);
...
const precallCallStatusOptions = ['坐席未接', '坐席接听', '关机', '无人接听', '不在服务区', '忙音', '停机', '空号'];
...
const precallAttributeConfigs = {
  ...
  '通话状态': { label: '通话状态', options: precallCallStatusOptions, isCallStatus: true }
};
...
function renderSmartCodePushTimeSlot(slot = {}, index = 0, total = 1) {
  ...
  return `
    <div class="push-time-slot smart-code-push-slot" data-slot-index="${rowNo}">
      ...
      <input class="form-input time-input smart-code-push-start" type="time" step="60" list="smartCodeTimeOptions" value="${escapeAttr(slot.start || '')}" ${renderUiActionCallback('assignment-sync-push-time')} />
      <div class="push-time-separator">-</div>
      <input class="form-input time-input smart-code-push-end" type="time" step="60" list="smartCodeTimeOptions" value="${escapeAttr(slot.end || '')}" ${renderUiActionCallback('assignment-sync-push-time')} />
      <button class="push-time-remove" type="button" ${renderUiActionCallback('assignment-remove-push-time', ['$self'])} style="${total <= 1 ? 'visibility:hidden' : ''}">删除</button>
    </div>
  `;
}
  ...
  return `
    <div class="push-time-inline optional-push-time smart-code-push-time-config" data-expanded="${isExpanded}">
      <div class="optional-push-time-header">
        ...
        <button class="optional-push-time-toggle" type="button" ${renderUiActionCallback('assignment-toggle-push-time', ['$self'])}>${isExpanded ? '收起配置' : '配置推送时间'}</button>
      </div>
      <div class="optional-push-time-body" ${isExpanded ? '' : 'hidden'}>
        ...
        <div class="push-time-slots">
          ...
          <button class="push-time-add" type="button" ${renderUiActionCallback('assignment-add-push-time')}>新增时段</button>
...
function renderSmartCodeValueInput(selectedValues = [], inputMode = 'picker') {
  ...
  const triggerText = formatAssignmentMultiSelectSummary(selectedValues, '请选择SmartCode');
  return `
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
      <div class="smart-code-input-mode" style="margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between;">
        ...
        <div class="series-mode-toggle" style="margin-top: 0; margin-bottom: 0;">
          <button class="duration-tab ${inputMode === 'picker' ? 'active' : ''}" type="button" id="smartCodePickerMode" ${renderUiActionCallback('assignment-smartcode-mode', ['picker'])}>下拉勾选</button>
          <button class="duration-tab ${inputMode === 'manual' ? 'active' : ''}" type="button" id="smartCodeManualMode" ${renderUiActionCallback('assignment-smartcode-mode', ['manual'])}>手动输入</button>
        </div>
      </div>
      <div id="smartCodePickerInputPanel" style="${inputMode === 'manual' ? 'display:none' : ''}">
        <div class="tag-picker project-name-picker">
          <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" id="projectNameTrigger" type="button" ${renderUiActionCallback('assignment-project-toggle')}><span data-picker-summary>${escapeHtml(triggerText)}</span></button>
          <div class="tag-picker-panel" id="projectNamePanel">
            <div class="tag-picker-toolbar">
              ...
              <div class="tag-picker-actions">
                <button class="tag-picker-action" type="button" ${renderUiActionCallback('assignment-project-all')}>全选</button>
                <button class="tag-picker-action" type="button" ${renderUiActionCallback('assignment-project-clear')}>清空</button>
              </div>
            </div>
            <div class="tag-picker-search">
              <input type="search" id="projectNameSearchInput" placeholder="搜索SmartCode" ${renderUiActionCallback('assignment-project-filter', ['$value'])} />
            </div>
            <div class="tag-picker-list" id="projectNameList">
              ${assignmentSmartCodeOptions.map(item => `
                <label class="tag-option" data-project-option="${escapeAttr(item)}">
                  <input type="checkbox" name="assignmentSmartCodeValue" value="${item}" ${selectedValues.includes(item) ? 'checked' : ''} ${renderUiActionCallback('assignment-project-option', ['$self'])} />
                  ${item}
                </label>
              `).join('')}
            </div>
      ...
      <div id="smartCodeManualInputPanel" style="${inputMode === 'manual' ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" id="assignmentSmartCodeManualInput" rows="4" style="margin-top: 0;" placeholder="可一次粘贴多个 SmartCode，支持换行、逗号、分号或空格分隔">${escapeHtml(manualValue)}</textarea>
...
function renderProjectNameValueInput(selectedValues = [], inputMode = 'picker') {
  const manualValue = selectedValues.join('\n');
  const triggerText = formatAssignmentMultiSelectSummary(selectedValues, '请选择大项目名');
  return `
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
      <div class="smart-code-input-mode" style="margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between;">
        <div class="form-field-helper-label">录入方式</div>
        <div class="series-mode-toggle" style="margin-top: 0; margin-bottom: 0;">
          <button class="duration-tab ${inputMode === 'picker' ? 'active' : ''}" type="button" id="projectNamePickerMode" ${renderUiActionCallback('assignment-project-mode', ['picker'])}>下拉勾选</button>
          <button class="duration-tab ${inputMode === 'manual' ? 'active' : ''}" type="button" id="projectNameManualMode" ${renderUiActionCallback('assignment-project-mode', ['manual'])}>手动输入</button>
        </div>
      </div>
      <div id="projectNamePickerInputPanel" style="${inputMode === 'manual' ? 'display:none' : ''}">
        <div class="tag-picker project-name-picker">
          <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" id="projectNameTrigger" type="button" onclick="toggleProjectNamePicker()"><span data-picker-summary>${escapeHtml(triggerText)}</span></button>
          <div class="tag-picker-panel" id="projectNamePanel">
            <div class="tag-picker-toolbar">
      ...
      <div id="projectNameManualInputPanel" style="${inputMode === 'manual' ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" id="assignmentProjectNameManualInput" rows="4" style="margin-top: 0;" placeholder="可一次粘贴多个大项目名，支持换行、逗号、分号或空格分隔">${escapeHtml(manualValue)}</textarea>
  ...
  return `
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
      <div class="smart-code-input-mode" style="margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between;">
        <div class="form-field-helper-label">录入方式</div>
        <div class="series-mode-toggle" style="margin-top: 0; margin-bottom: 0;">
          <button class="duration-tab ${inputMode === 'picker' ? 'active' : ''}" type="button" id="dealerPickerMode" ${renderUiActionCallback('assignment-dealer-mode', ['picker'])}>下拉勾选</button>
          <button class="duration-tab ${inputMode === 'manual' ? 'active' : ''}" type="button" id="dealerManualMode" ${renderUiActionCallback('assignment-dealer-mode', ['manual'])}>手动输入</button>
        </div>
      </div>
      ...
      <div id="dealerManualInputPanel" style="${inputMode === 'manual' ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" id="assignmentDealerManualInput" rows="4" style="margin-top: 0;" placeholder="可一次粘贴多个意向门店编码，支持换行、逗号、分号或空格分隔">${escapeHtml(manualValue)}</textarea>
        <div class="series-form-hint" style="margin-bottom: 0;">保存时会自动拆分并去重；不在下拉选项中的意向门店也可直接录入。</div>
      </div>
    </div>
  `;
}
...
function renderCompactAssignmentRuleForm(rule, dimension) {
  const config = getCompactAssignmentConfig(dimension);
  const selectedValues = getCompactAssignmentSelectedValues(rule, dimension);
  const compactValueSummary = formatAssignmentMultiSelectSummary(selectedValues, `请选择${config.valueLabel}`);
  const relatedSeriesValues = hasRelatedSeriesCondition(dimension) ? getCompactSelectedSeries(rule, dimension) : [];
  const dealerLeadStatusValues = dimension.key === 'dealer' ? getDealerSelectedLeadStatuses(rule) : [];
  const leadStatusReasonValues = dimension.key === 'lead-status' ? getLeadStatusReasonValues(rule) : [];
      ...
      <section class="series-form-section">
        ...
        <div class="series-action-box">
          ${dimension.key === 'sc' ? `
            <div class="form-group">
              ...
              <div class="series-mode-toggle">
                <button class="duration-tab ${smartCodeMode === 'single' ? 'active' : ''}" type="button" id="seriesAllocationSingleTab" ${renderUiActionCallback('assignment-series-mode', ['$self', 'single'])}>不按比例</button>
                <button class="duration-tab ${smartCodeMode === 'ratio' ? 'active' : ''}" type="button" id="seriesAllocationRatioTab" ${renderUiActionCallback('assignment-series-mode', ['$self', 'ratio'])}>按比例</button>
              </div>
            </div>
            <div class="form-group" id="seriesSingleAllocation" style="${smartCodeMode === 'ratio' ? 'display:none' : ''}">
              <div class="form-label">外呼类型 <span class="required">*</span></div>
              ${renderAssignmentSingleChannelPicker(singleChannel, ['series', 'sc', 'project', 'channel', 'lead-status', 'dealer'].includes(dimension.key))}
          ...
          <div class="form-group">
            <div class="form-label">是否按比例 <span class="required">*</span></div>
            <div class="series-mode-toggle">
              <button class="duration-tab ${mode === 'single' ? 'active' : ''}" type="button" id="seriesAllocationSingleTab" ${renderUiActionCallback('assignment-series-mode', ['$self', 'single'])}>不按比例</button>
              <button class="duration-tab ${mode === 'ratio' ? 'active' : ''}" type="button" id="seriesAllocationRatioTab" ${renderUiActionCallback('assignment-series-mode', ['$self', 'ratio'])}>按比例</button>
            </div>
          </div>
      ...
      <section class="series-form-section">
        ...
        ${hasRelatedSeriesCondition(dimension) ? `
          <div class="form-group">
            ...
            ${renderAssignmentSeriesMultiPicker('assignmentRelatedSeriesValue', relatedSeriesValues)}
        ...
        <div class="form-group">
          ...
          ${dimension.key === 'sc' ? renderSmartCodeValueInput(selectedValues, smartCodeInputMode) : dimension.key === 'project' ? renderProjectNameValueInput(selectedValues, projectNameInputMode) : dimension.key === 'dealer' ? renderDealerValueInput(selectedValues, dealerInputMode) : dimension.key === 'channel' ? `
            <div class="tag-picker project-name-picker">
              <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" id="projectNameTrigger" type="button" ${renderUiActionCallback('assignment-project-toggle')}><span data-picker-summary>${escapeHtml(compactValueSummary)}</span></button>
              <div class="tag-picker-panel" id="projectNamePanel">
                <div class="tag-picker-toolbar">
                ...
                <div class="tag-picker-search">
                  <input type="search" id="projectNameSearchInput" placeholder="搜索${config.valueLabel}" ${renderUiActionCallback('assignment-project-filter', ['$value'])} />
                </div>
                <div class="tag-picker-list" id="projectNameList">
                  ${config.options.map(item => `
                    <label class="tag-option" data-project-option="${escapeAttr(item)}">
                      <input type="checkbox" name="${config.inputName}" value="${item}" ${selectedValues.includes(item) ? 'checked' : ''} ${renderUiActionCallback('assignment-project-option', ['$self'])} />
                      ${item}
                    </label>
                  `).join('')}
                </div>
          ...
          ` : dimension.key === 'series' ? `
            ${renderAssignmentSeriesMultiPicker(config.inputName, selectedValues)}
          ` : `
            <div class="series-select-panel">
              <div class="series-series-grid">
                ${config.options.map(item => `
