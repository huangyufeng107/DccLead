...
function renderAiRedialPolicyPage() {
  ...
  page.innerHTML = `
    ...
    <div class="filter-row">
      ...
      <input class="filter-select" id="aiRedialKeywordFilter" placeholder="请输入内容" ${renderUiActionCallback('ai-redial-filter')} />
      <span class="filter-label">原外呼类型：</span>
      <select class="filter-select" id="aiRedialOutboundTypeFilter" ${renderUiActionCallback('ai-redial-filter')}>${renderSimpleOptions(aiRedialOutboundTypeOptions, '', '全部')}</select>
      <span class="filter-label">新外呼类型：</span>
      <select class="filter-select" id="aiRedialNewOutboundTypeFilter" ${renderUiActionCallback('ai-redial-filter')}>${renderSimpleOptions(aiRedialNewOutboundTypeOptions, '', '全部')}</select>
      <span class="filter-label">命中类型：</span>
      <select class="filter-select" id="aiRedialAttributeFilter" ${renderUiActionCallback('ai-redial-filter')}>${renderSimpleOptions(aiRedialAttributeOptions, '', '全部')}</select>
      <span class="filter-label">状态：</span>
      <select class="filter-select" id="aiRedialStatusFilter" ${renderUiActionCallback('ai-redial-filter')}><option value="">全部</option><option value="启用">启用</option><option value="停用">停用</option></select>
      <button class="btn-secondary" type="button" ${renderUiActionCallback('ai-redial-reset')}>重置</button>
    </div>
    <div class="card">
      <div class="section-header">
        <div class="section-title">外呼重推配置列表</div>
        <button class="btn-add strategy-add-btn" type="button" ${renderUiActionCallback('ai-redial-open-modal', ['add'])}>新增配置</button>
      </div>
      ...
      <div class="pagination">
        ...
        <div class="pagination-btns">
          <select class="hit-page-size" id="aiRedialPageSize" ${renderUiActionCallback('ai-redial-page-size', ['$value'])}>
            <option value="5">每页 5 条</option>
            <option value="10">每页 10 条</option>
            <option value="20">每页 20 条</option>
          </select>
          <button class="page-btn" type="button" ${renderUiActionCallback('ai-redial-page', [-1])}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></button>
          <select class="hit-page-size" id="aiRedialPageSelect" ${renderUiActionCallback('ai-redial-page-select', ['$value'])}></select>
          <button class="page-btn" type="button" ${renderUiActionCallback('ai-redial-page', [1])}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
      </div>
    </div>
  `;
...
function renderAiRedialOutboundLabelPicker(selectedLabels = [], readonly = false) {
  ...
  return `
    <div class="tag-picker dispatch-tag-picker">
      <button class="tag-picker-trigger placeholder" id="aiRedialOutboundLabelTrigger" type="button" ${readonly ? 'disabled' : renderUiActionCallback('ai-redial-label-toggle')}>请选择外呼标签</button>
      <div class="tag-picker-panel" id="aiRedialOutboundLabelPanel">
        <div class="tag-picker-toolbar">
          ...
          <div class="tag-picker-actions">
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('ai-redial-label-all')}>全选</button>
            <button class="tag-picker-action" type="button" ${renderUiActionCallback('ai-redial-label-clear')}>清空</button>
          </div>
        </div>
        <div class="tag-picker-search">
          <input type="search" placeholder="搜索外呼标签" ${renderUiActionCallback('ai-redial-label-filter', ['$value'])} />
        </div>
        <div class="tag-picker-list">
          ${leadDispatchTagOptions.map(tag => `
            <label class="tag-option">
              <input type="checkbox" name="aiRedialOutboundLabel" value="${escapeAttr(tag)}" ${safeSelected.includes(tag) ? 'checked' : ''} ${readonly ? 'disabled' : renderUiActionCallback('ai-redial-label-option', ['$self'])} />
              ${tag}
            </label>
          `).join('')}
        </div>
...
function getAiRedialIntentOptions(leadStatuses = [], currentValue = '') {
  // Keep the existing option range/order; the shared renderer resolves dictionary labels.
...
function renderAiRedialTable() {
  ...
  body.innerHTML = pageRows.length ? pageRows.map(rule => `
    <tr>
      ...
      <td><div class="action-btns"><button class="action-btn view" type="button" ${renderUiActionCallback('ai-redial-open-modal', ['view', rule.id])}>查看</button><button class="action-btn edit" type="button" ${renderUiActionCallback('ai-redial-open-modal', ['edit', rule.id])}>编辑</button><button class="action-btn delete" type="button" ${renderUiActionCallback('ai-redial-delete', [rule.id])}>删除</button></div></td>
    </tr>
  `).join('') : `<tr><td colspan="10"><div class="empty-state">${(aiRedialRules || []).length === 0 ? '暂无数据' : '暂无匹配的外呼重推配置'}</div></td></tr>`;
}
...
function openAiRedialRuleModal(mode, id) {
  ...
  document.getElementById('aiRedialRuleModalFooter').innerHTML = mode === 'view'
    ? `<button class="btn-secondary" type="button" ${renderUiActionCallback('ai-redial-close', ['aiRedialRuleModal'])}>关闭</button><button class="btn-add" type="button" ${renderUiActionCallback('ai-redial-open-modal', ['edit', rule.id])}>编辑</button>`
    : `<button class="btn-secondary" type="button" ${renderUiActionCallback('ai-redial-close', ['aiRedialRuleModal'])}>取消</button><button class="btn-add" type="button" ${renderUiActionCallback('ai-redial-save', [mode])}>保存</button>`;
...
function renderAiRedialRuleForm(rule, readonly = false) {
  ...
  return `
    ...
    <section class="dispatch-form-section"><div class="dispatch-section-title">命中条件</div><div class="dispatch-form-grid">
        ...
        <div class="form-group"><div class="form-label">命中类型 <span class="required">*</span></div><select class="form-input" id="aiRedialFormAttribute" ${renderUiActionCallback('ai-redial-condition-sync')} ${disabled}>${renderSimpleOptions(aiRedialAttributeOptions, attributeType, '请选择')}</select></div>
...
// 外呼重推配置的模板交互统一通过受控回调桥接，避免内联事件分散在动态内容中。
registerUiActionCallback('ai-redial-filter', filterAiRedialTable);
registerUiActionCallback('ai-redial-reset', resetAiRedialFilters);
registerUiActionCallback('ai-redial-open-modal', openAiRedialRuleModal);
registerUiActionCallback('ai-redial-page-size', changeAiRedialPageSize);
registerUiActionCallback('ai-redial-page', changeAiRedialPage);
registerUiActionCallback('ai-redial-page-select', selectAiRedialPage);
registerUiActionCallback('ai-redial-label-toggle', toggleAiRedialOutboundLabelPicker);
registerUiActionCallback('ai-redial-label-all', selectAllAiRedialOutboundLabels);
registerUiActionCallback('ai-redial-label-clear', clearAiRedialOutboundLabels);
registerUiActionCallback('ai-redial-label-filter', value => filterTagPickerOptions('aiRedialOutboundLabelPanel', value));
registerUiActionCallback('ai-redial-label-option', toggleAiRedialOutboundLabelOption);
registerUiActionCallback('ai-redial-condition-sync', syncAiRedialConditionFields);
registerUiActionCallback('ai-redial-lead-status-sync', syncAiRedialLeadStatusFields);
registerUiActionCallback('ai-redial-close', (...args) => closeModal(...args));
registerUiActionCallback('ai-redial-save', saveAiRedialRule);
registerUiActionCallback('ai-redial-delete', deleteAiRedialRule);
