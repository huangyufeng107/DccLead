...
function renderTestDriveCoreTableRowHtml(item) {
  ...
  return `<tr>
    ...
    <td style="text-align:center;"><button class="mw-detail-link" type="button" onclick="openOpsTestDriveDetailPage('${item.testDriveCode}')">查看</button></td>
  </tr>`;
}
...
function exportOpsTestDriveData() {
  ...
  const exportCols = [
    ...
    { key: 'taskFollowTime', label: '总部首次外呼时间' },
    { key: 'contactStatus', label: '外呼接通状态' },
        ...
        <table id="testDriveDetailTable" class="mw-report-table" style="width:100%; border-collapse:separate; border-spacing:0; font-size:12px; text-align:left; white-space:nowrap;">
          <thead>
            ...
            <tr style="background:#f8fafc; color:#334155; font-weight:600;">
              ...
              <th id="groupColHq" colspan="12" style="position:sticky; top:0; z-index:15; background:var(--surface-container-low, #f3f3fd); color:var(--on-surface-variant, #434654); text-align:center; padding:6px 8px; font-weight:700; border-right:1px solid var(--surface-container-highest, #e2e2ec); border-bottom:1px solid var(--surface-container-highest, #e2e2ec);">
                ⑤ 总部外呼与回访督导 (12项)
              </th>
            ...
            <tr style="background:#f1f5f9; color:#334155; font-weight:600;">
              ...
              <th style="position:sticky; top:31px; z-index:14; background:#f1f5f9; padding:8px 8px; border-bottom:2px solid #cbd5e1;">总部首次外呼时间</th>
              <th style="position:sticky; top:31px; z-index:14; background:#f1f5f9; padding:8px 8px; border-bottom:2px solid #cbd5e1;">外呼接通状态</th>
...
const opsTestDriveMockData = [
  ...
  },
  {
    ...
    scheduleDispatchVisitDesc: '排程系统下发成功，滨江店前台已接单',
    contactStatus: '正常接通',
  ...
  return `
    ...
    <div class="detail-tab-panel active" id="testDriveDetailInfoPanel">
      ...
      <section class="detail-section">
        ...
        <div class="detail-section-body">
          <div class="lead-detail-item">
            <div class="lead-detail-label">外呼坐席账号</div>
            <div class="lead-detail-value" style="font-family:monospace;">${item.agentAccount || '—'}</div>
          </div>
          <div class="lead-detail-item">
          ...
          <div class="lead-detail-item">
            <div class="lead-detail-label">总部首次外呼时间</div>
            <div class="lead-detail-value">${item.taskFollowTime || '—'}</div>
          </div>
          <div class="lead-detail-item">
            <div class="lead-detail-label">外呼接通状态</div>
            <div class="lead-detail-value"><span class="tag-chip green">${item.contactStatus || '—'}</span></div>
          </div>
          <div class="lead-detail-item">
            <div class="lead-detail-label">电销初始触达状态</div>
            <div class="lead-detail-value">${item.teleInitialStatus || '—'}</div>
          </div>
          <div class="lead-detail-item" style="grid-column:span 3;">
              ...
              <div style="border:1px solid ${isLatest ? '#93c5fd' : '#e2e8f0'}; border-radius:8px; background:#ffffff; box-shadow:${isLatest ? '0 2px 6px rgba(37,99,235,0.06)' : '0 1px 2px rgba(0,0,0,0.02)'}; overflow:hidden;">
                <div style="padding:10px 14px; background:${isLatest ? '#eff6ff' : '#f8fafc'}; border-bottom:1px solid ${isLatest ? '#dbeafe' : '#edf2f7'}; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
                  ...
                  <div style="font-size:12px; color:#64748b; display:flex; align-items:center; gap:14px;">
                    ...
                    ${rec.agentAccount ? `<span>坐席账号：<code style="background:${isLatest ? '#dbeafe' : '#f1f5f9'}; padding:1px 6px; border-radius:4px; font-size:11px;">${rec.agentAccount}</code></span>` : ''}
                  </div>
                </div>
...
function getTestDriveSystemLogs(item) {
  ...
  const followTime = item.taskFollowTime && item.taskFollowTime !== '-' ? item.taskFollowTime : '2026-09-15 10:15:00';
  ...
  return [
    ...
    },
    {
      ...
      type: '电话外呼',
      desc: `坐席完成下发确认沟通，外呼接通状态：${item.contactStatus || '正常接通'}，下发纪要：${item.scheduleDispatchVisitDesc || '排程下发至专营店'}。`,
      result: '已下发'
    },
    {
      time: storeTime,
