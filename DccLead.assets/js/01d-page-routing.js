...
function setLeadDispatchAuditTabsVisible(visible) {
  document.getElementById('leadDispatchAuditTabs')?.classList.toggle('show', visible);
}
...
function switchBlacklistTab(tab = 'blacklist-policy-rules') {
  ...
  const dealerListPage = document.getElementById('blacklistDealerListPage');
  
  if (tab === 'blacklist-policy-rules') {
    setPageName('培育策略 / 黑名单配置 / 黑名单策略');
    if (userListPage) userListPage.classList.remove('show');
    if (dealerListPage) dealerListPage.classList.remove('show');
  ...
  } else if (tab === 'blacklist-dealer-list') {
    setPageName('培育策略 / 黑名单配置 / 经销商黑名单');
    rulesPageElements.forEach(el => el.style.display = 'none');
    if (userListPage) userListPage.classList.remove('show');
    if (dealerListPage) dealerListPage.classList.add('show');
    if (typeof renderBlacklistDealerListPage === 'function') renderBlacklistDealerListPage();
  }
}
...
function switchStrategyConfigTab(tab) {
  const labels = {
    ...
    'retention-data-config': '滞留数据配置',
    'tianwang-ai-dispatch': '天网付费自动下发'
  };
  ...
  } else if (tab === 'retention-data-config') {
    document.getElementById('retentionDataConfigPage').classList.add('show');
    renderRetentionDataConfigPage();
  } else if (tab === 'tianwang-ai-dispatch') {
    document.getElementById('tianwangAiDispatchConfigPage').classList.add('show');
    renderTianwangPaidDispatchConfig('ai');
  }
}
...
function switchManualAiConfigTab(tab) {
  const labels = {
    ...
    'next-visit-precall-config': '回访预外呼配置',
    'return-visit-order-config': '回访工单配置',
    'amap-map-config': '高德地图配置',
    'frequent-intent-policy': '关键行为提醒',
    'tianwang-manual-dispatch': '天网付费自动下发'
  };
  const configuratorKey = strategyConfiguratorPageMap[tab];
  setStrategyConfigTabsVisible(false);
  ...
  hideLeadPages();
  setPolicyContentVisible(false);
  if (tab === 'frequent-intent-policy') {
    document.getElementById('frequentIntentPolicyPage').classList.add('show');
  } else if (tab === 'tianwang-manual-dispatch') {
    document.getElementById('tianwangManualDispatchConfigPage').classList.add('show');
    renderTianwangPaidDispatchConfig('manual');
...
function showLeadImportPage() {
  ...
  document.querySelector('.scheduling-nav')?.classList.remove('show');
  setSidebarActiveByName('线索导入');
  setPolicyContentVisible(false);
  document.getElementById('designStage')?.classList.remove('show');
  setPageName('NEV培育 / 线索导入');
  document.getElementById('leadImportPage')?.classList.add('show');
  if (typeof renderLeadImportPage === 'function') renderLeadImportPage();
}
...
document.querySelectorAll('.sidebar .sidebar-item').forEach(item => {
  item.addEventListener('click', () => {
    ...
    document.querySelector('.test-drive-nav')?.classList.remove('show');
    if (name === '培育策略') {
      showPolicyPage();
    } else if (name === '线索管理') {
      showLeadsPage();
    } else if (name === '试驾排程') {
      showTestDriveSchedulingPage();
    } else if (name === '线索导入') {
      showLeadImportPage();
    } else if (name === '培育任务') {
      showNurtureTaskPage();
...
function showLeadsPage(defaultName = '留资线索') {
  ...
  setManualAiConfigTabsVisible(false);
  setExcellentConfigManageTabsVisible(false);
  setLeadDispatchAuditTabsVisible(defaultName === '线索分发审核');
  document.querySelector('nav[aria-label="培育策略三级菜单"]').classList.add('hidden');
  ...
  if (defaultName === '留资线索') {
    ...
    const page = document.getElementById('resourceLeadPage');
  ...
  } else if (defaultName === '线索分发审核') {
    designStage.classList.remove('show');
    if (typeof switchLeadDispatchAuditTab === 'function') {
      switchLeadDispatchAuditTab('lead-dispatch-audit-ai-cold');
    } else {
      const auditPage = document.getElementById('leadDispatchAuditPage');
      if (auditPage) auditPage.classList.add('show');
    }
  } else {
    designStage.classList.add('show');
...
function showTestDriveSchedulingPage() {
  hideLeadPages();
  setStrategyConfigTabsVisible(false);
  setManualAiConfigTabsVisible(false);
  setExcellentConfigManageTabsVisible(false);
  setLeadDispatchAuditTabsVisible(false);
  document.querySelector('nav[aria-label="培育策略三级菜单"]')?.classList.add('hidden');
  document.querySelector('.leads-nav')?.classList.remove('show');
  ...
  setTestDriveNavActive('ai');
  setPolicyContentVisible(false);
  document.getElementById('designStage')?.classList.remove('show');
  setPageName('NEV培育 / 试驾排程 / 中台推送监控');
  document.getElementById('aiTestDriveSchedulePage')?.classList.add('show');
  if (typeof renderAiTestDriveSchedulePage === 'function') renderAiTestDriveSchedulePage();
}

function setTestDriveNavActive(source) {
  document.querySelectorAll('.test-drive-nav .strategy-nav-item').forEach(item => {
    item.classList.toggle('active', item.dataset.source === source);
  });
}

function switchTestDriveSource(source) {
  if (source === 'manual') {
    if (typeof openManualTestDriveAppointmentLeadPage === 'function') openManualTestDriveAppointmentLeadPage();
    return;
  }
  showTestDriveSchedulingPage();
}
...
function switchReportsNav(el) {
  document.querySelectorAll('.reports-nav .strategy-nav-item').forEach(item => item.classList.remove('active'));
  el.classList.add('active');
  showReportsPage(el.textContent.trim());
}

// Static top-level navigation is routed through the same declarative action
// bridge as dynamic controls; callbacks resolve only to these named handlers.
registerUiActionCallback('strategy-nav-switch', el => switchStrategyNav(el));
registerUiActionCallback('leads-nav-switch', el => switchLeadsNav(el));
registerUiActionCallback('test-drive-source-switch', source => switchTestDriveSource(source));
registerUiActionCallback('reports-nav-switch', el => switchReportsNav(el));
registerUiActionCallback('ops-nav-switch', el => switchOpsNav(el));
registerUiActionCallback('settings-subpage-switch', sub => switchSettingsSubPage(sub));
registerUiActionCallback('strategy-config-tab-switch', tab => switchStrategyConfigTab(tab));
registerUiActionCallback('manual-config-tab-switch', tab => switchManualAiConfigTab(tab));
registerUiActionCallback('excellent-config-tab-switch', tab => switchExcellentConfigManageTab(tab));
registerUiActionCallback('blacklist-tab-switch', tab => switchBlacklistTab(tab));
