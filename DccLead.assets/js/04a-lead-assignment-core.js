...
function renderAllocationPolicyPanel(targetId = 'leadAssignmentDimensionPanel') {
  ...
  panel.innerHTML = `
    ...
    <div class="filter-row">
      <span class="filter-label">关键字：</span>
      <input class="lead-input" style="width:130px; margin-right: 12px;" id="allocationPolicyKeywordFilter" placeholder="请输入" oninput="filterAllocationPolicyTable()" />
    ...
    <tr>
      ...
      <td>
        <div class="action-btns">
          ...
          <button class="action-btn ${policy.status === '启用' ? 'delete' : 'view'}" type="button" onclick="toggleAllocationPolicy('${policy.id}')">${policy.status === '启用' ? '停用' : '启用'}</button>
