...
function renderBlacklistUserListPage() {
  ...
  if (info) {
    info.textContent = `共 ${rows.length} 条记录，当前第 ${blacklistUserCurrentPage} / ${totalPages} 页`;
    syncPaginationButtons(info.closest('.pagination') || '#blacklistUserPagination', blacklistUserCurrentPage, totalPages);
  }
...
function openBlacklistUserDetailModal(id) {
  ...
  body.innerHTML = `
    <div class="rule-detail-body" style="grid-template-columns: repeat(2, 1fr); gap: 16px;">
      ...
      <div class="view-field"><div class="view-field-label">手机号码</div><div class="view-field-value">${escapeHtml(user.phone || '—')}</div></div>
