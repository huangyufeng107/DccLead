...
function renderOutboundLabelMappingPage() {
  ...
  page.innerHTML = `
    ...
    <div class="filter-row">
      ...
      <input class="lead-input" style="width:130px; margin-right: 12px;" id="outboundMappingKeywordFilter" placeholder="请输入" data-ui-action="outbound-mapping-filter" />
      <span class="filter-label">标签分类：</span>
      <select class="filter-select" id="outboundMappingCategoryFilter" data-ui-action="outbound-mapping-filter">
      ...
      <select class="filter-select" id="outboundMappingSourceFilter" data-ui-action="outbound-mapping-filter">
        <option value="">全部</option>
        ${outboundMappingSourceOptions.map(item => `<option value="${escapeAttr(item)}">${escapeHtml(item)}</option>`).join('')}
      </select>
      <span class="filter-label">状态：</span>
      <select class="filter-select" id="outboundMappingStatusFilter" data-ui-action="outbound-mapping-filter">
        <option value="">全部</option>
        <option value="启用">启用</option>
        <option value="停用">停用</option>
      </select>
      <button class="btn-secondary" type="button" data-ui-action="outbound-mapping-filter-reset">重置</button>
    </div>
    <div class="card">
      <div class="section-header">
        <div class="section-title">外呼标签映射列表</div>
        <button class="btn-add strategy-add-btn" type="button" data-ui-action="outbound-mapping-add">新增映射</button>
      </div>
      ...
      <div class="pagination">
        ...
        <div class="pagination-btns">
          <select class="hit-page-size" id="outboundLabelMappingPageSize" data-ui-action="outbound-mapping-page-size">
            <option value="5">每页 5 条</option>
            <option value="10">每页 10 条</option>
            <option value="20">每页 20 条</option>
          </select>
          <button class="page-btn" type="button" data-ui-action="outbound-mapping-page" data-direction="-1"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></button>
          <select class="hit-page-size" id="outboundLabelMappingPageSelect" data-ui-action="outbound-mapping-page-select"></select>
          <button class="page-btn" type="button" data-ui-action="outbound-mapping-page" data-direction="1"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
      </div>
    </div>
  `;
...
function getFilteredOutboundLabelMappingRules() {
  const keyword = document.getElementById('outboundMappingKeywordFilter')?.value.trim().toLowerCase() || '';
  ...
  return outboundLabelMappingRules
    .filter(rule => {
      ...
      const text = [
        rule.name,
        `OLM-${String(rule.id).padStart(3, '0')}`,
        rule.category,
        (getOutboundMappingRuleSources(rule) || []).join(' '),
        (getOutboundMappingDisplayMatchValues(rule) || []).join(' '),
        rule.output
...
function renderOutboundLabelMappingTable() {
  ...
  if (info) {
    info.textContent = `共 ${rows.length} 条记录，当前第 ${outboundLabelMappingCurrentPage} / ${totalPages} 页`;
    syncPaginationButtons(info.closest('.pagination'), outboundLabelMappingCurrentPage, totalPages);
  }
    ...
    <tr>
      ...
      <td>
        <div class="action-btns">
          <button class="action-btn view" type="button" data-ui-action="outbound-mapping-view" data-rule-id="${rule.id}">查看</button>
          <button class="action-btn edit" type="button" data-ui-action="outbound-mapping-edit" data-rule-id="${rule.id}">编辑</button>
          <button class="action-btn delete" type="button" data-ui-action="outbound-mapping-delete" data-rule-id="${rule.id}">删除</button>
        </div>
      </td>
    </tr>
...
function resetOutboundLabelMappingFilters() {
  ['outboundMappingKeywordFilter', 'outboundMappingCategoryFilter', 'outboundMappingSourceFilter', 'outboundMappingStatusFilter'].forEach(id => {
...
function renderOutboundMappingValueField(source, selected = [], logic = '或') {
  ...
  const triggerText = formatOutboundMappingValueSelection(safeSelected, config.placeholder);
  return `
    <div class="tag-picker dispatch-tag-picker outbound-mapping-picker" data-source="${escapeAttr(source)}">
      <button class="tag-picker-trigger ${safeSelected.length ? '' : 'placeholder'}" id="${config.triggerId}" type="button" onclick="toggleOutboundMappingValuePicker('${source}')"><span class="tag-picker-trigger-text" data-picker-summary>${escapeHtml(triggerText)}</span></button>
...
function formatOutboundMappingValueSelection(selected, placeholder) {
  if (!selected?.length) return placeholder;
  const visibleLabels = selected.slice(0, 3);
  return visibleLabels.join('、') + (selected.length > 3 ? ` 等${selected.length}项` : '');
}
...
registerUiAction('outbound-mapping-filter', (target, event) => {
  if (event.type === 'input' || event.type === 'change') filterOutboundLabelMappingTable();
});
registerUiAction('outbound-mapping-filter-reset', resetOutboundLabelMappingFilters);
registerUiAction('outbound-mapping-add', () => openOutboundLabelMappingModal('add'));
registerUiAction('outbound-mapping-page-size', (target, event) => {
  if (event.type === 'change') changeOutboundLabelMappingPageSize(target.value);
});
registerUiAction('outbound-mapping-page', target => changeOutboundLabelMappingPage(Number(target.dataset.direction)));
registerUiAction('outbound-mapping-page-select', (target, event) => {
  if (event.type === 'change') selectOutboundLabelMappingPage(target.value);
});
registerUiAction('outbound-mapping-view', target => openOutboundLabelMappingModal('view', Number(target.dataset.ruleId)));
registerUiAction('outbound-mapping-edit', target => openOutboundLabelMappingModal('edit', Number(target.dataset.ruleId)));
registerUiAction('outbound-mapping-delete', target => deleteOutboundLabelMappingRule(Number(target.dataset.ruleId)));
