...
function renderOutboundLabelMappingPage() {
  ...
  page.innerHTML = `
    ...
    <div class="filter-row">
      ...
      <span class="filter-label">匹配来源：</span>
...
function renderOutboundLabelMappingDetail(rule) {
  ...
  const fields = [
    ['规则名称', rule.name],
    ['标签分类', rule.category],
    ['匹配来源', getOutboundMappingRuleSources(rule).join('、')],
    ['匹配值', getOutboundMappingDisplayMatchValues(rule).join('、')],
    ...(getOutboundMappingRuleSources(rule).includes('外呼标签') ? [['外呼标签判断条件', logicBySource['外呼标签'] === '且' ? '且（AND）' : '或（OR）']] : []),
    ...(getOutboundMappingRuleSources(rule).includes('通话状态') ? [['通话状态判断条件', logicBySource['通话状态'] === '且' ? '且（AND）' : '或（OR）']] : []),
    ['映射结果', rule.output],
    ['状态', rule.enabled ? '启用' : '停用'],
    ['更新时间', rule.updatedAt],
    ['备注', rule.remark || '—']
  ];
  return `<div class="rule-detail-layout"><section class="rule-detail-section"><div class="rule-detail-title">规则详情</div><div class="rule-detail-body">${fields.map(([label, value]) => `<div class="rule-detail-field"><div class="rule-detail-label">${label}</div><div class="rule-detail-value">${detailValue(value)}</div></div>`).join('')}</div></section></div>`;
}
    ...
    <div class="dispatch-rule-form">
      <section class="dispatch-form-section">
        ...
        <div class="dispatch-form-grid">
          <div class="form-group wide"><div class="form-label">规则名称 <span class="required">*</span></div><input class="form-input" id="outboundMappingName" value="${escapeAttr(target.name)}" placeholder="例：客观理由拒绝归因" /></div>
      ...
      <section class="dispatch-form-section">
        <div class="dispatch-section-title">匹配关系</div>
        <div class="form-group">
          <div class="form-label">匹配来源 <span class="required">*</span></div>
...
function saveOutboundLabelMappingRule() {
  ...
  const sources = getSelectedOutboundMappingSources();
  if (!sources.length) { showToast('请选择匹配来源', false); return; }
  ...
  if (sources.some(source => !getOutboundMappingSources(output).includes(source))) { showToast('请选择有效匹配来源', false); return; }
  const payload = {
    name,
    category,
    sources,
