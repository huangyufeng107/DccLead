...
function renderExcellentConfigImportUpload() {
  ...
  return `<div class="assignment-import-panel"><div class="assignment-import-tip">Step1 操作说明：下载模板了解格式要求 → 按模板填写 → 上传Excel文件。模板包含必填字段，上传后将校验空值、状态和意向车系枚举。</div><div class="action-btns" style="margin-bottom:12px;"><button class="btn-secondary" type="button" data-ui-action="excellent-import-download-template">下载模板</button></div><label class="assignment-import-upload"><input id="excellentConfigImportFile" type="file" accept=".xls,.xlsx,.xml,.csv,.tsv,.txt" ${renderUiActionCallback('excellent-import-file', ['$self'])} /><div><strong>上传Excel文件</strong><span>支持模板导出的 .xls 文件，也兼容 CSV/TSV 文本文件</span></div></label><div class="assignment-import-file" id="excellentConfigImportFileName">${excellentConfigImportState.fileName ? `已选择：${escapeHtml(excellentConfigImportState.fileName)}` : ''}</div><div class="series-form-hint">导入字段：${config.columns.join('、')}。</div></div>`;
}
...
function renderExcellentConfigImportComplete() {
  ...
  return `<div class="assignment-import-complete"><div class="import-complete-title">导入完成</div><div class="assignment-import-complete-desc">成功导入 <strong>${result.success}</strong> 条；失败 <strong>${result.failed}</strong> 条。</div></div>`;
}
...
function renderExcellentConfigImportFooter() {
  ...
  if (excellentConfigImportState.step === 1) return `<button class="btn-cancel" type="button" ${renderUiActionCallback('excellent-import-close', ['excellentConfigImportModal'])}>取消</button><button class="btn-save" type="button" ${excellentConfigImportState.validatedRows.length ? '' : 'disabled'} ${renderUiActionCallback('excellent-import-preview')}>下一步</button>`;
  if (excellentConfigImportState.step === 2) return `<button class="btn-cancel" type="button" ${renderUiActionCallback('excellent-import-back')}>上一步</button><button class="btn-save" type="button" ${validCount ? '' : 'disabled'} ${renderUiActionCallback('excellent-import-confirm')}>确认导入</button>`;
  return `<button class="btn-save" type="button" ${renderUiActionCallback('excellent-import-close', ['excellentConfigImportModal'])}>完成</button>`;
}
...
registerUiAction('excellent-import-download-template', downloadExcellentConfigImportTemplate);
registerUiActionCallback('excellent-import-file', input => handleExcellentConfigImportFile(input.files && input.files[0]));
registerUiActionCallback('excellent-import-close', (...args) => closeModal(...args));
registerUiActionCallback('excellent-import-preview', goExcellentConfigImportPreview);
registerUiActionCallback('excellent-import-back', backExcellentConfigImportUpload);
registerUiActionCallback('excellent-import-confirm', confirmExcellentConfigImport);
