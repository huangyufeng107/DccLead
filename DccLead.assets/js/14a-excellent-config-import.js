...
function renderExcellentConfigImportUpload() {
  ...
  return `<div class="assignment-import-panel"><div class="assignment-import-tip">Step1 操作说明：下载模板了解格式要求 → 按模板填写 → 上传Excel文件。模板包含必填字段，上传后将校验空值、状态和意向车系枚举。</div><div class="action-btns" style="margin-bottom:12px;"><button class="btn-secondary" type="button" data-ui-action="excellent-import-download-template">下载模板</button></div><label class="assignment-import-upload"><input id="excellentConfigImportFile" type="file" accept=".xls,.xlsx,.xml,.csv,.tsv,.txt" onchange="handleExcellentConfigImportFile(this.files && this.files[0])" /><div><strong>上传Excel文件</strong><span>支持模板导出的 .xls 文件，也兼容 CSV/TSV 文本文件</span></div></label><div class="assignment-import-file" id="excellentConfigImportFileName">${excellentConfigImportState.fileName ? `已选择：${escapeHtml(excellentConfigImportState.fileName)}` : ''}</div><div class="series-form-hint">导入字段：${config.columns.join('、')}。</div></div>`;
}
