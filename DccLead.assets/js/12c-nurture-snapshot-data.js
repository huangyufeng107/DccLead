...
function createTaskWorkorderSnapshot(lead, task, assignmentSnapshot, index) {
  ...
  const makeManualOrder = (round, options = {}) => {
    ...
    return {
      ...
      redialSourceWorkorderCode: options.redialSourceWorkorderCode || '—',
      redialNextWorkorderCode: options.redialNextWorkorderCode || '—',
      redialNextContactRound: options.redialNextContactRound || '',
  ...
  if (scenarioType === 'manual-success') {
    ...
    workorders = [
      makeAiOrder(1, {
        ...
        redialNextWorkorderCode: `${baseCode}2`,
        redialNextContactRound: 2,
        matchedRules: ['通话状态配置', '外呼重推配置']
      }),
      makeManualOrder(2, {
        isAiRedialTask: '是',
        redialSourceWorkorderCode: `${baseCode}1`,
