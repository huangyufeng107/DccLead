...
function renderResourceLeadTable() {
  ...
  const pageInfoEl = document.getElementById('resourceLeadPageInfo');
  if (pageInfoEl) {
    pageInfoEl.textContent = `共 ${rows.length} 条记录，当前第 ${resourceLeadCurrentPage} / ${totalPages} 页`;
    syncPaginationButtons(pageInfoEl.closest('.pagination'), resourceLeadCurrentPage, totalPages);
  }
...
function formatResourceDispatchStatusTag(status) {
  const value = status || '—';
  const statusClass = {
    '已下发': 'green',
    '待下发': 'blue',
  ...
  return `<span class="tag-chip ${statusClass}">${value}</span>`;
}
...
function renderResourceLeadDetailSummary(lead) {
  const items = [
    ...
    { label: '下发状态', value: formatResourceDispatchStatusTag(lead.dispatchStatus) },
...
function getResourceLeadDetailGroups(lead) {
  // 与“查看培育线索详情”共用同一套 SmartCode 投放信息口径。
...
registerUiAction('resource-leads-export', exportResourceLeads);
