...
function openViewModal(id) {
  ...
  document.getElementById('policyDetailSubtitle').textContent = `培育策略 / 黑名单配置 / ${p.name}`;
...
function renderPolicyDetailPageContent(p) {
  ...
  return `
    ...
    <div class="hit-record-section">
        ...
        <table class="hit-record-table">
          <thead>
            <tr>
              ...
              <th style="width:112px">总部线索ID</th>
      ...
      <div class="audio-player" id="recordAudioPlayer" style="margin-top:16px; margin-bottom:14px;">
        <button class="audio-toggle" type="button" onclick="toggleRecordAudioPlayback(event)" aria-label="暂停或继续播放" id="recordAudioToggle">
        ...
        <button class="modal-close" type="button" style="margin-left:12px; color:#64748b;" onclick="stopRecordAudio()" aria-label="关闭播放器" title="关闭播放器">×</button>
      </div>
  ...
  body.innerHTML = pageRecords.length
    ? pageRecords.map(record => `
      <tr class="hit-record-row ${record.callId === activeTranscriptCallId ? 'active' : ''}" onclick="openTranscriptByCallId('${record.callId}', false)">
        <td><input type="checkbox" ${isRecordEligibleForReview(record, viewingId) ? '' : 'disabled'} ${selectedHitRecordIds.includes(record.callId) ? 'checked' : ''} onclick="event.stopPropagation()" onchange="toggleHitRecordSelection(this)" value="${record.callId}" /></td>
        <td><span class="cell-text">${record.headquarterLeadId || '—'}</span></td>
        ...
        <td><button class="listen-btn ${activeAudioCallId === record.callId && audioPlaying ? 'playing' : ''}" type="button" onclick="listenRecordAudio(event, '${record.callId}', this)">${activeAudioCallId === record.callId && audioPlaying ? '播放中' : '播放'}</button></td>
        <td>
    ...
    `).join('')
    : `<tr><td colspan="14"><div class="empty-state" style="padding:28px 20px">暂无命中线索记录</div></td></tr>`;
...
function listenRecordAudio(event, callId, button) {
  ...
  const records = hitRecords[viewingId] || [];
  const recordIndex = records.findIndex(r => r.callId === callId);
  if (recordIndex !== -1) {
    activeTranscriptCallId = callId;
    if (typeof openTranscript === 'function') {
      openTranscript(viewingId, recordIndex, false);
    }
  }
...
function stopRecordAudio() {
  ...
  if (document.getElementById('hitRecordTableBody')) renderHitRecordTable();
}
...
function updateRecordAudioPlayer(record) {
  ...
  document.getElementById('recordAudioTitle').textContent = `${audioPlaying ? '正在播放' : '已暂停'} · ${record.phone} · ${record.callId}`;
...
function renderReviewStatus(record, policyId) {
  const policy = policies.find(p => p.id === policyId);
  if (!policy?.manualReview) return '<span style="color:#64748b">—</span>';
...
function isReviewBlockedByConflict(record, policyId) {
  return getHitExceptionReason(record, policyId) === '冲突任务';
}

function isRecordEligibleForReview(record, policyId) {
  return isRecordPendingReview(record, policyId) && !isReviewBlockedByConflict(record, policyId);
}

function renderReviewActions(record, policyId) {
  const policy = policies.find(p => p.id === policyId);
  if (!policy?.manualReview) return '<span style="color:#64748b">—</span>';
  if (isReviewBlockedByConflict(record, policyId)) {
    return '<span class="review-blocked" title="该手机号在其他黑名单策略中已有待审核数据，当前记录不可审核">其他策略待审核</span>';
  }
  if ((record.reviewStatus || '待审核') !== '待审核') return '<span style="color:#64748b">已处理</span>';
  return `
...
function renderHitBatchTools(pageRecords) {
  ...
  const pendingPageRecords = pageRecords.filter(record => isRecordEligibleForReview(record, viewingId));
...
function toggleAllPendingHitRecords(checked) {
  ...
  const start = (hitRecordPage - 1) * hitRecordPageSize;
  const pageRecords = records.slice(start, start + hitRecordPageSize);
  const pendingIds = pageRecords.filter(record => isRecordEligibleForReview(record, viewingId)).map(record => record.callId);
...
function batchReviewHitRecords(status) {
  const records = hitRecords[viewingId] || [];
  const eligibleCount = records.filter(record => selectedHitRecordIds.includes(record.callId) && isRecordEligibleForReview(record, viewingId)).length;
  if (!eligibleCount) {
    showToast('请先选择可审核的记录', false);
    return;
  }
  const actionText = status === '已通过' ? '通过' : '驳回';
  const consequence = status === '已通过' ? '通过后，相关号码将加入黑名单。' : '驳回后，相关号码不会加入黑名单。';
  if (!confirm(`确认批量${actionText}已选的 ${eligibleCount} 条记录吗？${consequence}`)) return;
  let changed = 0;
  records.forEach(record => {
    if (selectedHitRecordIds.includes(record.callId) && isRecordEligibleForReview(record, viewingId)) {
...
function reviewHitRecord(callId, status) {
  ...
  if (!record) return;
  if (!isRecordEligibleForReview(record, viewingId)) {
    showToast('该手机号在其他黑名单策略中已有待审核数据，当前记录不可审核', false);
    return;
  }
  const actionText = status === '已通过' ? '通过' : '驳回';
  const consequence = status === '已通过' ? '通过后，该号码将加入黑名单。' : '驳回后，该号码不会加入黑名单。';
  if (!confirm(`确认${actionText}该条审核记录吗？${consequence}`)) return;
  record.reviewStatus = status;
  record.systemStatus = '已处理';
  record.exceptionReason = '—';
