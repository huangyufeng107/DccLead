...
function renderManualWorkorderDetailContent(item) {
  const tabs = [['detail', '工单详情'], ['quality', '员工质检'], ['tags', '线索评级'], ['timeline', '时光轴']];
  ...
  const ratingContent = `<div class="nurture-rating-panel mw-rating-panel"><div class="rating-result-label">评级结果</div><div class="rating-score-strip"><strong class="rating-level" style="color:${rating.color}">${rating.text}</strong><strong class="rating-score">${rating.score}<small>分</small></strong><div class="rating-progress"><i style="width:${rating.score}%; background:${rating.color}"></i></div><span class="rating-percent">${rating.score}%</span></div><section class="rating-summary-block"><h5>评级小结</h5><div class="rating-summary-copy">当前评为<strong>${rating.text}等级</strong>：计划<strong>1个月内</strong>购车，关注<strong>${item.intentSeries}</strong>，预算<strong>16-20万</strong>、倾向<strong>贷款</strong>；已产生<strong>${item.followCount}次</strong>跟进，近期平台访问活跃，建议围绕金融方案与到店试驾持续推进。</div></section><section class="rating-features-block"><h5>客户特征</h5><div class="tag-summary-block"><h6>标签小结</h6><p>25-35岁企业管理人员，处于家庭成长期；计划1个月内换购，预算16-20万，重点关注智能化与金融方案。</p></div>${renderRatingGroup('基础画像', ratingTags.basic)}${renderRatingGroup('互动表现', ratingTags.interaction)}${renderRatingGroup('兴趣偏好', ratingTags.preference)}${renderRatingGroup('购车需求', ratingTags.purchase)}</section></div>`;
