  ...
  return `
    <div class="assignment-allocation-row">
      <div class="assignment-allocation-main">
        ...
        <button class="assignment-mini-btn" type="button" ${renderUiActionCallback('assignment-remove-series-row', ['$self'])}>删除</button>
      </div>
...
function renderAllocationChannelPicker(selectedChannel = '') {
  ...
  return `
    <div class="allocation-channel-picker">
      <button class="allocation-channel-trigger ${selected ? '' : 'placeholder'}" type="button" ${renderUiActionCallback('assignment-channel-toggle', ['$self'])}>
      ...
      <div class="allocation-channel-panel">
        ...
        <div class="allocation-channel-list">
          ${assignmentOutboundChannels.map(channel => `
            <button class="allocation-channel-option ${selected === channel ? 'selected' : ''}" type="button" data-channel="${escapeAttr(channel)}" ${renderUiActionCallback('assignment-channel-select', ['$self'])}>
...
function renderAssignmentSingleChannelPicker(selectedChannel = '', useNativeSelect = false) {
  const selected = assignmentOutboundChannels.includes(selectedChannel) ? selectedChannel : '';
  if (useNativeSelect) {
    return `<select class="form-input" id="assignmentSingleChannel"><option value="">请选择外呼类型</option>${assignmentOutboundChannels.map(channel => `<option value="${escapeAttr(channel)}" ${selected === channel ? 'selected' : ''}>${escapeHtml(channel)}</option>`).join('')}</select>`;
  }
  return `
    <div class="allocation-channel-picker assignment-single-channel-picker">
      ...
      <div class="allocation-channel-panel">
        ...
        <div class="allocation-channel-list">
          ${assignmentOutboundChannels.map(channel => `
            <button class="allocation-channel-option ${selected === channel ? 'selected' : ''}" type="button" data-channel="${escapeAttr(channel)}" ${renderUiActionCallback('assignment-single-channel-select', ['$self'])}>
              <span class="allocation-channel-mark"></span>
              <span>${escapeHtml(channel)}</span>
            </button>
          `).join('')}
...
function renderBlacklistCallTypePicker(selectedCallType = '', disabled = false) {
  ...
  return `
    <div class="allocation-channel-picker blacklist-call-type-picker">
      ...
      <button class="allocation-channel-trigger ${selected ? '' : 'placeholder'}" type="button" ${renderUiActionCallback('assignment-channel-toggle', ['$self'])} ${disabled ? 'disabled' : ''}>
        <span class="allocation-channel-trigger-text">${escapeHtml(selected || '请选择外呼类型')}</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
      </button>
      <div class="allocation-channel-panel">
        ...
        <div class="allocation-channel-list">
          ${blacklistPolicyCallTypeOptions.map(type => `
            <button class="allocation-channel-option ${selected === type ? 'selected' : ''}" type="button" data-channel="${escapeAttr(type)}" ${renderUiActionCallback('assignment-blacklist-calltype-select', ['$self'])}>
              <span class="allocation-channel-mark"></span>
...
function renderAiRedialOutboundTypePicker(selectedType = '', disabled = false) {
  ...
  return `
    <select class="form-input" id="aiRedialFormOutboundType" ${renderUiActionCallback('assignment-sync-redial-type')} ${disabled ? 'disabled' : ''}>
      <option value="">请选择外呼类型</option>
...
function renderAllocationPushTimeSlot(slot = {}, index = 0, total = 1) {
  ...
  return `
    <div class="push-time-slot allocation-push-slot" data-slot-index="${rowNo}">
      ...
      <button class="push-time-remove" type="button" ${renderUiActionCallback('assignment-remove-allocation-push-time', ['$self'])} style="${total <= 1 ? 'visibility:hidden' : ''}">删除</button>
    </div>
  `;
}
    ...
    <div class="allocation-row-push-time optional-push-time" data-expanded="${isExpanded}">
      ...
      <div class="optional-push-time-body" ${isExpanded ? '' : 'hidden'}>
        ...
        <div class="push-time-slots">
          ...
          <button class="push-time-add" type="button" ${renderUiActionCallback('assignment-add-allocation-push-time', ['$self'])}>新增时段</button>
    ...
    <div class="city-allocation-row${isFallback ? ' fallback' : ''}" data-fallback="${isFallback ? 'true' : 'false'}">
      <div class="city-allocation-head">
        ...
        <div class="city-allocation-actions">
          <button class="assignment-mini-btn" type="button" ${renderUiActionCallback('assignment-remove-city-row', ['$self'])} style="${isFallback ? 'visibility:hidden' : ''}">删除</button>
        </div>
      </div>
      <div class="city-allocation-body">
        <div class="city-ratio-allocation">
          ...
          <button class="series-add-channel-btn" type="button" ${renderUiActionCallback('assignment-add-city-ratio', ['$self'])}>添加外呼类型</button>
        </div>
      </div>
    </div>
  `;
...
function renderAreaMultiPicker(options = [], selectedValues = [], label = '关联区域') {
  ...
  return `
    <div class="area-multi-picker city-allocation-city" data-area-picker="true" data-picker-type="${isCityPicker ? 'city' : 'province'}">
      <button class="area-multi-trigger${selectedValues.length ? '' : ' placeholder'}" type="button" ${renderUiActionCallback('assignment-area-toggle', ['$self'])}>${summary}</button>
...
function renderProvinceMultiPicker(options = [], selectedSet = new Set()) {
  return `
    <div class="province-picker-search">
      <input type="search" class="province-picker-search-input" placeholder="搜索省份" ${renderUiActionCallback('assignment-province-filter', ['$self'])} />
    </div>
    <div class="province-picker-list">
      ${options.map(value => `
        <label class="area-multi-option" data-area-option="${value}">
          <input type="checkbox" class="area-multi-checkbox" value="${value}" ${selectedSet.has(value) ? 'checked' : ''} ${renderUiActionCallback('assignment-refresh-city-mutex')} />
          <span>${value}</span>
        </label>
      `).join('')}
    </div>
...
function renderCityCascadePicker(selectedSet, activeProvince = '') {
  return `
    <div class="city-picker-search">
      <input type="search" class="city-picker-search-input" placeholder="搜索城市" ${renderUiActionCallback('assignment-city-filter', ['$self'])} />
    </div>
    <div class="city-picker-toolbar">
      <span class="city-picker-count">已选 ${selectedSet.size} 项</span>
      <div class="city-picker-actions">
        <button class="city-picker-action" type="button" ${renderUiActionCallback('assignment-current-province-all', ['$self'])}>选择当前省</button>
        <button class="city-picker-action" type="button" ${renderUiActionCallback('assignment-current-city-clear', ['$self'])}>清空本组</button>
      </div>
    </div>
    <div class="city-picker-layout">
      <div class="city-province-list">
        ${assignmentCityGroups.map(group => `
          <button class="city-province-item ${group.province === activeProvince ? 'active' : ''}" type="button" data-province="${group.province}" ${renderUiActionCallback('assignment-city-province-switch', ['$self'])}>${group.province}</button>
        `).join('')}
      </div>
          ...
          <div class="city-group" data-city-group="${group.province}" style="${group.province === activeProvince ? '' : 'display:none'}">
            ${group.cities.map(city => `
              <label class="area-multi-option" data-area-option="${city}" data-province="${group.province}">
                <input type="checkbox" class="area-multi-checkbox" value="${city}" ${selectedSet.has(city) ? 'checked' : ''} ${renderUiActionCallback('assignment-refresh-city-mutex')} />
                <span>${city}</span>
              </label>
            `).join('')}
          </div>
...
function renderSmartCodeCityAllocationPanel(rule = {}, scope = 'city') {
  ...
  return `
    ...
    <button class="series-add-channel-btn" type="button" ${renderUiActionCallback('assignment-add-city-row')}>${config.addLabel}</button>
...
function updateProjectNameSelectedCount() {
  ...
  if (!selected.length) {
    summary.textContent = `请选择${config.valueLabel}`;
    return;
  }
  summary.textContent = selected.slice(0, 3).join('、') + (selected.length > 3 ? ` 等${selected.length}项` : '');
}
...
function saveSeriesAssignmentRule() {
  ...
  if (dimension.key === 'series') {
    selectedValues = Array.from(document.querySelectorAll(`input[name="${config.inputName}"]:checked`)).map(input => input.value);
  }
  ...
  if (isRatio) {
    ...
    const channel = document.getElementById('assignmentSingleChannel')?.value || document.querySelector('input[name="seriesSingleChannel"]:checked')?.value || '';
...
registerUiActionCallback('assignment-close-modal', () => document.getElementById('leadDispatchRuleModal')?.classList.remove('show'));
registerUiActionCallback('assignment-save-rule', saveLeadAssignmentRule);
registerUiActionCallback('assignment-sync-push-time', syncSmartCodePushTime);
registerUiActionCallback('assignment-remove-push-time', removeSmartCodePushTimeSlot);
registerUiActionCallback('assignment-toggle-push-time', toggleOptionalPushTime);
registerUiActionCallback('assignment-add-push-time', addSmartCodePushTimeSlot);
registerUiActionCallback('assignment-smartcode-mode', switchSmartCodeInputMode);
registerUiActionCallback('assignment-project-mode', switchProjectNameInputMode);
registerUiActionCallback('assignment-dealer-mode', switchDealerInputMode);
registerUiActionCallback('assignment-project-toggle', toggleProjectNamePicker);
registerUiActionCallback('assignment-project-all', selectAllProjectNameOptions);
registerUiActionCallback('assignment-project-clear', clearProjectNameOptions);
registerUiActionCallback('assignment-project-filter', filterProjectNameOptions);
registerUiActionCallback('assignment-project-option', toggleProjectNameOption);
registerUiActionCallback('assignment-series-mode', switchSeriesAllocationMode);
registerUiActionCallback('assignment-remove-series-row', removeSeriesAllocationRow);
registerUiActionCallback('assignment-channel-toggle', toggleAllocationChannelPicker);
registerUiActionCallback('assignment-channel-select', selectAllocationChannelOption);
registerUiActionCallback('assignment-single-channel-select', selectAssignmentSingleChannelOption);
registerUiActionCallback('assignment-blacklist-calltype-select', selectBlacklistCallTypeOption);
// 函数定义位于后加载的 AI 外呼脚本中；使用包装函数可避免初始化阶段
// 因脚本顺序造成引用错误，同时仍在实际操作时执行对应联动。
registerUiActionCallback('assignment-sync-redial-type', () => syncAiRedialNewOutboundType());
registerUiActionCallback('assignment-remove-allocation-push-time', removeAllocationPushTimeSlot);
registerUiActionCallback('assignment-add-allocation-push-time', addAllocationPushTimeSlot);
registerUiActionCallback('assignment-remove-city-row', removeSmartCodeCityAllocationRow);
registerUiActionCallback('assignment-add-city-ratio', addCityAllocationRatioRow);
registerUiActionCallback('assignment-area-toggle', toggleAreaMultiPicker);
registerUiActionCallback('assignment-province-filter', filterProvinceMultiPicker);
registerUiActionCallback('assignment-refresh-city-mutex', refreshCityAllocationSelectionMutex);
registerUiActionCallback('assignment-city-filter', filterCityCascadePicker);
registerUiActionCallback('assignment-current-province-all', selectCurrentProvinceCities);
registerUiActionCallback('assignment-current-city-clear', clearCurrentCityPicker);
registerUiActionCallback('assignment-city-province-switch', switchCityPickerProvince);
registerUiActionCallback('assignment-add-city-row', addSmartCodeCityAllocationRow);
