...
const sdReportTabMeta = {
  performance: {
    name: '业绩晾晒报告',
    title: '业绩晾晒报告',
    intro: '两个板块互不影响，均可单独选择「开始日期＋小时 ～ 结束日期＋小时」；筛选一变，相关指标与图表会同步更新。点击右上角「📋 看板指标说明」可查阅完整指标口径手册。',
    modalTitle: '坐席业绩看板 · 时段业绩指标口径与规则说明手册',
    modalSub: '包含排程、通话、目标、完成度、每日两王及全员明细统计逻辑，供业务与产研对照执行',
    docFile: 'DccLead.assets/docs/DccLead_时段业绩看板指标说明.html'
  },
  underfilled: {
    name: '留资未满报告',
    title: '留资未满报告',
    intro: '仅统计关联“留资未满任务”的回访记录。两个大点均可单独选择时段；筛选一变，相关指标与分布同步更新。点击右上角「📋 看板指标说明」可查阅留资未满指标手册。',
    modalTitle: '坐席业务看板 · 留资未满业绩报告指标说明手册',
    modalSub: '包含留资未满数、激活量、排程量、通话时长及各小组人均统计口径，供业务与产研对照执行',
    docFile: 'DccLead.assets/docs/DccLead_留资未满业绩报告指标说明.html'
  },
  employee: {
    name: '员工维度报告',
    title: '员工维度报告',
    intro: '按日期、时段及 11 项筛选维度查看员工执行数据，任务、话单与有效接通实时计算，后端转化为 T+1 数据。点击右上角「📋 看板指标说明」可查阅员工维度手册。',
    modalTitle: '坐席业务看板 · 员工维度看板指标说明手册',
    modalSub: '包含新增任务、继续跟进任务、过程指标（首触/通话/分层）及后端转化（T+1）完整字段定义与对账关系',
    docFile: 'DccLead.assets/docs/DccLead_员工维度看板指标说明.html'
  }
};

function openSeatPerformanceMetricModal() {
  const meta = sdReportTabMeta[sdBusinessTab] || sdReportTabMeta.performance;
  let modal = document.getElementById('seatPerformanceMetricModal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'seatPerformanceMetricModal';
  ...
  modal.innerHTML = `
    <div style="background:#fff; width:94%; max-width:1380px; height:88vh; border-radius:12px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25); display:flex; flex-direction:column; overflow:hidden;">
      <header style="padding:16px 24px; background:#1e293b; color:#fff; display:flex; justify-content:space-between; align-items:center; border-bottom:3px solid #2563eb;">
        <div>
          <h3 style="margin:0; font-size:16px; font-weight:700; display:flex; align-items:center; gap:8px;">
            <span>📋 ${meta.modalTitle}</span>
            <span style="font-size:11px; padding:2px 8px; border-radius:999px; background:#2563eb; color:#fff;">${meta.name} · 业务口径标准</span>
          </h3>
          <p style="margin:4px 0 0; font-size:12px; color:#94a3b8;">${meta.modalSub}</p>
        </div>
        <div style="display:flex; gap:10px; align-items:center;">
          <button type="button" style="color:#93c5fd; border:1px solid #3b82f6; background:rgba(37,99,235,0.25); padding:6px 14px; font-size:12px; font-weight:600; border-radius:4px; cursor:pointer;" onclick="window.open('${meta.docFile}', '_blank')">在新窗口打开完整手册 ↗</button>
          <button type="button" style="background:none; border:none; color:#94a3b8; font-size:22px; cursor:pointer; line-height:1;" onclick="closeSeatPerformanceMetricModal()">✕</button>
        </div>
      </header>
      <div style="flex:1; overflow:hidden; background:#f8fafc;">
        <iframe src="${meta.docFile}" style="width:100%; height:100%; border:none;"></iframe>
      </div>
      <footer style="padding:12px 24px; background:#f8fafc; border-top:1px solid #e2e8f0; display:flex; justify-content:flex-end; align-items:center;">
        <button type="button" style="height:32px; padding:0 20px; background:#2563eb; border:1px solid #2563eb; color:#fff; border-radius:4px; font-size:13px; font-weight:500; cursor:pointer;" onclick="closeSeatPerformanceMetricModal()">完成查阅，返回看板</button>
      </footer>
    </div>
  `;
  modal.style.display = 'flex';
}
function closeSeatPerformanceMetricModal() {
  const modal = document.getElementById('seatPerformanceMetricModal');
  if (modal) modal.style.display = 'none';
}
window.openSeatPerformanceMetricModal = openSeatPerformanceMetricModal;
window.closeSeatPerformanceMetricModal = closeSeatPerformanceMetricModal;

let sdReportIntroExpanded=false;
function sdCompactReportIntro(root){
  if(!root)root=document.querySelector('#seatPerformanceDashboardPage .sd');
  const header=root?.querySelector('header'),intro=header?.querySelector('p');
  ...
  header.querySelector('.sd-report-intro-toggle')?.remove();
  header.insertAdjacentHTML('beforeend','<button type="button" class="sd-report-intro-toggle" onclick="openSeatPerformanceMetricModal()" title="查看'+meta.name+'指标说明"><span style="font-size:14px;line-height:1;">📋</span> 看板指标说明</button>');
}
function toggleSdReportIntro(){
  openSeatPerformanceMetricModal();
}
window.sdCompactReportIntro = sdCompactReportIntro;
window.toggleSdReportIntro = toggleSdReportIntro;
renderSeatPerformanceDashboard=function(){
  ...
  if(reportIntro)reportIntro.innerHTML='两个板块互不影响，均可单独选择「开始日期＋小时 ～ 结束日期＋小时」；筛选一变，相关指标与图表会同步更新。';
  ...
  if(!document.getElementById('sd-report-intro-collapse-css'))document.head.insertAdjacentHTML('beforeend','<style id="sd-report-intro-collapse-css">#seatPerformanceDashboardPage .sd>header{padding-right:210px}#seatPerformanceDashboardPage .sd .sd-report-intro-toggle{position:absolute;z-index:2;top:28px;right:34px;height:32px;display:inline-flex;align-items:center;gap:6px;padding:0 12px;border:1px solid #93c5fd;border-radius:6px;background:#eff6ff;color:#1d4ed8;font:600 12px "Microsoft YaHei","PingFang SC",sans-serif;cursor:pointer;transition:border-color .18s,background .18s,box-shadow .18s,transform .18s}#seatPerformanceDashboardPage .sd .sd-report-intro-toggle:hover{border-color:#60a5fa;background:#dbeafe;color:#1e40af;box-shadow:0 3px 9px rgba(37,99,235,.15);transform:translateY(-1px)}@media(max-width:760px){#seatPerformanceDashboardPage .sd>header{padding-right:20px;padding-bottom:68px}#seatPerformanceDashboardPage .sd .sd-report-intro-toggle{top:auto;right:20px;bottom:20px}}</style>');
...
function sdResetTargetFilters(){sdTargetKeyword='';sdTargetGroup='all';sdTargetStatus='all';renderSdReportConfiguration()}
function sdRefreshTargetList(){sdApplyTargetFilter();if(typeof showToast==='function')showToast('业绩目标配置数据已刷新',true)}
...
function sdNormalizeConfigToolbar(){
  ...
  if(sdReportConfigTab==='mapping'){
    ...
    if(!toolbar.querySelector('.sd-filter-reset'))toolbar.insertAdjacentHTML('beforeend','<button class="sd-btn sd-filter-reset" type="button" onclick="sdResetMappingFilters()">重置</button>');
    ...
    if(!toolbar.querySelector('.sd-filter-reset'))toolbar.insertAdjacentHTML('beforeend','<button class="sd-btn sd-filter-reset" type="button" onclick="sdResetTargetFilters()">重置</button>');
...
function sdResetMappingFilters(){sdMappingKeyword='';sdMappingGroup='all';sdMappingStatus='all';renderSdReportConfiguration()}
function sdRefreshMappingList(){sdApplyMappingFilter();if(typeof showToast==='function')showToast('坐席与小组映射数据已刷新',true)}
...
renderSeatPerformanceDashboard=function(){
  ...
  if(root)sdCompactReportIntro(root);
...
function sdNormalizeConfigToggleActions(){
  document.querySelectorAll('#seatPerformanceDashboardPage .sd-map-status').forEach(function(status){
    var isEnabled=status.classList.contains('on');
    var label=status.textContent.trim();
    status.className=isEnabled?'col-status-on':'col-status-off';
    status.textContent='● '+label;
  });
  document.querySelectorAll('#seatPerformanceDashboardPage .sd-table-actions .mute').forEach(function(button){
    var isDisable=button.textContent.trim()==='停用';
    button.classList.toggle('disable',isDisable);
    button.classList.toggle('enable',!isDisable);
  });
}
...
renderSdReportConfiguration=function(){
  ...
  sdNormalizeConfigToggleActions();
};
