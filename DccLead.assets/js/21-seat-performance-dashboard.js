...
function sdDrawCharts(){document.querySelectorAll('#seatPerformanceDashboardPage canvas[data-sd-chart]').forEach(c=>{const d=sdCalc(sdF[c.dataset.panel]),k=c.dataset.sdChart;if(k==='bar'){sdDrawBar(c,d);sdBindBarHover(c,d)}else if(k==='donut'){sdDrawDonut(c,d);sdBindDonutHover(c,d)}else if(k==='schedule'){sdDrawSchedule(c,d);sdBindScheduleHover(c,d)}else{sdDrawCalls(c,d);sdBindCallsHover(c,d)}})}
...
renderSeatPerformanceDashboard=function(){
  ...
  document.querySelectorAll('#seatPerformanceDashboardPage .kpis > div:nth-child(4) span').forEach(item=>item.textContent='本时段不在排班');
...
sdCalc=function(filter){
  ...
  data.r.forEach((seat,index)=>{
    // 模拟来源字段：已接收线索、意向下发、外呼次数和企微添加，用于榜单破局与每日两王口径。
    seat.received=Math.max(1,seat.task*2+(index%4)+1);
    seat.intent=Math.max(0,Math.floor(seat.p/2)+(index%3));
    seat.scheduleRate=seat.p/seat.received;
    seat.outbound=Math.max(seat.task,Math.ceil(seat.c/95));
...
function sdCallRank(rows){return [...rows].sort((a,b)=>b.c-a.c||b.p-a.p||b.scheduleRate-a.scheduleRate||a.name.localeCompare(b.name,'zh-CN'))}
...
sdOutcomeRankings=function(data){
  ...
  return '<div class="box"><h3>五、全员多指标榜单 TOP 10</h3><div class="charts sd-outcome-ranks">'+sdOutcomeRankTable(data.r,'排程量 TOP 10（批）',sdScheduleRank,'p',' 批',{label:'有效排程率',value:s=>(s.scheduleRate*100).toFixed(1)+'%'})+sdOutcomeRankTable(data.r,'通话时长 TOP 10',sdLaborRank,'c',' 秒',{label:'外呼次数',value:s=>s.outbound+' 次'})+sdOutcomeRankTable(data.r,'试驾量 TOP 10（批）',sdDriveRank,'drive',' 批',{label:'排程量',value:s=>s.p+' 批'})+sdOutcomeRankTable(data.r,'锁单量 TOP 10（台/单）',sdLockRank,'locked',' 台/单',{label:'试驾量',value:s=>s.drive+' 批'})+sdOutcomeRankTable(monthly,'月累销冠王次数 TOP 10',countRank('salesKing'),'salesKing',' 次',{label:'排程量',value:s=>s.p+' 批'})+sdOutcomeRankTable(monthly,'月累劳模王次数 TOP 10',countRank('laborKing'),'laborKing',' 次',{label:'通话时长',value:s=>sdDur(s.c)})+'</div></div>';
};
...
function sdRankFocusConfig(data){
  ...
  return {
    ...
    call:{tab:'通话时长',title:'通话时长 TOP 10',metric:'c',unit:'秒',rows:data.r,ranker:sdCallRank,rule:'并列时依次按排程量、有效排程率排序',aux:seat=>'排程 '+seat.p+' 批'},
...
sdOpenMappingModal=function(id){sdOpenMappingModalBeforeRequiredFields(id);const modal=document.querySelector('.sd-map-modal');sdNormalizeRequiredLabels(modal);modal?.querySelectorAll('input[readonly]').forEach(field=>field.classList.add('is-autofill-readonly'))};
const sdOpenTargetModalBeforeRequiredFields=sdOpenTargetModal;
sdOpenTargetModal=function(id,seatId){sdOpenTargetModalBeforeRequiredFields(id,seatId);const modal=document.querySelector('.sd-map-modal');sdNormalizeRequiredLabels(modal);modal?.querySelectorAll('input[readonly]').forEach(field=>field.classList.add('is-autofill-readonly'))};
