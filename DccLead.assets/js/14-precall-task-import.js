  ...
  return `
    <div class="assignment-import-panel">
      ...
      <label class="assignment-import-upload">
        <input id="precallTaskImportFile" type="file" accept=".xls,.xlsx,.xml,.csv,.tsv,.txt" ${renderUiActionCallback('precall-task-import-file', ['$self'])} />
        <div>
          ...
          <tbody>
            ${rows.length ? rows.map(row => `
              <tr class="${row.valid ? '' : 'has-error'}" style="${row.valid ? '' : 'background-color: #fff1f0;'}">
                ...
                <td>${row.valid ? '<span class="assignment-import-pass" style="color: #16a34a;">通过</span>' : `<div class="assignment-import-error" style="color: #ff4d4f; font-size: 12px; line-height: 1.4;">${row.errors.map(escapeHtml).join('<br>')}</div>`}</td>
              </tr>
...
function renderPrecallTaskImportStepComplete() {
  ...
  return `
    <div class="assignment-import-complete">
      <div class="import-complete-title">导入完成</div>
...
function renderPrecallTaskImportFooter() {
  ...
  if (step === 1) {
    return `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('precall-task-import-close', ['precallTaskImportModal'])}>取消</button>
      <button class="btn-save" type="button" ${precallTaskImportState.validatedRows.length ? '' : 'disabled'} ${renderUiActionCallback('precall-task-import-preview')}>下一步</button>
    `;
  }
  if (step === 2) {
    return `
      <button class="btn-cancel" type="button" ${renderUiActionCallback('precall-task-import-back')}>上一步</button>
      <button class="btn-save" type="button" ${validCount ? '' : 'disabled'} ${renderUiActionCallback('precall-task-import-confirm')}>确认导入</button>
    `;
  }
  return `<button class="btn-save" type="button" ${renderUiActionCallback('precall-task-import-close', ['precallTaskImportModal'])}>完成</button>`;
}
  ...
  const sheets = [
    ...
    },
    {
      ...
      rows: [
        ...
        ['命中类型', '必填', '命中类型。支持“渠道编码”、“SmartCode”、“大项目名”、“线索状态”；“通话状态”类型请在列表新增/编辑弹窗中配置。', '渠道编码、SmartCode、大项目名、线索状态'],
...
registerUiActionCallback('precall-task-import-file', input => handlePrecallTaskImportFile(input.files && input.files[0]));
registerUiActionCallback('precall-task-import-close', (...args) => closeModal(...args));
registerUiActionCallback('precall-task-import-preview', goPrecallTaskImportPreview);
registerUiActionCallback('precall-task-import-back', backPrecallTaskImportUpload);
registerUiActionCallback('precall-task-import-confirm', confirmPrecallTaskImport);
...
function validatePrecallTaskImportRows(rows) {
  return rows.map(row => {
    ...
    if (!attributeType) errors.push('命中类型必填');
      ...
      errors.push('命中类型只支持：渠道编码、SmartCode、大项目名、线索状态；“通话状态”类型请在页面上新增/编辑弹窗中配置');
    }
...
function exportPrecallTask() {
  const rows = strategyConfiguratorData.precallTask.map(rule => {
    const isCallStatusType = rule.attributeType === '通话状态';
    const attrValueText = isCallStatusType
      ? (rule.callStatusRules || []).map(r => {
          const durText = (r.status === '坐席接听' && r.duration && Number(r.duration) > 0) ? `通话时长≤${r.duration}秒, ` : '';
    ...
    return [
      ...
      rule.attributeType || '',
      attrValueText,
      isCallStatusType ? '' : (rule.leadStatuses || []).join('、'),
      (rule.carSeries || []).join('、'),
      rule.precallLine || '',
