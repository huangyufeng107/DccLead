...
const configuratorOptions = {
  ...
  precallTaskStatus: ['不限制', 'DCC呼叫成功', 'DCC呼叫完成'],
...
let strategyConfiguratorData = {
  ...
  manualUnconnected: [
    { sortOrder: 1, id: 1012, ruleName: '分配配置A', status: '启用', conditionMode: 'contactStatus', conditionModeLabel: '按接触状态', contactStatus: '无人接听', carSeries: ['N7'], contactRepeatCount: 0, maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['科大讯飞-冷线索'], belongType: 'SmartCode', belongValues: ['SC-N6-0891', 'SC-GZ-2026'], includeIncomplete: '是', matchConditionSummary: '无人接听 / N7 / 含留资未满' },
    { sortOrder: 2, id: 1013, ruleName: '分配配置B', status: '启用', conditionMode: 'precallTask', conditionModeLabel: '按预外呼任务', lastContactStatus: '关机', carSeries: ['N6'], taskReturnVisitCount: 2, precallTaskStatus: ['DCC呼叫成功', 'DCC呼叫完成'], maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['科大讯飞-留资未满'], belongType: 'SmartCode', belongValues: ['SC-SY-0234'], includeIncomplete: '是', matchConditionSummary: '关机 / N6 / 工单2次 / DCC呼叫成功、DCC呼叫完成 / 含留资未满' },
    { sortOrder: 3, id: 1014, ruleName: '分配配置C', status: '停用', conditionMode: 'contactStatus', conditionModeLabel: '按接触状态', contactStatus: '无人接听', carSeries: ['华为天籁'], contactRepeatCount: 0, maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['一知-冷线索'], belongType: 'SmartCode', belongValues: ['SC-XK-7712'], includeIncomplete: '是', matchConditionSummary: '无人接听 / 华为天籁 / 含留资未满' }
  ],
  manualUnconverted: [
...
const strategyConfiguratorDefs = {
  ...
  manualUnconnected: {
    ...
    title: '未接通激活配置',
    desc: '配置人工外呼无人接听后转交 AI 外呼的规则，支持按接触状态或按预外呼任务两种命中条件模式。',
    addLabel: '新增配置',
    search: [
      { label: '关键字', field: 'keyword', type: 'text' },
    ...
    actions: ['add'],
    columns: [
      { label: '#', field: 'sortOrder', width: 60 },
    ...
    form: [
      ...
      { label: '接触状态重复次数', field: 'contactRepeatCount', type: 'number', required: true, allowZero: true, section: '命中条件', hintHtml: '填写非负整数，<strong>0次代表不限制</strong>。', placeholder: '请输入次数', conditionMode: 'contactStatus' },
      ...
      { label: '任务回访工单次数', field: 'taskReturnVisitCount', type: 'number', required: true, allowZero: true, section: '命中条件', hintHtml: '填写非负整数，<strong>0次代表不限制</strong>。', placeholder: '请输入次数', conditionMode: 'precallTask' },
      { label: '预外呼任务状态', field: 'precallTaskStatus', type: 'multiSelectMutex', options: 'precallTaskStatus', required: true, section: '命中条件', defaultValue: ['不限制'], conditionMode: 'precallTask' },
      ...
      },
      {
        ...
        fields: [
          ...
          row.conditionMode === 'precallTask'
            ? ['任务回访工单次数', `${row.taskReturnVisitCount ?? 0}次`]
            : ['接触状态重复次数', `${row.contactRepeatCount ?? 0}次`],
          row.conditionMode === 'precallTask' ? ['预外呼任务状态', Array.isArray(row.precallTaskStatus) ? (row.precallTaskStatus.includes('不限制') ? '不限制' : row.precallTaskStatus.join('、')) : (row.precallTaskStatus || '不限制')] : null,
...
function switchManualUnconnectedConditionMode(mode) {
  ...
  const precallFields = ['lastContactStatus', 'taskReturnVisitCount', 'precallTaskStatus'];
...
function renderStrategyConfiguratorFormField(key, field, row, readonly = false) {
  ...
  if (field.type === 'multiSelectMutex') {
    const selected = Array.isArray(value) ? value : (value ? [value] : ['不限制']);
    const options = configuratorOptions[field.options] || [];
    const optionsHtml = options.map(opt => {
      const isChecked = selected.includes(opt);
      const disabled = readonly ? 'disabled' : '';
      return `<label class="tag-option ${isChecked ? 'selected' : ''}"><input type="checkbox" name="${id}Value" value="${escapeAttr(opt)}" ${isChecked ? 'checked' : ''} ${disabled} onchange="handlePrecallTaskStatusMutex(this)" /><span>${escapeHtml(opt)}</span></label>`;
    }).join('');
    return `
      <div class="${groupClass}" id="formGroup_${field.field}">
        <div class="form-label">${field.label}${required}</div>
        <div class="unconnected-select-picker mutex-status-picker" id="${id}_picker">
          <button type="button" class="unconnected-select-toggle" onclick="toggleUnconnectedSelectDropdown('${id}')" id="${id}_toggle" ${readonly ? 'disabled' : ''}>
            <span class="unconnected-select-text" id="${id}_text">${formatMutexSelectText(selected)}</span>
            <span class="unconnected-select-arrow"></span>
          </button>
          <div class="unconnected-select-dropdown" id="${id}_dropdown" style="display:none;">
            <div class="unconnected-select-dropdown-head">
              <span id="${id}_count">已选 ${selected.length} 项</span>
              ${readonly ? '' : '<button type="button" onclick="clearPrecallTaskStatusMutex(this)">清空</button>'}
            </div>
            <div class="unconnected-select-options">${optionsHtml}</div>
            ${readonly ? '' : '<div class="unconnected-select-dropdown-foot"><button type="button" onclick="toggleUnconnectedSelectDropdown(\'' + id + '\')">完成</button></div>'}
          </div>
        </div>
        ${hint}
      </div>
...
function isManualUnconvertedNonNegativeIntegerField(key, fieldName) {
  if (key === 'manualUnconnected') return ['contactRepeatCount', 'taskReturnVisitCount', 'maxOutboundCount', 'pushInterval'].includes(fieldName);
...
function getManualUnconnectedMatchSummary(payload) {
  ...
  if (payload.conditionMode === 'contactStatus') {
    ...
    if (payload.contactRepeatCount > 0) parts.push(`重复${payload.contactRepeatCount}次`);
  } else {
    parts.push(payload.lastContactStatus || '—');
    if (payload.carSeries?.length) parts.push(payload.carSeries.join('、'));
    if (payload.taskReturnVisitCount > 0) parts.push(`工单${payload.taskReturnVisitCount}次`);
    if (Array.isArray(payload.precallTaskStatus)) {
    const statuses = payload.precallTaskStatus.filter(s => s && s !== '不限制');
    if (statuses.length) parts.push(statuses.join('、'));
  } else if (payload.precallTaskStatus && payload.precallTaskStatus !== '不限制') {
    parts.push(payload.precallTaskStatus);
  }
  }
...
function collectStrategyConfiguratorFormValue(def) {
  ...
  for (const field of def.form) {
    ...
    } else if (field.type === 'multiSelectMutex') {
      value = getPrecallTaskStatusSelectedValues();
    } else {
      value = el.value.trim();
    }
...
function formatMutexSelectText(selected) {
  if (!selected || selected.length === 0) return '请选择';
  if (selected.includes('不限制')) return '不限制';
...
function updatePrecallTaskStatusMutexSummary(picker, selected) {
  const id = picker.id.replace('_picker', '');
  const textEl = document.getElementById(`${id}_text`);
  const countEl = document.getElementById(`${id}_count`);
  if (textEl) textEl.textContent = formatMutexSelectText(selected);
  if (countEl) countEl.textContent = `已选 ${selected.length} 项`;
}

function clearPrecallTaskStatusMutex(button) {
  const picker = button.closest('.unconnected-select-picker');
  if (!picker) return;
  const noLimitCb = picker.querySelector('input[value="不限制"]');
  picker.querySelectorAll('input[type="checkbox"]').forEach(cb => {
    cb.checked = cb === noLimitCb;
    cb.closest('.tag-option')?.classList.toggle('selected', cb.checked);
  });
  const selected = noLimitCb ? ['不限制'] : [];
  updatePrecallTaskStatusMutexSummary(picker, selected);
  window.unconnectedPrecallTaskStatusState = { selected };
}

function handlePrecallTaskStatusMutex(checkbox) {
  const picker = checkbox.closest('.unconnected-select-picker');
  const allCheckboxes = picker.querySelectorAll('input[type="checkbox"]');
  const id = picker.id.replace('_picker', '');

  if (checkbox.value === '不限制') {
    if (checkbox.checked) {
    ...
    if (checkbox.checked) {
      const noLimitCb = picker.querySelector('input[value="不限制"]');
      if (noLimitCb) {
        noLimitCb.checked = false;
        noLimitCb.closest('.tag-option').classList.remove('selected');
      }
    }
  }
  ...
  const newSelected = [];
  allCheckboxes.forEach(cb => { if (cb.checked) newSelected.push(cb.value); });
  if (!newSelected.length) {
    const noLimitCb = picker.querySelector('input[value="不限制"]');
    if (noLimitCb) { noLimitCb.checked = true; noLimitCb.closest('.tag-option').classList.add('selected'); newSelected.push('不限制'); }
  }
  updatePrecallTaskStatusMutexSummary(picker, newSelected);

  if (!window.unconnectedPrecallTaskStatusState) window.unconnectedPrecallTaskStatusState = {};
  window.unconnectedPrecallTaskStatusState.selected = newSelected;
}

function getPrecallTaskStatusSelectedValues() {
  if (window.unconnectedPrecallTaskStatusState?.selected) return window.unconnectedPrecallTaskStatusState.selected;
  const picker = document.getElementById('cfgForm_precallTaskStatus_picker');
  if (!picker) return ['不限制'];
  const checked = [];
  picker.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => checked.push(cb.value));
  return checked.length ? checked : ['不限制'];
}
