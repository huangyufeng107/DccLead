var safePushWeekdays = ['周一', '周二', '周三', '周四', '周五', '周六', '周日'];

function renderSimpleOptions(options = [], selected = '', placeholder = '请选择') {
  const items = placeholder ? [`<option value="">${escapeHtml(placeholder)}</option>`] : [];
  (options || []).forEach(opt => {
    const val = typeof opt === 'object' ? opt.value : opt;
    const label = typeof opt === 'object' ? opt.label : opt;
    items.push(`<option value="${escapeAttr(val)}" ${val === selected ? 'selected' : ''}>${escapeHtml(label)}</option>`);
  });
  return items.join('');
}
...
const strategyConfiguratorPageMap = {
  ...
  'next-visit-precall-config': 'nextVisitPrecall',
...
const configuratorOptions = {
  ...
  returnVisitInitialLeadStatus: (typeof returnVisitInitialLeadStatusOptions !== 'undefined' ? returnVisitInitialLeadStatusOptions : ['总部_高意向', '总部_休眠未购', '总部_休眠失联', '总部_战败', '总部_暂败', '总部_阶段性战败', '总部_无效', '总部_异地']),
  returnVisitMatchingDimension: ['按线索状态', '按渠道与媒体'],
  leadDispatchLeadStatus: (typeof leadDispatchFormLeadStatusOptions !== 'undefined' && Array.isArray(leadDispatchFormLeadStatusOptions)) ? leadDispatchFormLeadStatusOptions.filter(Boolean) : ['总部_高意向', '经销商_跟进中', '战败待复核'],
  manualUnconvertedReviewResult: ['下次回访', '试驾排程下发', '试驾线索下发', '意向线索下发', '无人接听下发', '休眠失联', '休眠未购', '无效结案', '战败'],
  contactStatus: ['正常接通', '无人接听', '企微跟进', '客户拒绝', '忙音/秘书台', '空号', '停机', '关机'],
  reviewStatus: ['休眠失联-被拉黑', '休眠未购-资金不足', '阶段性战败', '下次回访'],
  carSeries: ['全部车系', 'N6', '轩逸', '天籁', '逍客', '奇骏', 'ARIYA', '探陆'],
  // 高德地图配置中的默认车系必须是具体车系，不能选择“全部车系”。
  amapDefaultCarSeries: ['N6', '轩逸', '天籁', '逍客', '奇骏', 'ARIYA', '探陆'],
  outboundType: (typeof unifiedPolicyCallTypeOptions !== 'undefined' ? unifiedPolicyCallTypeOptions : ['人工外呼', 'AI外呼']),
  ...
  returnVisitOrderContactStatus: ['空号', '停机', '忙音', '不在服务区', '无人接听', '关机', '坐席未接', '线路拒呼', '线路未呼'],
  precallCallStatusOptions: ['坐席未接', '坐席接听', '关机', '无人接听', '不在服务区', '忙音', '停机', '空号'],
  lastContactStatus: ['正常接通', '无人接听', '企微跟进', '客户拒绝', '忙音/秘书台', '空号', '停机', '关机'],
  conditionMode: ['按接触状态', '按预外呼任务'],
  nextReturnVisitTime: [
    '下次预外呼时间',
    'N+1天 18时',
    'N+2天 18时',
...
let strategyConfiguratorData = {
  ...
  manualUnconnected: [
    { sortOrder: 1, id: 1012, ruleName: '分配配置A', priority: 1, status: '启用', conditionMode: 'contactStatus', conditionModeLabel: '按接触状态', contactStatus: ['无人接听'], carSeries: ['N7'], contactRepeatCount: {operator: '=', count: 0}, maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['科大讯飞-冷线索'], belongType: 'SmartCode', belongValues: ['SC-N6-0891', 'SC-GZ-2026'], includeIncomplete: '是', matchConditionSummary: '无人接听 / N7 / 含留资未满' },
    { sortOrder: 2, id: 1013, ruleName: '分配配置B', priority: 2, status: '启用', conditionMode: 'precallTask', conditionModeLabel: '按预外呼任务', lastContactStatus: ['关机'], carSeries: ['N6'], taskReturnVisitCount: {operator: '≥', count: 2}, maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['科大讯飞-留资未满'], belongType: 'SmartCode', belongValues: ['SC-SY-0234'], includeIncomplete: '是', matchConditionSummary: '关机 / N6 / 工单≥2次 / 含留资未满' },
    { sortOrder: 3, id: 1014, ruleName: '分配配置C', priority: 3, status: '停用', conditionMode: 'contactStatus', conditionModeLabel: '按接触状态', contactStatus: ['无人接听'], carSeries: ['华为天籁'], contactRepeatCount: {operator: '=', count: 0}, maxOutboundCount: '10天2次', pushInterval: 24, outboundType: ['一知-冷线索'], belongType: 'SmartCode', belongValues: ['SC-XK-7712'], includeIncomplete: '是', matchConditionSummary: '无人接听 / 华为天籁 / 含留资未满' }
  ],
  manualUnconverted: [
  ...
  nextVisitPrecall: [
    { sortOrder: 1, id: 1012, ruleName: 'N7/N6预约跟进预外呼', priority: 1, contactStatus: '无人接听', carSeries: ['N7', 'N6'], enableStickyRouting: '是', nextVisitTimeoutMinutes: 30, precallLine: '虚拟号-大众', systemStatus: '启用', createdAt: '2026-08-24 10:00:00 操作人：管理员', updatedAt: '2026-08-24 10:00:00 操作人：管理员' },
    { sortOrder: 2, id: 1013, ruleName: '全车系超时降级回访策略', priority: 2, contactStatus: '正常接通', carSeries: ['全部车系'], enableStickyRouting: '是', nextVisitTimeoutMinutes: 60, precallLine: '虚拟号-厚扑', systemStatus: '启用', createdAt: '2026-08-24 10:00:00 操作人：管理员', updatedAt: '2026-08-24 10:00:00 操作人：管理员' }
  ],
  excellentProject: [
  ...
  returnVisitOrder: [
    { sortOrder: 1, id: 1001, ruleName: '无人接听异常生成回访工单', priority: 1, status: '启用', precallSystemStatus: '已处理', triggerCondition: '无人接听', matchingDimension: '按渠道与媒体', mediaName: ['懂车帝', '汽车之家'], initialLeadStatus: [], channelCode: ['R1'], contactStatus: '无人接听', nurtureResult: '下次回访', abnormalReason: '', intentLevelAction: 'E', nextReturnVisitTime: 'N+1天 18时', agentRouting: '原始归属人', descriptionTemplate: '[系统自动话单转换] 懂车帝/之家渠道连续3次无人接听，系统已自动建立回访工单归属原始归属人，并自动降级为N。', createdAt: '2026-07-01 10:00:00 操作人：系统管理员', updatedAt: '2026-07-01 10:00:00 操作人：系统管理员' },
    { sortOrder: 2, id: 1002, ruleName: '客户拒绝投诉工单直接指派', priority: 2, status: '启用', precallSystemStatus: '已处理', triggerCondition: '空号', matchingDimension: '按线索状态', mediaName: [], initialLeadStatus: ['总部_战败'], channelCode: [], contactStatus: '正常接通', nurtureResult: '战败', abnormalReason: '价格因素', intentLevelAction: 'F', nextReturnVisitTime: 'N+1天 18时', agentRouting: '原始归属人', descriptionTemplate: '[系统自动话单转换] 客户呼叫反馈为空号，直接触发投诉工单指派给原始归属人。', createdAt: '2026-07-02 11:30:00 操作人：李四', updatedAt: '2026-07-02 11:30:00 操作人：李四' },
    { sortOrder: 3, id: 1003, ruleName: '忙音多次重试提醒', priority: 3, status: '停用', precallSystemStatus: '处理中', triggerCondition: '忙音', matchingDimension: '按渠道与媒体', mediaName: ['抖音'], initialLeadStatus: [], channelCode: [], contactStatus: '忙音/秘书台', nurtureResult: '无人接听下发', abnormalReason: '', intentLevelAction: 'C', nextReturnVisitTime: '下次预外呼时间', agentRouting: '原始归属人', descriptionTemplate: '[系统自动话单转换] 抖音渠道连续5次忙音，触发系统默认分配回访工单。', createdAt: '2026-07-03 09:15:00 操作人：张三', updatedAt: '2026-07-03 09:15:00 操作人：张三' },
    { sortOrder: 4, id: 1004, ruleName: '高意向未接通回访工单配置', priority: 4, status: '启用', precallSystemStatus: '已处理', triggerCondition: '无人接听', matchingDimension: '按线索状态', mediaName: [], initialLeadStatus: ['总部_高意向'], channelCode: [], contactStatus: '无人接听', nurtureResult: '下次回访', abnormalReason: '', intentLevelAction: 'A', nextReturnVisitTime: 'N+1天 10时', agentRouting: '原始归属人', descriptionTemplate: '[系统自动话单转换] 高意向线索呼叫未接通，建立回访工单指派给原始归属人。', createdAt: '2026-07-20 09:00:00 操作人：系统管理员', updatedAt: '2026-07-20 09:00:00 操作人：系统管理员' }
  ]
};
...
const strategyConfiguratorDefs = {
  ...
  manualUnconnected: {
    ...
    title: '未接通激活配置',
    desc: '配置人工外呼无人接听后转交 AI 外呼的规则，支持按接触状态或按预外呼任务两种命中条件模式。<br><strong>多配置命中处理：</strong>同一线索命中多个未接通激活配置时，按权重判定，数字越小越先判定；同权重取创建时间倒序。',
    addLabel: '新增配置',
    search: [
      { label: '关键字', field: 'keyword', type: 'text' },
      { label: '条件模式', field: 'conditionModeLabel', type: 'selectFromData' },
      { label: '接触状态', field: 'contactStatus', type: 'multiselect', options: 'contactStatus' },
    ...
    actions: ['add'],
    columns: [
      { label: '权重', field: 'priority', width: 64, priority: true },
      { label: '配置名称', field: 'ruleName', width: 140 },
      { label: '条件模式', field: 'conditionModeLabel', width: 110 },
      { label: '外呼类型', field: 'outboundType', width: 130 },
      { label: '命中条件摘要', field: 'matchConditionSummary', width: 300 },
      { label: 'AI外呼次数≤x次', field: 'maxOutboundCount', width: 130 },
      { label: '命中类型', field: 'belongType', width: 100 },
      { label: '包含留资未满', field: 'includeIncomplete', width: 110 },
      { label: '状态', field: 'status', width: 80, status: true }
    ],
    formSections: ['命中条件', '执行动作'],
    form: [
      { label: '配置名称', field: 'ruleName', type: 'text', required: true, section: '基础信息', placeholder: '请输入配置名称', wide: true },
      { label: '权重', field: 'priority', type: 'priority', required: true, section: '基础信息', defaultValue: 255, placeholder: '请输入权重' },
      { label: '状态', field: 'status', type: 'select', options: 'status', required: true, section: '基础信息', defaultValue: '启用' },
      { label: '接触状态', field: 'contactStatus', type: 'multiselect', options: 'contactStatus', required: true, section: '命中条件', placeholderOption: '请选择', conditionMode: 'contactStatus' },
      { label: '接触状态重复次数', field: 'contactRepeatCount', type: 'countCompare', required: true, allowZero: true, section: '命中条件', hintHtml: '填写非负整数，<strong>0次代表不限制</strong>。', conditionMode: 'contactStatus' },
      { label: '最后接触状态', field: 'lastContactStatus', type: 'multiselect', options: 'lastContactStatus', required: true, section: '命中条件', placeholderOption: '请选择', conditionMode: 'precallTask' },
      { label: '任务回访工单次数', field: 'taskReturnVisitCount', type: 'countCompare', required: true, allowZero: true, section: '命中条件', hintHtml: '填写非负整数，<strong>0次代表不限制</strong>。', conditionMode: 'precallTask' },
      ...
      { label: 'AI外呼次数≤x次', field: 'maxOutboundCount', type: 'dayCount', required: true, section: '命中条件', hintHtml: '填写非负整数，<strong>0次代表不限制</strong>。' },
  ...
  manualUnconverted: {
    ...
    form: [
      { label: '配置名称', field: 'ruleName', type: 'text', required: true, section: '基础信息', placeholder: '请输入配置名称', wide: true },
      { label: '权重', field: 'priority', type: 'priority', required: true, section: '基础信息', defaultValue: 255 },
      { label: '状态', field: 'status', type: 'select', options: 'status', required: true, section: '基础信息', defaultValue: '启用' },
      ...
      { label: '异常原因', field: 'abnormalReason', type: 'leadStatusReason', section: '命中条件' },
  ...
  precallTask: {
    ...
    search: [
      ...
      { label: '预外呼路由', field: 'precallLine', type: 'select', options: 'precallLine', excludeValues: ['请选择'] }
    ],
  ...
  nextVisitPrecall: {
    pageId: 'nextVisitPrecallConfigPage',
    title: '回访预外呼配置',
    desc: '配置预约回访任务的自动化外拨路线、坐席粘性路由及超时降级规则。',
    addLabel: '新增配置',
    search: [
      { label: '关键字', field: 'keyword', type: 'text' },
      { label: '配置名称', field: 'ruleName', type: 'text' },
      { label: '接触状态', field: 'contactStatus', type: 'select', options: 'contactStatus', includeAll: true },
      { label: '意向车系', field: 'carSeries', type: 'select', options: 'carSeries', includeAll: true },
      { label: '预外呼路由', field: 'precallLine', type: 'select', options: 'precallLine', excludeValues: ['请选择'] },
      { label: '状态', field: 'systemStatus', type: 'select', options: 'status', includeAll: true }
    ],
    actions: ['import', 'export', 'add'],
    columns: [
      { label: '权重', field: 'priority', width: 70 },
      { label: '配置名称', field: 'ruleName', width: 180 },
      { label: '接触状态', field: 'contactStatus', width: 140 },
      { label: '意向车系', field: 'carSeries', width: 140 },
      { label: '预外呼路由', field: 'precallLine', width: 140 },
      { label: '粘性路由(优先原坐席)', field: 'enableStickyRouting', width: 160 },
      { label: '超时降级阀值(分钟)', field: 'nextVisitTimeoutMinutes', width: 150 },
      { label: '状态', field: 'systemStatus', width: 90, status: true }
    ],
    form: []
  },
  ...
  returnVisitOrder: {
    ...
    search: [
      { label: '关键字', field: 'keyword', type: 'text' },
      { label: '回访结果', field: 'nurtureResult', type: 'select', options: 'manualUnconvertedReviewResult' },
      { label: '接触状态', field: 'contactStatus', type: 'multiselect', options: 'contactStatus' },
      { label: '状态', field: 'status', type: 'select', options: 'status' }
    ],
    actions: ['add'],
    ...
    form: [
      ...
      { label: '预外呼通话状态', field: 'triggerCondition', type: 'select', options: 'returnVisitOrderContactStatus', required: true, section: '匹配条件', placeholderOption: '请选择' },
      ...
      { label: '回访结果', field: 'nurtureResult', type: 'select', options: 'manualUnconvertedReviewResult', required: true, section: '执行动作', placeholderOption: '请选择' },
      { label: '接触状态', field: 'contactStatus', type: 'select', options: 'contactStatus', required: true, section: '执行动作', placeholderOption: '请选择' },
      { label: '异常原因', field: 'abnormalReason', type: 'leadStatusReason', required: true, section: '执行动作' },
      { label: '意向级别', field: 'intentLevelAction', type: 'select', options: 'intentLevelAction', required: true, section: '执行动作', placeholderOption: '请选择意向级别动作' },
...
strategyConfiguratorData.amapMap = strategyConfiguratorData.returnVisitOrder.map((row, index) => ({
  ...
  projectName: [['新能源专项'], ['DCC培育项目'], ['高意向培育'], ['深度试驾项目']][index] || ['新能源专项'],
  initialLeadStatus: [['总部_高意向'], ['总部_战败'], ['总部_休眠未购'], ['总部_留资']][index] || ['总部_高意向'],
  agentRouting: '原始归属人',
  defaultCarSeries: ['ARIYA', 'N6', '天籁', '轩逸'][index] || 'ARIYA'
}));
strategyConfiguratorDefs.amapMap = {
  ...
  search: [
    { label: '关键字', field: 'keyword', type: 'text' },
    { label: '初始线索状态', field: 'initialLeadStatus', type: 'select', options: 'returnVisitInitialLeadStatus' },
    { label: '回访结果', field: 'nurtureResult', type: 'select', options: 'manualUnconvertedReviewResult' },
  ...
  form: [
    ...
    ...strategyConfiguratorDefs.returnVisitOrder.form
      .filter(field => field.section === '执行动作' && field.field !== 'descriptionTemplate')
      .map(field => ({ ...field, required: field.required })),
    { label: '默认车系', field: 'defaultCarSeries', type: 'select', options: 'amapDefaultCarSeries', required: true, section: '执行动作', placeholderOption: '请选择' },
    ...strategyConfiguratorDefs.returnVisitOrder.form
      .filter(field => field.section === '执行动作' && field.field === 'descriptionTemplate')
...
window.precallGlobalExclusionState = window.precallGlobalExclusionState || {
  enabled: true,
  excludedScList: ['SC-SZ-001 (深圳华熙店)', 'SC-GZ-2026 (广州体验中心)'],
  excludedProjects: ['华南试驾专场大项目'],
  excludedMedias: ['懂车帝', '抖音']
};

function renderExclusionTags(list = [], tagClass = '', emptyText = '暂无') {
  if (!list || !list.length) return `<span class="exclusion-empty">${emptyText}</span>`;
  const limit = 3;
  const visibleItems = list.slice(0, limit);
  const hiddenCount = list.length - limit;
  let html = visibleItems.map(item => `<span class="exclusion-tag ${tagClass}">${escapeHtml(item)}</span>`).join('');
  if (hiddenCount > 0) {
    const hiddenListText = escapeAttr(list.slice(limit).join('、'));
    html += `<span class="exclusion-more-badge" title="点击展开管理。更多隐藏项: ${hiddenListText}" onclick="openPrecallGlobalExclusionModal()">+ ${hiddenCount} 项</span>`;
  }
  return html;
}

function renderPrecallGlobalExclusionBanner() {
  const state = window.precallGlobalExclusionState;
  const isEnabled = state.enabled;
  const scTags = renderExclusionTags(state.excludedScList, 'tag-sc', '暂无全局排除SC');
  const prjTags = renderExclusionTags(state.excludedProjects, 'tag-prj', '暂无全局排除大项目');
  const mediaTags = renderExclusionTags(state.excludedMedias, 'tag-media', '暂无全局排除媒体');

  return `
    <div class="precall-exclusion-banner ${isEnabled ? 'active' : 'disabled'}">
      <div class="exclusion-banner-header">
        <div class="exclusion-title-wrap">
          <span class="exclusion-shield-icon">🛡️</span>
          <div>
            <div class="exclusion-title">预外呼全局排除防线 (优先拦截池)</div>
            <div class="exclusion-subtitle">命中所配置全局排除项的线索，不论是否符合下方任何推送/回访预外呼配置，均优先直接拦截不发外呼。</div>
          </div>
        </div>
        <div class="exclusion-banner-actions">
          <label class="switch-toggle" title="开关全局排除防线">
            <input type="checkbox" ${isEnabled ? 'checked' : ''} onchange="togglePrecallGlobalExclusionState(this.checked)" />
            <span class="switch-slider"></span>
            <span class="switch-label">${isEnabled ? '已开启防护' : '已关闭防护'}</span>
          </label>
          <button class="btn-secondary compact" type="button" onclick="openPrecallGlobalExclusionModal()">⚙️ 配置全局排除项</button>
        </div>
      </div>
      <div class="exclusion-banner-body" style="${isEnabled ? '' : 'display:none;'}">
        <div class="exclusion-group">
          <div class="exclusion-group-label">全局排除SC (${state.excludedScList.length})：</div>
          <div class="exclusion-tags">${scTags}</div>
        </div>
        <div class="exclusion-group">
          <div class="exclusion-group-label">全局排除大项目 (${state.excludedProjects.length})：</div>
          <div class="exclusion-tags">${prjTags}</div>
        </div>
        <div class="exclusion-group">
          <div class="exclusion-group-label">全局排除媒体 (${state.excludedMedias.length})：</div>
          <div class="exclusion-tags">${mediaTags}</div>
        </div>
      </div>
    </div>
  `;
...
function togglePrecallGlobalExclusionState(checked) {
  window.precallGlobalExclusionState.enabled = checked;
  const currentKey = window.currentStrategyConfiguratorKey || 'precallTask';
  renderStrategyConfiguratorPage(currentKey);
}

function openPrecallGlobalExclusionModal() {
  const state = window.precallGlobalExclusionState;
  const scOptions = ['SC-SZ-001 (深圳华熙店)', 'SC-GZ-2026 (广州体验中心)', 'SC-N6-0891', 'SC-XK-7712', 'SC-BJ-1002', 'SC-SH-2088'];
  const projectOptions = (configuratorOptions.projectName || []).concat(['华南试驾专场项目']);
  const mediaOptions = ['懂车帝', '汽车之家', '易车', '抖音', '快手', '微信朋友圈', '百度', '腾讯'];

  document.getElementById('leadDispatchRuleModalTitle').textContent = '⚙️ 配置预外呼全局排除防线 (排除池)';
  document.getElementById('leadDispatchRuleModalBody').innerHTML = `
    <div class="dispatch-rule-form">
      <div style="background:#fff7e6; border:1px solid #ffd591; padding:10px 14px; border-radius:6px; margin-bottom:16px; font-size:12px; color:#d46b08;">
        💡 <strong>提示：</strong>在此处选择的 SC、大项目名与媒体来源，将作为全平台预外呼规则（推送预外呼 / 回访预外呼）的第一道全局防护网。线索一旦命中，无需在每条规则里重复配置排除。
      </div>
      <section class="dispatch-form-section">
        <div class="dispatch-section-title">全局排除维度管理</div>
        <div class="dispatch-form-grid">
          <div class="form-group wide">
            <div class="form-label">全局排除 SC (SmartCode)</div>
            ${renderGenericTagPicker({ idPrefix: 'globalExclusionSc', inputName: 'globalExclusionScCheckbox', options: scOptions, selected: state.excludedScList, placeholder: '请选择排除的SmartCode', isSearch: true, width: '100%' })}
            <div class="series-form-hint">命中此列表中任何 SmartCode 的线索，均直接终止预外呼。</div>
          </div>
          <div class="form-group wide">
            <div class="form-label">全局排除大项目名</div>
            ${renderGenericTagPicker({ idPrefix: 'globalExclusionProject', inputName: 'globalExclusionProjectCheckbox', options: projectOptions, selected: state.excludedProjects, placeholder: '请选择排除的大项目名', isSearch: true, width: '100%' })}
          </div>
          <div class="form-group wide">
            <div class="form-label">全局排除媒体来源</div>
            ${renderGenericTagPicker({ idPrefix: 'globalExclusionMedia', inputName: 'globalExclusionMediaCheckbox', options: mediaOptions, selected: state.excludedMedias, placeholder: '请选择排除的媒体来源', width: '100%' })}
          </div>
        </div>
      </section>
    </div>
  ...
  document.getElementById('leadDispatchRuleModalFooter').innerHTML = `
    ...
    <button class="btn-save" type="button" onclick="savePrecallGlobalExclusionModal()">保存全局防线</button>
  `;
  document.getElementById('leadDispatchRuleModal').classList.add('show');
  updateGenericTagPickerSummary('globalExclusionSc');
  updateGenericTagPickerSummary('globalExclusionProject');
  updateGenericTagPickerSummary('globalExclusionMedia');
}

function savePrecallGlobalExclusionModal() {
  const getSelected = idPrefix => getGenericTagPickerInputs(idPrefix).filter(input => input.checked).map(input => input.value);
  window.precallGlobalExclusionState.excludedScList = getSelected('globalExclusionSc');
  window.precallGlobalExclusionState.excludedProjects = getSelected('globalExclusionProject');
  window.precallGlobalExclusionState.excludedMedias = getSelected('globalExclusionMedia');
...
function renderStrategyConfiguratorPage(key) {
  window.currentStrategyConfiguratorKey = key;
  ...
  page.innerHTML = `
    ...
    `}
    ${(key === 'precallTask' || key === 'nextVisitPrecall') ? renderPrecallGlobalExclusionBanner() : ''}
  ...
  `;
  bindStrategyConfiguratorAddAction(page, key);
...
function bindStrategyConfiguratorAddAction(page, key) {
  const trigger = page.querySelector('[data-strategy-configurator-open]');
  if (!trigger) return;
  trigger.addEventListener('click', event => {
    event.preventDefault();
    event.stopPropagation();
    openStrategyConfiguratorModal(key, 'add');
  });
}
...
function renderStrategyConfiguratorSummary(key) {
  ...
  if (key === 'followCount') {
  ...
  } else if (key === 'precallTask' || key === 'nextVisitPrecall') {
    ...
    items = [
      { label: '配置总数', value: rows.length },
      { label: '已启用', value: enabled },
      { label: '大项目名', value: projectNameCount },
      { label: '初始线索状态', value: initialLeadStatusCount }
    ];
    ...
    items = [
      { label: '配置总数', value: rows.length },
      { label: '已启用', value: enabled },
      { label: '预外呼通话状态', value: triggerConditionCount },
      { label: '工单类型', value: nurtureResultCount },
      { label: '指派规则', value: agentRoutingCount }
    ];
  } else {
    const enabled = rows.filter(row => row.systemStatus === '启用').length;
    items = [
...
function renderStrategyConfiguratorSearch(key) {
  ...
  if (['manualUnconnected', 'manualUnconverted', 'precallTask', 'nextVisitPrecall', 'excellentProject', 'excellentChannel', 'excellentSc', 'returnVisitOrder', 'amapMap', 'followCount'].includes(key)) return renderFlatStrategyConfiguratorFilterRow(key, def);
...
function renderStrategyConfiguratorActions(key) {
  ...
  return def.actions.map(action => {
    ...
    if (action === 'add') return `<button class="btn-add strategy-add-btn" type="button" onclick="openStrategyConfiguratorModal('${key}', 'add')" data-strategy-configurator-open="${escapeAttr(key)}" data-strategy-configurator-mode="add">${def.addLabel || '新增'}</button>`;
...
function getFilteredStrategyConfiguratorRows(key) {
  ...
  return (strategyConfiguratorData[key] || [])
    .filter(row => def.search.every(field => {
      ...
      if (field.field === 'keyword') {
        if (key === 'followCount') {
          const fieldsToCheck = [
            row.leadStatus,
            `FUC-${String(row.id).padStart(3, '0')}`,
            row.followUpCount
          ];
          return fieldsToCheck.some(item => String(item ?? '').toLowerCase().includes(value.toLowerCase()));
        }
    ...
    .sort((a, b) => {
      if (key === 'precallTask' || key === 'manualUnconnected' || isReturnVisitOrderConfigurator(key)) {
...
function renderStrategyConfiguratorTable(key) {
  ...
  if (!body) return;
  const allData = strategyConfiguratorData[key] || [];
  const emptyText = getStrategyConfiguratorEmptyText(key, allData.length > 0);
  ...
  `).join('') : `<tr><td colspan="${def.columns.length + 1}"><div class="empty-state">${emptyText}</div></td></tr>`;
}

function getStrategyConfiguratorEmptyText(key, hasData) {
  if (!hasData) return '暂无数据';
  const emptyMap = {
    manualUnconnected: '暂无匹配的未接通激活配置',
    manualUnconverted: '暂无匹配的未成交激活配置',
    precallTask: '暂无匹配的预外呼任务配置',
    returnVisitOrder: '暂无匹配的回访工单配置',
    amapMap: '暂无匹配的高德地图配置',
    followCount: '暂无匹配的跟进次数配置',
    excellentProject: '暂无匹配的优秀项目配置',
    excellentChannel: '暂无匹配的优秀渠道配置',
    excellentSc: '暂无匹配的优秀SmartCode配置'
  };
  return emptyMap[key] || '暂无匹配的数据';
}
...
function renderStrategyConfiguratorCell(key, row, col) {
  ...
  if (key === 'manualUnconnected' && col.field === 'priority') {
    return `<span class="policy-plain-text">P${row.priority || 255}</span>`;
  }
  ...
  if (isReturnVisitOrderConfigurator(key) && col.field === 'matchSummary') {
    ...
    const items = [
      ...
      `预外呼通话状态：${escapeHtml(row.triggerCondition || '不限')}`,
  ...
  if (isReturnVisitOrderConfigurator(key) && col.field === 'actionSummary') {
    const items = [
      ...
      key === 'amapMap' && row.defaultCarSeries ? `默认车系 → ${escapeHtml(row.defaultCarSeries)}` : ''
    ].filter(Boolean);
...
function openStrategyConfiguratorModal(key, mode, id = null) {
  ...
  if (mode === 'view') {
    ...
    document.getElementById('leadDispatchRuleModalFooter').innerHTML = `<button class="btn-cancel" type="button" onclick="closeModal('leadDispatchRuleModal')">取消</button><button class="btn-save" type="button" onclick="saveStrategyConfiguratorRow()">保存</button>`;
  }
  
  document.getElementById('leadDispatchRuleModal').classList.add('show');
  bindPrecallModalInteractions();
  if (mode !== 'view') {
  ...
  if (key === 'manualUnconnected' && mode !== 'view') {
    window.unconnectedConditionModeState = { mode: row?.conditionMode || 'contactStatus' };
  ...
  if ((key === 'manualUnconverted' || isReturnVisitOrderConfigurator(key)) && mode !== 'view') {
    if (isReturnVisitOrderConfigurator(key)) {
      handleReturnVisitOrderNurtureResultChange(row?.abnormalReason || '', row?.intentLevelAction || '', row?.nextReturnVisitTime || '', row?.contactStatus || '');
  ...
  if ((key === 'precallTask' || key === 'nextVisitPrecall') && mode !== 'view') {
    const root = getPrecallFormRoot(key);
    if (row?.attributeType === 'SmartCode') updatePrecallPickerSummary(root, 'smart', '请选择SmartCode', strategyPrecallAttributeConfigs.SmartCode.options.length);
    if (row?.attributeType === '大项目名') updatePrecallPickerSummary(root, 'project', '请选择大项目名', strategyPrecallProjectNameOptions.length);
  }
  if (key === 'excellentSc') {
  ...
  if (key === 'nextVisitPrecall' && mode !== 'view') {
    updateGenericTagPickerSummary('nextVisitContactStatus');
  }
}
...
function renderStrategyConfiguratorDetail(key, row) {
  ...
  if (key === 'manualUnconnected') {
    ...
    const sections = [
      ...
      },
      {
        title: '命中条件',
        fields: [
          ['条件模式', row.conditionModeLabel || '按接触状态'],
          row.conditionMode === 'precallTask'
            ? ['最后接触状态', Array.isArray(row.lastContactStatus) ? (row.lastContactStatus.length ? row.lastContactStatus.join('、') : '—') : (row.lastContactStatus || '—')]
            : ['接触状态', Array.isArray(row.contactStatus) ? (row.contactStatus.length ? row.contactStatus.join('、') : '—') : (row.contactStatus || '—')],
          ['回访车系', Array.isArray(row.carSeries) ? row.carSeries.join('、') : (row.carSeries || '不限')],
          row.conditionMode === 'precallTask'
            ? ['任务回访工单次数', formatCountCompareForDetail(row.taskReturnVisitCount)]
            : ['接触状态重复次数', formatCountCompareForDetail(row.contactRepeatCount)],
  ...
  if (key === 'nextVisitPrecall') return renderNextVisitPrecallDetail(row);
...
function formatPrecallDetailValues(value, fallback = '—') {
  if (Array.isArray(value)) return value.length ? value.join('、') : fallback;
  return value || fallback;
}

function renderPrecallConfigDetail(row, variant) {
  if (!row || !Object.keys(row).length) return renderEmptySnapshot('未找到配置');
  const isNextVisit = variant === 'nextVisit';
  const matchFields = [
    ...(isNextVisit ? [['回访结果', '下次回访'], ['接触状态', formatPrecallDetailValues(row.contactStatus)]] : []),
    ['命中类型', row.attributeType || '—'],
    ['类型值', formatPrecallDetailValues(row.attributeValues)],
    ...(row.attributeType !== '线索状态' && row.leadStatuses?.length ? [['线索状态（且）', formatPrecallDetailValues(row.leadStatuses)]] : []),
    ['意向车系', formatPrecallDetailValues(row.carSeries, '不限')]
  ];
  const actionFields = [
    ['预外呼线路', row.precallLine || '—'],
    ...(isNextVisit
      ? [['粘性路由优先分配', row.enableStickyRouting === '是' ? '是（优先原归属坐席）' : row.enableStickyRouting === '否' ? '否（直接池化分配）' : '—'], ['超时降级阈值', row.nextVisitTimeoutMinutes ? `${row.nextVisitTimeoutMinutes} 分钟` : '—']]
      : []),
    ['工作时段优先级', row.workTimePriority ? `P${row.workTimePriority}` : '—'],
    ['工作时段技能组', row.workTimeSkillGroup || '—'],
    ['非工作时段优先级', row.offWorkPriority ? `P${row.offWorkPriority}` : '—'],
    ['非工作时段技能组', row.offWorkSkillGroup || '—'],
    ['生效时段', getPrecallWorkTimeSummary(row.workTime)]
  ];
  return renderRuleDetailSections([
    {
      title: '基础信息',
      fields: [
        ['配置名称', row.ruleName || '—'],
        ['权重优先级', `P${row.priority || 255}`],
        ['状态', row.systemStatus || '启用']
      ]
    },
    { title: '命中条件', fields: matchFields },
    { title: '执行动作', fields: actionFields }
  ]);
}

function renderPrecallTaskDetail(row) {
  return renderPrecallConfigDetail(row, 'precallTask');
}

function renderNextVisitPrecallDetail(row) {
  return renderPrecallConfigDetail(row, 'nextVisit');
}
...
function renderReturnVisitOrderDetail(row) {
  ...
  const sections = [
    ...
    },
    {
      title: '执行动作',
      fields: [
        ['回访结果', row.nurtureResult],
        ['接触状态', row.contactStatus],
        ['异常原因', row.abnormalReason || '不更新'],
        ['意向级别', row.intentLevelAction],
...
function renderRuleDetailSections(sections) {
  return `<div class="rule-detail-layout">${sections.map(section => `<section class="rule-detail-section"><div class="rule-detail-title">${section.title}</div><div class="rule-detail-body">${section.fields.map(([label, value]) => `<div class="rule-detail-field"><div class="rule-detail-label">${escapeHtml(label)}</div><div class="rule-detail-value">${renderStrategyDetailValue(value)}</div></div>`).join('')}</div></section>`).join('')}</div>`;
}

function renderStrategyDetailValue(value) {
...
function renderPrecallCheckboxes(name, options, selected, readonly = false) {
  const isAllSeriesSelected = name === 'precallCarSeries' && selected.includes('全部车系');
  return `<div class="series-select-panel"><div class="series-series-grid">${options.map(item => {
    const isDisableItem = isAllSeriesSelected && item !== '全部车系';
    const isDisabled = readonly || isDisableItem;
    const opacityStyle = isDisableItem ? 'style="opacity: 0.45; cursor: not-allowed;"' : '';
    const role = name === 'precallCarSeries' ? ' data-precall-role="car-series"' : '';
    return `<label class="series-series-option" ${opacityStyle}><input type="checkbox" name="${name}" value="${escapeAttr(item)}" ${selected.includes(item) ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}${role}/><span>${escapeHtml(item)}</span></label>`;
  }).join('')}</div></div>`;
}

function handlePrecallCarSeriesChange(checkbox) {
  if (checkbox.name !== 'precallCarSeries') return;
  const val = checkbox.value;
  const isChecked = checkbox.checked;
  const formRoot = checkbox.closest('[data-precall-form-key]') || document;
  const inputs = Array.from(formRoot.querySelectorAll('input[name="precallCarSeries"]'));
  const allInput = inputs.find(input => input.value === '全部车系');
  if (val === '全部车系') {
    inputs.forEach(input => {
      if (input.value !== '全部车系') {
        input.checked = false;
        input.disabled = isChecked;
        const label = input.closest('.series-series-option');
        if (label) {
          label.style.opacity = isChecked ? '0.45' : '1';
          label.style.cursor = isChecked ? 'not-allowed' : 'pointer';
        }
      }
    });
  } else if (isChecked && allInput) {
    allInput.checked = false;
  }
}


// 预外呼配置在本文件内独立维护，避免与线索分配表单的同名全局配置发生冲突。
const strategyPrecallProjectNameOptions = ['新能源专项', '置换活动', 'DCC培育项目', '高意向培育', '保客激活', '金融置换专项'];
const strategyPrecallLeadStatusOptions = ['总部_高意向', '总部_休眠未购', '总部_休眠失联', '总部_战败', '总部_暂败', '总部_阶段性战败', '总部_无效', '总部_异地'];
const strategyPrecallAttributeConfigs = Object.freeze({
  '渠道编码': { label: '渠道编码', options: (typeof configuratorOptions !== 'undefined' && configuratorOptions.channelCode) ? configuratorOptions.channelCode : ['R1', 'R2', 'R3', 'R4', 'R5'] },
  'SmartCode': { label: 'SmartCode', options: ['SC-GZ-2026', 'SC-BJ-8899', 'SC-SH-6688', 'SC-SZ-7711', 'SC-HZ-3322'] },
  '大项目名': { label: '大项目名', options: strategyPrecallProjectNameOptions },
  '线索状态': { label: '线索状态', options: strategyPrecallLeadStatusOptions },
  '媒体名称': { label: '媒体名称', options: (typeof configuratorOptions !== 'undefined' && configuratorOptions.mediaName) ? configuratorOptions.mediaName : ['懂车帝', '汽车之家', '易车', '抖音', '小红书'] }
});

const precallInputModeState = {
  precallTask: { smartCode: 'picker', projectName: 'picker' },
  nextVisitPrecall: { smartCode: 'picker', projectName: 'picker' }
};

function getPrecallFormKey(key = strategyConfiguratorEditing.key) {
  return key === 'nextVisitPrecall' ? 'nextVisitPrecall' : 'precallTask';
}

function getPrecallInputMode(key, type) {
  return precallInputModeState[getPrecallFormKey(key)][type];
}

function setPrecallInputMode(key, type, mode) {
  precallInputModeState[getPrecallFormKey(key)][type] = mode === 'manual' ? 'manual' : 'picker';
  return precallInputModeState[getPrecallFormKey(key)][type];
}

function getPrecallFormRoot(key = strategyConfiguratorEditing.key) {
  return document.querySelector(`[data-precall-form-key="${getPrecallFormKey(key)}"]`);
}

function getPrecallControlRoot(element) {
  return element?.closest?.('[data-precall-form-key]') || getPrecallFormRoot();
}

function getPrecallFormValue(root, id) {
  return root?.querySelector(`#${id}`)?.value.trim() || '';
}

function getPrecallCheckedValues(root, name) {
  return Array.from(root?.querySelectorAll(`input[name="${name}"]:checked`) || []).map(input => input.value);
}

function getPrecallPickerParts(root, type) {
  return {
    picker: root?.querySelector(`[data-precall-element="${type}-picker"]`),
    manual: root?.querySelector(`[data-precall-element="${type}-manual"]`),
    panel: root?.querySelector(`[data-precall-element="${type}-options"]`),
    trigger: root?.querySelector(`[data-precall-element="${type}-trigger"]`),
    count: root?.querySelector(`[data-precall-element="${type}-count"]`),
    pickerMode: root?.querySelector(`[data-precall-element="${type}-picker-mode"]`),
    manualMode: root?.querySelector(`[data-precall-element="${type}-manual-mode"]`),
    manualInput: root?.querySelector(`[data-precall-element="${type}-manual-input"]`)
  };
}

function updatePrecallPickerSummary(root, type, placeholder, total) {
  const parts = getPrecallPickerParts(root, type);
  const inputs = Array.from(parts.panel?.querySelectorAll('input[name="precallAttributeValue"]') || []);
  const selected = inputs.filter(input => input.checked).map(input => input.value);
  if (parts.count) parts.count.textContent = `已选 ${selected.length} / ${total}`;
  if (parts.trigger) {
    parts.trigger.textContent = selected.length ? selected.join('、') : placeholder;
    parts.trigger.classList.toggle('placeholder', !selected.length);
  }
  return selected;
}

function switchPrecallPickerInputMode(root, type, mode, placeholder, total) {
  const formKey = root?.dataset?.precallFormKey || strategyConfiguratorEditing.key;
  const currentMode = setPrecallInputMode(formKey, type === 'smart' ? 'smartCode' : 'projectName', mode);
  const isManual = currentMode === 'manual';
  const parts = getPrecallPickerParts(root, type);
  const inputs = Array.from(parts.panel?.querySelectorAll('input[name="precallAttributeValue"]') || []);
  if (isManual) {
    if (parts.manualInput) parts.manualInput.value = inputs.filter(input => input.checked).map(input => input.value).join('\n');
  } else if (parts.manualInput) {
    const values = [...new Set(parts.manualInput.value.split(/[\n,;，；\s]+/).map(value => value.trim()).filter(Boolean))];
    inputs.forEach(input => {
      input.checked = values.includes(input.value);
      input.closest('.tag-option')?.classList.toggle('selected', input.checked);
    });
  }
  parts.pickerMode?.classList.toggle('active', !isManual);
  parts.manualMode?.classList.toggle('active', isManual);
  if (parts.picker) parts.picker.style.display = isManual ? 'none' : '';
  if (parts.manual) parts.manual.style.display = isManual ? '' : 'none';
  updatePrecallPickerSummary(root, type, placeholder, total);
}

function syncPrecallAttributeFields(formRoot = document.querySelector('[data-precall-form-key]')) {
  const type = formRoot?.querySelector('[data-precall-role="attribute-type"]')?.value || '请选择';
  const formKey = formRoot?.dataset?.precallFormKey || strategyConfiguratorEditing.key;
  const config = strategyPrecallAttributeConfigs[type];
  const valueGroup = formRoot?.querySelector('[data-precall-role="attribute-value-group"]');
  const statusGroup = formRoot?.querySelector('[data-precall-role="lead-status-group"]');
  if (valueGroup) {
    valueGroup.innerHTML = renderPrecallAttributeValueControl(type, [], 'picker', false, formKey);
  }
  if (statusGroup) {
    statusGroup.style.display = (config && type !== '线索状态') ? '' : 'none';
  }
}

function bindPrecallModalInteractions() {
  const modalBody = document.getElementById('leadDispatchRuleModalBody');
  if (!modalBody || modalBody.dataset.precallInteractionsBound === 'true') return;
  modalBody.dataset.precallInteractionsBound = 'true';

  modalBody.addEventListener('change', event => {
    const target = event.target;
    if (target.dataset.precallRole === 'attribute-type') {
      syncPrecallAttributeFields(target.closest('[data-precall-form-key]'));
      return;
    }
    if (target.dataset.precallRole === 'car-series') {
      handlePrecallCarSeriesChange(target);
      return;
    }
    if (target.matches('[data-precall-element="project-options"] input[name="precallAttributeValue"]')) updatePrecallPickerSummary(getPrecallControlRoot(target), 'project', '请选择大项目名', strategyPrecallProjectNameOptions.length);
    if (target.matches('[data-precall-element="smart-options"] input[name="precallAttributeValue"]')) updatePrecallPickerSummary(getPrecallControlRoot(target), 'smart', '请选择SmartCode', strategyPrecallAttributeConfigs.SmartCode.options.length);
  });

  modalBody.addEventListener('input', event => {
    const target = event.target;
    if (target.dataset.precallRole === 'integer-input') target.value = target.value.replace(/[^0-9]/g, '');
  });

  modalBody.addEventListener('click', event => {
    const control = event.target.closest('[data-precall-control]');
    if (!control || control.disabled) return;
    event.preventDefault();
    const action = control.dataset.precallControl;
    const root = getPrecallControlRoot(control);
    if (action === 'project-mode') switchPrecallPickerInputMode(root, 'project', control.dataset.mode, '请选择大项目名', strategyPrecallProjectNameOptions.length);
    if (action === 'project-toggle') getPrecallPickerParts(root, 'project').panel?.classList.toggle('show');
    if (action === 'project-select-all' || action === 'project-clear') {
      getPrecallPickerParts(root, 'project').panel?.querySelectorAll('input[name="precallAttributeValue"]').forEach(input => { input.checked = action === 'project-select-all'; input.closest('.tag-option')?.classList.toggle('selected', input.checked); });
      updatePrecallPickerSummary(root, 'project', '请选择大项目名', strategyPrecallProjectNameOptions.length);
    }
    if (action === 'smart-mode') switchPrecallPickerInputMode(root, 'smart', control.dataset.mode, '请选择SmartCode', strategyPrecallAttributeConfigs.SmartCode.options.length);
    if (action === 'smart-toggle') getPrecallPickerParts(root, 'smart').panel?.classList.toggle('show');
    if (action === 'smart-select-all' || action === 'smart-clear') {
      getPrecallPickerParts(root, 'smart').panel?.querySelectorAll('input[name="precallAttributeValue"]').forEach(input => { input.checked = action === 'smart-select-all'; input.closest('.tag-option')?.classList.toggle('selected', input.checked); });
      updatePrecallPickerSummary(root, 'smart', '请选择SmartCode', strategyPrecallAttributeConfigs.SmartCode.options.length);
    }
    if (action === 'sticky-help') togglePrecallStickyHelp(control);
    if (action === 'sticky-help-content') event.stopPropagation();
    if (action === 'time-add') {
      control.dataset.timeContainer === 'nextVisitWorkTimeSlots' ? addNextVisitWorkTimeSlot() : addPrecallWorkTimeSlot();
    }
    if (action === 'time-remove') removePrecallWorkTimeSlot(control);
  });
}

function togglePrecallStickyHelp(trigger) {
  const formRoot = getPrecallControlRoot(trigger);
  const popover = formRoot?.querySelector('[data-precall-element="sticky-help-popover"]');
  if (!popover) return;
  const isOpening = popover.style.display === 'none' || !popover.style.display;
  popover.style.display = isOpening ? 'block' : 'none';
  if (trigger) trigger.setAttribute('aria-expanded', String(isOpening));
}

function renderPrecallAttributeValueControl(attributeType, selectedValues = [], smartCodeInputMode = 'picker', readonly = false, formKey = strategyConfiguratorEditing.key) {
  const config = strategyPrecallAttributeConfigs[attributeType];
  if (!config) return '<div class="form-label">类型值</div><div class="series-form-hint">请先选择命中类型</div>';
  if (attributeType === '大项目名') return renderPrecallProjectNameInput(selectedValues, smartCodeInputMode, readonly, formKey);
  ...
  const activeFormKey = getPrecallFormKey(formKey);
  const isManual = setPrecallInputMode(activeFormKey, 'smartCode', smartCodeInputMode) === 'manual';
  ...
  return `<div class="form-label">SmartCode <span class="required">*</span></div>
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
      <div class="smart-code-input-mode" style="margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between;">
        <div class="form-field-helper-label">录入方式</div>
        <div class="series-mode-toggle" style="margin-top: 0; margin-bottom: 0;">
          <button class="duration-tab ${!isManual ? 'active' : ''}" type="button" data-precall-element="smart-picker-mode" ${readonly ? 'disabled' : 'data-precall-control="smart-mode" data-mode="picker"'}>下拉勾选</button>
          <button class="duration-tab ${isManual ? 'active' : ''}" type="button" data-precall-element="smart-manual-mode" ${readonly ? 'disabled' : 'data-precall-control="smart-mode" data-mode="manual"'}>手动输入</button>
        </div>
      </div>
      
      <div data-precall-element="smart-picker" style="${isManual ? 'display:none' : ''}">
        <div class="tag-picker project-name-picker ${readonly ? 'readonly' : ''}">
          <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" data-precall-element="smart-trigger" type="button" ${readonly ? 'disabled' : 'data-precall-control="smart-toggle"'}>${selectedValues.length ? escapeHtml(selectedValues.join('、')) : '请选择SmartCode'}</button>
          <div class="tag-picker-panel" data-precall-element="smart-options">
            <div class="tag-picker-toolbar"><span data-precall-element="smart-count">已选 ${selectedValues.length} / ${config.options.length}</span>${readonly ? '' : '<div class="tag-picker-actions"><button class="tag-picker-action" type="button" data-precall-control="smart-select-all">全选</button><button class="tag-picker-action" type="button" data-precall-control="smart-clear">清空</button></div>'}</div>
            <div class="tag-picker-list">${config.options.map(item => `<label class="tag-option ${selectedValues.includes(item) ? 'selected' : ''}"><input type="checkbox" name="precallAttributeValue" value="${escapeAttr(item)}" ${selectedValues.includes(item) ? 'checked' : ''} ${disabled} />${escapeHtml(item)}</label>`).join('')}</div>
          </div>
        </div>
      </div>
      
      <div data-precall-element="smart-manual" style="${isManual ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" data-precall-element="smart-manual-input" rows="4" style="margin-top: 0;" placeholder="可一次粘贴多个 SmartCode，支持换行、逗号、分号或空格分隔" ${disabled}>${escapeHtml(selectedValues.join('\n'))}</textarea>
        <div class="series-form-hint" style="margin-bottom: 0;">保存时会自动拆分并去重；不在下拉选项中的 SmartCode 也可直接录入。</div>
      </div>
    </div>`;
}

function renderPrecallProjectNameInput(selectedValues = [], inputMode = 'picker', readonly = false, formKey = strategyConfiguratorEditing.key) {
  const isManual = setPrecallInputMode(formKey, 'projectName', inputMode) === 'manual';
  const disabled = readonly ? 'disabled' : '';
  return `<div class="form-label">大项目名 <span class="required">*</span></div>
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
      <div class="smart-code-input-mode" style="margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between;">
        <div class="form-field-helper-label">录入方式</div>
        <div class="series-mode-toggle" style="margin-top: 0; margin-bottom: 0;">
          <button class="duration-tab ${!isManual ? 'active' : ''}" type="button" data-precall-element="project-picker-mode" ${readonly ? 'disabled' : 'data-precall-control="project-mode" data-mode="picker"'}>下拉勾选</button>
          <button class="duration-tab ${isManual ? 'active' : ''}" type="button" data-precall-element="project-manual-mode" ${readonly ? 'disabled' : 'data-precall-control="project-mode" data-mode="manual"'}>手动输入</button>
        </div>
      </div>
      
      <div data-precall-element="project-picker" style="${isManual ? 'display:none' : ''}">
        <div class="tag-picker project-name-picker ${readonly ? 'readonly' : ''}">
          <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" data-precall-element="project-trigger" type="button" ${readonly ? 'disabled' : 'data-precall-control="project-toggle"'}>${selectedValues.length ? escapeHtml(selectedValues.join('、')) : '请选择大项目名'}</button>
          <div class="tag-picker-panel" data-precall-element="project-options">
            <div class="tag-picker-toolbar"><span data-precall-element="project-count">已选 ${selectedValues.length} / ${strategyPrecallProjectNameOptions.length}</span>${readonly ? '' : '<div class="tag-picker-actions"><button class="tag-picker-action" type="button" data-precall-control="project-select-all">全选</button><button class="tag-picker-action" type="button" data-precall-control="project-clear">清空</button></div>'}</div>
            <div class="tag-picker-list">${strategyPrecallProjectNameOptions.map(item => `<label class="tag-option ${selectedValues.includes(item) ? 'selected' : ''}"><input type="checkbox" name="precallAttributeValue" value="${escapeAttr(item)}" ${selectedValues.includes(item) ? 'checked' : ''} ${disabled} />${escapeHtml(item)}</label>`).join('')}</div>
          </div>
        </div>
      </div>
      
      <div data-precall-element="project-manual" style="${isManual ? '' : 'display:none'}">
        <textarea class="form-input assignment-manual-textarea" data-precall-element="project-manual-input" rows="4" style="margin-top: 0;" placeholder="可一次粘贴多个大项目名，支持换行、逗号、分号或空格分隔" ${disabled}>${escapeHtml(selectedValues.join('\n'))}</textarea>
...
function renderPrecallTimeSlot(slot = {}, index = 0, total = 1, readonly = false) {
  return `<div class="push-time-slot precall-work-slot"><div class="push-time-slot-label">时段${index + 1}</div><input class="form-input time-input precall-work-start" type="time" value="${escapeAttr(slot.start || '')}" ${readonly ? 'disabled' : ''}/><div class="push-time-separator">-</div><input class="form-input time-input precall-work-end" type="time" value="${escapeAttr(slot.end || '')}" ${readonly ? 'disabled' : ''}/>${readonly ? '' : `<button class="push-time-remove" type="button" data-precall-control="time-remove" style="${total <= 1 ? 'visibility:hidden' : ''}">删除</button>`}</div>`;
}

function refreshNextVisitWorkTimeSlots() {
  const container = document.getElementById('nextVisitWorkTimeSlots');
  if (!container) return;
  const rows = [...container.querySelectorAll('.precall-work-slot')];
  rows.forEach((row, index) => {
    const label = row.querySelector('.push-time-slot-label');
...
function addNextVisitWorkTimeSlot() {
  const container = document.getElementById('nextVisitWorkTimeSlots');
  if (!container) return;
...
function removePrecallWorkTimeSlot(btn) {
  const slotRow = btn.closest('.precall-work-slot');
  const container = btn.closest('#nextVisitWorkTimeSlots, #precallWorkTimeSlots');
  if (slotRow) slotRow.remove();
  if (container) {
    if (container.id === 'nextVisitWorkTimeSlots') refreshNextVisitWorkTimeSlots();
    else refreshPrecallWorkTimeSlots();
  }
}
...
function renderPrecallTaskForm(row = {}, readonly = false) {
  ...
  const attributeValueControl = renderPrecallAttributeValueControl(attributeType, attributeValues, attributeInputMode || 'picker', readonly, 'precallTask');
  return `<div class="dispatch-rule-form strategy-configurator-form strategy-configurator-form-precallTask" data-precall-form-key="precallTask">
    <section class="dispatch-form-section"><div class="dispatch-section-title">基础信息</div><div class="dispatch-form-grid">
      ...
      <div class="form-group"><div class="form-label">权重 <span class="required">*</span></div><input class="form-input" id="precallPriority" data-precall-role="integer-input" type="number" min="1" max="255" value="${escapeAttr(row.priority || 255)}" ${disabled} /><div class="series-form-hint">数字越小优先级越高，命中多条规则时优先按权重判断</div></div>
    ...
    <section class="dispatch-form-section"><div class="dispatch-section-title">命中条件</div><div class="dispatch-form-grid">
      <div class="form-group wide"><div class="form-label">命中类型 <span class="required">*</span></div><select class="form-input" id="precallAttributeType" data-precall-role="attribute-type" ${disabled}>${configuratorOptions.attributeType.map(item => `<option value="${escapeAttr(item)}" ${attributeType === item ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select></div>
      <div class="form-group wide" id="precallAttributeValueGroup" data-precall-role="attribute-value-group">${attributeValueControl}</div>
      <div class="form-group wide" id="precallLeadStatusGroup" data-precall-role="lead-status-group" style="${config && attributeType !== '线索状态' ? '' : 'display:none'}"><div class="form-label">线索状态 <span class="optional-push-time-tag">选填</span></div>${renderPrecallCheckboxes('precallLeadStatus', strategyPrecallLeadStatusOptions, leadStatuses, readonly)}<div class="series-form-hint">选择后与命中类型按“且（AND）”组合。</div></div>
    ...
    <section class="dispatch-form-section"><div class="dispatch-section-title">执行动作</div><div class="dispatch-form-grid">
      ...
      <div class="form-group"><div class="form-label">工作时段优先级 <span class="required">*</span></div><input class="form-input" id="precallWorkPriority" data-precall-role="integer-input" type="number" min="1" value="${escapeAttr(row.workTimePriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
      ...
      <div class="form-group"><div class="form-label">非工作时段优先级 <span class="required">*</span></div><input class="form-input" id="precallOffWorkPriority" data-precall-role="integer-input" type="number" min="1" value="${escapeAttr(row.offWorkPriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
      ...
      <div class="form-group wide"><div class="form-label">生效时段 <span class="required">*</span></div><div class="precall-work-time-panel"><div class="push-time-days">${safePushWeekdays.map(day => `<label class="push-time-day"><input type="checkbox" name="precallWorkDay" value="${day}" ${workTime.days?.includes(day) ? 'checked' : ''} ${disabled}/><span>${day}</span></label>`).join('')}</div><div class="push-time-slots"><div id="precallWorkTimeSlots">${slots.map((slot, index) => renderPrecallTimeSlot(slot, index, slots.length, readonly)).join('')}</div>${readonly ? '' : '<button class="push-time-add" type="button" data-precall-control="time-add" data-time-container="precallWorkTimeSlots">新增时段</button>'}</div><div class="series-form-hint">至少选择一天和一个完整时段；结束时间须晚于开始时间，多个时段不可重叠。</div></div></div>
    </div></section>
  </div>`;
}

function renderNextVisitPrecallForm(row = {}, readonly = false) {
  const isAdd = !row || !Object.keys(row).length || !row.id;
  const status = row.systemStatus || '启用';
  const attributeType = row.attributeType || '请选择';
  const config = strategyPrecallAttributeConfigs[attributeType];
  const attributeValues = row.attributeValues || [];
  const leadStatuses = row.leadStatuses || [];
  const carSeries = row.carSeries || (isAdd ? [] : ['全部车系']);
  const contactStatus = Array.isArray(row.contactStatus) ? row.contactStatus : (row.contactStatus ? [row.contactStatus] : []);
  const workTime = row.workTime || (isAdd ? { days: [], slots: [{ start: '', end: '' }] } : { days: ['周一', '周二', '周三', '周四', '周五'], slots: [{ start: '09:00', end: '18:00' }] });
  const slots = workTime.slots?.length ? workTime.slots : [{ start: '', end: '' }];
  const disabled = readonly ? 'disabled' : '';
  const attributeInputMode = attributeType === 'SmartCode' ? row.smartCodeInputMode : attributeType === '大项目名' ? row.projectNameInputMode : 'picker';
  const attributeValueControl = renderPrecallAttributeValueControl(attributeType, attributeValues, attributeInputMode || 'picker', readonly, 'nextVisitPrecall');

  return `<div class="dispatch-rule-form strategy-configurator-form strategy-configurator-form-nextVisitPrecall" data-precall-form-key="nextVisitPrecall">
    <section class="dispatch-form-section"><div class="dispatch-section-title">基础信息</div><div class="dispatch-form-grid">
      <div class="form-group wide"><div class="form-label">配置名称 <span class="required">*</span></div><input class="form-input" id="nextVisitRuleName" value="${escapeAttr(row.ruleName || '')}" placeholder="请输入配置名称" ${disabled} /></div>
      <div class="form-group"><div class="form-label">权重 <span class="required">*</span></div><input class="form-input" id="nextVisitPriority" data-precall-role="integer-input" type="number" min="1" max="255" value="${escapeAttr(row.priority || 255)}" ${disabled} /><div class="series-form-hint">数字越小优先级越高，命中多条规则时优先按权重判断</div></div>
      <div class="form-group"><div class="form-label">状态 <span class="required">*</span></div><select class="form-input" id="nextVisitStatus" ${disabled}><option value="启用" ${status === '启用' ? 'selected' : ''}>启用</option><option value="停用" ${status === '停用' ? 'selected' : ''}>停用</option></select></div>
    </div></section>
    <section class="dispatch-form-section"><div class="dispatch-section-title">命中条件</div><div class="dispatch-form-grid">
      <div class="form-group wide"><div class="form-label">回访结果 <span class="required">*</span></div><input class="form-input" value="下次回访" style="background:#f1f5f9; color:#475569; font-weight:bold; cursor:not-allowed;" readonly disabled /><div class="series-form-hint">此条件参数值固定锁定为“下次回访”，不可编辑修改</div></div>
      <div class="form-group wide"><div class="form-label">接触状态 <span class="required">*</span></div>${renderGenericTagPicker({ idPrefix: 'nextVisitContactStatus', inputName: 'nextVisitContactStatusCheckbox', options: configuratorOptions.contactStatus || [], selected: contactStatus, placeholder: '请选择接触状态', readonly, width: '100%' })}</div>
      <div class="form-group wide"><div class="form-label">命中类型 <span class="required">*</span></div><select class="form-input" id="precallAttributeType" data-precall-role="attribute-type" ${disabled}>${configuratorOptions.attributeType.map(item => `<option value="${escapeAttr(item)}" ${attributeType === item ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select></div>
      <div class="form-group wide" id="precallAttributeValueGroup" data-precall-role="attribute-value-group">${attributeValueControl}</div>
      <div class="form-group wide" id="precallLeadStatusGroup" data-precall-role="lead-status-group" style="${config && attributeType !== '线索状态' ? '' : 'display:none'}"><div class="form-label">线索状态 <span class="optional-push-time-tag">选填</span></div>${renderPrecallCheckboxes('precallLeadStatus', strategyPrecallLeadStatusOptions, leadStatuses, readonly)}<div class="series-form-hint">选择后与命中类型按“且（AND）”组合。</div></div>
      <div class="form-group wide"><div class="form-label">意向车系 <span class="required">*</span></div>${renderPrecallCheckboxes('precallCarSeries', configuratorOptions.carSeries, carSeries, readonly)}</div>
    ...
    <section class="dispatch-form-section"><div class="dispatch-section-title">执行动作</div><div class="dispatch-form-grid">
      <div class="form-group"><div class="form-label">预外呼线路 <span class="required">*</span></div><select class="form-input" id="nextVisitLine" ${disabled}>${configuratorOptions.precallLine.map(item => `<option value="${escapeAttr(item)}" ${(row.precallLine || '请选择') === item ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select></div>
      <div class="form-group"><div class="form-label" style="display:flex; align-items:center; gap:6px; position:relative;">粘性路由优先分配 <span class="required">*</span><button type="button" data-precall-control="sticky-help" aria-label="查看粘性路由说明" aria-expanded="false" style="display:inline-flex; align-items:center; justify-content:center; width:16px; height:16px; padding:0; border:0; border-radius:50%; background:#e5e6eb; color:#4e5969; font-size:11px; font-weight:bold; cursor:pointer; user-select:none;" title="点击查看模式对比">?</button><div data-precall-element="sticky-help-popover" data-precall-control="sticky-help-content" style="display:none; position:absolute; top:24px; left:0; width:320px; background:#ffffff; border:1px solid #e5e6eb; box-shadow:0 4px 16px rgba(0,0,0,0.12); padding:12px; border-radius:6px; z-index:99; font-weight:normal;"><div style="font-weight:700; color:#1f2329; margin-bottom:6px; font-size:12px;">💡 分配模式对比说明：</div><div style="font-size:12px; color:#4e5969; line-height:1.5;">• <strong>优先分配原归属坐席(推荐)：</strong>外呼前预校验原坐席状态，仅当原坐席空闲时拨号并转接，保障1v1专属服务体验与沟通连贯性。<br/>• <strong>直接池化分配：</strong>不绑定原坐席，拨通后转给当前最空闲的任意坐席，追求最高拨打履率。</div></div></div><select class="form-input" id="precallStickyRouting" ${disabled}><option value="" ${!row.enableStickyRouting ? 'selected' : ''}>请选择</option><option value="是" ${row.enableStickyRouting === '是' ? 'selected' : ''}>是 (优先分配原归属坐席)</option><option value="否" ${row.enableStickyRouting === '否' ? 'selected' : ''}>否 (直接池化轮询分配)</option></select></div>
      <div class="form-group"><div class="form-label">超时降级阈值(分钟) <span class="required">*</span></div><input class="form-input" id="precallDegradeTimeout" type="number" min="5" value="${escapeAttr(row.nextVisitTimeoutMinutes || '')}" placeholder="如: 30" ${disabled} /><div class="series-form-hint">原坐席忙碌且超过预设时间，自动降级转给同组AB角/最闲坐席</div></div>
      <div class="form-group"><div class="form-label">工作时段优先级 <span class="required">*</span></div><input class="form-input" id="nextVisitWorkPriority" data-precall-role="integer-input" type="number" min="1" value="${escapeAttr(row.workTimePriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
      <div class="form-group"><div class="form-label">工作时段技能组 <span class="required">*</span></div><select class="form-input" id="nextVisitWorkSkill" ${disabled}>${renderSimpleOptions(configuratorOptions.skillGroup, row.workTimeSkillGroup || '', '请选择')}</select></div>
      <div class="form-group"><div class="form-label">非工作时段优先级 <span class="required">*</span></div><input class="form-input" id="nextVisitOffWorkPriority" data-precall-role="integer-input" type="number" min="1" value="${escapeAttr(row.offWorkPriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
      <div class="form-group"><div class="form-label">非工作时段技能组 <span class="required">*</span></div><select class="form-input" id="nextVisitOffWorkSkill" ${disabled}>${renderSimpleOptions(configuratorOptions.skillGroup, row.offWorkSkillGroup || '', '请选择')}</select></div>
      <div class="form-group wide"><div class="form-label">生效时段 <span class="required">*</span></div><div class="precall-work-time-panel"><div class="push-time-days">${safePushWeekdays.map(day => `<label class="push-time-day"><input type="checkbox" name="precallWorkDay" value="${day}" ${workTime.days?.includes(day) ? 'checked' : ''} ${disabled}/><span>${day}</span></label>`).join('')}</div><div class="push-time-slots"><div id="nextVisitWorkTimeSlots">${slots.map((slot, index) => renderPrecallTimeSlot(slot, index, slots.length, readonly)).join('')}</div>${readonly ? '' : '<button class="push-time-add" type="button" data-precall-control="time-add" data-time-container="nextVisitWorkTimeSlots">新增时段</button>'}</div><div class="series-form-hint">至少选择一天和一个完整时段；结束时间须晚于开始时间，多个时段不可重叠。</div></div></div>
    </div></section>
  </div>`;
}

function renderStrategyConfiguratorForm(key, row, readonly = false) {
  if (key === 'nextVisitPrecall') return renderNextVisitPrecallForm(row, readonly);
...
function renderStrategyConfiguratorFormSections(key, def, row, readonly = false) {
  ...
  return sectionNames.map(sectionName => {
    ...
    if (key === 'manualUnconnected' && sectionName === '命中条件') {
      const currentMode = row.conditionMode || 'contactStatus';
      gridContent += `<div class="form-group wide" style="margin-bottom: 4px;">
        <div class="form-label">命中条件模式 <span class="required">*</span></div>
        <div class="duration-tabs unconnected-condition-tabs" style="max-width: 360px;">
          <button type="button" class="duration-tab ${currentMode === 'contactStatus' ? 'active' : ''}" ${readonly ? 'disabled' : ''} onclick="switchManualUnconnectedConditionMode('contactStatus')">按接触状态</button>
          <button type="button" class="duration-tab ${currentMode === 'precallTask' ? 'active' : ''}" ${readonly ? 'disabled' : ''} onclick="switchManualUnconnectedConditionMode('precallTask')">按预外呼任务</button>
        </div>
      </div>`;
    }
    gridContent += fields.map(field => {
      if (key === 'manualUnconnected' && field.conditionMode) {
        const currentMode = row.conditionMode || 'contactStatus';
        const isVisible = field.conditionMode === currentMode;
        return `<div id="formGroup_${field.field}_wrap" style="${isVisible ? '' : 'display:none'}">${renderStrategyConfiguratorFormField(key, field, row, readonly)}</div>`;
      }
      return renderStrategyConfiguratorFormField(key, field, row, readonly);
    }).join('');
    if (key === 'manualUnconnected' && sectionName === '命中条件') {
      gridContent += `<div id="unconnectedBelongTypeSubContainer" class="form-group wide" style="display:none"></div>`;
    }
    let extraHint = '';
    if (key === 'manualUnconnected' && sectionName === '基础信息') {
      extraHint = `<div class="series-form-hint" style="margin-top: 6px; margin-bottom: 0;">数字越小优先级越高，命中多条规则时优先按权重判断。</div>`;
    }
    return `<section class="dispatch-form-section"><div class="dispatch-section-title">${sectionName}</div><div class="dispatch-form-grid">${gridContent}</div>${extraHint}</section>`;
  }).join('');
}

function switchManualUnconnectedConditionMode(mode) {
  const contactFields = ['contactStatus', 'contactRepeatCount'];
  const precallFields = ['lastContactStatus', 'taskReturnVisitCount'];
  contactFields.forEach(f => {
    const wrap = document.getElementById(`formGroup_${f}_wrap`);
    if (wrap) wrap.style.display = mode === 'contactStatus' ? '' : 'none';
  });
  precallFields.forEach(f => {
    const wrap = document.getElementById(`formGroup_${f}_wrap`);
    if (wrap) wrap.style.display = mode === 'precallTask' ? '' : 'none';
  });
  if (!window.unconnectedConditionModeState) window.unconnectedConditionModeState = {};
  window.unconnectedConditionModeState.mode = mode;
  const tabs = document.querySelectorAll('.dispatch-form-section .duration-tab');
  tabs.forEach((tab, i) => {
    tab.classList.toggle('active', (i === 0 && mode === 'contactStatus') || (i === 1 && mode === 'precallTask'));
  });
}
...
function renderStrategyConfiguratorFormField(key, field, row, readonly = false) {
  ...
  if (field.type === 'dayCount') {
    ...
    return `<div class="${groupClass}" id="formGroup_${field.field}"><div class="form-label">${field.label}${required}</div><div class="day-count-control"><span class="day-count-prefix">最近</span><div class="day-count-input"><input class="form-input" id="${id}_days" type="number" min="0"${integerAttrs} value="${escapeAttr(parsed.days)}" placeholder="0" ${disabled} /><span class="day-count-suffix">天</span></div><span class="day-count-connector">内 ≤</span><div class="day-count-input"><input class="form-input" id="${id}_count" type="number" min="0"${integerAttrs} value="${escapeAttr(parsed.count)}" placeholder="0" ${disabled} /><span class="day-count-suffix">次</span></div></div>${hint}</div>`;
  }
  if (field.type === 'priority') {
    const priorityVal = value ?? field.defaultValue ?? 255;
    const ph = field.placeholder || '请输入权重';
    return `<div class="${groupClass}" id="formGroup_${field.field}"><div class="form-label">${field.label}${required}</div><input class="form-input" id="${id}" type="number" min="1" max="255" value="${escapeAttr(priorityVal)}" placeholder="${escapeAttr(ph)}" ${readonly ? 'readonly' : ''} /></div>`;
  }
  if (field.type === 'countCompare') {
    const parsed = parseCountCompareValue(value);
    const integerAttrs = getNonNegativeIntegerInputAttrs(key, field.field);
    const operators = ['>', '≥', '=', '≤', '<'];
    const operatorLabels = { '>': '大于', '≥': '大于等于', '=': '等于', '≤': '小于等于', '<': '小于' };
    const opHtml = operators.map(op => {
      const selected = parsed.operator === op;
      return `<button type="button" class="count-compare-op ${selected ? 'active' : ''}" ${readonly ? 'disabled' : ''} data-op="${op}" onclick="setCountCompareOperator(this, '${op}')" title="${operatorLabels[op]}">${op}</button>`;
    }).join('');
    return `<div class="${groupClass}" id="formGroup_${field.field}"><div class="form-label">${field.label}${required}</div><div class="count-compare-control"><div class="count-compare-op-group">${opHtml}</div><div class="count-compare-input"><input class="form-input" id="${id}_value" type="number" min="0"${integerAttrs} value="${escapeAttr(parsed.count)}" placeholder="0" ${disabled} /><span class="count-compare-suffix">次</span></div></div>${hint}</div>`;
  }
  if (isReturnVisitOrderConfigurator(key) && field.field === 'intentLevelAction') {
    ...
    if (nurtureResult === '意向线索下发' && options.includes('A')) {
      defaultOption = 'A';
    }
    const selected = options.includes(value) ? value : defaultOption;
    return `<div class="${groupClass}"><div class="form-label">${field.label}${required}</div><select class="form-input" id="${id}" ${locked || readonly ? 'disabled' : ''}>${options.map(item => `<option value="${escapeAttr(item)}" ${selected === item ? 'selected' : ''}>${escapeHtml(formatReturnVisitOrderIntentLevel(item))}</option>`).join('')}</select>${hint}</div>`;
  }
...
function isManualUnconvertedNonNegativeIntegerField(key, fieldName) {
  if (key === 'manualUnconnected') return ['maxOutboundCount', 'pushInterval'].includes(fieldName);
...
function formatCountCompareForDetail(val) {
  if (typeof val === 'object' && val !== null && 'operator' in val) {
    if (val.count === 0) return '不限制';
    return `${val.operator}${val.count}次`;
  }
...
function formatReturnVisitOrderIntentLevel(value = '') {
  return getSharedIntentLevelLabel(value);
}
...
function syncReturnVisitOrderIntentLevelAction(currentIntent = '') {
  ...
  const selected = options.includes(previousValue) ? previousValue : defaultOption;
  
  intentSelect.disabled = locked;
  intentSelect.innerHTML = options.map(item => `<option value="${escapeAttr(item)}" ${selected === item ? 'selected' : ''}>${escapeHtml(formatReturnVisitOrderIntentLevel(item))}</option>`).join('');
}
...
const returnVisitNurtureResultToContactStatusMap = {
  '下次回访': ['正常接通', '无人接听', '企微跟进', '客户拒绝', '忙音/秘书台', '空号', '停机', '关机'],
  '试驾排程下发': ['正常接通', '企微跟进'],
  '试驾线索下发': ['正常接通', '企微跟进'],
  '意向线索下发': ['正常接通', '企微跟进'],
  '无人接听下发': ['无人接听', '客户拒绝', '忙音/秘书台'],
  '无人接下发': ['无人接听', '客户拒绝', '忙音/秘书台'],
  '休眠失联': ['无人接听', '忙音/秘书台', '关机'],
  '休眠未购': ['正常接通', '客户拒绝'],
  '无效结案': ['空号', '停机'],
  '战败': ['正常接通', '客户拒绝']
...
function syncReturnVisitOrderContactStatus(currentContactStatus = '') {
  const nurtureResultEl = document.getElementById('cfgForm_nurtureResult');
  const contactStatusEl = document.getElementById('cfgForm_contactStatus');
  if (!nurtureResultEl || !contactStatusEl) return;

  const nurtureResult = nurtureResultEl.value || '';
  const allowedOptions = returnVisitNurtureResultToContactStatusMap[nurtureResult] || [];
  const previousValue = currentContactStatus || contactStatusEl.value || '';

  if (!nurtureResult) {
    const allOptions = configuratorOptions.contactStatus || [];
    const selected = allOptions.includes(previousValue) ? previousValue : '';
    contactStatusEl.disabled = false;
    contactStatusEl.innerHTML = `<option value="" ${!selected ? 'selected' : ''}>请选择</option>` + allOptions.map(item =>
      `<option value="${escapeAttr(item)}" ${selected === item ? 'selected' : ''}>${escapeHtml(item)}</option>`
    ).join('');
  } else {
    contactStatusEl.disabled = false;
    const selected = allowedOptions.includes(previousValue) ? previousValue : (allowedOptions.length > 0 ? allowedOptions[0] : '');
    contactStatusEl.innerHTML = `<option value="" ${!allowedOptions.includes(selected) ? 'selected' : ''}>请选择</option>` + allowedOptions.map(item =>
      `<option value="${escapeAttr(item)}" ${selected === item ? 'selected' : ''}>${escapeHtml(item)}</option>`
    ).join('');
  }
}

function handleReturnVisitOrderNurtureResultChange(currentReason = '', currentIntent = '', currentTime = '', currentContactStatus = '') {
  ...
  syncReturnVisitOrderContactStatus(currentContactStatus);
}
...
function syncReturnVisitOrderAgentRouting() {
  ...
  if (!agentRoutingEl) return;
  if (strategyConfiguratorEditing.key === 'amapMap') {
  ...
  const lockedValues = ['坐席未接', '线路拒呼', '线路未呼', '超时未接', '线索拒呼', '线索未呼'];
...
function parseCountCompareValue(value) {
  if (typeof value === 'object' && value !== null && 'operator' in value) {
    return { operator: value.operator || '=', count: value.count ?? 0 };
  }
  if (typeof value === 'string') {
    const match = value.match(/^([><=≥≤]+)?(\d+)$/);
    if (match) return { operator: match[1] || '=', count: parseInt(match[2], 10) };
  }
...
function setCountCompareOperator(btn, op) {
  const control = btn.closest('.count-compare-control');
  if (!control) return;
  control.querySelectorAll('.count-compare-op').forEach(function(b) { b.classList.remove('active'); });
  btn.classList.add('active');
  if (!window.countCompareOperatorState) window.countCompareOperatorState = {};
  var inputId = control.querySelector('.count-compare-input input')?.id;
  if (inputId) window.countCompareOperatorState[inputId.replace('_value', '')] = op;
}

function formatCountCompareDisplay(operator, count) {
  if (count === 0) return '不限制';
  return `${operator}${count}次`;
}
...
function getManualUnconnectedMatchSummary(payload) {
  const parts = [];
  if (payload.conditionMode === 'contactStatus') {
    const cs = Array.isArray(payload.contactStatus) ? payload.contactStatus.filter(Boolean).join('、') : (payload.contactStatus || '—');
    parts.push(cs);
    if (payload.carSeries?.length) parts.push(payload.carSeries.join('、'));
    const crc = payload.contactRepeatCount;
    if (typeof crc === 'object' && crc !== null) {
      if (crc.count > 0) parts.push(`重复${crc.operator}${crc.count}次`);
    } else if (crc > 0) {
      parts.push(`重复${crc}次`);
    }
  } else {
    const lcs = Array.isArray(payload.lastContactStatus) ? payload.lastContactStatus.filter(Boolean).join('、') : (payload.lastContactStatus || '—');
    parts.push(lcs);
    if (payload.carSeries?.length) parts.push(payload.carSeries.join('、'));
    const trvc = payload.taskReturnVisitCount;
    if (typeof trvc === 'object' && trvc !== null) {
      if (trvc.count > 0) parts.push(`工单${trvc.operator}${trvc.count}次`);
    } else if (trvc > 0) {
      parts.push(`工单${trvc}次`);
    }
  }
  if (payload.includeIncomplete === '是') parts.push('含留资未满');
  return parts.join(' / ');
}

function collectPrecallSharedFields(formKey, ids, slotContainerId) {
  const root = getPrecallFormRoot(formKey);
  const valueOf = id => getPrecallFormValue(root, id);
  return {
    root,
    valueOf,
    checkedValues: name => getPrecallCheckedValues(root, name),
    ruleName: valueOf(ids.ruleName),
    priority: Number(valueOf(ids.priority)),
    attributeType: valueOf('precallAttributeType'),
    attributeValues: getPrecallCheckedValues(root, 'precallAttributeValue'),
    leadStatuses: getPrecallCheckedValues(root, 'precallLeadStatus'),
    carSeries: getPrecallCheckedValues(root, 'precallCarSeries'),
    precallLine: valueOf(ids.line),
    workTimePriority: Number(valueOf(ids.workPriority)),
    workTimeSkillGroup: valueOf(ids.workSkill),
    offWorkPriority: Number(valueOf(ids.offWorkPriority)),
    offWorkSkillGroup: valueOf(ids.offWorkSkill),
    systemStatus: valueOf(ids.status) || '启用',
    days: getPrecallCheckedValues(root, 'precallWorkDay'),
    slots: Array.from(root?.querySelectorAll(`#${slotContainerId} .precall-work-slot`) || []).map(row => ({
...
function resolvePrecallAttributeValues(shared, formKey) {
  const { root, attributeType } = shared;
  const smartCodeInputMode = attributeType === 'SmartCode' ? getPrecallInputMode(formKey, 'smartCode') : 'picker';
  const projectNameInputMode = attributeType === '大项目名' ? getPrecallInputMode(formKey, 'projectName') : 'picker';
  let attributeValues = shared.attributeValues;
  const manualInput = attributeType === 'SmartCode'
    ? getPrecallPickerParts(root, 'smart').manualInput
    : attributeType === '大项目名'
      ? getPrecallPickerParts(root, 'project').manualInput
      : null;
  if ((attributeType === 'SmartCode' && smartCodeInputMode === 'manual') || (attributeType === '大项目名' && projectNameInputMode === 'manual')) {
    attributeValues = [...new Set((manualInput?.value || '').split(/[\n,;，；\s]+/).map(item => item.trim()).filter(Boolean))];
  }
  return { attributeValues, smartCodeInputMode, projectNameInputMode };
}

function validatePrecallCommonFields(shared, options = {}) {
  const { requireTimeRouting = false } = options;
  if (!shared.ruleName) return '请填写配置名称';
  if (!Number.isInteger(shared.priority) || shared.priority < 1 || shared.priority > 255) return '请填写1-255的权重';
  if (!strategyPrecallAttributeConfigs[shared.attributeType]) return '请选择命中类型';
  if (!shared.carSeries.length) return '请选择意向车系';
  if (!shared.precallLine || shared.precallLine === '请选择') return '请选择预外呼线路';
  if (requireTimeRouting) {
    if (!Number.isInteger(shared.workTimePriority) || shared.workTimePriority < 1) return '请填写有效工作时段优先级';
    if (!shared.workTimeSkillGroup) return '请选择工作时段技能组';
    if (!Number.isInteger(shared.offWorkPriority) || shared.offWorkPriority < 1) return '请填写有效非工作时段优先级';
    if (!shared.offWorkSkillGroup) return '请选择非工作时段技能组';
  }
  return '';
}

function validatePrecallWorkTime(days, slots) {
  if (!days.length) return { message: '请选择生效星期' };
  if (!slots.length || slots.some(slot => !slot.start || !slot.end || slot.end <= slot.start)) return { message: '请填写有效生效时段（结束时间须晚于开始时间）' };
  const orderedSlots = [...slots].sort((a, b) => a.start.localeCompare(b.start));
  if (orderedSlots.some((slot, index) => index > 0 && slot.start < orderedSlots[index - 1].end)) return { message: '生效时段不可重叠' };
  return { orderedSlots };
}

function collectNextVisitPrecallFormValue() {
  const shared = collectPrecallSharedFields('nextVisitPrecall', {
    ruleName: 'nextVisitRuleName', priority: 'nextVisitPriority', line: 'nextVisitLine',
    workPriority: 'nextVisitWorkPriority', workSkill: 'nextVisitWorkSkill',
    offWorkPriority: 'nextVisitOffWorkPriority', offWorkSkill: 'nextVisitOffWorkSkill', status: 'nextVisitStatus'
  }, 'nextVisitWorkTimeSlots');
  const { valueOf, ruleName, priority, attributeType, leadStatuses, carSeries, precallLine, workTimePriority: rawWorkTimePriority, workTimeSkillGroup: rawWorkTimeSkillGroup, offWorkPriority: rawOffWorkPriority, offWorkSkillGroup: rawOffWorkSkillGroup, systemStatus, days } = shared;
  const contactStatus = getGenericTagPickerInputs('nextVisitContactStatus').filter(input => input.checked).map(input => input.value);
  const enableStickyRouting = valueOf('precallStickyRouting');
  const nextVisitTimeoutMinutes = Number(valueOf('precallDegradeTimeout'));
  const workTimePriority = rawWorkTimePriority;
  const workTimeSkillGroup = rawWorkTimeSkillGroup;
  const offWorkPriority = rawOffWorkPriority;
  const offWorkSkillGroup = rawOffWorkSkillGroup;
  const commonError = validatePrecallCommonFields(shared, { requireTimeRouting: true });
  if (commonError) return { ok: false, message: commonError };
  const { attributeValues, smartCodeInputMode, projectNameInputMode } = resolvePrecallAttributeValues(shared, 'nextVisitPrecall');

  if (!attributeValues.length) return { ok: false, message: `请选择或输入${attributeType === 'SmartCode' ? 'SmartCode' : attributeType === '大项目名' ? '大项目名' : attributeType}` };
  if (!contactStatus.length) return { ok: false, message: '请选择接触状态' };
  if (!carSeries.length) return { ok: false, message: '请选择意向车系' };
  if (!precallLine || precallLine === '请选择') return { ok: false, message: '请选择预外呼线路' };
  if (!enableStickyRouting) return { ok: false, message: '请选择粘性路由优先分配' };
  if (!Number.isInteger(nextVisitTimeoutMinutes) || nextVisitTimeoutMinutes < 5) return { ok: false, message: '请填写不小于5分钟的超时降级阈值' };
  const timeValidation = validatePrecallWorkTime(days, shared.slots);
  if (timeValidation.message) return { ok: false, message: timeValidation.message };

  const payload = {
    ruleName, priority, attributeType, attributeValues, leadStatuses,
    smartCodeInputMode, projectNameInputMode,
    carSeries, contactStatus, precallLine,
    enableStickyRouting, nextVisitTimeoutMinutes,
    workTimePriority, workTimeSkillGroup, offWorkPriority, offWorkSkillGroup,
    workTime: { days, slots: timeValidation.orderedSlots },
    systemStatus
  };
...
function collectStrategyConfiguratorFormValue(def) {
  if (strategyConfiguratorEditing.key === 'nextVisitPrecall') return collectNextVisitPrecallFormValue();
  ...
  for (const field of def.form) {
    ...
    if (isReturnVisitOrderConfigurator(strategyConfiguratorEditing.key) && field.field === 'intentLevelAction') {
      ...
      let defaultOption = allowed[0];
      if (nurtureResult === '意向线索下发' && allowed.includes('A')) {
        defaultOption = 'A';
      }
      payload[field.field] = allowed.includes(val) ? val : defaultOption;
      continue;
    }
    ...
    if (strategyConfiguratorEditing.key === 'amapMap' && field.field === 'agentRouting') {
      payload[field.field] = '原始归属人';
      continue;
    }
    ...
    if (field.type === 'multiselect') {
      ...
      const prioVal = document.getElementById(id)?.value.trim() || '255';
      const numPrio = Math.min(255, Math.max(1, parseInt(prioVal, 10) || 255));
      value = numPrio;
    } else if (field.type === 'countCompare') {
      const countVal = document.getElementById(`${id}_value`)?.value.trim() || '0';
      let op = '=';
      const activeOp = document.querySelector(`#formGroup_${field.field} .count-compare-op.active`);
      if (activeOp) op = activeOp.dataset.op || '=';
      else if (window.countCompareOperatorState?.[id]) op = window.countCompareOperatorState[id];
      const numCount = Number(countVal);
      value = { operator: op, count: numCount };
    } else {
      value = el.value.trim();
    }
    ...
    if (field.type === 'countCompare') {
      const numberValue = Number(value.count);
      if (Number.isNaN(numberValue) || numberValue < 0) return { ok: false, message: `请填写有效${field.label}` };
      if (!Number.isInteger(numberValue)) return { ok: false, message: `请填写非负整数${field.label}` };
      if (field.required && numberValue === 0 && value.operator !== '=') {
        // 0次时只能是=（代表不限制）
      }
  ...
  if (strategyConfiguratorEditing.key === 'manualUnconnected') {
    const conditionMode = (window.unconnectedConditionModeState?.mode) || payload.conditionMode || 'contactStatus';
    payload.conditionMode = conditionMode;
    payload.conditionModeLabel = conditionMode === 'contactStatus' ? '按接触状态' : '按预外呼任务';
    if (conditionMode === 'contactStatus') {
      payload.lastContactStatus = [];
      payload.taskReturnVisitCount = 0;
    } else {
      payload.contactStatus = [];
      payload.contactRepeatCount = 0;
    }
    payload.matchConditionSummary = getManualUnconnectedMatchSummary(payload);
  ...
  if (isReturnVisitOrderConfigurator(strategyConfiguratorEditing.key)) {
    const nurtureResult = payload.nurtureResult || '';
    const contactStatus = payload.contactStatus || '';
    const allowed = returnVisitNurtureResultToContactStatusMap[nurtureResult] || [];
    if (nurtureResult && allowed.length > 0 && !allowed.includes(contactStatus)) {
      return { ok: false, message: `接触状态「${contactStatus}」不在回访结果「${nurtureResult}」的联动允许范围内` };
    }
  }
...
function collectPrecallTaskFormValue() {
  const shared = collectPrecallSharedFields('precallTask', {
    ruleName: 'precallRuleName', priority: 'precallPriority', line: 'precallLine',
    workPriority: 'precallWorkPriority', workSkill: 'precallWorkSkill',
    offWorkPriority: 'precallOffWorkPriority', offWorkSkill: 'precallOffWorkSkill', status: 'precallStatus'
  }, 'precallWorkTimeSlots');
  const { ruleName, priority, attributeType, leadStatuses, carSeries, precallLine, workTimePriority, workTimeSkillGroup, offWorkPriority, offWorkSkillGroup, systemStatus, days, slots } = shared;
  const commonError = validatePrecallCommonFields(shared, { requireTimeRouting: true });
  if (commonError) return { ok: false, message: commonError };
  const { attributeValues, smartCodeInputMode, projectNameInputMode } = resolvePrecallAttributeValues(shared, 'precallTask');
  if (!attributeValues.length) {
    return { ok: false, message: `请选择或输入${strategyPrecallAttributeConfigs[attributeType].label}` };
  }
  const timeValidation = validatePrecallWorkTime(days, slots);
  if (timeValidation.message) return { ok: false, message: timeValidation.message };
  const orderedSlots = timeValidation.orderedSlots;
  const payload = {
    ruleName, priority, attributeType, attributeValues, smartCodeInputMode, projectNameInputMode,
    leadStatuses: (attributeType === '线索状态' || isCallStatusType) ? [] : leadStatuses,
    carSeries, precallLine,
    workTime: { days, slots: orderedSlots }, workTimePriority, workTimeSkillGroup, offWorkPriority, offWorkSkillGroup,
    systemStatus
  };
  if (isCallStatusType) payload.callStatusRules = callStatusRules;
  return { ok: true, payload };
}
...
function saveStrategyConfiguratorRow() {
  ...
  const finalPayload = { ...payload, updatedAt: now };
  if (id) {
    const index = strategyConfiguratorData[key].findIndex(row => row.id === id);
    if (index > -1) strategyConfiguratorData[key][index] = { ...strategyConfiguratorData[key][index], ...finalPayload };
    showToast('编辑成功', true);
  } else {
    ...
    strategyConfiguratorData[key].push({ id: nextId, createdAt: now, systemStatus: key === 'precallTask' ? '启用' : undefined, ...finalPayload });
    showToast('新增成功', true);
  }
  closeModal('leadDispatchRuleModal');
  renderStrategyConfiguratorPage(key);
...
function openStrategyConfiguratorQuickEdit(key, id, field) {
  ...
  document.getElementById('leadDispatchRuleModalFooter').innerHTML = `<button class="btn-cancel" type="button" onclick="closeModal('leadDispatchRuleModal')">取消</button><button class="btn-save" type="button" onclick="saveStrategyConfiguratorQuickEdit()">保存</button>`;
  document.getElementById('leadDispatchRuleModal').classList.add('show');
}
...
function openStrategyConfiguratorImport(key) {
  ...
  if (['excellentProject', 'excellentChannel', 'excellentSc'].includes(key)) {
    openExcellentConfigImportWizard(key);
    return;
  }
  const def = getStrategyConfiguratorDef(key);
...
function formatExcellentMultiSelectSummary(selectedValues = [], placeholder = '请选择') {
  const selected = Array.isArray(selectedValues) ? selectedValues : [];
  return selected.length ? `${selected.slice(0, 3).join('、')}${selected.length > 3 ? ` 等${selected.length}项` : ''}` : placeholder;
}
...
function renderExcellentScForm(row = {}, readonly = false) {
  ...
  const disabled = readonly ? 'disabled' : '';
  const smartCodeSummary = formatExcellentMultiSelectSummary(selectedSmartCodes, '请选择SmartCode');
      ...
      <section class="dispatch-form-section">
        ...
        <div class="dispatch-form-grid">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
              ...
              <div id="excellentScSmartCodePickerInputPanel" style="${smartCodeInputMode === 'manual' ? 'display:none' : ''}">
                <div class="tag-picker project-name-picker">
                  <button class="tag-picker-trigger ${selectedSmartCodes.length ? '' : 'placeholder'}" id="excellentScSmartCodeTrigger" type="button" ${disabled} onclick="toggleExcellentScSmartCodePicker()"><span data-picker-summary>${escapeHtml(smartCodeSummary)}</span></button>
          ...
          <div class="form-group wide">
            ...
            ${renderGenericTagPicker({ idPrefix: 'excellentScCarSeries', inputName: 'excellentScCarSeriesCheckbox', options: configuratorOptions.carSeries || [], selected: selectedCarSeries, placeholder: '请选择意向车系', readonly, width: '100%' })}
...
function updateExcellentScSmartCodeSelectedCount() {
  ...
  if (trigger) {
    const summary = trigger.querySelector('[data-picker-summary]') || trigger;
    if (selected.length) {
      summary.textContent = formatExcellentMultiSelectSummary(selected);
      trigger.classList.remove('placeholder');
    } else {
      summary.textContent = '请选择SmartCode';
...
function renderExcellentProjectForm(row = {}, readonly = false) {
  ...
  const inputMode = row.projectNameInputMode === 'manual' ? 'manual' : 'picker';
  const disabled = readonly ? 'disabled' : '';
  const projectSummary = formatExcellentMultiSelectSummary(selectedValues, '请选择大项目名');
    ...
    <div class="dispatch-rule-form strategy-configurator-form strategy-configurator-form-excellentProject">
      ...
      <section class="dispatch-form-section">
        ...
        <div class="dispatch-form-grid">
          <div class="form-group wide">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
              ...
              <div id="excellentProjectPickerInputPanel" style="${inputMode === 'manual' ? 'display:none' : ''}">
                <div class="tag-picker project-name-picker">
                  <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" id="excellentProjectTrigger" type="button" ${disabled} onclick="toggleExcellentProjectPicker()"><span data-picker-summary>${escapeHtml(projectSummary)}</span></button>
                  <div class="tag-picker-panel" id="excellentProjectPanel">
                    <div class="tag-picker-toolbar">
                      <span id="excellentProjectSelectedCount">已选 ${selectedValues.length} / ${strategyPrecallProjectNameOptions.length}</span>
                      ${readonly ? '' : `
                        <div class="tag-picker-actions">
                    ...
                    <div class="tag-picker-list" id="excellentProjectList">
                      ${strategyPrecallProjectNameOptions.map(item => `
          ...
          <div class="form-group wide">
            <div class="form-label">意向车系</div>
            ${renderGenericTagPicker({ idPrefix: 'excellentProjectCarSeries', inputName: 'excellentProjectCarSeriesCheckbox', options: configuratorOptions.carSeries || [], selected: selectedCarSeries, placeholder: '请选择意向车系', readonly, width: '100%' })}
            <div class="series-form-hint">选择后白名单将与指定意向车系组合判断；不选择表示不限意向车系</div>
          </div>
        </div>
      </section>
...
function updateExcellentProjectSelectedCount() {
  ...
  if (countSpan) countSpan.textContent = `已选 ${selected.length} / ${strategyPrecallProjectNameOptions.length}`;
...
function renderExcellentChannelForm(row = {}, readonly = false) {
  ...
  const disabled = readonly ? 'disabled' : '';
  const channelSummary = formatExcellentMultiSelectSummary(selectedValues, '请选择渠道编码');
    ...
    <div class="dispatch-rule-form strategy-configurator-form strategy-configurator-form-excellentChannel">
      ...
      <section class="dispatch-form-section">
        ...
        <div class="dispatch-form-grid">
          <div class="form-group wide">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
              ...
              <div id="excellentChannelPickerInputPanel" style="${inputMode === 'manual' ? 'display:none' : ''}">
                <div class="tag-picker project-name-picker">
                  <button class="tag-picker-trigger ${selectedValues.length ? '' : 'placeholder'}" id="excellentChannelTrigger" type="button" ${disabled} onclick="toggleExcellentChannelPicker()"><span data-picker-summary>${escapeHtml(channelSummary)}</span></button>
          ...
          <div class="form-group wide">
            <div class="form-label">意向车系</div>
            ${renderGenericTagPicker({ idPrefix: 'excellentChannelCarSeries', inputName: 'excellentChannelCarSeriesCheckbox', options: configuratorOptions.carSeries || [], selected: selectedCarSeries, placeholder: '请选择意向车系', readonly, width: '100%' })}
            <div class="series-form-hint">选择后白名单将与指定意向车系组合判断；不选择表示不限意向车系</div>
          </div>
        </div>
      </section>
...
function renderGenericTagPicker({ idPrefix, inputName, options = [], selected = [], placeholder = '请选择', onchange = '', readonly = false, width = '100%', isSearch = false }) {
  ...
  const selectedSummary = safeSelected.length
    ? `${safeSelected.slice(0, 3).join('、')}${safeSelected.length > 3 ? ` 等${safeSelected.length}项` : ''}`
    : placeholder;
  ...
  return `
    <div class="tag-picker dispatch-tag-picker ${readonly ? 'readonly' : ''}" style="display: inline-block; vertical-align: middle; width: ${width};">
      <button class="tag-picker-trigger ${safeSelected.length ? '' : 'placeholder'}" id="${idPrefix}Trigger" data-placeholder="${escapeAttr(placeholder)}" type="button" ${triggerStyle} ${onclickAttr}>${escapeHtml(selectedSummary)}</button>
      <div class="tag-picker-panel" id="${idPrefix}Panel">
        <div class="tag-picker-toolbar">
          <span id="${idPrefix}Count">已选 ${safeSelected.length} / ${options.length}</span>
          ${readonly ? '' : `
          <div class="tag-picker-actions">
...
function updateGenericTagPickerSummary(idPrefix) {
  if (typeof updatePickerSummaryFromInputs === 'function') {
    ...
    const checkboxes = document.querySelectorAll(`#${idPrefix}Panel input[type="checkbox"]:checked`);
    const total = document.querySelectorAll(`#${idPrefix}Panel input[type="checkbox"]`).length;
    const placeholderText = (trigger && typeof trigger.getAttribute === 'function') ? trigger.getAttribute('data-placeholder') : '请选择';
    if (countEl) countEl.textContent = `已选 ${checkboxes.length} / ${total}`;
    if (trigger) trigger.textContent = checkboxes.length ? `已选 ${checkboxes.length} 项` : (placeholderText || '请选择');
  }
}

function runGenericTagPickerChange(onchange) {
  if (!onchange) return;
  try {
    new Function(onchange)();
  } catch (error) {
    showToast('选项更新失败，请重试', false);
  }
}
...
function toggleGenericTagOption(idPrefix, input, onchange) {
  togglePickerOption(input, () => {
    updateGenericTagPickerSummary(idPrefix);
    runGenericTagPickerChange(onchange);
  });
}
...
function toggleUnconnectedSelectDropdown(id) {
...
// 动态策略配置页统一通过声明式事件桥打开弹窗，避免内联 onclick
// 在原型运行环境中失效，导致“新增配置”点击无响应。
registerUiActionCallback('strategy-configurator-open-modal', (key, mode, id = null) => {
  openStrategyConfiguratorModal(key, mode, id);
});

document.addEventListener('click', function(e) {
  document.querySelectorAll('.unconnected-select-dropdown').forEach(dropdown => {
    if (!dropdown.contains(e.target) && !e.target.closest('.unconnected-select-toggle')) {
      dropdown.style.display = 'none';
      const toggle = dropdown.parentElement.querySelector('.unconnected-select-toggle');
