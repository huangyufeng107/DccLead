...
let strategyConfiguratorData = {
  ...
  nextVisitPrecall: [
    { sortOrder: 1, id: 1012, ruleName: 'N7/N6预约跟进预外呼', priority: 1, contactStatus: '无人接听', carSeries: ['N7', 'N6'], enableNextVisit: '启用', enableStickyRouting: '是', nextVisitTimeoutMinutes: 30, precallLine: '虚拟号-大众', systemStatus: '启用', createdAt: '2026-08-24 10:00:00 操作人：管理员', updatedAt: '2026-08-24 10:00:00 操作人：管理员' },
    { sortOrder: 2, id: 1013, ruleName: '全车系超时降级回访策略', priority: 2, contactStatus: '正常接通', carSeries: ['全部车系'], enableNextVisit: '启用', enableStickyRouting: '是', nextVisitTimeoutMinutes: 60, precallLine: '虚拟号-厚扑', systemStatus: '启用', createdAt: '2026-08-24 10:00:00 操作人：管理员', updatedAt: '2026-08-24 10:00:00 操作人：管理员' }
  ],
  excellentProject: [
...
const strategyConfiguratorDefs = {
  ...
  nextVisitPrecall: {
    ...
    columns: [
      ...
      { label: '下次回访预外呼', field: 'enableNextVisit', width: 140 },
...
function renderPrecallCheckboxes(name, options, selected, readonly = false) {
  ...
  return `<div class="series-select-panel"><div class="series-series-grid">${options.map(item => {
    ...
    return `<label class="series-series-option" ${opacityStyle}><input type="checkbox" name="${name}" value="${escapeAttr(item)}" ${selected.includes(item) ? 'checked' : ''} ${isDisabled ? 'disabled' : ''} onchange="handlePrecallCarSeriesChange(this)"/><span>${escapeHtml(item)}</span></label>`;
  }).join('')}</div></div>`;
}
...
function handlePrecallCarSeriesChange(checkbox) {
  ...
  const inputs = Array.from(document.querySelectorAll('input[name="precallCarSeries"]'));
...
function removeNextVisitWorkTimeSlot(button) {
  removePrecallWorkTimeSlot(button);
}

function refreshNextVisitWorkTimeSlots() {
  const rows = [...document.querySelectorAll('#nextVisitWorkTimeSlots .precall-work-slot')];
...
function togglePrecallStickyHelp(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  }
  const popover = document.getElementById('precallStickyHelpPopover');
  if (popover) {
...
document.addEventListener('click', (e) => {
  ...
  if (popover && popover.style.display === 'block') {
    if (!popover.contains(e.target) && !e.target.closest('[onclick*="togglePrecallStickyHelp"]')) {
...
function syncPrecallAttributeFields() {
  ...
  if (valueGroup) valueGroup.innerHTML = renderPrecallAttributeValueControl(type, [], 'picker', false);
...
function removePrecallWorkTimeSlot(button) {
  const container = button.closest('#nextVisitWorkTimeSlots, #precallWorkTimeSlots');
  if (container) {
    const slots = container.querySelectorAll('.precall-work-slot');
    if (slots.length <= 1) {
      if (typeof showToast === 'function') showToast('至少保留一个生效时段', false);
      return;
    }
  }
...
function renderPrecallTaskDetail(row) {
  const attributeDetail = [[row.attributeType || '类型值', (row.attributeValues || []).join('、')], ['线索状态（且）', row.attributeType !== '线索状态' && (row.leadStatuses || []).length ? row.leadStatuses.join('、') : '不限']];
  const nextVisitSummary = row.enableNextVisit === '停用' ? '停用 (坐席主动外拨)' : `启用 (粘性路由: ${row.enableStickyRouting || '是'}, 超时降级: ${row.nextVisitTimeoutMinutes || 30}分钟)`;
  const sections = [
    { title: '基础信息', fields: [['配置名称', row.ruleName], ['权重', row.priority ? `P${row.priority}` : '—'], ['状态', row.systemStatus]] },
    { title: '命中条件', fields: [['命中类型', row.attributeType], ...attributeDetail, ['意向车系', (row.carSeries || []).join('、') || '不限']] },
    { title: '执行动作', fields: [['预外呼线路', row.precallLine], ['下次回访预测策略', nextVisitSummary], ['生效时段', getPrecallWorkTimeSummary(row.workTime)], ['工作时段优先级', row.workTimePriority], ['工作时段技能组', row.workTimeSkillGroup], ['非工作时段优先级', row.offWorkPriority], ['非工作时段技能组', row.offWorkSkillGroup]] }
  ];
  return renderRuleDetailSections(sections);
}
...
function renderNextVisitPrecallDetail(row) {
  const nextVisitSummary = row.enableNextVisit === '停用' ? '停用 (坐席主动外拨)' : `启用 (粘性路由: ${row.enableStickyRouting || '是'}, 超时降级: ${row.nextVisitTimeoutMinutes || 30}分钟)`;
  const contactStatusSummary = Array.isArray(row.contactStatus) ? (row.contactStatus.length ? row.contactStatus.join('、') : '—') : (row.contactStatus || '—');
  const sections = [
    { title: '基础信息', fields: [['配置名称', row.ruleName], ['权重', row.priority ? `P${row.priority}` : '—'], ['状态', row.systemStatus]] },
    { title: '命中条件', fields: [['意向车系', (row.carSeries || []).join('、') || '全部车系'], ['接触状态', contactStatusSummary]] },
    { title: '执行动作', fields: [['预外呼线路', row.precallLine || '虚拟号-大众'], ['下次回访预测策略', nextVisitSummary], ['生效时段', getPrecallWorkTimeSummary(row.workTime)], ['工作时段优先级', row.workTimePriority || 1], ['工作时段技能组', row.workTimeSkillGroup || '技能组A'], ['非工作时段优先级', row.offWorkPriority || 2], ['非工作时段技能组', row.offWorkSkillGroup || '技能组B']] }
  ];
  return renderRuleDetailSections(sections);
}
...
function renderNextVisitPrecallForm(row = {}, readonly = false) {
  ...
  return `<div class="dispatch-rule-form strategy-configurator-form strategy-configurator-form-nextVisitPrecall">
    <section class="dispatch-form-section"><div class="dispatch-section-title">基础信息</div><div class="dispatch-form-grid">
      ...
      <div class="form-group"><div class="form-label">权重 <span class="required">*</span></div><input class="form-input" id="nextVisitPriority" type="number" min="1" max="255" oninput="this.value = this.value.replace(/[^0-9]/g, '')" value="${escapeAttr(row.priority || 255)}" ${disabled} /><div class="series-form-hint">数字越小优先级越高，命中多条规则时优先按权重判断</div></div>
    ...
    <section class="dispatch-form-section"><div class="dispatch-section-title">执行动作</div><div class="dispatch-form-grid">
      ...
      <div class="form-group"><div class="form-label">下次回访预外呼 <span class="required">*</span></div><select class="form-input" id="precallEnableNextVisit" onchange="togglePrecallNextVisitFields()" ${disabled}><option value="" ${!row.enableNextVisit ? 'selected' : ''}>请选择</option><option value="启用" ${row.enableNextVisit === '启用' ? 'selected' : ''}>启用 (系统自动重推)</option><option value="停用" ${row.enableNextVisit === '停用' ? 'selected' : ''}>停用 (坐席主动拨打)</option></select></div>
      <div class="form-group" id="precallStickyRoutingGroup" style="${row.enableNextVisit === '停用' ? 'display:none;' : ''}"><div class="form-label" style="display:flex; align-items:center; gap:6px; position:relative;">粘性路由优先分配 <span class="required">*</span><span style="display:inline-flex; align-items:center; justify-content:center; width:16px; height:16px; border-radius:50%; background:#e5e6eb; color:#4e5969; font-size:11px; font-weight:bold; cursor:pointer; user-select:none;" title="点击查看模式对比" onclick="togglePrecallStickyHelp(event)">?</span><div id="precallStickyHelpPopover" style="display:none; position:absolute; top:24px; left:0; width:320px; background:#ffffff; border:1px solid #e5e6eb; box-shadow:0 4px 16px rgba(0,0,0,0.12); padding:12px; border-radius:6px; z-index:99; font-weight:normal;" onclick="event.stopPropagation()"><div style="font-weight:700; color:#1f2329; margin-bottom:6px; font-size:12px;">💡 分配模式对比说明：</div><div style="font-size:12px; color:#4e5969; line-height:1.5;">• <strong>优先分配原归属坐席(推荐)：</strong>外呼前预校验原坐席状态，仅当原坐席空闲时拨号并转接，保障1v1专属服务体验与沟通连贯性。<br/>• <strong>直接池化分配：</strong>不绑定原坐席，拨通后转给当前最空闲的任意坐席，追求最高拨打履率。</div></div></div><select class="form-input" id="precallStickyRouting" ${disabled}><option value="" ${!row.enableStickyRouting ? 'selected' : ''}>请选择</option><option value="是" ${row.enableStickyRouting === '是' ? 'selected' : ''}>是 (优先分配原归属坐席)</option><option value="否" ${row.enableStickyRouting === '否' ? 'selected' : ''}>否 (直接池化轮询分配)</option></select></div>
      <div class="form-group" id="precallDegradeTimeoutGroup" style="${row.enableNextVisit === '停用' ? 'display:none;' : ''}"><div class="form-label">超时降级阈值(分钟) <span class="required">*</span></div><input class="form-input" id="precallDegradeTimeout" type="number" min="5" value="${escapeAttr(row.nextVisitTimeoutMinutes || '')}" placeholder="如: 30" ${disabled} /><div class="series-form-hint">原坐席忙碌且超过预设时间，自动降级转给同组AB角/最闲坐席</div></div>
      <div class="form-group"><div class="form-label">工作时段优先级 <span class="required">*</span></div><input class="form-input" id="nextVisitWorkPriority" type="number" min="1" oninput="this.value = this.value.replace(/[^0-9]/g, '')" value="${escapeAttr(row.workTimePriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
      ...
      <div class="form-group"><div class="form-label">非工作时段优先级 <span class="required">*</span></div><input class="form-input" id="nextVisitOffWorkPriority" type="number" min="1" oninput="this.value = this.value.replace(/[^0-9]/g, '')" value="${escapeAttr(row.offWorkPriority || '')}" placeholder="请输入优先组" ${disabled} /></div>
...
function collectNextVisitPrecallFormValue() {
  ...
  const ruleName = valueOf('nextVisitRuleName');
  const priority = Number(valueOf('nextVisitPriority'));
  const carSeries = checkedValues('precallCarSeries');
  const contactStatus = getGenericTagPickerInputs('nextVisitContactStatus').filter(input => input.checked).map(input => input.value);
  const precallLine = valueOf('nextVisitLine');
  const enableNextVisit = valueOf('precallEnableNextVisit') || '启用';
  const enableStickyRouting = valueOf('precallStickyRouting') || '是';
  const nextVisitTimeoutMinutes = Number(valueOf('precallDegradeTimeout')) || 30;
  const workTimePriority = Number(valueOf('nextVisitWorkPriority')) || 1;
  const workTimeSkillGroup = valueOf('nextVisitWorkSkill') || '技能组A';
  const offWorkPriority = Number(valueOf('nextVisitOffWorkPriority')) || 2;
  const offWorkSkillGroup = valueOf('nextVisitOffWorkSkill') || '技能组B';
  const systemStatus = valueOf('nextVisitStatus') || '启用';

  const days = checkedValues('precallWorkDay');
  const slotRows = document.querySelectorAll('#nextVisitWorkTimeSlots .precall-work-slot');
  ...
  if (!enableNextVisit) return { ok: false, message: '请选择下次回访预外呼' };
  if (enableNextVisit === '启用') {
    if (!enableStickyRouting) return { ok: false, message: '请选择粘性路由优先分配' };
    if (!nextVisitTimeoutMinutes) return { ok: false, message: '请填写超时降级阈值(分钟)' };
  }

  const payload = {
    ruleName, priority, carSeries, contactStatus, precallLine,
    enableNextVisit, enableStickyRouting, nextVisitTimeoutMinutes,
...
function collectPrecallTaskFormValue() {
  ...
  if (!attributeValues.length) {
    return { ok: false, message: `请选择或输入${precallAttributeConfigs[attributeType].label}` };
  }
  if (!precallLine || precallLine === '请选择') return { ok: false, message: '请选择预外呼线路' };
  ...
  const payload = {
    ...
    leadStatuses: (attributeType === '线索状态' || isCallStatusType) ? [] : checkedValues('precallLeadStatus'),
    carSeries: checkedValues('precallCarSeries'), precallLine,
    enableNextVisit: valueOf('precallEnableNextVisit') || '启用',
    enableStickyRouting: valueOf('precallStickyRouting') || '是',
    nextVisitTimeoutMinutes: Number(valueOf('precallDegradeTimeout')) || 30,
