...
const configuratorOptions = {
  ...
  attributeType: ['请选择', '渠道编码', 'SmartCode', '大项目名', '线索状态', '通话状态'],
...
function renderStrategyConfiguratorSearch(key) {
  ...
  if (['manualUnconnected', 'manualUnconverted', 'precallTask', 'excellentProject', 'excellentChannel', 'excellentSc', 'returnVisitOrder', 'amapMap', 'followCount'].includes(key)) return renderFlatStrategyConfiguratorFilterRow(key, def);
...
function getPrecallCallStatusSummary(rules = []) {
  const list = Array.isArray(rules) ? rules : [];
  if (!list.length) return '—';
  return list.map(r => {
    const durText = (r.status === '坐席接听' && r.duration && Number(r.duration) > 0) ? `≤${r.duration}秒, ` : '';
    return `${r.status}(${durText}${r.redialCount}次/${r.redialInterval}分钟)`;
  }).join('、');
}

function getPrecallMatchSummary(row = {}) {
  if (row.attributeType === '通话状态') {
    return `通话状态：${getPrecallCallStatusSummary(row.callStatusRules)}`;
  }
...
function renderPrecallCallStatusRow(row = {}, index = 0, readonly = false) {
  const statusOptions = (precallAttributeConfigs['通话状态']?.options) || configuratorOptions.precallCallStatusOptions || [];
  const statusVal = row.status || '';
  const countVal = row.redialCount != null ? row.redialCount : '';
  const intervalVal = row.redialInterval != null ? row.redialInterval : '';
  const durationVal = row.duration != null ? row.duration : '';
  const showDuration = statusVal === '坐席接听';
  const disabled = readonly ? 'disabled' : '';
  return `<tr class="precall-call-status-row" data-call-status-row="${index}">
    <td class="col-status">
      <select class="precall-call-status-select" name="precallCallStatusType" onchange="updatePrecallCallStatusDurationVisibility(this)" ${disabled}>
        <option value="">请选择</option>
        ${statusOptions.map(opt => `<option value="${escapeAttr(opt)}" ${statusVal === opt ? 'selected' : ''}>${escapeHtml(opt)}</option>`).join('')}
      </select>
    </td>
    <td class="col-duration">
      ${showDuration ? `<div class="precall-duration-input-wrap">
        <span class="precall-duration-prefix">≤</span>
        <input type="number" min="1" class="precall-call-status-duration" name="precallCallStatusDuration" value="${escapeAttr(durationVal)}" placeholder="秒数" oninput="this.value = this.value.replace(/[^0-9]/g, '')" ${disabled} />
        <span class="precall-duration-suffix">秒</span>
      </div>` : `<span class="precall-duration-empty">—</span>`}
    </td>
    <td class="col-count">
      <input type="number" min="0" class="precall-call-status-count" name="precallCallStatusCount" value="${escapeAttr(countVal)}" placeholder="请输入" oninput="this.value = this.value.replace(/[^0-9]/g, '')" ${disabled} />
    </td>
    <td class="col-interval">
      <input type="number" min="0" class="precall-call-status-interval" name="precallCallStatusInterval" value="${escapeAttr(intervalVal)}" placeholder="请输入" oninput="this.value = this.value.replace(/[^0-9]/g, '')" ${disabled} />
    </td>
    <td class="col-action">
      ${readonly ? '' : `<button type="button" class="precall-call-status-remove" title="删除" onclick="removePrecallCallStatusRow(this)">删除</button>`}
    </td>
  </tr>`;
}

function renderPrecallCallStatusPanel(callStatusRules = [], readonly = false) {
  const rules = Array.isArray(callStatusRules) && callStatusRules.length ? callStatusRules : [{ status: '', redialCount: '', redialInterval: '', duration: '' }];
  return `<div class="form-label">通话状态<span class="required">*</span><span class="form-label-desc">（重推机制：超时/异常自动补推）</span></div>
    <div class="precall-call-status-panel">
      <table class="precall-call-status-table">
        <thead>
          <tr>
            <th class="col-status">状态类型</th>
            <th class="col-duration">通话时长(秒)</th>
            <th class="col-count">重推次数</th>
            <th class="col-interval">重推间隔</th>
            <th class="col-action">操作</th>
          </tr>
        </thead>
        <tbody id="precallCallStatusRows">
          ${rules.map((row, i) => renderPrecallCallStatusRow(row, i, readonly)).join('')}
        </tbody>
      </table>
      ${readonly ? '' : `<div class="precall-call-status-add-row">
        <button type="button" class="precall-call-status-add-btn" onclick="addPrecallCallStatusRow()">+ 添加通话状态</button>
      </div>`}
    </div>
    <div class="precall-call-status-hint">重推次数（0为不重推，单位：次）；重推间隔（单位：分钟）；"坐席接听"时可设通话时长 ≤ N 秒为额外命中条件</div>`;
}

function addPrecallCallStatusRow() {
  const tbody = document.getElementById('precallCallStatusRows');
  if (!tbody) return;
  const index = tbody.querySelectorAll('.precall-call-status-row').length;
  tbody.insertAdjacentHTML('beforeend', renderPrecallCallStatusRow({ status: '', redialCount: '', redialInterval: '', duration: '' }, index, false));
}

function updatePrecallCallStatusDurationVisibility(selectEl) {
  const row = selectEl.closest('.precall-call-status-row');
  if (!row) return;
  const durationCell = row.querySelector('.col-duration');
  if (!durationCell) return;
  if (selectEl.value === '坐席接听') {
    durationCell.innerHTML = `<div class="precall-duration-input-wrap">
      <span class="precall-duration-prefix">≤</span>
      <input type="number" min="1" class="precall-call-status-duration" name="precallCallStatusDuration" value="" placeholder="秒数" oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
      <span class="precall-duration-suffix">秒</span>
    </div>`;
  } else {
    durationCell.innerHTML = '<span class="precall-duration-empty">—</span>';
  }
}

function removePrecallCallStatusRow(btn) {
  const row = btn.closest('.precall-call-status-row');
  const tbody = row?.parentElement;
  if (!tbody) return;
  if (tbody.querySelectorAll('.precall-call-status-row').length <= 1) {
    showToast('至少保留一条通话状态配置', false);
    return;
  }
  row.remove();
}

function renderPrecallAttributeValueControl(attributeType, selectedValues = [], smartCodeInputMode = 'picker', readonly = false, extraState = {}) {
  ...
  if (config.isCallStatus) return renderPrecallCallStatusPanel(extraState.callStatusRules || [], readonly);
...
function renderPrecallTaskForm(row = {}, readonly = false) {
  ...
  const disabled = readonly ? 'disabled' : '';
  const isCallStatusType = attributeType === '通话状态';
  const callStatusRules = row.callStatusRules || (isCallStatusType ? [{ status: '', redialCount: '', redialInterval: '' }] : []);
  ...
  const attributeValueControl = renderPrecallAttributeValueControl(attributeType, attributeValues, attributeInputMode || 'picker', readonly, { callStatusRules });
...
function syncPrecallAttributeFields() {
  ...
  const isCallStatusType = type === '通话状态';
  ...
  const extraState = isCallStatusType
    ? { callStatusRules: [{ status: '', redialCount: '', redialInterval: '' }] }
    : {};
  if (valueGroup) valueGroup.innerHTML = renderPrecallAttributeValueControl(type, [], 'picker', false, extraState);
  if (statusGroup) statusGroup.style.display = config && type !== '线索状态' && !isCallStatusType ? '' : 'none';
}
...
function renderPrecallTaskDetail(row) {
  const isCallStatusType = row.attributeType === '通话状态';
  const callStatusDetail = isCallStatusType
    ? [['通话状态重推策略', getPrecallCallStatusSummary(row.callStatusRules)]]
    : [[row.attributeType || '类型值', (row.attributeValues || []).join('、')], ['线索状态（且）', row.attributeType !== '线索状态' && (row.leadStatuses || []).length ? row.leadStatuses.join('、') : '不限']];
  const sections = [
    { title: '基础信息', fields: [['配置名称', row.ruleName], ['权重', row.priority ? `P${row.priority}` : '—'], ['状态', row.systemStatus]] },
    { title: '命中条件', fields: [['命中类型', row.attributeType], ...callStatusDetail, ['意向车系', (row.carSeries || []).join('、') || '不限']] },
...
function collectPrecallTaskFormValue() {
  ...
  const isCallStatusType = attributeType === '通话状态';
  if (isCallStatusType) {
    const rows = Array.from(document.querySelectorAll('#precallCallStatusRows .precall-call-status-row'));
    const usedStatuses = new Set();
    for (let i = 0; i < rows.length; i += 1) {
      const r = rows[i];
      const status = r.querySelector('.precall-call-status-select')?.value || '';
      const countRaw = (r.querySelector('.precall-call-status-count')?.value || '').trim();
      const intervalRaw = (r.querySelector('.precall-call-status-interval')?.value || '').trim();
      const durationRaw = (r.querySelector('.precall-call-status-duration')?.value || '').trim();
      if (!status && !countRaw && !intervalRaw && !durationRaw) continue;
      if (!status) return { ok: false, message: `第 ${i + 1} 行请选择通话状态类型` };
      if (usedStatuses.has(status)) return { ok: false, message: `通话状态「${status}」重复配置` };
      usedStatuses.add(status);
      const count = Number(countRaw);
      if (countRaw === '' || Number.isNaN(count) || !Number.isInteger(count) || count < 0) return { ok: false, message: `第 ${i + 1} 行重推次数请填写非负整数` };
      const interval = Number(intervalRaw);
      if (intervalRaw === '' || Number.isNaN(interval) || !Number.isInteger(interval) || interval < 0) return { ok: false, message: `第 ${i + 1} 行重推间隔请填写非负整数（分钟）` };
      if (count > 0 && interval <= 0) return { ok: false, message: `第 ${i + 1} 行重推次数 > 0 时，重推间隔需大于 0 分钟` };
      let duration = null;
      if (status === '坐席接听' && durationRaw !== '') {
        duration = Number(durationRaw);
        if (Number.isNaN(duration) || !Number.isInteger(duration) || duration <= 0) return { ok: false, message: `第 ${i + 1} 行通话时长请填写正整数（秒）` };
      }
      callStatusRules.push({ status, redialCount: count, redialInterval: interval, duration });
    }
    if (!callStatusRules.length) return { ok: false, message: '请至少添加一条通话状态重推配置' };
    attributeValues = callStatusRules.map(r => r.status);
  } else if (!attributeValues.length) {
...
function saveStrategyConfiguratorRow() {
  ...
  if (key === 'precallTask' && payload.attributeType === '通话状态' && Array.isArray(payload.callStatusRules)) {
    const currentStatuses = new Set(payload.callStatusRules.map(r => r.status));
    for (const existingRow of strategyConfiguratorData[key]) {
      if (id && existingRow.id === id) continue;
      if (existingRow.attributeType !== '通话状态' || !Array.isArray(existingRow.callStatusRules)) continue;
      for (const rule of existingRow.callStatusRules) {
        if (currentStatuses.has(rule.status)) {
          showToast(`通话状态「${rule.status}」已在配置「${existingRow.ruleName}」中配置，不可重复`, false);
          return;
        }
      }
    }
