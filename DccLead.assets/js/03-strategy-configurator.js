...
const strategyConfiguratorDefs = {
  ...
  returnVisitOrder: {
    ...
    form: [
      ...
      { label: '回访描述模板', field: 'descriptionTemplate', type: 'textarea', section: '执行动作', placeholder: '请输入描述模板，例如：[系统自动话单转换] 通话状态为 {callStatus}，已重呼...' }
    ]
  }
};
...
function renderStrategyConfiguratorSearch(key) {
  ...
  if (['manualUnconnected', 'manualUnconverted', 'precallTask', 'excellentProject', 'excellentChannel', 'excellentSc'].includes(key)) return renderFlatStrategyConfiguratorFilterRow(key, def);
...
function getFilteredStrategyConfiguratorRows(key) {
  ...
  return (strategyConfiguratorData[key] || [])
    ...
    .sort((a, b) => {
      if (key === 'precallTask' || key === 'returnVisitOrder') {
...
function renderStrategyConfiguratorCell(key, row, col) {
  if (key === 'returnVisitOrder' && col.field === 'priority') {
    return `<span class="policy-plain-text">P${row.priority || 99}</span>`;
  }
  if (key === 'returnVisitOrder' && col.field === 'ruleName') {
    return `
      <div class="rule-text-stack">
        <div class="rule-text-main">${escapeHtml(row.ruleName || '—')}</div>
        <div class="rule-muted">规则ID：RVO-${String(row.id).padStart(3, '0')}</div>
      </div>
    `;
  }
  if (key === 'returnVisitOrder' && col.field === 'matchSummary') {
  ...
  if (key === 'returnVisitOrder' && col.field === 'actionSummary') {
    const items = [
...
function openStrategyConfiguratorModal(key, mode, id = null) {
  ...
  if ((key === 'manualUnconverted' || key === 'returnVisitOrder') && mode !== 'view') {
    if (key === 'returnVisitOrder') {
...
function renderStrategyConfiguratorDetail(key, row) {
  ...
  if (key === 'returnVisitOrder') return renderReturnVisitOrderDetail(row);
...
function renderStrategyConfiguratorFormField(key, field, row, readonly = false) {
  ...
  const disabled = readonly ? 'disabled' : '';
  if (key === 'returnVisitOrder' && field.field === 'mediaName') {
    ...
    return `
      <div class="${groupClass}" id="formGroup_${field.field}">
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px;">
          ...
          <div class="condition-row" style="margin-top: 12px; margin-bottom: 0; padding: 6px 12px; min-height: 40px;">
  ...
  if (key === 'returnVisitOrder' && field.field === 'intentLevelAction') {
  ...
  if (key === 'returnVisitOrder' && field.field === 'agentRouting') {
  ...
  if (key === 'returnVisitOrder' && field.field === 'nextReturnVisitTime') {
    const precallSystemStatus = row.precallSystemStatus || '';
    const allOptions = configuratorOptions.nextReturnVisitTime || [];
    if (precallSystemStatus === '处理中') {
      // 处理中 → 禁止选择，默认"下次预外呼时间"
      return `<div class="${groupClass}"><div class="form-label">${field.label}${required}</div><select class="form-input" id="${id}" disabled>${allOptions.map(item => `<option value="${escapeAttr(item)}" ${item === '下次预外呼时间' ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select>${hint}</div>`;
    } else if (precallSystemStatus === '已处理') {
      // 已处理 → 可以选择，但禁止选中"下次预外呼时间"
      const currentVal = value === '下次预外呼时间' ? '' : (value || '');
      return `<div class="${groupClass}"><div class="form-label">${field.label}${required}</div><select class="form-input" id="${id}"><option value="" ${!currentVal ? 'selected' : ''}>请选择</option>${allOptions.map(item => `<option value="${escapeAttr(item)}" ${item === '下次预外呼时间' ? 'disabled' : ''} ${currentVal === item ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select>${hint}</div>`;
    } else {
      // 未选择 → 禁止选择
      return `<div class="${groupClass}"><div class="form-label">${field.label}${required}</div><select class="form-input" id="${id}" disabled><option value="" selected>请选择</option>${allOptions.map(item => `<option value="${escapeAttr(item)}">${escapeHtml(item)}</option>`).join('')}</select>${hint}</div>`;
    }
  }
  ...
  if (field.type === 'select') {
    ...
    const onchange = field.field === 'belongType'
      ...
      : (key === 'manualUnconverted' && field.field === 'leadStatus' ? 'onchange="syncStrategyConfiguratorAbnormalReason()"'
        : (key === 'returnVisitOrder' && field.field === 'nurtureResult' ? 'onchange="handleReturnVisitOrderNurtureResultChange()"'
          : (key === 'returnVisitOrder' && field.field === 'precallSystemStatus' ? 'onchange="syncReturnVisitOrderAgentRouting()"'
            : (key === 'returnVisitOrder' && field.field === 'triggerCondition' ? 'onchange="syncReturnVisitOrderAgentRouting()"'
              : (key === 'returnVisitOrder' && field.field === 'matchingDimension' ? 'onchange="syncReturnVisitOrderMatchingDimension()"'
                : '')))));
  ...
  if (field.type === 'leadStatusReason') {
    const statusVal = (key === 'returnVisitOrder' ? row.nurtureResult : row.leadStatus) || '';
...
function syncStrategyConfiguratorAbnormalReason(currentReason = '') {
  ...
  const statusField = key === 'returnVisitOrder' ? 'nurtureResult' : 'leadStatus';
...
function handleReturnVisitOrderNurtureResultChange(currentReason = '', currentIntent = '') {
...
function syncReturnVisitOrderAgentRouting() {
  ...
  // --- 下次回访时间联动 ---
  if (nextReturnVisitTimeEl) {
    const allOptions = configuratorOptions.nextReturnVisitTime || [];
    if (precallStatus === '处理中') {
      // 预外呼系统状态 = 处理中 → 禁止选择，默认值"下次预外呼时间"
      nextReturnVisitTimeEl.disabled = true;
      nextReturnVisitTimeEl.innerHTML = allOptions.map(item =>
        `<option value="${escapeAttr(item)}" ${item === '下次预外呼时间' ? 'selected' : ''}>${escapeHtml(item)}</option>`
      ).join('');
      nextReturnVisitTimeEl.value = '下次预外呼时间';
    } else if (precallStatus === '已处理') {
      // 预外呼系统状态 = 已处理 → 可以选择，但禁止选中"下次预外呼时间"
      nextReturnVisitTimeEl.disabled = false;
      nextReturnVisitTimeEl.innerHTML = `<option value="">请选择</option>` + allOptions.map(item =>
        `<option value="${escapeAttr(item)}" ${item === '下次预外呼时间' ? 'disabled' : ''}>${escapeHtml(item)}</option>`
      ).join('');
      // 如果当前值是"下次预外呼时间"，需要清除
      if (nextReturnVisitTimeEl.value === '下次预外呼时间') {
        nextReturnVisitTimeEl.value = '';
      }
    } else {
      // 未选择预外呼系统状态 → 禁止选择
      nextReturnVisitTimeEl.disabled = true;
      nextReturnVisitTimeEl.innerHTML = `<option value="" selected>请选择</option>` + allOptions.map(item =>
...
function collectStrategyConfiguratorFormValue(def) {
  ...
  for (const field of def.form) {
    ...
    if (field.type === 'leadStatusReason') {
      const statusField = strategyConfiguratorEditing.key === 'returnVisitOrder' ? 'nurtureResult' : 'leadStatus';
    ...
    if (strategyConfiguratorEditing.key === 'returnVisitOrder' && field.field === 'intentLevelAction') {
    ...
    if (strategyConfiguratorEditing.key === 'returnVisitOrder' && field.field === 'mediaName') {
      const id = `cfgForm_${field.field}`;
