...
function getFilteredPolicies() {
  const keyword = document.getElementById('blacklistPolicyKeywordFilter')?.value.trim().toLowerCase() || '';
...
function renderTable() {
  ...
  if (pageInfoEl) {
    pageInfoEl.textContent = `共 ${filtered.length} 条记录，当前第 ${currentPage} / ${totalPages} 页`;
    syncPaginationButtons(pageInfoEl.closest('.pagination'), currentPage, totalPages);
  }
  ...
  if (filtered.length === 0) {
    const emptyText = policies.length === 0
      ? '<div class="empty-state-icon">📋</div>暂无策略数据，点击"新增策略"开始配置'
      : '暂无匹配的黑名单策略';
      ...
      <tr>
        ...
        <td class="col-field">
          <div class="rule-text-stack">
            <div class="rule-text-main">${p.name}${pendingBadge}</div>
            <div class="rule-muted">规则ID: BLP-${p.id}</div>
            <div class="policy-row-desc" style="margin-top: 2px;">${p.desc || '—'}</div>
          </div>
        </td>
...
function resetPolicyFilters() {
  ['blacklistPolicyKeywordFilter', 'callTypeFilter', 'durationFilter', 'statusFilter'].forEach(id => {
...
function handlePolicyBlacklistReasonChange(value) {
  const customReasonGroup = document.getElementById('policyBlacklistCustomReasonGroup');
  if (customReasonGroup) customReasonGroup.style.display = value === '其它原因' ? 'block' : 'none';
}

function handlePolicyBlacklistDurationChange(value) {
  const hint = document.getElementById('policyBlacklistDurationHint');
  if (!hint) return;
  if (!value) {
    hint.textContent = '请选择封禁期限后，系统将说明生效范围与到期规则。';
  } else if (value === 'perm') {
    hint.textContent = '电话号码命中该策略后将永久加入黑名单，不会自动解除。';
  } else {
    hint.textContent = `电话号码命中该策略后封禁 ${value} 天，到期后自动从黑名单中移除。`;
  }
}
...
function openModal(mode, id) {
  ...
  document.getElementById('policyBlacklistReason').value = '';
  document.getElementById('policyBlacklistCustomReason').value = '';
  document.getElementById('policyBlacklistDuration').value = '';
  handlePolicyBlacklistReasonChange('');
  handlePolicyBlacklistDurationChange('');
  switchCondition('and');
  ...
  if (mode === 'edit' && id) {
    ...
    if (p) {
      ...
      const storedReason = p.blacklistReason || '';
      const storedDuration = p.durationType === 'perm' ? 'perm' : String(p.validityNum || '');
      document.getElementById('policyBlacklistReason').value = ['频繁投诉打扰', '主观强拒绝', '客观已购车', '空号停机异常'].includes(storedReason) ? storedReason : (storedReason ? '其它原因' : '');
      document.getElementById('policyBlacklistCustomReason').value = ['频繁投诉打扰', '主观强拒绝', '客观已购车', '空号停机异常'].includes(storedReason) ? '' : storedReason;
      document.getElementById('policyBlacklistDuration').value = ['perm', '30', '60', '90', '180'].includes(storedDuration) ? storedDuration : '';
      handlePolicyBlacklistReasonChange(document.getElementById('policyBlacklistReason').value);
      handlePolicyBlacklistDurationChange(document.getElementById('policyBlacklistDuration').value);
    }
  }
...
function openPolicyFormPage(mode = 'add', id = null) {
  ...
  const subtitle = policy
    ? `培育策略 / 黑名单配置 / ${policy.name} / 编辑策略`
    : '培育策略 / 黑名单配置 / 新增策略';
  setSidebarActiveByName('培育策略');
  setPageName(`培育策略 / 黑名单配置 / ${mode === 'edit' ? '编辑策略' : '新增策略'}`);
...
function savePolicy() {
  ...
  const reasonSelection = document.getElementById('policyBlacklistReason').value;
  const customReason = document.getElementById('policyBlacklistCustomReason').value.trim();
  const durationSelection = document.getElementById('policyBlacklistDuration').value;
  if (!reasonSelection) { showToast('请选择拉黑原因', false); return; }
  if (reasonSelection === '其它原因' && !customReason) { showToast('请填写详细拉黑原因', false); return; }
  if (!durationSelection) { showToast('请选择封禁期限', false); return; }
  const blacklistReason = reasonSelection === '其它原因' ? customReason : reasonSelection;
  const durationType = durationSelection === 'perm' ? 'perm' : 'temp';
  const validityNum = durationType === 'temp' ? Number(durationSelection) : null;
  const validityUnit = durationType === 'temp' ? '天' : null;
  ...
  if (editingId) {
    ...
    if (idx > -1) {
      policies[idx] = { ...policies[idx], name, desc, priority, callType, tags, tagCondition, blacklistReason, durationType, validityNum, validityUnit, status, manualReview, notificationConfig };
    }
    showToast('策略已更新', true);
  } else {
    policies.push({ id: nextId++, name, desc, priority, callType, tags, tagCondition, blacklistReason, durationType, validityNum, validityUnit, status, manualReview, notificationConfig, hitCount: 0 });
