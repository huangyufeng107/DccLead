...
function renderTaskSnapshotTabs(task) {
  ...
  const tabs = [
    ...
    { key: 'workorder', label: '回访工单', html: renderTaskWorkorderSnapshotPanel(task.workorderSnapshot, task), active: true }
  ];
  return `
    <div class="task-snapshot-shell" data-task-snapshot="${taskKey}">
      <div class="task-snapshot-tabs">
        ${tabs.map(tab => `
          <button class="task-snapshot-tab ${tab.active ? 'active' : ''}" type="button" data-tab="${tab.key}" ${renderUiActionCallback('task-snapshot-tab', ['$self'])}>
            ${tab.label}
          </button>
        `).join('')}
      </div>
...
function renderTaskWorkorderSnapshotPanel(snapshot, task) {
      ...
      <div class="workorder-flow">
        <div class="workorder-steps">
          ${workorders.map((order, index) => {
            ...
            const testDriveSchedule = getAiTestDriveScheduleRelation(order, task);
            return `
            <button class="workorder-step ${index === 0 ? 'active' : ''} ${testDriveSchedule ? 'has-testdrive-schedule' : ''}" type="button" data-workorder-step="${index}" ${renderUiActionCallback('workorder-step', ['$self'])}>
              <div class="workorder-step-title">
                <span>第${order.contactRound}次建联</span>
                <span class="workorder-step-flags">
                  ${testDriveSchedule ? '<span class="ai-testdrive-generated-badge">已生成试驾排程</span>' : ''}
                  ${showRedialFlag ? '<span class="tag-chip orange">触发重推规则</span>' : ''}
                </span>
              </div>
        ...
        <div class="workorder-step-panels">
          ${workorders.map((order, index) => `
            <div class="workorder-step-panel ${index === 0 ? 'active' : ''}" data-workorder-panel="${index}">
              ${renderWorkorderPanelCard(order, task)}
            </div>
          `).join('')}
        </div>
      </div>
...
function renderWorkorderPanelCard(order, task) {
  ...
  return `
    <div class="workorder-panel-card">
      ...
      ${renderAiTestDriveScheduleRelation(order, task)}
...
function getAiTestDriveScheduleRelation(order, task) {
  if (order.communicationMethod !== 'AI智能外呼' || typeof window.findAiTestDriveScheduleByTrace !== 'function') return null;
  return window.findAiTestDriveScheduleByTrace({
    leadCode: task?.leadCode,
    taskCode: task?.taskCode,
    callbackWorkorderCode: order.callbackWorkorderCode
  });
}

function renderAiTestDriveScheduleRelation(order, task) {
  const schedule = getAiTestDriveScheduleRelation(order, task);
  if (!schedule) return '';
  const nevClass = schedule.nev === '已推送' ? 'green' : (schedule.nev === '待重试' || schedule.nev === '推送超时' ? 'orange' : 'red');
  return `
    <section class="ai-testdrive-relation-card">
      <div class="ai-testdrive-relation-head">
        <div class="ai-testdrive-relation-title-wrap">
          <span class="ai-testdrive-relation-icon">AI</span>
          <div><div class="ai-testdrive-relation-title">AI试驾预约线索</div><div class="ai-testdrive-relation-note">本次 AI 智能外呼已生成预约试驾，可继续追溯 DCC 与 NEV 处理结果。</div></div>
        </div>
        <button class="ai-testdrive-relation-link" type="button" onclick="openAiTestDriveScheduleFromWorkorder('${escapeAttr(schedule.id)}')">查看排程详情 <span>→</span></button>
      </div>
      <div class="ai-testdrive-relation-data">
        <div class="ai-testdrive-relation-item"><span>预约试驾编码</span><b class="relation-code">${detailValue(schedule.booking)}</b></div>
        <div class="ai-testdrive-relation-item"><span>DCC处理结果</span><b><span class="tag-chip green">${detailValue(schedule.dcc)}</span></b></div>
        <div class="ai-testdrive-relation-item"><span>NEV推送状态</span><b><span class="tag-chip ${nevClass}">${detailValue(schedule.nev)}</span></b></div>
        <div class="ai-testdrive-relation-item"><span>试驾排程编码</span><b class="relation-code">${detailValue(schedule.schedule)}</b></div>
      </div>
    </section>
  `;
}
...
function getWorkorderFieldSections(order) {
  ...
  const baseFields = [
    ...
    ...(order.isAiRedialTask === '是'
      ? [
          { label: '任务来源', value: 'AI重推任务' },
          { label: '重推来源回访工单', value: detailValue(order.redialSourceWorkorderCode) }
        ]
      : []),
    { label: '中止回访工单', value: formatYesNoText(order.stopTask) },
    {
      label: order.postCallSnapshot?.redialHit ? '重推规则处理' : '回访工单异常原因',
      value: order.postCallSnapshot?.redialHit
        ? `已创建第${order.redialNextContactRound || (Number(order.contactRound) + 1)}次AI建联任务${order.redialNextWorkorderCode ? `（${order.redialNextWorkorderCode}）` : ''}`
        : detailValue(order.taskAbnormalReason)
    },
...
function renderWorkorderAudioControl(order) {
  ...
  return `
    <div class="workorder-audio-control">
      <button class="listen-btn" type="button" data-call-id="${callId}" ${renderUiActionCallback('workorder-audio-toggle', ['$self'])}>试听</button>
      <div class="workorder-audio-player" data-player-for="${callId}" data-phone="${phone}" data-duration="${escapeAttr(duration)}">
        <button class="workorder-audio-toggle" type="button" ${renderUiActionCallback('workorder-audio-pause', ['$self'])} aria-label="暂停或继续试听">Ⅱ</button>
...
function renderWorkorderRecordingText(text, dialogue) {
  ...
  return `
    ...
    <button class="workorder-link-btn" type="button" ${renderUiActionCallback('workorder-text-toggle', ['$self'])}>展开对话</button>
  `;
}
...
function renderWorkorderPayload(payload, title = '报文详情', buttonText = '查看报文') {
  ...
  return `
    <button class="workorder-link-btn" type="button" data-title="${escapeAttr(title)}" data-payload="${escapeAttr(encodeURIComponent(payload))}" ${renderUiActionCallback('workorder-payload-open', ['$self'])}>${buttonText}</button>
  `;
}
...
registerUiActionCallback('task-snapshot-tab', switchTaskSnapshotTab);
registerUiActionCallback('workorder-step', switchWorkorderStep);
registerUiActionCallback('workorder-audio-toggle', toggleWorkorderAudio);
registerUiActionCallback('workorder-audio-pause', (button, event) => toggleWorkorderAudioPause(event, button));
registerUiActionCallback('workorder-text-toggle', toggleWorkorderText);
registerUiActionCallback('workorder-payload-open', openPayloadModal);
